...
def time_shift(variables, dataframe, id='id', time='time', shift=1):
    ...
    #Columns to exclude from the conflict check (e.g., 'id', 'time')
    exclude_columns = [id, time]

    # Check for conflicts, excluding 'id' and 'time' columns
    existing_conflicts = [col for col in df_merged.columns if col in dataframe.columns and col not in exclude_columns]
    if existing_conflicts:
        raise ValueError(f"The following lag/lead columns already exist: {', '.join(existing_conflicts)}")
    else:
        # Safe to merge
        return df_merged
