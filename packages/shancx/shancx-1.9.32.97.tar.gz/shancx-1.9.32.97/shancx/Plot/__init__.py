...
def plotGrey(img,name="plotGrey", saveDir="plotGrey",cmap='gray', title='Image'):
    ...
    plt.title(f"Image ")
    plt.axis('off')  # 不显示坐标轴
    plt.savefig(f'./{saveDir}/{name}_{now_str}.png')
    plt.close()
...
def plotRadar(array_dt,name="plotRadar", saveDir="plotRadar",ty="CR"):
    ...
    if len(array_dt.shape) == 2 and ty == "pre":
        ...
        fig.tight_layout()     
        outpath = f"./{saveDir}/{name}_pre_{now_str}.png"
        crDir(outpath)
        plt.savefig(outpath)
        plt.close()    
    else:
        ...
        plt.colorbar(im, cax=cax)   
        fig.tight_layout()     
        outpath = f"./{saveDir}/{name}_CR_{now_str}.png"
        crDir(outpath)
        plt.savefig(outpath)
        plt.close()
...
def plotA2b(a, b, name='plotA2b', saveDir="plotA2b", title='plotA2b Plot',class1 = "H9",class2 = "FB4", cmap='summer'):
    # 创建时间戳用于输出文件名
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    sublen = a.shape[0]
    # 创建画布和子图
    fig, axes = plt.subplots(2, sublen, figsize=(20, 6))  # 调整画布高度以减少空白
    # 绘制数组 `a` 中的图像并添加色标
    for i in range(sublen):
        ...
        # 为 `a` 添加色标，高度与图像一致
        divider_a = make_axes_locatable(axes[0, i])  # 创建子图的分割器
        cax_a = divider_a.append_axes("right", size="5%", pad=0.1)  # 在子图右侧添加色标轴
        cbar_a = fig.colorbar(im_a, cax=cax_a)  # 在色标轴上创建色标
        cbar_a.ax.tick_params(labelsize=8)  # 调整色标刻度标签大小
    # 绘制数组 `b` 中的图像并添加色标
    for i in range(sublen):
        ...
        # 为 `b` 添加色标，高度与图像一致
        divider_b = make_axes_locatable(axes[1, i])  # 创建子图的分割器
        cax_b = divider_b.append_axes("right", size="5%", pad=0.1)  # 在子图右侧添加色标轴
        cbar_b = fig.colorbar(im_b, cax=cax_b)  # 在色标轴上创建色标
        cbar_b.ax.tick_params(labelsize=8)  # 调整色标刻度标签大小
    plt.tight_layout()
    plt.subplots_adjust(top=0.9, bottom=0.1, hspace=0.05, wspace=0.1)  # 减少子图之间的间距
    outpath = f'./{saveDir}/{name}_{now_str}.png'
...
def plotScatter(df1,saveDir="plotScatter"):
    ...
    plt.savefig(f"./{saveDir}/plotScatter_{now_str}.png", dpi=300, bbox_inches="tight")  # 保存图像，设置分辨率和边界
    plt.close()
...
def plotVal( epoch=0,*datasets, title=["input","prediction","truth"], saveDir="plotVal", cmap='summer'):
    ...
    """   使用方法
        ...
        if total >= 3:
            break
    if epoch % 2 == 0:                    
       plot_data(epoch,   inputs[0]  --->example shape 为(256,256)
               inputs[0], 
               pre[0], 
               targets[0], 
...
def plotValplus(epoch=0, *datasets, title=["input", "prediction", "truth"], saveDir="plotValplus", cmap='summer'):
    ...
    for i, (data, t) in enumerate(zip(datasets, title)):
        ...
        im = axes[i].matshow(data, cmap=cmap, vmin=np.min(data), vmax=np.max(data))
        ...
        divider = make_axes_locatable(axes[i])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        cbar = fig.colorbar(im, cax=cax, ticks=np.linspace(np.min(data), np.max(data), 15))
        cbar.set_ticks(np.linspace(np.min(data), np.max(data), 15))
