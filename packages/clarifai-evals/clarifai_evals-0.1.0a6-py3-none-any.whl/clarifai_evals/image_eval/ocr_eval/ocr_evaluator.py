...
class OCREvaluator(BaseImageEvaluator):
  ...
  def _evaluate_gt_metric(
    ...
    summary_scores, persample_scores = metric_obj.evaluate(predictions_batch, ground_truth_batch)
    graph_data = None
    if graph:
  ...
  def _verify_inputs(self, prediction_batch: List[resources_pb2.Data],
    ...
    elif len(prediction_batch) > 128:
      raise ValueError("prediction_batch cannot have more than 128 samples")
