...
class TextGenerationEvaluator(BaseTextEvaluator):
  ...
  def _evaluate_gt_metric(
    ...
    metric_scores, summary_scores, explanations = obj.evaluate(query_batch, prediction_batch,
                                                               ground_truth_batch, context_batch)
    metric_results = None
    if graph:
  ...
  def _evaluate_nongt_metric(
    ...
    metric_scores, summary_score, explanations = obj.evaluate(
        query_batch, prediction_batch, context_batch=context_batch)
    graph_data = None
    if graph:
