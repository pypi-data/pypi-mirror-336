...
class LLMJudgeTextClassificationEvaluator(BaseLLMJudgeTextEvaluator):
  ...
  def evaluate(
    ...
    num_workers = max(5, len(prediction_onehot) // self.batch_size)
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
      futures = [
          executor.submit(
              ...
              prediction_onehot[start:start + self.batch_size],
              available_labels,
              query_batch[start:start + self.batch_size],
              context_batch[start:start + self.batch_size] if context_batch is not None else None,
          ) for start in range(0, len(prediction_onehot), self.batch_size)
      ]
