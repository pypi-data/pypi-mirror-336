...
class BaseWorkflowSpan(BaseStep):
    type: Literal[StepType.workflow] = Field(default=StepType.workflow)
...
class LlmSpan(BaseStep):
    type: Literal[StepType.llm] = Field(default=StepType.llm)
...
class RetrieverSpan(BaseStep):
    type: Literal[StepType.retriever] = Field(default=StepType.retriever)
    input: str = Field()
...
class ToolSpan(BaseStep):
    type: Literal[StepType.tool] = Field(default=StepType.tool)
    input: str = Field()
