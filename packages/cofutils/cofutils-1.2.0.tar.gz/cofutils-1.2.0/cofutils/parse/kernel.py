...
class Kernel(object):
    ...
    def setRunTimeInfo(self, info):
        ...
        assert (self.rStartTime < self.kStartTime) or self.device != 0, f"profStart:{Kernel.profStart} kLongName: {self.kLongName} device: {self.device} rstarttime: {self.rStartTime}, kstarttime: {self.kStartTime}, kendtime: {self.kEndTime}, kdur: {self.kDuration/float(1e9)}"
