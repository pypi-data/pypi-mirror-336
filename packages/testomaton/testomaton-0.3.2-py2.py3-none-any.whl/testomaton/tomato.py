...
def tomato():
    ...
    if input_file is not None:
        try:
            with open(input_file, 'r', encoding='utf-8') as f:
                parsed_yaml_model = yaml.load(f.read(), Loader=DuplicateCheckLoader)
        except OSError as e:
    ...
    if validate_model and function_name is None:
        functions_to_parse = model.get_function_names(parsed_yaml_model, whitelist, blacklist)
        if len(functions_to_parse) == 0:
            print(f'No function to validate', file=sys.stderr)
            sys.exit(int(ec.MODEL_VALIDATION_ERROR))
    else: 
