...
def get_eu_otu(args):
    ...
    cmd_txt = '%s/%s_db_update_cmds.sh' % (f_path, f_base)
    ...
    for query in query_to_hit_dict:
        ...
        # print(hit_tax_list)
        # print('%s\t%s\t%s\t%s\t%s\t%s' % (query, hit_num_bac, hit_num_ar, hit_num_eu, hit_num_virus, hit_num_na))
        # print()
    ...
    if len(unclassified_refseq_set) > 0:
        ...
        print('Note:')
        print('Taxon ID for %s RefSeqs is missing, to increase accuracy, add their classifications to %s and rerun this script. Classification commands exported to: %s' % (len(unclassified_refseq_set), refseq_tax_txt, cmd_txt))
        print('Identified %s eukaryotic OTUs exported to: %s' % (len(eu_otu_set), eu_otu_txt))
        print('Done')
...
if __name__ == '__main__':
    ...
    get_eu_otu_parser.add_argument('-n',    required=False, type=int, default=20,   help='number of top hits to consider, default is 20')
