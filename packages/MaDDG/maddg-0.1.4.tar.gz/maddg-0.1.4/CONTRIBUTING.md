...
Welcome to `MaDDG` contributor's guide.
...
If you experience bugs or general issues with `MaDDG`, please have a look
...
You can help improve `MaDDG` docs by making them more readable and coherent, or
by adding missing information and correcting mistakes.

`MaDDG` documentation uses [Sphinx] as its main documentation compiler.
...
:::{tip}
   Please notice that the [GitHub web interface] provides a quick way to
   propose changes in `MaDDG`'s files. While this mechanism can
...
```
conda create -n MaDDG python=3 six virtualenv pytest pytest-cov
conda activate MaDDG
```
...
3. Clone this copy to your local disk:
   ...
   ```
   git clone git@github.com:YourLogin/MaDDG.git
   cd MaDDG
   ```
...
5. Install [pre-commit]:
   ...
   `MaDDG` comes with a lot of hooks configured to automatically help the
...
[repository]: https://github.com/<USERNAME>/MaDDG
[issue tracker]: https://github.com/<USERNAME>/MaDDG/issues
