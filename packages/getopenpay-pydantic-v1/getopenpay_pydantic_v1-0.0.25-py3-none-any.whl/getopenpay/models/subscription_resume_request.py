...
class SubscriptionResumeRequest(BaseModel):
  ...
  is_preview: Optional[StrictBool] = Field(
    False,
    description="Whether the request is in preview mode (subscriptions won't actually be resumed)"
  )
  proration_behavior: Optional[ProrationEnum] = Field(
    None, description='Determines how to handle prorations when the subscription is resumed.'
  )
