...
def generate_lrc_at_song(song_path: str, model: whisper.Whisper) -> None:
    """Generates a lrc file at the location of the provided song location.

    Args:
        song_path (str): The path/location where the song file is located.
    ...
    song_dir: str = os.path.dirname(song_path)
    lrc_path: str = os.path.splitext(song_path)[0] + "." + "lrc"

    if os.path.exists(lrc_path):
        raise FileExistsError(f"lrc file for '{song_dir}' already exists.")

    lrc_data: str = generate_lrc(song_path, model)
...
def generate_lrc_for_album(album_path: str, model: whisper.Whisper) -> None:
    ...
    with console.status(status_message.format(0, total_files)) as status:
        for file_num, song_path in enumerate(music_file_paths, 1):
            song_name: str = get_filename_without_extension(song_path)
