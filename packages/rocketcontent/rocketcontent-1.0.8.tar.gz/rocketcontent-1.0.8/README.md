...
* Create a YAML configuration file (e.g., `rocketcontent.yaml`) to store your Content Repository ID, credentials and settings. For the first connection, leave the `repo_id` field blank—it will be populated automatically.
    ...
    repository:
        ...
        repo_pass: admin
        repo_user: admin
        repo_url: https://services-us-virginia-m-1.skytap.com:8682        
    ```
...
* **Logging:**
    ...
    2025-03-11 11:10:05,549 - INFO -   URL: https://services-us-virginia-m-1.skytap.com:8682/mobius/rest
    2025-03-11 11:10:05,549 - INFO -   Repo Name: Mobius
    ...
    2025-03-11 11:10:05,549 - INFO - Smart Chat URL : https://services-us-virginia-m-1.skytap.com:8682/mobius/rest/conversations
    ...
    2025-03-11 11:10:05,549 - DEBUG - Starting new HTTPS connection (1): services-us-virginia-m-1.skytap.com:8682
    2025-03-11 11:10:25,871 - DEBUG - https://services-us-virginia-m-1.skytap.com:8682 "POST /mobius/rest/conversations HTTP/1.1" 200 None
    ```
---
#### Document List
[Smart Chat](#smart-chat)
...
[Table of Contents](#table-of-contents)
   ...
   The `archive_metadata` method archives content in Content Repository with associated metadata.
...
* **Parameters**:
    ...
    - `ArchiveMetadataBuilder`:**[Mandatory]** Object containing metadata (indexes) for the content.
...
* **Example:**
    ...
    import os
     from rocketcontent.content_services_api import ContentServicesApi
     from rocketcontent.content_archive import ArchiveMetadataBuilder
     try:
         ...
         png_metadata = ArchiveMetadataBuilder(document_class_id="LISTFILE")
         ...
         status = content_obj.archive_metadata(myfile, png_metadata)
     
         print(f"File PNG {myfile} archived successfully. Response Status: {status}")  
     except (FileNotFoundError, ValueError) as e:
         print(f"Error: {e}") 
    ```
    > **Supported Formats: TXT, PNG, PDF, JPG.**
