...
class CheckoutPreferences(BaseModel):
  """
    Preferences for the checkout session. All changes to this object must be backwards-compatible, old objects should still be valid with all new schemas.  Note: this object is stored in both PaymentLinks (in string form) and CheckoutSessions
    """ # noqa: E501
  fallback_cascade_preferences: Optional[List[FallbackConfigurationInput]] = None
  fallback_cascade_selected_product_price_quantity: Optional[List[List[SelectedPriceQuantity]]
                                                             ] = None
  processor_preferences: Optional[CheckoutProcessorsPreferences] = None
  __properties: ClassVar[List[str]] = [
    'fallback_cascade_preferences', 'fallback_cascade_selected_product_price_quantity',
  ...
  def to_dict(self) -> Dict[str, Any]:
    ...
    if self.fallback_cascade_selected_product_price_quantity:
      for _item in self.fallback_cascade_selected_product_price_quantity:
        if _item:
          _items.append([_inner_item.to_dict() for _inner_item in _item if _inner_item is not None])
  ...
  def from_dict(cls, obj: Dict) -> Self:
    ...
    _obj = cls.model_validate(
      {
        'fallback_cascade_preferences':
          [
            FallbackConfigurationInput.from_dict(_item)
            for _item in obj.get('fallback_cascade_preferences')
          ] if obj.get('fallback_cascade_preferences') is not None else None,
        'fallback_cascade_selected_product_price_quantity':
          [
            [SelectedPriceQuantity.from_dict(_inner_item)
