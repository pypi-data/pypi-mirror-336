...
class QuestionsExtract(BaseComponent):
    ...
    def build_prompt():
        GeneralPromptBuilder(
            instruction="根据提供的文本知识，列举能够被准确回答的问题，确保完全覆盖知识内容且每个问题的答案都能在知识中找到对应信息。问题需要具体明确，形式应为问句，并以JSON数组格式输出。",
...
- 量词需与知识原文中的数量描述精确对应
- 禁止出现"这些"、"它"、"该"等指代性表述""",
        ).export_yaml(practice_dir + "/questions_extract.yaml")
    ...
    def run(
        ...
        **kwargs,
        ...
        _prompt = GeneralPromptBuilder.load("questions_extract").get_instruction(
            question
        )
