...
class LimitOffsetPagination(BasePaginationStrategy):
    ...
    def generate_metadata(
        ...
        links["last"] = link_builder.build_link({
            self.limit_param: limit,
            self.offset_param: max(0, total_items - limit)
        })
...
class CursorPagination(BasePaginationStrategy):
    ...
    def calculate_offset_limit(self, cursor: Optional[str], page_size: int) -> Tuple[int, int]: #type:ignore
        return 0, page_size
    ...
    def generate_metadata(
        ...
        link_builder = LinkBuilder(
            base_url,
            request_params,
            [self.cursor_param, self.page_size_param]
        )
