...
class TCPKeepAliveAdapter(HTTPAdapter):
    """
    An HTTP adapter that allows TCP keepalives to be configured

    Defaults are to wait 10 minutes before sending the first keepalive probe,
    then send a probe every 10 minutes, and consider the connection dead after
    5 unsuccessful probes. This will keep the connection alive for up to 60 minutes.
    """

    def __init__(
        self,
        keepalive_time: int = 600,
    ...
    def init_poolmanager(
        ...
        # These options might not be available on all platforms (generally Windows)
        # so we need to add them conditionally
        if hasattr(socket, "TCP_KEEPIDLE"):
            socket_options.append((socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, self.keepalive_time))
        if hasattr(socket, "TCP_KEEPINTVL"):
            socket_options.append(
                (socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, self.keepalive_interval)
            )
        if hasattr(socket, "TCP_KEEPCNT"):
            socket_options.append((socket.IPPROTO_TCP, socket.TCP_KEEPCNT, self.keepalive_probes))
...
class RESTClientObject(requests.Session, BrowserMixin):
    ...
    def __init__(
        ...
        if not use_tcp_keepalive:
            ...
            adapter = TCPKeepAliveAdapter(max_retries=max_retries)
