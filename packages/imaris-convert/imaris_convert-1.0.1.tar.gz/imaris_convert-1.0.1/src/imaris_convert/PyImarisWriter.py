...
class ImageSize(PyClassnameExceptionRaiser):
    ...
    def __str__(self):
        return '{} (x: {} y: {} z: {} c: {} t: {})'.format(self.__class__.__name__, self.x, self.y, self.z, self.c,
                                                           self.t)

    def __truediv__(self, other):
        return ImageSize(x=self.x // other.x, y=self.y // other.y, z=self.z // other.z, c=self.c // other.c,
                         t=self.t // other.t)
...
class DimensionSequence(PyClassnameExceptionRaiser):
    def __init__(self, *args):
        ...
        if missing_keys:
            self.raise_creating_clex(
                'Missing arguments: {}. You provided {}'.format(','.join(missing_keys), ','.join(args)))
...
class Color(PyClassnameExceptionRaiser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if missing_keys:
            self.raise_creating_clex(
                'Missing arguments: {}. You provided {}'.format(','.join(missing_keys), ','.join(kwargs)))
    ...
    def __str__(self):
        return '{} (red: {} green: {} blue: {} alpha: {})'.format(self.__class__.__name__, self.mRed, self.mGreen,
                                                                  self.mBlue, self.mAlpha)
...
class Parameters:
    ...
    def _get_c_parameters(self):
        ...
        c_parameters = bpConverterTypesC_ParametersPtr(
            bpConverterTypesC_Parameters(parameter_section_data, number_of_sections))
        return c_parameters
...
class ImageConverter(PyClassnameExceptionRaiser):
    ...
    def _store_dimension_sequence(self, dimension_sequence):
        ...
        self.mDimensionSequence = bpConverterTypesC_DimensionSequence5DPtr(
            bpConverterTypesC_DimensionSequence5D(*sequence))
    ...
    def _store_progress_callback(self, progress_callback_class):
        if not hasattr(progress_callback_class, 'RecordProgress'):
            text = 'The provided progress callback class "{}" does not have the required method RecordProgress(float, int)'.format(
    ...
    def _get_np_type(self):
        ...
        if self.mDataType == bpConverterTypesC_UInt8Type:
            ...
            raise PyImarisWriterException(
                'Could not create numpy type for PyImarisWriter Type "{}"'.format(self.mDataType))
    ...
    def NeedCopyBlock(self, block_index: ImageSize) -> bool:
        c_block_index = bpConverterTypesC_Index5D(block_index.x, block_index.y, block_index.z, block_index.c,
                                                  block_index.t, )
