...
class RegisteredEventHandler(RegisteredHandler[Event[T], T]):
    def __init__(self, command_name: str, handler: MessageHandler[Event[T], Event], input_class: Type[T]):
        super().__init__(command_name, handler, input_class)

    def __repr__(self) -> str:
        return f"RegisteredEventHandler({self.message_name}, {self.handler}, {self.input_class})"
