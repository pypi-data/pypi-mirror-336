...
- the median computation is now much faster. Yori now approximates the median using the
  remedian algorithm (see Cantone and Hofri, 2013) during the gridding process while 
  buffers the values of each cell during the aggregation and then uses the numpy.median 
  function to provide the result. This process is much faster than the previous version 
  but the results might not be as accurate (see documentation for more details).
...
### Bug Fixes
- fixed an issue in `yori-aggr` that woul cause an error if the `only_histograms` option
  was used in combination with `min_valid_days`
...
- New option `-F`, `--final` in `yori-aggr` allows users to save a "finalized"
  ...
  statistics (i.e. Sum, Sum\_Squares, Median\_Distribution). This action will
  reduce the file size, but will make the output unusable with Yori.

### Other Changes
- Reduced number of variables in fulltest\_config and generated new reference
  test files
