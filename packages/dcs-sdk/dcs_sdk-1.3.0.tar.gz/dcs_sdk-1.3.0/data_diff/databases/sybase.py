...
class Sybase(ThreadedDatabase):
    ...
    _args: Dict[str, Any]
    _sybase: Any
    table_name_one: Optional[str]
    table_name_two: Optional[str]
    ...
    def __init__(self, host, port, user, password, *, database, thread_count, **kw) -> None:
        ...
        self._sybase = None
        self.table_name_one = None
        self.table_name_two = None
        ...
        try:
            if "iq" in self._args.get("driver", "").lower():
                ...
                conn_dict = {
                    ...
                    # "options": f"-csearch_path={self.default_schema}",
                }
                if host:
                    conn_dict["HOST"] = host
    ...
    def select_table_schema(self, path: DbPath) -> str:
        ...
        if self.table_name_one is None:
            self.table_name_one = name
        if self.table_name_two is None and self.table_name_one != name:
            self.table_name_two = name
        if "iq" in self._args.get("driver", "").lower():
            return (
    ...
    def _query_cursor(self, c, sql_code):
        ...
        if self.table_name_one:
            table_name = self.table_name_one
            bracketed_table = f"[{table_name}]"

            if table_name.lower() in sql_code.lower() or bracketed_table.lower() in sql_code.lower():
                if "d.domain_name" not in sql_code.lower():
                    if bracketed_table.lower() in sql_code.lower():
                        sql_code = sql_code.replace(bracketed_table, f"{self.default_schema}.{table_name}")
                    else:
                        sql_code = sql_code.replace(table_name, f"{self.default_schema}.{table_name}")

        if self.table_name_two:
            table_name = self.table_name_two
            bracketed_table = f"[{table_name}]"
        ...
        if sql_code != original_sql and "[" in sql_code:
