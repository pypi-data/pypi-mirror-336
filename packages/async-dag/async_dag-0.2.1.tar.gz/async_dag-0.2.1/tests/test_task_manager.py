...
async def test_task_manager_sanity() -> None:
    ...
    starting_node = tm.add_node(from_args, tm.parameter_node)

    inc_1 = tm.add_node(inc, starting_node)

    str_node = tm.add_node(to_str, starting_node)
    str_to_int_node = tm.add_node(to_int, str_node, tm.parameter_node)
...
async def test_multiple_invocation() -> None:
    with build_dag(Input) as tm:
        from_args_node = tm.add_node(from_args, tm.parameter_node)
...
async def test_calling_order_of_dag() -> None:
    ...
    with build_dag(Input) as tm:
        ...
        single_path_to_end_node_1 = tm.add_node(
            define_step(1, 0), starting_node, tm.parameter_node
        )
        single_path_to_end_node_2 = tm.add_node(
            define_step(2, 0), single_path_to_end_node_1, tm.parameter_node
        )

        merge_path_to_end_node_1_1 = tm.add_node(
            define_step(3, 0.1), starting_node, tm.parameter_node
        )
        merge_path_to_end_node_1_2 = tm.add_node(
            define_step(4, 0.15), starting_node, tm.parameter_node
        )
...
async def test_duplicate_task_node_input() -> None:
    ...
    with build_dag(Input) as tm:
        ...
        tm.add_node(merge, starting_node, starting_node, tm.parameter_node)

    await tm.invoke(Input(expected_value))


async def test_parameter_node_should_be_a_singleton() -> None:
    with build_dag(Input) as tm:
        assert tm.parameter_node is tm.parameter_node


async def test_parameter_node_result_should_be_the_parameter() -> None:
    expected = "FOO BAR"
    with build_dag(str) as tm:
        node = tm.parameter_node
    result = await tm.invoke(expected)

    assert node.extract_result(result) == expected


async def test_parameter_node_should_never_be_called() -> None:
    with build_dag() as tm:
        assert tm.parameter_node is tm.parameter_node
...
async def test_immediate_value_get_converted_to_node() -> None:
    expected_value = 1
    with build_dag() as tm:
        node = tm.add_node(inc, 0)
