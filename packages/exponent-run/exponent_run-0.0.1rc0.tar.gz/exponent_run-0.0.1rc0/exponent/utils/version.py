...
def _get_install_command() -> List[str] | None:
    """Get the install command for exponent."""

    installer = get_installer()
    if installer == "pip":
        if "pipx" in str(sys.executable):
            return ["pipx", "upgrade", "exponent-run"]
        else:
            return ["pip", "install", "--upgrade", "exponent-run"]
    elif installer == "uv":
        return ["pipx", "upgrade", "exponent-run"]
    else:
        return None
...
def upgrade_exponent(
    *,
    ...
    Returns:
        None
    """
    new_version_str = f"New version available: exponent-run=={new_version} (current: {current_version})\n"
    upgrade_command = _get_install_command()

    if upgrade_command is None:
        click.secho(
            f"{new_version_str}See https://docs.exponent.run/installation for instructions on how to upgrade.",
            fg="yellow",
            bold=True,
        )
