...
from exponent.core.config import Environment, Settings, is_editable_install
...
def shell(
    ...
    # If Claude 3.7 is selected, use the new XML strategy
    if (
        is_editable_install()
        and strategy == "NATURAL_EDIT"
        and "CLAUDE_3_7_SONNET" in model
    ):
        strategy = "NATURAL_EDIT_CLAUDE_3_7_XML"
        print("> Using Claude 3.7 XML NATURAL_EDIT strategy\n")
    ...
    if headless:
        ...
        kitty_kbd_proto = enable_kitty_kbd_protocol()

        if kitty_kbd_proto:
            atexit.register(disable_kitty_kbd_protocol)

        input_handler = InputHandler(kitty_kbd_proto)
    ...
    def _handle_step_event(self, event: dict[str, Any]) -> None:
        ...
        if require_confirmation:
            seqs.append(
                [
                    *self._render_block_footer(
                        "Confirm step with <C+y>. Sending a new message will cancel this step."
                    ),
                    "\n",
                ]
            )
...
class InputHandler:
    def __init__(self, kitty_kbd_proto: bool) -> None:
        self.bindings = KeyBindings()
        self.kitty_kbd_proto = kitty_kbd_proto
        self.shortcut = None
        ...
        @self.bindings.add("enter", filter=kitty_kbd_proto)
        ...
        # shift-enter
        @self.bindings.add(
            "escape", "[", "1", "3", ";", "2", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
        ...
        @self.bindings.add(
            "escape", "[", "9", "7", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
            self.shortcut = "<c-a>"
            event.app.exit()
        ...
        @self.bindings.add(
            "escape", "[", "9", "9", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
            self.shortcut = "<c-c>"
            event.app.exit()
        ...
        @self.bindings.add(
            "escape", "[", "1", "0", "0", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
            self.shortcut = "<c-d>"
            event.app.exit()
        ...
        @self.bindings.add(
            "escape", "[", "1", "0", "1", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
        ...
        @self.bindings.add(
            "escape", "[", "1", "1", "6", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
            self.shortcut = "<c-t>"
            event.app.exit()
        ...
        @self.bindings.add(
            "escape", "[", "1", "2", "1", ";", "5", "u", filter=kitty_kbd_proto
        )
        def _(event: Any) -> None:
            self.shortcut = "<c-y>"
            event.app.exit()
    ...
    def prompt(self, auto_confirm_mode: AutoConfirmMode, thinking: bool) -> str:
        ...
        if auto_confirm_mode == AutoConfirmMode.ALL:
            toolbar.append(("ansired", "➔ autorun: on"))
        elif auto_confirm_mode == AutoConfirmMode.READ_ONLY:
            toolbar.append(("ansigreen", "➔ autorun: read only"))
        else:
            toolbar.append(("#777777", "➔ autorun: off"))

        toolbar.append(("", " "))
        toolbar.append(("#777777", "(toggle with <ctrl-a>)"))
        toolbar.append(("", "   "))

        if thinking:
            toolbar.append(("ansiblue", "⋯ thinking: on"))
        else:
            toolbar.append(("#777777", "⋯ thinking: off"))

        toolbar.append(("", " "))
        toolbar.append(("#777777", "(toggle with <ctrl-t>)"))

        self.shortcut = None

        user_input = self.session.prompt(
            HTML("<b><ansigreen>></ansigreen></b> "),
            multiline=self.kitty_kbd_proto,
...
class Shell:
    ...
    def _confirm_execution(self) -> None:
        ...
        # Debug: Add a marker so we know confirmation was triggered
        print(f"\n(Confirming {self.chat.pending_event['__typename']})\n")
