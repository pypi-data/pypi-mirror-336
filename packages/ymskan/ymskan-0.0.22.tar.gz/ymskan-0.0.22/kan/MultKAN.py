...
class MultKAN(nn.Module):
    ...
    def fix(self, dataset: dict, batch_size, batch_size_test, labels, opt="LBFGS", epochs=100, log=1, lamb=0.,
        ...
        pbar = tqdm(range(epochs), desc='description', ncols=200)
        ...
        def closure():
            ...
            objective.backward()
            # train_pbar.set_postfix(loss=objective.item(), train_loss=train_loss.item())
            return objective
        ...
        for epoch in pbar:
            ...
            # train_pbar = tqdm(range(steps), desc='Training Progress', colour='GREEN')
            for batch_num, i in enumerate(range(0, len(train_indices), batch_size), start=0):
                ...
                if opt == "Adam":
                    ...
                    loss.backward()
                    optimizer.step()
                    # train_pbar.set_postfix(loss=loss.item(), train_loss=train_loss.item())

            test_loss = torch.zeros(1).to(self.device)
            with torch.no_grad():
                # test_pbar = tqdm(range(test_steps), desc='Testing Progress', colour='blue')
                ...
                for batch_num, i in enumerate(range(0, len(test_indices), batch_size_test)):
                    ...
                    # test_pbar.set_postfix(loss=loss.item(),val_loss=test_loss.item())
