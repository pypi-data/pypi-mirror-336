...
from uni_model.auto_generated import UniLayerConst
...
class UniModelToGraphDefConverter:
    ...
    def convert(model: UniModel, uni_model_path: UniModelPath,
        ...
        UniModelToGraphDefConverter._serialize(
            UniModelToGraphDefConverter._cast_model(model, fixed_const_saving_method), uni_model_path, fixed_const_saving_method
        )

    @staticmethod
    def _cast_model(model: UniModel, const_saving_method: ConstSavingMethod) -> GraphDef:
        ...
        graph_nodes = [UniModelToGraphDefConverter._generate_graph_nodes(g, const_saving_method) for g in model.uni_graphs]
        graphdef = GraphDef()
    ...
    @staticmethod
    def _generate_graph_nodes(graph: UniGraph, const_saving_method: ConstSavingMethod) -> List[NodeDef]:
        layer_input = {}
        ...
        if const_saving_method == ConstSavingMethod.OUT_MODEL_FILE:
            for node in graph.layer_nodes:
                if isinstance(node, UniLayerConst):
                    node._set_extra(ConstExtraAttr(const_compressor_holder.hold_const_float_data(node.float_data),
                                                   const_compressor_holder.hold_const_indices_data(node.indices_data, node.dtype.value_n_bits)))
        return [n.to_node_def(layer_input.get(n.name, []), graph.id) for n in graph.layer_nodes]
