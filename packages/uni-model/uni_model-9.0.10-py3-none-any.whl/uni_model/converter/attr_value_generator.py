...
from uni_model.auto_generated import Dtype, Dfloat, QtypePerAxis, QtypePerTensor, PlaceholderDtype, TensorDesc
        ...
        for k, v in node.attr.items():
            if node.op == "Const":
                if const_compressor_holder.has_const_to_read():
                    if k == "indicesData":
                        tv = const_compressor_holder.read_const_indices_data(cls._to_key_type_value(v), node.attr["dtype"].func.attr["valueNBits"].i)
                    elif k == "floatData":
