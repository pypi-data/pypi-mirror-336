...
from .dolphindb_info import *
from .instrument_class import Instrument


class GJDataC:

    def __init__(self, host, port, user_id, password, dolphindb_service):
        """
    ...
    def get_trading_dates_validate_date_n_start_end(self, date, n, start_date, end_date, is_include_start,
        ...
        elif trade_time and date_count and date_type:  # 查询前/后 N个交易日/时间数据
    ...
    def get_active_contract_tick_data(self, data, frequency, start_date, end_date):
        ...
        # 使用 strftime 方法将 Timestamp 转换为所需格式
        formatted_dates = [ts.strftime('%Y.%m.%d') for ts in data['trading_day'].tolist()]
    ...
    def get_trading_days(trading_dates, trade_time, date_count):
        ...
        return fina_result[0] if len(fina_result) <= 1 else fina_result
