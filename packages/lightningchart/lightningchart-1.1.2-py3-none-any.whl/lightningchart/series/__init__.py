...
class PointLineAreaSeries(
    Series, ComponentWithPointPaletteColoring, ComponentWithLinePaletteColoring
):
    def append_json(self, data_array, data_properties):
        ...
        self.instance.send(
            self.id, 'appendJSON', {'array': data_array, 'properties': data_properties}
        )
        return self
    ...
    def set_max_sample_count(self, max_sample_count: int, automatic: bool = True):
        ...
        """
        self.instance.send(
            self.id, 'setMaxSampleCount', {'max': max_sample_count, 'auto': automatic}
        )
        return self

    def set_curve_preprocessing(
        self, type: str, step: str = None, resolution: int | float = 20
    ):
...
class SeriesWithAddDataPoints(Series):
    def add_dict_data(
        self, data: dict[str, int | float] | list[dict[str, int | float]]
    ):
...
class SeriesWithAddArray(Series):
    ...
    def add_arrays_xy(self, array_x: list[int | float], array_y: list[int | float]):
        ...
        self.instance.send(
            self.id, 'addArraysXY', {'arrayX': array_x, 'arrayY': array_y}
        )
        return self
...
class SeriesWithIndividualPoint(Series):
    def set_individual_point_color_enabled(self, enabled: bool):
        ...
        """
        self.instance.send(
            self.id, 'setIndividualPointColorEnabled', {'enabled': enabled}
        )
        return self
...
class SeriesWith2DLines(Series):
    ...
    def set_dashed(
        ...
        color: lightningchart.Color = None,
    ):
        ...
        self.instance.send(
            self.id,
            'setDashedStroke',
            {'pattern': pattern, 'thickness': thickness, 'color': color.get_hex()},
        )
        return self
...
class SeriesWithCull(Series):
    def set_cull_mode(self, mode: str = 'disabled'):
        ...
        if mode not in cull_modes:
            raise ValueError(
                f"Expected mode to be one of {cull_modes}, but got '{mode}'."
            )
...
class PointSeriesStyle(Series):
    def set_point_image_style(self, source: str):
        ...
        """
        base64_image = convert_source_to_base64(source)
