...
from typing import Any, overload, SupportsIndex, Iterator
...
class ListState(list):
    ...
    def __setitem__(self, key: SupportsIndex, value: Any | Iterator[Any], /, **kwargs) -> None:  # type: ignore
        old_value = self[key]
    ...
    def insert(self, index: SupportsIndex, object: Any, /, **kwargs) -> None:
        ...
        for callback in self.on_change_callbacks:
            callback(new_value=object, action="add", index=index)  # TODO: add `index` argument for list updates. Index changes should also be observable.
...
class State(type):
    def __new__(cls, json_state: dict | list, *args, **kwargs) -> DictState | ListState | SetState:
        if isinstance(json_state, dict):
            return DictState(json_state)
        elif isinstance(json_state, list):
            return ListState(json_state)
        elif isinstance(json_state, set):
