...
from tecton_core.offline_store import get_time_column_for_offline
...
def get_offline_store_reader(
    ...
    if case == "delta":
        ...
        return IcebergReader(spark, fd.id, time_column=get_time_column_for_offline(fd))
    else:
        msg = f"Invalid store type {case} for feature definition {fd.name}."
        raise ValueError(msg)
...
class IcebergWriter(OfflineStoreWriter):
    ...
    def _ensure_table_exists(self, schema: StructType) -> None:
        if not self._spark.catalog.tableExists(self._full_table_name):
