...
class MetadataCatalog(MetastoreCatalog):
    ...
    def __commit_table(self, updated_staged_table: StagedTable) -> CommitTableResponse:
        ...
        if int(metadata_content["version"]) != new_metadata_version - 1:
            # Another concurrent commit already happened.
            msg = f"Commit version mismatch. Attempted new commit version {new_metadata_version} is not an increment on the current metadata version {metadata_content['version']}."
