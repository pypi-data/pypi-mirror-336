...
def handleInputs2D(
    ...
    for i, record in enumerate(records):
        if i < num_settings:
            ...
            intv = int(setting['intv'] if "intv" in setting.keys() else (len(record)/ 1E7))
            ...
            for typ in curve_types:
                ...
                curves.append((retrs[0] if isinstance(retrs, tuple) else retrs, typ))
...
def main():
    ...
    if output is not None:
        ...
        for item in curve_list:
            ...
            contents.append({name: {typ: [round(item, 2) for item in curve] for curve, typ in curves}})
