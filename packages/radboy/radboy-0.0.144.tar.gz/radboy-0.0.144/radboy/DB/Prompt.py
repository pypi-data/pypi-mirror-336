...
{Fore.light_steel_blue}if 'b' returns to previous menu, try '#b' to return to barcode input, in ListMode@$LOCATION_FIELD, 'e' does the same{Style.reset}''')
    ...
    {Fore.grey_70}[+CRV({i.CRV})*Total({total}){Fore.slate_blue_1}=TotalCRV({crv})]
    {Fore.grey_70}[+Tax({i.Tax}) w/o CRV({i.CRV})*Total({total}){Fore.slate_blue_1}=TaxNoCRVTotal({tax})]
    {Fore.grey_70}[+Tax({i.Tax}) w/ CRV({i.CRV})*Total({total}){Fore.slate_blue_1}=TaxCRVTotal({tax_crv})]{Style.reset}"""
                        print(msg)
...
{Fore.light_green}Total Product Value Taxed w/ CRV({master_total_tax_crv}):{Fore.slate_blue_1}{master_total_tax_crv+master_total}
                    {Style.reset}"""
                    print(msg)
