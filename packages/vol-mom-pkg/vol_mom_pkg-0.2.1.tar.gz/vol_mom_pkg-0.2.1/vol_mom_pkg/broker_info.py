...
def get_incremental_pnl(start_date=None, end_date=None, timeframe="1D"):
    ...
    spy_prices = get_ohlc_alpaca(["SPY"], start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"))

    # Merge SPY close prices with history DataFrame
    history = history.join(spy_prices["SPY"], how="left")
    history['Close'] = history['Close'].ffill()
    history['SPY Daily Chg'] = history["Close"].pct_change()
    history['SPY Value'] = history['SPY Daily Chg']*300000
    history['SPY Value'].iloc[0] = 300000
    history['Account Value'].iloc[0] = 300000
    history['SPY Value'] = history['SPY Value'].cumsum()
    history = history[['Account Value', 'SPY Value']]
    return history
