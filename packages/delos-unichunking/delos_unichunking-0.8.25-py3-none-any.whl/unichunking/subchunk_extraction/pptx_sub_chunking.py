...
async def extract_subchunks_pptx(
...
) -> tuple[list[SubChunk], list[MatrixTable], list[str], Path]:
    ...
    images: list[str] = []
    image_idx = 0
    images_jobs: list[dict[str, str | int | ChunkPosition]] = []
    ...
    tasks = [
        handle_image(
            str(res["image_idx"]),
            int(res["r_id"]) if not isinstance(res["r_id"], ChunkPosition) else 0,
            int(res["slide_idx"])
            if not isinstance(res["slide_idx"], ChunkPosition)
            else 0,
            res["position"]
            if isinstance(res["position"], ChunkPosition)
            else ChunkPosition(x0=0, y0=0, x1=1, y1=1),
            semaphore,
        )
    ...
    return subchunks, tables, images, path
