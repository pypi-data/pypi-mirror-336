...
async def extract_subchunks(
    ...
    if extension == "pdf":
        formatted_subchunks, num_pages = await extract_subchunks_pdf(
            path,
        ...
        match extension:
            case "docx" | "doc" | "odt":
                subchunks, tables, images, new_path = await extract_subchunks_docx(
                    path,
                    extension,
                    function=function,
                )
            ...
            case "xlsx" | "xls" | "ods":
                subchunks, tables, images, new_path = extract_subchunks_xlsx(
                    path,
                    extension,
                )

            case "pptx" | "ppt" | "odp":
                subchunks, tables, images, new_path = await extract_subchunks_pptx(
                    path,
                    extension,
                    function=function,
                )
    ...
    return subchunks_final, num_pages
