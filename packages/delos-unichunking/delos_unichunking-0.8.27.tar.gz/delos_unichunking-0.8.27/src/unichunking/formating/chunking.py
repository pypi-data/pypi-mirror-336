...
async def build_chunked_pages(
    ...
    chunks_extracted, num_pages_default = await extract_subchunks(
        path,
        k_max=k_max,
    ...
    if len(pages_numbers) and num_pages_default == 0:
        return [[]], 0

    num_pages = max(pages_numbers) + 1
    ...
    return edit_positions_on_page(chunks_pages, extension), num_pages
