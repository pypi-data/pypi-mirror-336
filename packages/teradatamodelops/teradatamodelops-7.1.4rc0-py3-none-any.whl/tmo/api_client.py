...
class TmoClient(object):
    ...
    def __init__(self, **kwargs):
        ...
        logging.info(f"Connected to {self.vmo_url}")
        ...
        if os.path.isfile(self.DEFAULT_OLD_CONFIG_FILE_PATH):
            ...
            for key in conf:
                if key.startswith("aoa_url"):
                    ...
                    del conf[key]
            yaml.safe_dump_all(conf, open(self.DEFAULT_CONFIG_FILE_PATH, "w"))
    ...
    def __chase_tls_cert_chain(self):
        if self.vmo_url.startswith("https") and not (
            ...
            try:
                if re.search("http|https", self.vmo_url):
                    ...
                    raise Exception("Invalid VMO_URL")
