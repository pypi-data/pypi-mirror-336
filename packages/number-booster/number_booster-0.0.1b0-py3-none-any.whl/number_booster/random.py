...
class RandomBoosterStrategy(BaseBoosterStrategy):
    ...
    .. note::
        If the minimum and maximum multipliers are between 1 and 2, and the
        value provided is an integer, the resulting value after boosting may
        remain the same. Specifically, if the multiplier is close to 1 (within
        this range), the boosted integer value may still round back to 1.
        In such cases, the boost will have no significant effect on the
        integer value.
    """
