...
Djblets.AddIntegrationPopupView = Backbone.View.extend({
 ...
 ${_.escape(gettext("There are no integrations currently installed."))}
  ...
  show($button) {
    ...
    const scrollbarWidth = el.offsetWidth - el.clientWidth - popupBorderWidths;
    const popupOffset = $el.offset();
    const popupHeight = Math.floor($window.height() - popupOffset.top - $el.getExtents('bm', 'tb'));
    ...
    if ($el.hasClass('-is-empty')) {
       ...
       */
      const tileWidth = $el.find('.djblets-c-integration').filter(':first').outerWidth(true);
      const winWidth = $window.width();
      const availWidth = winWidth - popupOffset.left - scrollbarWidth - $el.getExtents('m', 'r');
      let popupWidth = Math.max(Math.floor(availWidth / tileWidth), 1) * tileWidth + scrollbarWidth + popupBorderWidths;
       ...
       */
      const newScrollbarWidth = el.offsetWidth - el.clientWidth - popupBorderWidths;
     ...
     * the window resized.
     */
    $(document).one('click.djblets-integrations-popup', () => this.hide());
    $(window).one('resize.djblets-integrations-popup', () => this.hide());
  },
  /**
   * Hide the popup.
   *
