...
(function () {
  const entryTemplate = _.template(`<li class="djblets-c-list-edit-widget__entry"
 ...
 <a href="#" class="djblets-c-list-edit-widget__remove-item"
  ...
  Djblets.Forms.ListEditView = Backbone.View.extend({
    ...
    render() {
      ...
      this._$numRowsEl = this.$(`input[name="${this._fieldName}_num_rows"]`).val(this._numItems);
      return this;
    },
    /**
    ...
    _createDefaultEntry(index) {
      ...
      const renderedRowHTML = this._renderedDefaultRow.replaceAll('__EDIT_LIST_ROW_ID__', rowID).replaceAll('__EDIT_LIST_ROW_INDEX__', index);
    ...
    _updateEntryInputName($entry, index) {
      ...
      if ($inputs.length > 1) {
        $inputs.each((idx, el) => $(el).attr('name', `${fieldName}_value[${index}]_${idx}`));
      } else {
    ...
    _removeItem(e) {
      ...
      if (this._numItems > 1) {
        $entry.remove();
        this._numItems -= 1;
        this._$list.find('.djblets-c-list-edit-widget__entry').each((idx, el) => {
          const $el = $(el);
