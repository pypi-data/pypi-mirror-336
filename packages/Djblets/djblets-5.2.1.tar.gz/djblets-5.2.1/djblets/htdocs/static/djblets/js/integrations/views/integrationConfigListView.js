...
(function () {
  ...
  const IntegrationConfigItemView = Djblets.Config.TableItemView.extend({
    className: 'djblets-c-integration-config djblets-c-config-forms-list__item',
    actionHandlers: {
...
<td></td>`),
    ...
    getRenderContext() {
      ...
      const integration = this.model.collection.options.integrationsMap[integrationID];
      return {
     ...
     */
    _onDeleteClicked() {
      $('<p>').text(gettext("This integration will be permanently removed. This cannot be undone.")).modalBox({
        buttons: [$('<button>').text(gettext("Cancel")), $('<button class="danger">').text(gettext("Delete Integration")).click(() => this.model.destroy({
          beforeSend: xhr => {
            xhr.setRequestHeader('X-CSRFToken', this.model.collection.options.csrfToken);
          }
        }))],
  ...
  Djblets.IntegrationConfigListView = Backbone.View.extend({
    ...
    render() {
      ...
      this.listView.render().$el.removeAttr('aria-busy');
