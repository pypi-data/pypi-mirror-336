...
suite('djblets/forms/views/ConditionValueFormFieldView', function () {
  ...
  function setupConditionSetView(conditionsData) {
    const conditionsTemplate = ` <div class="conditions-field">
  ...
  describe('Rendering', function () {
    it('Loaded rows', function () {
      ...
      const $lastID = conditionSetView.$('input[name=my_conditions_last_id]');
      expect($lastID.val()).toBe('1');

      /* Check the first row. */
      let $row = $rows.eq(0);
      let $choice = $row.find('.conditions-field-choice');
      expect($choice.html()).toBe(['<select name="my_conditions_choice[0]">', '<option value="my-choice-1">My Choice 1</option>', '<option value="my-choice-2">My Choice 2</option>', '</select>'].join(''));
      expect($choice.children('select').val()).toBe('my-choice-1');
      let $operator = $row.find('.conditions-field-operator');
      expect($operator.html()).toBe(['<select name="my_conditions_operator[0]">', '<option value="my-op-1">My Op 1</option>', '<option value="my-op-2">My Op 2</option>', '</select>'].join(''));
      ...
      expect($choice.html()).toBe(['<select name="my_conditions_choice[1]">', '<option value="my-choice-1">My Choice 1</option>', '<option value="my-choice-2">My Choice 2</option>', '</select>'].join(''));
      expect($choice.children('select').val()).toBe('my-choice-2');
      $operator = $row.find('.conditions-field-operator');
      expect($operator.html()).toBe(['<select name="my_conditions_operator[1]">', '<option value="my-op-3">My Op 3</option>', '<option value="my-op-4">My Op 4</option>', '</select>'].join(''));
  ...
  describe('Actions', function () {
    ...
    it('Changing a choice updates model', function () {
      ...
      const $rows = conditionSetView.$('.conditions-field-row');
      const condition = conditionSetView.model.conditions.at(0);
      $rows.eq(0).find('.conditions-field-choice select').val('my-choice-2').trigger('change');
    ...
    it('Changing an operator updates model', function () {
      ...
      const $rows = conditionSetView.$('.conditions-field-row');
      const condition = conditionSetView.model.conditions.at(0);
      $rows.eq(0).find('.conditions-field-operator select').val('my-op-2').trigger('change');
      expect(condition.get('operator').id).toBe('my-op-2');
    });
  });
  describe('Model events', function () {
    let conditionSetView, conditionSet, condition, $row, $choice, $operator, $valueWrapper;
    beforeEach(function () {
    ...
    describe('Choice changed', function () {
      it('Updates UI state', function () {
        condition.set('choice', conditionSet.choices.get('my-choice-2'));
...
//# sourceMappingURL=conditionSetViewTests.js.map