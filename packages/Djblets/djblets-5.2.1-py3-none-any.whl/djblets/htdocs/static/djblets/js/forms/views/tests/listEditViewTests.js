 ...
 <span class="fa fa-plus"></span>
  ...
  const makeView = function makeView() {
    ...
    const $el = $(formTemplate({
      renderedDefaultRow: renderedDefaultRow.replaceAll('__EDIT_LIST_ROW_ID__', '0').replaceAll('__EDIT_LIST_ROW_INDEX__', '0'),
  ...
  describe('Removal', function () {
    it('With no values', function () {
      ...
      expect($input.val()).toEqual('');
      expect($input.attr('id')).toMatch(/^_value_djblets-list-edit-row\d+/);
      expect($input.attr('name')).toEqual('_value[0]');
    });
    ...
    it('With multiple values', function () {
      ...
      const $inputThree = view.$('.djblets-c-list-edit-widget__input').eq(2);
      ...
      expect($inputOne.attr('id')).toMatch(/^_value_djblets-list-edit-row\d+/);
      expect($inputOne.attr('name')).toEqual('_value[0]');
    });
  });
  describe('Addition', function () {
    it('With values', function () {
      ...
      const $inputOne = view.$('.djblets-c-list-edit-widget__input').eq(0);
      expect($inputOne.val()).toEqual('One');
      expect($inputOne.attr('id')).toEqual('_value_UUID1');
      expect($inputOne.attr('name')).toEqual('_value[0]');
      const $inputTwo = view.$('.djblets-c-list-edit-widget__input').eq(1);
      expect($inputTwo.val()).toEqual('Two');
      expect($inputTwo.attr('id')).toEqual('_value_UUID2');
      expect($inputTwo.attr('name')).toEqual('_value[1]');
      const $inputThree = view.$('.djblets-c-list-edit-widget__input').eq(2);
      ...
      expect($inputThree.attr('id')).toEqual('_value_UUID3');
      expect($inputThree.attr('name')).toEqual('_value[2]');
      const $inputFour = view.$('.djblets-c-list-edit-widget__input').eq(3);
      expect($inputFour.val()).toEqual('');
      expect($inputFour.attr('id')).toMatch(/^_value_djblets-list-edit-row\d+/);
    ...
    it('With no values', function () {
      ...
      expect($inputTwo.attr('id')).toMatch(/^_value_djblets-list-edit-row\d+/);
      expect($inputTwo.attr('name')).toEqual('_value[1]');
    });
  });
});
