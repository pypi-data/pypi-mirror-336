...
suite('djblets/forms/views/ConditionValueFormFieldView', function () {
  ...
  describe('Methods', function () {
    describe('getValue', function () {
      it('<input>', function () {
        const view = createValueField('<input type="text" value="abc123">');
        expect(view.getValue()).toBe('abc123');
      });
      it('<select>', function () {
        const view = createValueField(['<select>', '<option value="1">One</option>', '<option value="2" selected="selected">Two</option>', '</select>'].join(''));
    ...
    describe('setValue', function () {
      ...
      it('<select>', function () {
        const view = createValueField(['<select>', '<option value="1">One</option>', '<option value="2">Two</option>', '</select>'].join(''));
        view.setValue('2');
...
//# sourceMappingURL=conditionValueFormFieldViewTests.js.map