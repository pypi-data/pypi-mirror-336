...
<% } %>`),
  ...
  render() {
    ...
    const renderItem = this.options.multivalued ? () => '' : this.renderOption;
    const selectizeOptions = _.defaults(this._selectizeOptions, {
      ...
      load(query, callback) {
        self.loadOptions(query, data => callback(data.filter(item => !self._selectedIDs.has(item.id))));
      },
      onChange(selected) {
        if (selected) {
  ...
  _onItemSelected(item, addToInput) {
    if (this.options.multivalued) {
      const $item = $(`<${this.optionTagName}>`).html(this.renderSelectedOption(item));
      const $items = this._$selected.children();
      const text = $item.text();
      if (this.autoAddClose) {
        $('<span class="remove-item ink-i-delete-item">').click(() => this._onItemRemoved($item, item)).appendTo($item);
      }
      let attached = false;
