...
suite('djblets/integrations/views/IntegrationConfigListView', function () {
  const template = _.template(`<div class="djblets-c-integration-configs">
 <div class="djblets-l-config-forms-container">
  ...
  describe('Configurations', function () {
    ...
    describe('Rendering', function () {
      it('Icon', function () {
        const $icon1 = $row1.find('.djblets-c-integration-config__name img');
        const $icon2 = $row2.find('.djblets-c-integration-config__name img');
        const $icon3 = $row3.find('.djblets-c-integration-config__name img');
        const $icon4 = $row4.find('.djblets-c-integration-config__name img');
        ...
        expect($icon1.attr('srcset')).toBe('data:test,int1 1x, data:test,int1@2x 2x');
        expect($icon2.attr('srcset')).toBe('data:test,int2 1x, data:test,int2@2x 2x');
        expect($icon3.attr('srcset')).toBe('data:test,int3 1x, data:test,int3@2x 2x');
        expect($icon4.attr('srcset')).toBe('data:test,int1 1x, data:test,int1@2x 2x');
      });
      it('Name', function () {
        const $name1 = $row1.find('.djblets-c-integration-config__name a');
        const $name2 = $row2.find('.djblets-c-integration-config__name a');
        const $name3 = $row3.find('.djblets-c-integration-config__name a');
        const $name4 = $row4.find('.djblets-c-integration-config__name a');
      ...
      it('Integration name', function () {
        const $intName1 = $row1.find('.djblets-c-integration-config__integration-name');
        const $intName2 = $row2.find('.djblets-c-integration-config__integration-name');
        const $intName3 = $row3.find('.djblets-c-integration-config__integration-name');
        const $intName4 = $row4.find('.djblets-c-integration-config__integration-name');
      ...
      it('Status', function () {
        ...
        expect($row1.find('.djblets-c-config-forms-list__item-state').text()).toBe('Enabled');
        expect($row2.find('.djblets-c-config-forms-list__item-state').text()).toBe('Enabled');
        expect($row3.find('.djblets-c-config-forms-list__item-state').text()).toBe('Disabled');
        expect($row4.find('.djblets-c-config-forms-list__item-state').text()).toBe('Enabled');
      });
    });
    describe('Actions', function () {
      it('Delete', function () {
        ...
        spyOn(config, 'sync');
        spyOn($.fn, 'modalBox').and.callFake(options => options.buttons[1].click());
...
//# sourceMappingURL=integrationConfigListViewTests.js.map