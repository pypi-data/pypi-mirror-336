  ...
  const BaseConditionRowView = Backbone.View.extend({
    ...
    initialize(options) {
      this.listenTo(this.model, 'destroy', () => {
        this.$el.slideUp(options.rowAnimationSpeedMS, this.remove.bind(this));
      });
    },
    /**
...
<% } %>`),
    ...
    render() {
      ...
      this.conditionSet.choices.each(choice => {
        this._$choice.append($('<option>').val(choice.id).text(choice.get('name')));
      });
       ...
       */
      this._$operator = $('<select>').attr('name', `${fieldName}_operator[${rowNum}]`).appendTo(this.$el.children('.conditions-field-operator'));
    ...
    _onChoiceChanged() {
      ...
      choice.operators.each(operator => {
        this._$operator.append($('<option>').val(operator.id).text(operator.get('name')));
      });
  ...
  Djblets.Forms.ConditionSetView = Backbone.View.extend({
    ...
    render() {
      ...
      this.listenTo(conditions, 'add', condition => this._addConditionRow(condition));
    ...
    _addConditionRow(condition, $rowEl) {
      const RowViewCls = condition.get('valid') ? ConditionRowView : DisabledConditionRowView;
