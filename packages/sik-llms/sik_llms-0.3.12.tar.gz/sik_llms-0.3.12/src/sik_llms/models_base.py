...
class Tool(BaseModel):
    ...
    def to_json(self) -> dict[str, object]:  # noqa: PLR0912
        """
        Convert the tool to a standard JSON Schema format.

        This produces a schema that follows JSON Schema standards
        and can be adapted for different AI providers if needed.
        """
        properties = {}
        required = []
        ...
        for param in self.parameters:
            ...
            # Special handling for object types with properties
            if json_type == 'object' and 'properties' in param_dict:
                # Standard practice is to set additionalProperties: false
                param_dict['additionalProperties'] = False

                # Handle nested required properties
                if param_dict.get('properties') and isinstance(param_dict['properties'], dict):
                    # This is an approach that works for both OpenAI and Anthropic:
                    # Set nested properties as required only if they're truly required
                    nested_required = []
                    for prop_name, prop_schema in param_dict['properties'].items():
                        # You might need to adjust this based on how your nested properties
                        # indicate their required status
                        if prop_schema.get('required') is True:
                            nested_required.append(prop_name)

                    if nested_required:
                        param_dict['required'] = nested_required
        ...
        # Set additionalProperties: false at the top level by default
        schema["schema"]["additionalProperties"] = False

        # Remove any default values that might be present
        self._remove_defaults_recursively(schema)
