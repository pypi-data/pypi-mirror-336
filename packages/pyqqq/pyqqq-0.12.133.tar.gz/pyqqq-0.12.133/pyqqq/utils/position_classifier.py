...
class PositionClassifier:
    ...
    DEF_DEFAULT_TYPE = 'auto' # 태깅되지 않은 종목을 무엇으로 간주할지 'auto' 'direct'

    def __init__(self, simple_data_api, kv_store_collection, default_type='auto'):
    ...
    def set_initial_position(self) -> None:
        ...
        for pos in cur_pos:
            ...
            if kv_auto_pos is not None and asset_code in kv_auto_pos:
                self.auto_positions[asset_code] = min([quantity, kv_auto_pos[asset_code], 0])
    ...
    def set_initial_order(self):
        ...
        kv_auto_tag_orders = self.kv_store.get(sefl.DEF_TAG_AUTO_ORDER_KEY) or []
        ...
        for order in cur_order:
            ...
            if order_no in kv_auto_orders:
                self.auto_pending_orders = min(quantity, kv_auto_orders[order_no])
            elif order_no in kv_direct_orders:
                self.direct_pending_orders = min(quantity, kv_direct_orders[order_no])
            else:
                if self.DEF_DEFAULT_TYPE == 'auto':
                    self.auto_pending_orders = min(quantity, kv_auto_orders[order_no])
                elif self.DEF_DEFAULT_TYPE == 'direct':
    ...
    def clear_orders(self) -> None:
        ...
        장 시작 전에 호출해주면 좋은 함수
        """
        self.logger.info(f"clear_orders")
        self.auto_pending_orders = {}
        self.direct_pending_orders = {}
        self.tagged_auto_pending_orders = []
        self.kv_store.remove(self.DEF_AUTO_ORDER_KEY)
        self.kv_store.remove(self.DEF_DIRECT_ORDER_KEY)
        self.kv_store.remove(self.DEF_TAG_AUTO_ORDER_KEY)
    ...
    def get_sellable_quantity_by_auto(self, asset_code, quantity) -> int:
        ...
        ret = 0
        if asset_code in self.auto_positions:
            diff = min(auto_positions[asset_code], remained)
            ret += diff
            remained -= diff

        if self.DEF_DEFAULT_TYPE == 'auto':
            if asset_code in self.direct_positions:
                diff = min(direct_positions[asset_code], remained)
                remained -= diff
    ...
    def _on_pending_order_update(self, status, order: StockOrder):
        ...
        if order.side  == OrderSide.BUY:
            if status in ["partial", "completed"]:
                ...
                elif order_no in self.tagged_auto_pending_orders or self.DEF_DEFAULT_TYPE == 'auto':
