...
class RemoveLicenseResponse(BaseModel):
    """
    Attributes:
        business_phones (Optional[list[str]]):
        display_name (Optional[str]): The full name displayed for the user. Example: John Doe.
        given_name (Optional[str]): The user's first name. Example: John.
        id (Optional[str]): A unique identifier for the user or action. Example: b4e7abb3-fc22-4610-9c3b-28ae703b3bec.
        job_title (Optional[str]): The user's professional job title. Example: Sales Manager.
        mail (Optional[str]): The user's primary email address. Example: john.doe@contoso.com.
        mobile_phone (Optional[str]): User's mobile phone number. Example: +1 425 555 0100.
        odata_context (Optional[str]): The Odata context
        office_location (Optional[str]): Location of the user's office. Example: 18/2111345.
        preferred_language (Optional[str]): The user's chosen language for communication. Example: en-US.
        surname (Optional[str]): User's last name or family name. Example: Sharma.
        user_principal_name (Optional[str]): The principal name associated with the user account. Example:
    ...
    @classmethod
    def from_dict(cls: Type["RemoveLicenseResponse"], src_dict: Dict[str, Any]):
