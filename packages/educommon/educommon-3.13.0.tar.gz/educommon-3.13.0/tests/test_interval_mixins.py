...
class DateIntervalMixinTestCase(TestCase):
    ...
    def test_intersections(self):
        ...
        models_params = (
            ...
            (
                DateIntervalModel_2_2,
                dict(field1='qwe', field2='asd', field3='zxc')
            ),
        )
    ...
    def test_date_in_intervals_filter(self):
        ...
        for day, exclude, test_ids in test_data:
            ...
            if exclude:
                q = ~q
            result_ids = model.objects.filter(
                q
        ...
        for pk, date_from, date_to in models_data:
            for include_bounds in (True, False):
                for day in (date_from, date_to):
                    ...
                    result_ids = model.objects.filter(q, pk=pk).values_list(
                        'pk', flat=True
                    )
                    self.assertEqual(
    ...
    def test_intersection_daterange_filter(self):
        ...
        for date_begin, date_end, exclude, test_ids in test_data:
            query_filter = model.get_intersection_daterange_filter(
                date_begin, date_end
            )
    ...
    def test_intersection_daterange_filter_with_null_field(self):
        ...
        for date_begin, date_end, test_ids in test_data:
            ...
            result_2 = model.objects.filter(query_filter).exclude(
                pk=result_1.pk
            ).first()
...
class DateTimeIntervalMixinTestCase(TestCase):
    ...
    def test_intersection_daterange_filter(self):
        ...
        test_data = (
            ...
            # за пределами справа
            (
                datetime(2020, 1, 31, 16, 1),
                datetime(2020, 1, 31, 17),
                True,
                [1]
            ),
            # проверка вхождения слева
            (
                datetime(2020, 1, 1, 15),
                datetime(2020, 1, 1, 15, 48, 21),
                False,
                [1]
            ),
            # проверка вхождения справа
            (
                datetime(2020, 1, 31, 16),
                datetime(2020, 1, 31, 17),
                False,
                [1]
            ),
            ...
            (
                datetime(2020, 1, 1, 15, 50),
                datetime(2020, 1, 31, 15, 59),
                False,
                [1]
            ),
        )
...
class ExtraIntervalTestCase(SimpleTestCase):
    ...
    def test(self):
        self.assertEqual(
            ExtraIntervalModel.interval_field_names,
            ('start_date', 'end_date')
        )
        self.assertEqual(
            ExtraIntervalModel.no_intersections_for, ('name',)
        )
        ...
        start_date_field, end_date_field, name_field = map(
            ExtraIntervalModel._meta.get_field,
