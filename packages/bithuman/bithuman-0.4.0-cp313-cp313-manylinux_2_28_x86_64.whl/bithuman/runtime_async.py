...
class AsyncBithuman(Bithuman):
    ...
    async def set_avatar_model(self, avatar_model_path: str) -> None:
        ...
        await loop.run_in_executor(None, super().set_avatar_model, avatar_model_path)
    ...
    async def validate_token(self, token: str) -> bool:
        ...
        """
        # Use loop.run_in_executor to run the synchronous validate_token method in a thread pool
        loop = self._loop or asyncio.get_running_loop()
        return await loop.run_in_executor(None, self.generator.validate_token, token)
        
    async def set_token(self, token: str) -> bool:
        """Set and validate the token for the Bithuman Runtime asynchronously.
        
        This pure async implementation validates and sets the token for the runtime.
        It validates the provided token against the hardware fingerprint
        ...
        Returns:
            bool: True if token is valid and set successfully.
            
        Raises:
            ValueError: If the token is invalid.
        """
        # Use loop.run_in_executor to run the validation in a thread pool
        loop = self._loop or asyncio.get_running_loop()
        is_valid = await loop.run_in_executor(None, self.generator.validate_token, token)
