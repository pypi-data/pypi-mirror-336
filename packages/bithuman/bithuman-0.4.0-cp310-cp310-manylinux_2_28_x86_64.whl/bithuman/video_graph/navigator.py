...
class VideoGraphNavigator:
    ...
    def from_workspace(
        ...
        """Create a navigator from the avatar model."""
        logger.info(f"Loading avatar model from {avatar_model_path}")
        ...
        for video in video_configs.videos:
            video.video_file = str(Path(avatar_model_path) / video.video_file)
    ...
    def next_n_frames(
        ...
        if self.frame_buffer:
            ...
            if video:
                if (on_user_speech and video.stop_on_user_speech) or (
                    on_agent_speech and video.stop_on_agent_speech
                ):
                    self.reset_buffer()
                    logger.trace(
