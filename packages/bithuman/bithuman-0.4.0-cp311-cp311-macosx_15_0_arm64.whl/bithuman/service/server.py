...
class SessionWorker(ABC):
    ...
    def __init__(
        ...
        self.runtime.set_avatar_model(
            avatar_model_path=self.init_request.avatar_model_path, load_data=False
        )
    ...
    def run(self) -> None:
        ...
        with self.init_semaphore:
            logger.info(f"Loading avatar model for client {self.client_id}")
            self.runtime.load_data()
            logger.info(f"Avatar model loaded for client {self.client_id}")
