...
from pyspark.sql.connect.profiler import detect_client_context
...
class SparkConnectClient(object):
    ...
    def metadata(self) -> Iterable[Tuple[str, str]]:
        ...
        A list of tuples (key, value)
        """
        return self._builder.metadata() + getattr(self.thread_local, "extra_request_metadata", [])
    ...
    def _execute_and_fetch(
        ...
        for response in self._execute_and_fetch_as_iterator(
            req, observations, extra_request_metadata or []
        ):
            if isinstance(response, StructType):
                schema = response
            elif isinstance(response, pa.RecordBatch):
