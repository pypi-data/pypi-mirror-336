...
class ExecutePlanResponseReattachableIterator(Generator):
    ...
    def send(self, value: Any) -> pb2.ExecutePlanResponse:
        ...
        if ret.HasField("result_complete"):
            # Result complete does not mean that the stream has been consumed
            # fully. To make sure we receive all trailers, consume all responses.
            is_last = not self._has_next(is_last=True)
            assert is_last, "Should not have any messages past 'result_complete'."
            self._release_all()
        else:
            self._release_until(self._last_returned_response_id)
        return ret

    def _has_next(self, is_last=False) -> bool:
        if self._result_complete:
            # After response complete response
            return False
        else:
            # There is a special case to call has next to consume the iterator of the
            # GRPC response stream fully. Semantically, the stream is already done
            # because we have received the `result_complete` message, but we still
            # need to let GRPC finish its own stream handling as well.
            result_complete = self._result_complete or is_last
            try:
                for attempt in self._retrying():
                    with attempt:
                        ...
                        if not result_complete and not has_next:
                            while not has_next:
    ...
    def _release_until(self, until_response_id: str) -> None:
        ...
        # BEGIN-EDGE
        if not self._send_release_until:
            return
        # END-EDGE
