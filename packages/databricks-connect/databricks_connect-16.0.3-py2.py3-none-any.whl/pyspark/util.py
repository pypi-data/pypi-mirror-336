...
def _normalize_path_for_spark_client_on_windows(sc: "SparkContext", path: str) -> str:
    from pyspark.sql import SparkSession

    session = SparkSession._instantiatedSession
    if (
        session is not None
        and session.conf.get("spark.databricks.service.client.enabled") == "true"
        and platform.system() == "Windows"
        and session.conf.get("spark.databricks.service.client.windows.path.normalizationEnabled")
        == "true"
    ):
...
# BEGIN-EDGE
def is_photon() -> bool:
    return os.getenv("ENABLE_PHOTON_TESTS", "false").lower() == "true"


def is_uc() -> bool:
    return os.getenv("ENABLE_UNITY_CATALOG_TESTS", "false").lower() == "true"


def is_serverless() -> bool:
    return os.getenv("ENABLE_SERVERLESS_TESTS", "false").lower() == "true"
