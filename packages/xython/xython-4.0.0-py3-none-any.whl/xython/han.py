...
import scolor, pynal, basic_data
...
class han:
	...
	def __init__(self, file_name=""):
		...
		self.var_action_name_vs_parameter_set_id = self.vars["han"]["action_name_vs_parameter_set_id"]
		self.var_action_name_vs_manual = self.vars["han"]["action_name_vs_manual"]
		self.current = {}

		# 한글 application이 실행중인지 확인한다
		if self.__check_hwp_object():
	...
	def apply_font_style_for_selection(self, my_range):
		...
		:return:
		"""
		if self.vars["han"]["basic_underline"]:
			my_range.Font.Underline = 1
		if self.vars["han"]["basic_size"]:
			my_range.Font.Size = self.vars["han"]["basic_size"]
		if self.vars["han"]["basic_bold"]:
			my_range.Font.Bold = 1
		if self.vars["han"]["rgb_int"]:
			my_range.Font.TextColor.RGB = self.vars["han"]["rgb_int"]
	...
	def check_action_name_as_dic(self, input_action_name):
		...
		parameter_name = self.vars["han"]["action_name_vs_parameter_set_id"][input_action_name]
		...
		for one in self.vars["han"]["parameter_set_id_vs_parameters"][parameter_name]:
	...
	def check_options(self, input_parameter):
		...
		try:
			result = self.vars["han"]["parameter_vs_parameter_option"][input_parameter]
		except:
			result = "없음"
		if not result:
			result = "없음"
	...
	def check_parameter_set_id(self, input_action):
		...
		result = "없음"
		try:
			result = self.vars["han"]['action_name_vs_parameter_set_id'][input_action]
		except:
			pass
		return result
	...
	def check_parameters(self, input_parameter_set_id):
		...
		# all_data = basic_data_for_han.basic_data().vars
		try:
			result = self.vars["han"]["parameter_set_id_vs_parameters"][input_parameter_set_id]
		except:
			result = "없음"
		if not result:
			result = "없음"
	...
	def delete_nth_word_from_selection(self):
		"""
		선택한 영역을 기준으로 왼쪽 여역을 기준으로 n번째 단어 삭제
	...
	def delete_word_at_cursor(self):
		"""
		커서가 있는 단어1개 삭제하는데, 만약 영역이 선택되었다면, 제일 왼쪽의 단어가 삭제된다
	...
	def delete_xline_in_table(self, table_no, x):
		"""
		현재 문서의 n번째 테이블의 x번째 가로줄 하나를 삭제
	...
	def delete_yline_in_table(self, table_no, y):
		"""
		현재 문서의 n번째 테이블의 y번째 세로줄 하나를 삭제
	...
	def filter_all_action_name_by_input_text(self, input_text):
		...
		"""
		result = []
		for one_action_name in list(self.vars["han"]['action_name_vs_manual'].keys()):
	...
	def free_basic_font_style(self):
		...
		:return:
		"""
		self.vars["han"]["apply_basic_font"] = False
		self.vars["han"]["basic_underline"] = False
		self.vars["han"]["basic_size"] = False
		self.vars["han"]["basic_bold"] = False
		self.vars["han"]["rgb_int"] = False
	...
	def get_all_action_name(self):
		...
		:return:
		"""
		result = list(self.vars["han"]['action_name_vs_manual'].keys())
		return result
	...
	def get_char_no_at_cursor(self):
		...
		#print(self.han_program.KeyIndicator())
	...
	def set_basic_font_style(self, **input_dic):
		...
		:return:
		"""
		self.vars["han"]["apply_basic_font"] = True

		if "size" in input_dic.keys():
			self.vars["han"]["basic_size"] = input_dic["size"]
		if "color" in input_dic.keys():
			self.vars["han"]["rgb_int"] = self.xcolor.change_scolor_to_rgbint(input_dic["color"])
		if "bold" in input_dic.keys():
			self.vars["han"]["basic_bold"] = True
		if "underline" in input_dic.keys():
			self.vars["han"]["basic_underline"] = True
		if "italic" in input_dic.keys():
			self.vars["han"]["basic_italic"] = True
	...
	def set_font_color_for_selection_by_scolor(self, input_scolor):
		"""
		...
		:return:
		"""
		input_rgb = self.xcolor.change_scolor_to_rgb(input_scolor)
	...
	def paint_background_for_selection_by_scolor(self, input_scolor="red50"):
		"""
		선택영역을 빨간색으로 칠하기
	...
	def paint_font_for_selection_by_scolor(self, input_scolor="red50"):
		"""
		선택영역 scolor형식으로 색칠하기
	...
	def set_font_underline_for_selection_with_scolor(self, input_scolor="bla", underline_position = 1):
		"""
		선택영역 rgb값으로 색칠하기
	...
	def insert_picture_at_selection_end(self, file_full_name="D:/my_code/icons/bibi2.png", input_w=100, input_h=100):
		self.han_program.InsertPicture(file_full_name, Embedded=True,  sizeoption=1, width=input_w, height=input_h)  # 이미지 삽입
	...
	def write_today_at_cursor(self):
		...
		:return:
		"""
		yyyymmdd = self.xtime.get_yyyy_mm_dd_for_today()
