...
class xymap:
	def __init__(self):
		...
		self.main_map = None
		self.all_cxy = xymap_basic_data.basic_data().vars
		...
		self.xcolor_list = ['lightred', 'gray', 'lightgreen', 'pink', 'lightblue', 'beige', 'black', 'darkgreen', 'darkblue',
	...
	def calculate_midpoint_and_angle(self, coord1, coord2):
		"""
	...
	def change_address_to_cxy(self, input_address=""):
		...
		for address_part in all_address_l1d[::-1]:
			...
			found_num = self.xre.search_with_jf_sql("[숫자:1~]", address_part)
	...
	def change_scolor_to_text_rgb(self, input_scolor):
		...
		:return:
		"""
		rgb_list = self.xcolor.change_scolor_to_rgb(input_scolor)
	...
	def draw_arrow_marker_one(self, input_cxy, i_angle, i_color):
		"""
		화살표 마커를 1개만 표시
		"""
	...
	def draw_line_with_number_marker(self, input_cxy_list, popup_list=None, tooltip_list=None, color_list="blue", use_line_detail=False, start_no=1):
		input_cxy_list = self._check_l2d(input_cxy_list)
	...
	def draw_line_with_numbered_marker(self, input_cxy_list, marker_style, input_popup, input_tool_tip, input_bgcolor, icon_style, input_line_color, icon_outline_shape, icon_border_width=2,
	...
	def draw_marker(self, input_cxy_list, i_tooltip=None, i_draggable_tf=False, i_popup=None, icon_style="이름 또는 index", icon_angle=0, icon_color=None, bg_color=None):
		"""
		마커를 넣는 것
	...
	def draw_marker_cluster(self, input_cxy_list, i_tooltip=None, i_popup=None, ):
		"""
	...
	def draw_marker_with_l2d(self, input_l2d):
		"""
		마커에 표시하는 아이콘을 선택할수가 있다
		icon의 색은 hex도 가능하다.
		"""
	...
	def draw_marker_with_list_2d_as_cx_cy_tooltip_menu(self, input_l2d):
		...
		"""
		input_l2d = self.xutil.change_any_data_to_list_2d(input_l2d)
		print(input_l2d)
		...
		icon_dic = {}
		icon_count = len(self.xcolor_list)
		for index, one in enumerate(list(unique_data)):
			icon_dic[one] = self.xcolor_list[divmod(index, icon_count)[1]]
	...
	def draw_marker_with_menu(self, cxy, tool_tip, menu):
	...
	def draw_marker_with_no(self, input_cxy_list, tooltip_l1d=None, color_l1d=None, start_no=1):
		"""
		숫자 번호를가진 마커를 만드는 것
	...
	def draw_marker_with_numbered_icon(self, input_cxy_l2d, i_bg_color_l1d, i_popup_l1d, i_tooltip_l1d, i_icon_shape="marker", i_start_no=1, i_icon_textcolor="black", i_border_width=2,
									   i_icon_linecolor="black"):
		"""
		icon=None, Font-Awesome의 아이콘 이름
		icon_shape=None, 아이콘의 모양, [circle, circle-dot, doughnut, rectangle, rectangle-dot, marker] 이렇게 6개가 적용가능
	...
	def draw_polyline(self, input_map_obj, input_cxy_list):
		"""
		다각형의 닫힌 도형을 만드는 것이다
	...
	def get_test_cxy_list(self, x_count=5, y_count=4, distance=0.1):
		...
		for x in range(x_count):
			for y in range(y_count):
				result.append([self.vars["basic_cxy"][0] + x * distance, self.vars["basic_cxy"][1] + y * distance])
		return result
	...
	def insert_plugin_for_filter(self, input_data):
		"""
		menu group와같이 비슷한 형태로 사용되는것으로, filter라는 개념으로 상요합니다
	...
	def make_sub_menu_group(self, top_menu_title, category_location, input_title, all_data_set):
		...
		:return:
		"""
		main_map_object = self.make_main_map_object("", 8)
		dic_sub_menus = {}
	...
	def make_top_menu_group(self, top_menu_name):
		...
		main_map_object.add_child(top_menu_name)
		return [top_menu_obj, main_map_object]
	...
	def set_for_line_detail(self, i_style=None, i_scolor=None, i_width=None, i_alpha_0to1=0):
		"""
		라인의 세부정보를 설정하는 것
		...
		if i_scolor: self.line_detail["color"] = i_scolor
	...
	def setup_dragging_by_0or1(self, input_tf=False):
		"""
	...
	def setup_height(self, input_no):
		"""
		화면의 크기(높이)를 설정
	...
	def write_text_in_box(self, input_cxy, input_text=""):
		"""
		사각형안에 글씨쓰기
	...
	def write_text_in_circle(self, input_cxy, input_text="123"):
		"""
		원안에 글자를 쓰도록 만든것
	...
	def write_text_with_box(self, input_cxy="", input_text="", i_dgree=0):
		"""
		사각형안에 글씨쓰기
		...
		folium.map.Marker(input_cxy,
						  icon=DivIcon(
							  icon_size=(150, 50),
							  icon_anchor=(0, 0),
							  html=f"""<div style="display: flex; justify-content: center; align items: center; border: 2px solid blue; 
							background-color: lightblue; padding: 10px;transform-origin:center; transform:translate(-50%, -50%) rotate(({i_dgree + 90}deg);p style="margin: 0;">{input_text}</p></div>""", )
						  ).add_to(self.main_map)

	def write_text_with_circle(self, input_cxy="", input_text="123"):
		"""
		원안에 글자를 쓰도록 만든것
		사각형의 형태가 기본이라, 너무 많이 쓰면 원을 조금 넘어간다
