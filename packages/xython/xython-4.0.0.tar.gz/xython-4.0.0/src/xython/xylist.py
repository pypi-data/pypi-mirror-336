...
class xylist(object):
	...
	2. 리스트에서 자주 사용하는 함수를 xylist의 객체로 만들었다
	3. 맨 처음 이 클래스의 이름을 xylist 로 만들었는데, 1차원 셀주소의 리스트 묶음으로 사용하고 있어서, 1로 시작하는 리스트 기능을 만들었기때문에, xylist라고 만듦
	4. for문을 사용해서 나타나는 각 객체를 xylist의 객체로 만듦
	5. i로 시작하는 것은 index의 의미로 기존의 리스트인 0부터 시작하는 의미
	6. pcell에서 리스트 값을 돌려주는 기본 객체를 xylist로 적용
			...
			if "~" in key:
				if ":" in key:
					...
					if no1 == no2 and no1 != "" and no2 != "":
						return xylist(self.main_list[int(no1) - 1])
					else:
						if no1 == "":
							no1 = None
						else:
						...
						if no2 == "":
							...
							no2 = int(no2)
						return xylist(self.main_list[no1:no2])
			elif ":" in key:
				# ~은 없고 :만 있을때
				no1, no2 = key.split(":")
				if no1 == "" and no2 == "":
					...
					return xylist(self.main_list[:int(no2)])
				elif no2 == "":
					return xylist(self.main_list[int(no1) - 1:])
				else:
					return xylist(self.main_list[int(no1) - 1:int(no2)])
			else:
				value = self.main_list[int(key.strip()) - 1]
				if type(value) == type([]) or type(value) == type(()):
					return xylist(list(value))
				else:
					return value
	...
	def __init__(self, input_data=None):
		...
		xylist의 내부적으로는 일반 list와 같은 형태로 계산이 된다

		만약 xylist가 들어오면 그 자료형으로 되는 것
		...
		elif type(input_data) == type(xylist):  # xylist일때
			self.main_list = input_data
	...
	def __setitem__(self, input_no, input_data):
		"""
		xylist[2] ="abc"와같이 값을 집어 넣을때 실행되는 것
		값을 입력할때 사용되는 부분
		"""
	...
	def check_max_len(self, input_data_2d):
		...
		"""
		temp = 0
		for list_1d in input_data_2d:
			temp = max(temp, len(list_1d))
		return temp
	...
	def filter_by_nth_style(self, input_list, nth_style, nth_only=""):
		...
		:return:
		"""
		if type(input_list) == type(xylist):
			print("xylist 입니다")
		...
		if nth_style == "even":
			...
			for one in c_list:
				aaa = self.xre.search_all_by_jf_sql("[+-:0~1][숫자:1~][~:1~1][+-:0~1][숫자:1~]", one)  # 3~5
				bbb = self.xre.search_all_by_jf_sql("[+-:0~1][숫자:1~][+-:1~1][숫자:1~]", one)  # 3+5,-3-5
				ccc = self.xre.search_all_by_jf_sql("[+-:0~1][숫자:1~]", one)  # 숫자만 분리
		...
		nth_jf = None
		nth_full = self.xre.search_all_by_jf_sql("[+-:0~1][숫자:1~]n[+-:0~1][숫자:1~]", nth_style)
		if nth_full:
			nth_jf = nth_full[0][0]
	...
	def get_dimension_for_input_data(self, input_any_data=""):
		...
		if data_style == "list_2d_same_len" or data_style == "list_2d":
			result = "2d"
		elif data_style == "list_1d" or data_style == "list_str_mix":
			result = "1d"
		else:
			result = "0d"
		return result
	...
	def insert_value_by_y(self, y_no, input_data=None):
		...
		if list_style in ["list_2d_same_len", "list_2d"]:
			for ix in range(len(self.main_list)):
				self.main_list[ix].insert(y_no - 1, input_data)
		elif list_style in ["list_1d", "list_str_mix"]:
	...
	def to_general_list(self, input_data=""):
		"""
		입력되는 모든 자료는 일반 list나 xylist만 가능
		...
		if type(xylist) == type(input_data):
			result = list(input_data)
		elif type(input_data) == type([]):
			result = input_data
		else:
	...
	def to_list(self):
		"""
		기본의 자료를 돌려주는 것이다
		xylist가 아닌 일반 list형으로 만들어서 돌려주는 것
	...
	def to_list_2d_with_same_len(self):
		"""
		...
		:return:
		"""
		list_2d = self.to_list_2d()
		max_len = self.check_max_len(list_2d)
		new_list_obj = []
		for list_1d in list_2d:
			differ_no = max_len - len(list_1d)
			if differ_no > 0:
				list_1d.extend([None] + differ_no)
			new_list_obj.append(list_1d)
		return new_list_obj
