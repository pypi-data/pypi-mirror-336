...
___CONFIG_PATH = '.upp.config'

___CONFIG_PATH_BAK = '_upp.config'
...
def to_print_date_file(*args, file_path='date_file', file_name=None, mode='a'):
    """
    将 文本输出到 date_file 中
    例如
    """
    to_txt(data_list=[to_print(*args)],
           file_name=(str(file_name) + '_' if file_name is not None else '') + datetime.today().strftime('%Y-%m-%d'),
...
def to_print_file(*args, file_name=None, mode='a'):
    """
...
def to_print_txt(*args, file_name=None, mode='a'):
    """
...
def to_log_file(*args, file_name=None, time_prefix=True, mode='a'):
    """
...
def to_log_txt(*args, file_name=None, time_prefix=True, mode='a'):
    """
...
def to_java_more(a1=None, a2=None, a3=None, a4=None, a5=None):
    r = list(map(lambda x: to_java(x), [a1, a2, a3, a4, a5]))
...
def get_data_from_path(file_name='config', file_path=None):
    data = get_data_from_path_detail(file_name, file_path, ___CONFIG_PATH)
    return data if data else get_data_from_path_detail(file_name, file_path, ___CONFIG_PATH_BAK)


def get_data_from_path_detail(file_name='config', file_path=None, path_name=___CONFIG_PATH):
...
def set_data_in_path(file_name='config', data=None, file_path=''):
    if data is None:
        data = {}
    config_path = file_path + '/' + ___CONFIG_PATH
    if not os.path.exists(config_path):
