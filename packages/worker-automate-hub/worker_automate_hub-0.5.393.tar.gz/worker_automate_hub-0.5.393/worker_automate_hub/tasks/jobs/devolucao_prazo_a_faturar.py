...
async def devolucao_prazo_a_faturar(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    ...
    """
    try:
        console.print("ESTA UTILIZANDO SIM A ULTIMA!!!")
                        ...
                        try:
                            ...
                            if 'gasolina' in descricao.lower() or 'diesel' in descricao.lower() or 'gnv' in descricao.lower() or 'etanol' in descricao.lower():
                                try:
                                    ...
                                    natureza_oper_select.send_keys("{ENTER}")
                                    await worker_sleep(1)
                                    natureza_oper_select.send_keys("{TAB}")
                                    await worker_sleep(1)
                                except:
                                ...
                                try:
                                    console.print("Selecionando NOP do item: '5102 - MERCAD. ADQ DE TERCEIRO-5.102 S/ ESTOQU C/ FINAN'")
                                    natureza_oper_select.select("5102 - MERCAD. ADQ DE TERCEIRO-5.102 S/ ESTOQU C/ FINAN")
                                    natureza_oper_select.send_keys("{ENTER}")
                                    await worker_sleep(1)
                                    natureza_oper_select.send_keys("{TAB}")
                                    await worker_sleep(1)
                ...
                if preenchimento_header_pre_venda.sucesso:
                    try:
                        ...
                        try:
                            ...
                            #     main_window = app["TMessageForm"]
                            # except Exception as ex:
                            #     console.print(f'{ex} - TMessageForm 6')
                            #     app = Application().connect(class_name="TMessageForm", timeout=10)
                            #     main_window = app["TMessageForm"]
