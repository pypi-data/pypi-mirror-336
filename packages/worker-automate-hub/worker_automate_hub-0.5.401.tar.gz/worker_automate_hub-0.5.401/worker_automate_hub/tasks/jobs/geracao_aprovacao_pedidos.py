...
async def geracao_aprovacao_pedidos_34(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    task["configEntrada"]["filialEmpresaOrigem"] = "34"
    retorno = await geracao_aprovacao_pedidos_processo(task)
    return retorno

async def geracao_aprovacao_pedidos_171(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    task["configEntrada"]["filialEmpresaOrigem"] = "171"
    retorno = await geracao_aprovacao_pedidos_processo(task)
    return retorno
...
async def geracao_aprovacao_pedidos_processo(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        filialEmpresaOrigem = task.get("configEntrada").get("filialEmpresaOrigem")
        periodo = task.get("configEntrada").get("periodo")
        # Abre um novo emsys
