...
class RDPConnection:
    ...
    async def desligar(self):
        ...
        """
        janelas_rdp = [win for win in gw.getAllTitles() if f" - {self.ip}" in win]
            ...
            for idx, janela in enumerate(janelas_encontradas):
                try:
                    ...
                    app = Application(backend="uia").connect(
                        title=titulo, found_index=idx, timeout=5
                    )
                    ...
                    # Fecha todos os modais antes de continuar
                    while True:
                        modais = [
                            win
                            ...
                            if any(
                                ...
                                for texto in [
                                    ...
                                    "Ligações ao Ambiente de Trabalho Remoto",
                                ]
                            )
                        ]

                        if not modais:
                            break  # Sai do loop se não houver mais modais

                        for modal_titulo in modais:
                            try:
                                console.print(f"Fechando modal: {modal_titulo}")
                                modal = Application(backend="uia").connect(
                                    title=modal_titulo, timeout=5
                                )
                                modal_window = modal.window(title=modal_titulo)

                                if modal_window.exists():
                                    modal_window.set_focus()
                                    await worker_sleep(1)

                                    # Simula um clique no modal para ativá-lo
                                    x, y = modal_window.rectangle().mid_point()
                                    pyautogui.click(x, y)
                                    await worker_sleep(1)

                                    if not modal_window.has_focus():
                                        console.print(
                                            "Forçando foco no modal com Alt+Tab..."
                                        )
                                    ...
                                    pyautogui.press("esc")
                                    await worker_sleep(1)
                                    console.print(f"Modal '{modal_titulo}' fechado.")

                            except Exception as e:
                                console.print(
                                    f"Erro ao fechar modal {modal_titulo}: {e}"
                                )
