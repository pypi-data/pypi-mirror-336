...
def test_ingest_udm_single_event(chronicle_client, mock_udm_event, mock_udm_response):
    ...
    with patch.object(chronicle_client.session, 'post', return_value=mock_udm_response):
        ...
        # Verify URL format
        url = call_args[0][0]
        assert "projects/test-project/locations/us/instances/test-customer/events:import" in url
        ...
        assert payload["inline_source"]["events"][0]["udm"]["metadata"]["id"] == "test-event-id"
        
        # Verify the result
        assert isinstance(result, dict)


def test_ingest_udm_multiple_events(chronicle_client, mock_udm_event, mock_udm_response):
    ...
    with patch.object(chronicle_client.session, 'post', return_value=mock_udm_response):
        ...
        event_ids = [e["udm"]["metadata"]["id"] for e in payload["inline_source"]["events"]]
...
def test_ingest_udm_validation_error_no_metadata(chronicle_client):
    ...
    with pytest.raises(ValueError, match="UDM event missing required 'metadata' section"):
        ingest_udm(
            client=chronicle_client,
            udm_events=event
        )
