import os
from magique.ai.tools.file_transfer.client import FileTransferClient
from magique.ai.tools.file_transfer.worker import FileTransferToolSet
...
async def test_file_transfer():
    ...
    with TemporaryDirectory() as temp_dir:
        toolset = FileTransferToolSet("file_manager", temp_dir)
        async with run_toolsets([toolset]):
            client = FileTransferClient(toolset.service_id, connect_params={"try_direct_connection": False})
...
async def test_file_manager():
    with TemporaryDirectory() as temp_dir:
        ...
        async with run_toolsets([toolset]):
            ...
            await service.invoke("write_file", {"file_name": "test.txt", "content": "Hello, world!1"})
            await service.invoke("write_file", {"file_name": "test1/test.txt", "content": "Hello, world!2"})
            await service.invoke("write_file", {"file_name": "test2/test.txt", "content": "Hello, world!3"})
            resp = await service.invoke("list_file_tree")
            assert len(resp['children']) == 3
            resp = await service.invoke("read_file", {"file_name": "test.txt"})
            assert resp["content"] == "Hello, world!1"
            resp = await service.invoke("read_file", {"file_name": "test1/test.txt"})
            assert resp["content"] == "Hello, world!2"
            resp = await service.invoke("read_file", {"file_name": "test2/test.txt"})
            assert resp["content"] == "Hello, world!3"
            resp = await service.invoke("delete_file", {"file_name": "test.txt"})
            assert resp["success"]
            resp = await service.invoke("list_file_tree")
