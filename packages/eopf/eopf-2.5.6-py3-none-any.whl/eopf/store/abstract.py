...
class EOProductStore(MutableMapping[str, "EOObject"]):
    ...
    def __init__(self, url: str | AnyPath, *args: Any, **kwargs: Any) -> None:
        self.url: AnyPath = AnyPath.cast(url, **kwargs.get("storage_options", {}))
        ...
        if not self.is_valid_url(self.url):
            raise EOStoreInvalidPathError(
                f"{str(self.url)} is no a valid filename for store {self.__class__.__name__}"
    ...
    def open(self, mode: OpeningMode = OpeningMode.OPEN, **kwargs: Any) -> "EOProductStore":
    ...
    def close(self) -> None:
        ...
        if hasattr(self, "_status"):
            ...
            if self._status == StorageStatus.CLOSE:
                self._logger.warning("Store is not open while trying to close it")
            self._status = StorageStatus.CLOSE
