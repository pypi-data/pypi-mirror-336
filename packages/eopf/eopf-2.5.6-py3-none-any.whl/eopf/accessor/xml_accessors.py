...
from eopf.formatting.abstract import EOAbstractXMLFormatter, EOListValuesFormatter
from eopf.formatting.basic_formatters import IsOptional, ToLowerStr
...
class XMLAnglesAccessor(EOAccessor):
    """
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        super().open(chunk_sizes=chunk_sizes)
        ...
        with self.url.open("r") as xml_fobj:
            try:
                ...
                raise XmlParsingError(f"Exception while parsing xml: {e}")
    ...
    def __getitem__(self, key: str) -> "EOObject":
        ...
        key: str
            ...
            "to_list(n1:Geometric_Info/Tile_Angles/Mean_Viewing_Incidence_Angle_List/Mean_Viewing_Incidence_Angle/ZENITH_ANGLE)"
        ----------
        ...
        xpath_result = xml_utils.get_xpath_results(self._root, xpath, self._namespaces)
        ...
        ::
                ...
                <Sun_Angles_Grid>
                 <Zenith>
                  ...
                  <Values_List>
                    <VALUES>30.9922 30.9691 30.9461 30.9232 30.9003 30.8774 30.8546 30.809 30.7863 30.7637 30.741</VALUES> # noqa: E501
                    ...
    ...
    def is_group(self, path: str) -> bool:
        ...
        if xpath.endswith("Values_List"):
            return False
        nodes_matched = xml_utils.filter_pos_list(
            xml_utils.get_xpath_results(self._root, xpath, namespaces=self._namespaces),
        )
...
class XMLTPAccessor(EOAccessor):
    """
        ...
        **kwargs: Any,
        ...
        if mode == OpeningMode.OPEN:
            try:
                ...
                raise MissingArgumentError(f"Missing configuration parameter: {e}")
            if not self.url.isfile():
    ...
    def _get_shape(self, xpath: str) -> list[int]:
        ...
        """
        list_ = xml_utils.get_xpath_results(self._root, xpath, namespaces=self._namespaces)
    ...
    def _get_tie_points_data(self, path: str) -> xr.DataArray:
        ...
        resolution = float(xml_utils.get_xpath_results(self._root, path, namespaces=self._namespaces)[0].text)
        step = float(xml_utils.get_xpath_results(self._root, self._xmltp_step, namespaces=self._namespaces)[0].text)
        if path[-1] == "Y":
    ...
    def __getitem__(self, key: str) -> "EOObject":
        ...
        filtered: Optional[str] = xml_utils.filter_pos_list(
            xml_utils.get_xpath_results(self._root, key, namespaces=self._namespaces),
        )
    ...
    def is_variable(self, path: str) -> bool:
        ...
        filtered: Optional[str] = xml_utils.filter_pos_list(
            xml_utils.get_xpath_results(self._root, path, namespaces=self._namespaces),
        )
...
class XMLManifestAccessor(EOAccessor):
    ...
    def __getitem__(self, key: str) -> "EOObject":
        ...
        if self._metada_mapping:
            self._attrs = self._translate_attributes(self._metada_mapping)
        else:
    ...
    def _translate_attributes(self, attributes_dict: Any) -> Any:
        ...
        for key, value in attributes_dict.items():
            if key == "id":
                try:
                    filtered = xml_utils.filter_pos_list(
                        xml_utils.get_xpath_results(self._parsed_xml, value, self._namespaces),
                    )
                    if filtered is None:
                        raise XmlXpathError()
                    self._product_name = filtered
            ...
            if match:
                # the xpath is iside a formatter function
                ...
                try:
                    ...
                    if isinstance(xpath_result, list):
                        internal_dict[key] = xml_utils.filter_pos_list(xpath_result)
                    else:
    ...
    def _translate_list_attributes(
        ...
        for idx in attributes_list:
            ...
            if isinstance(idx, dict):
                # Iterate over a dictionary ## maybe use local_dict[global_key] = _translate_attributes(idx)
                ...
                for key, value in idx.items():
                    ...
                    if isinstance(value, list):
                        local_dict[key] = self._translate_list_attributes(value)
                        continue

                    if isinstance(value, dict):
                        local_dict[key] = self._translate_attributes(value)
                        continue
    ...
    def stac_mapper(self, path: str) -> Any:
        ...
        if formatter_name is not None and formatter is not None:
            ...
            # Use match from 3.10
            if formatter_name in [ToProcessingHistory.name]:
            ...
            elif formatter_name == Text.name:
                return formatter.format(xpath)
            elif formatter_name in [ToProcessingSoftware.name]:
                try:
                    dict_attrib = self._get_single_xml_raw_data(xpath).attrib
                    data = {dict_attrib["name"]: dict_attrib["version"]}
                except Exception as e:
                    self.LOGGER.debug(f"Invalid xpath {xpath} : {e}")
                    data = {"": ""}
                return data
            elif formatter_name in [ToProductTimeliness.name]:
                # read https://github.com/stac-extensions/product/blob/main/README.md
                timeliness_category = self._get_single_xml_data(xpath)
                to_timeliness_map = {
                    "NR": "PT3H",
                    "NRT": "PT3H",
                    "NRT-10m": "PT10M",
            ...
            elif formatter_name in [ToSatOrbitState.name]:
                try:
                    data = self._get_xml_data(xpath)
                    if isinstance(data, list) and data and len(data) == 1:
                        # has usage for S1 products
                        return data[0].lower()
                    return data
                except Exception as e:
                    self.LOGGER.debug(f"Invalid xpath {xpath} : {e}")
                    return "No data about orbit state"
            elif formatter_name in [ToProviders.name]:
                try:
                    provider_info = self._get_single_xml_data(xpath)
                    return provider_info if provider_info else "No data about provider"
                except Exception as e:
                    self.LOGGER.debug(f"Invalid xpath {xpath} : {e}")
                    return "No data about provider"
            elif formatter_name == IsOptional.name:
                try:
                    data = self._get_single_xml_data(xpath)
                except Exception as e:
                    self.LOGGER.debug(f"Invalid xpath {xpath} : {e}")
                    data = None
                if data is None:
                    ...
                    try:
                        ...
                        if nested_data is not None:
                            ...
                            if isinstance(wf, EOListValuesFormatter):
                                return wf.format(nested_data)
                            else:
                                if len(nested_data) > 0:
                                    return wf.format(nested_data[0])
                    except Exception as e:
                        self.LOGGER.debug(f"Invalid xpath {wxpth} : {e}")
                    # Else, if there is no nested wrapper and data is empty, return is_optional()
                    return formatter.format(None)
            ...
            try:
                xml_data = self._get_xml_data(xpath)
                if xml_data:
                    if isinstance(formatter, (EOAbstractXMLFormatter, ToLowerStr)):
                        return formatter.format(xml_data)
                    else:
                        if len(xml_data) > 0:
                            return formatter.format(xml_data[0])
                    return formatter.format(xpath)
                else:
                    return formatter.format(xpath)
            except Exception as e:
...
class PatternWriter:
    ...
    def __init__(self, url: AnyPath, templates_path: str, template_name: str):
        ...
        self._url = url
        self._context_data: dict[str, dict[str, Any]] = dict()  # dict of attributes by group path
...
class NDimensionalXPath:
    ...
    def _recursive_xpath(
        ...
        if len(xpath_list) == 1:
            ...
            return [
                extracted[0] if len(extracted := elem.xpath(xpath_list[0])) == 1 else extracted for elem in elem_list
            ]
        else:
            return [self._recursive_xpath(elem.xpath(xpath_list[0]), xpath_list[1:]) for elem in elem_list]
    ...
    def _expand_array_nodes(self, sep: Optional[str] = None) -> None:
        ...
        for idx, elem in self.enumerate():
            if "count" in elem.attrib:
                split_array = elem.text.split(sep)
                self[idx] = []
                for value in split_array:
                    new_el = lxml.etree.Element(elem.tag)
                    new_el.text = value
                    self[idx].append(new_el)
    ...
    def _enumerate(lst: Any, index: Any) -> Iterator[Tuple[Any, lxml.etree._ElementTree]]:
        if isinstance(lst, lxml.etree._Element):
            yield (index,), lst
        else:
    ...
    def to_ndarray(
        ...
        for index, value in self.enumerate():
            ...
            array[tuple(array_index)] = decode_function(value)
        return array


@EOAccessorFactory.register_accessor("xmlmultifiles")
class XMLMultipleFilesAccessor(EOAccessor):
    ...
    def _decode_function_from_target_type(self) -> Callable[[lxml.etree._Element], Any]:
        ...
        elif self._target_type["name"] == "datetime64[us]":
            return lambda x: np.datetime64(datetime.fromisoformat(x.text))

        elif self._target_type["name"] == "float32":
            return lambda x: np.float32(x.text)

        elif self._target_type["name"] == "int32":
            return lambda x: np.int32(x.text)

        elif self._target_type["name"] == "bool":
            return lambda x: x.text in ("true", "True")

        elif self._target_type["name"] == "complex64":
            return lambda x: float(x.find("re").text) + 1j * float(x.find("im").text)

        elif self._target_type["name"] == "|S128":
...
class XMLSingleFileAccessor(EOAccessor):
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise MissingConfigurationParameterError(e)
...
# TODO remove this accessor and use MR #675
@EOAccessorFactory.register_accessor("xmlsinglefile_builder")
class XMLSingleFileBuilderAccessor(EOAccessor):

    _dynamic_params: list[str] = ["builder", "burst_idx"]
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        super().open(chunk_sizes=chunk_sizes, **kwargs)
    ...
    def get_data(self, key: str, **kwargs: Any) -> "EOObject":
        """
        This method is used to return eo_variables that allow to build coordinates
        depending on multiple variables.

        Parameters
        ----------
        key: str
            String containing a xpath expression separated by the '#' character.
        ...
        if self._builder == "azimuth_time":

            tree = self._root[0]

            number_of_lines = int(tree.xpath(f".//{key}/numberOfLines")[0].text)
            product_first_line_utc_time = tree.xpath(f".//{key}/productFirstLineUtcTime")[0].text
            product_last_line_utc_time = tree.xpath(f".//{key}/productLastLineUtcTime")[0].text
            ...
            ndarray = azimuth_time.values

            return EOVariable(data=ndarray)

        elif self._builder == "azimuth_time_burst":

            tree = self._root[0]
            burst_index = int(kwargs["burst_idx"]) + 1

            first_azimuth_time_burst = tree.xpath(f"//swathTiming/burstList/burst[{burst_index}]/azimuthTime")[0].text
            lines_per_burst = int(tree.xpath("//swathTiming/linesPerBurst")[0].text)
            azimuth_time_interval = float(tree.xpath("//imageInformation/azimuthTimeInterval")[0].text)
            ...
            ndarray = azimuth_time_burst.values
