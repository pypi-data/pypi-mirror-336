...
class EOProduct(EOGroup):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if (self.product_type is not None) and (self.processing_version is not None) and len(self.__short_names) == 0:
            # shortnames where never set -> generate the shortnames
            # self.__short_names = self._get_short_names(self.product_type, self.processing_version)
            # TMP FIX, need to change the manner in which short names are retrieved, TBD 2.5.1
            # currently retrieve from mapping is not possible with zarr products, as we would need also the legacy url
            pass
        self._strict = strict
    ...
    def __deepcopy__(self, memo: dict[int, Any]) -> "EOProduct":
        new_instance: EOProduct = EOProduct(
            ...
            self.processing_version if self.processing_version is not None else "",
            self._strict,
    ...
    def __setitem__(self, key: str, value: "EOObject") -> None:
        ...
        self.get_short_names_from_variables()
    ...
    def get_default_file_name_no_extension(self, mission_specific: Optional[str] = None) -> str:
        ...
        if mission_specific is not None:
            ...
            mission_specific = ""
        return get_default_file_name_no_extension(self.product_type, self.attrs, mission_specific=mission_specific)
    ...
    def _get_short_names(self, product_type: str, processing_version: str) -> dict[str, str]:
        """
        ...
        eop_short_names: dict[str, str] = dict()
    ...
    def set_type(
        ...
        """
        (Deprecated) Set the product_type
        """
        self._logger.warning(
            "eoproduct.set_type(intype, ...) is deprecated. Use eoproduct.set_type_and_version instead",
        )
        warn(
            "eoproduct.set_type(intype, ...) is deprecated. Use eoproduct.set_type_and_version instead",
            DeprecationWarning,
    ...
    def set_type_and_version(self, product_type: str, processing_version: str) -> None:
        ...
        self.attrs.setdefault("stac_discovery", {}).setdefault("properties", {})["product:type"] = product_type
        self.attrs.setdefault("stac_discovery", {}).setdefault("properties", {})[
            "processing:version"
        ] = processing_version

        if len(self.__short_names) == 0:
            # shortnames where never set -> generate the shortnames
            self.__short_names = self._get_short_names(product_type=product_type, processing_version=processing_version)
            self.dump_short_names_to_variables()
    ...
    @short_names.setter
    def short_names(self, short_names: dict[str, str]) -> None:
        """
    ...
    def _declare_as_product(self) -> None:
        self._attrs.setdefault("other_metadata", dict()).setdefault(EOPF_CATEGORY_ATTR, EOPRODUCT_CATEGORY)
    ...
    def dump_short_names_to_variables(self) -> None:
        """
    ...
    def get_short_names_from_variables(self, group: EOGroup | None = None) -> None:
        cur_group = self if group is None else group
        for _, var in cur_group.variables:
            if SHORT_NAME in var.attrs:
                self.short_names[var.path] = var.attrs[SHORT_NAME]
        for _, sub_group in cur_group.groups:
            self.get_short_names_from_variables(group=sub_group)

    def clean_short_names_in_variables(self, group: EOGroup | None = None) -> None:
        """
        ...
        for _, sub_group in cur_group.groups:
            self.clean_short_names_in_variables(group=sub_group)
