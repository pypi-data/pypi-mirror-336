    ...
    def _check(self, eoproduct: EOProduct) -> EOQCPartialCheckResult:
        ...
        for var_path in self.variables:
            try:
                ...
                continue
            res = res + self._var_check(var_path, attr_value, eoproduct)
        if res is None:
            return EOQCPartialCheckResult(status=False, message="ERROR: This should not happen")
        return res

    @abstractmethod
    def _var_check(self, var_path: str, value: Any, eoproduct: EOProduct) -> EOQCPartialCheckResult:
        pass


@dataclass
@EOQCFactory.register_eoqc("var_exists")
class EOQCVarAvailable(EOQCVarBase):
    """
...
class EOQCVarInRange(EOQCVarBase):
    ...
    def _var_check(self, var_path: str, value: Any, eoproduct: EOProduct) -> EOQCPartialCheckResult:
        if not isinstance(value, EOVariable):
            raise Exception("Target is not an EOVariable")
        if self.strict:
...
class EOQCCountVar(EOQCVarBase):
    ...
    def _var_check(self, var_path: str, value: Any, eoproduct: EOProduct) -> EOQCPartialCheckResult:
        ...
        if not isinstance(value, EOGroup):
            raise Exception("Target is not a group, can't count element")
