...
class DaskContext:
    ...
    def __init__(
        ...
        self._logger.info(
            f"Initialising an {cluster_type} cluster with client conf : {client_config} "
            f"and cluster config {cluster_config}",
        )
        ...
        if cluster_type == ClusterType.LOCAL:
            ...
            name=None, n_workers=None, threads_per_worker=None, processes=None, loop=None, start=None, host=None,
        ...
        elif cluster_type == ClusterType.SGE:
            try:
                ...
                raise PackageNotFoundError("Package dask_jobqueue should be installed.")
        elif cluster_type == ClusterType.LSF:
            try:
        ...
        elif cluster_type == ClusterType.GATEWAY:
            self._setup_gateway_cluster()
        elif cluster_type == ClusterType.ADDRESS:
            if address is None:
                raise TriggeringConfigurationError("addr parameter is mandatory for STR cluster connexion")
            if "address" not in self._client_config:
                self._client_config["address"] = address
        elif cluster_type == ClusterType.CUSTOM:
            self._setup_custom_cluster()
        elif cluster_type is None:
            # No cluster type provided but an address is given
            if address is not None and "address" not in self._client_config:
                self._client_config["address"] = address
        else:
            raise TriggeringConfigurationError("Unhandled dask context configuration")
    ...
    def __enter__(self) -> "DaskContext":
        self._logger.debug(f"Starting dask client and cluster {self}")
        if self._cluster is not None:
