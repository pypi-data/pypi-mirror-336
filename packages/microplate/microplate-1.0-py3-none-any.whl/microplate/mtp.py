...
class MTP:
    ...
    def normalize_percent(self, region_high: str, region_low: str, 
        ...
        block : int, optional
            ...
            then all blocks of the MTP are normalized on a bock-by-block basis.
        mode : str, optional
            Statistic used for normalization. 100%/0% can be either median or
            mean of the region_high/region_low.
        ...
        ------
        ValueError
            If regions do not exist, mode does not exist, or block is incorrect.
        """
        # Verify user input
        if not type(block) is int or block > self.blocks:
            raise ValueError
        ...
        for block_num in block_range:
            if mode == 'median':
                norm_plate.__arr[block_num] = 100 * (
                    (
                        norm_plate.__arr[block_num] 
    ...
    def normalize_minmax(self, block: int = -1):
        """Create a new MTP that is normalized on a percent max/min basis.
        ...
        for block_num in block_range:
            norm_plate.__arr[block_num] = 100 * (
                (
                    norm_plate.__arr[block_num] 
                    - np.min(norm_plate.__arr[block_num])
                )
                /(
                    np.max(norm_plate.__arr[block_num])
                    - np.min(norm_plate.__arr[block_num])
                )
            )
    ...
    # IMPROVEMENT: There is a lot of duplicated code between get/set/delete
    def __getitem__(self, key):
        ...
        # Range search
            ...
            if row == -1 and col == -1: 
                return self.__arr[block_num].flatten()
    ...
    def __delitem__(self, key):
        ...
        # Range search
            ...
            self.__arr[block_num,row_1:(row_2+1),col_1:(col_2+1)] = 0
        else:
            (row,col) = self.__well_transform(key)
            
            # Give me the whole array as 1D
            if row == -1 and col == -1: 
                self.__arr[block_num] = 0
            # Give me an entire row
            elif row == -1: 
                self.__arr[block_num,:,col] = 0
            # Give me an entire column
            elif col == -1: 
                self.__arr[block_num,row,:] = 0
            # Give me a well
            else: 
                self.__arr[block_num,row,col] = 0
