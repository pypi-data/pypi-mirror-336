...
def shell_wrapper(command, check=True, chdir=None, env=None):
    ...
    if chdir:
        dir_last = os.getcwd()
        os.chdir(chdir)
    rc = subprocess.call(shlex_split(command), env=env)  # equals to bash -c '<command>'
    if check and rc:
        if dir_last:
            os.chdir(dir_last)
...
def shell_retry(command, times=None, notify=True, env=None):
    ...
    while True:
        if isinstance(command, str):
            err = subprocess.call(shlex_split(command), env=env)
        else:
            try:
...
def shell_with_input(command, handler, env=None):
    p = subprocess.Popen(
        shlex_split(command), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE,
        bufsize=0, env=env)
...
class ShellInputHandler:
    def __init__(self, data, reduce=True):
        self.data = data
        self.reduce = reduce
    ...
    def run(self, p, text):
        ...
        for k, v in self.data.items():
            ...
            if match:
                ...
                self.input(p, v)
                break
        if key is not None and self.reduce:
            self.data.pop(key)
...
def shell_stdout(command, write=None, env=None):
    proc = subprocess.Popen(shlex_split(command),
                            env=env,
...
def shell_command(command, sync=True, headless=False, quiet=False, check=True, env=None):
    result = None
    try:
        if os.name == 'nt' and headless:
            ...
            if not sync:
                subprocess.Popen(shlex_split(command), env=env)
            elif quiet:
                rc, output = shell_output(command, env=env)
                if rc:
                    result = subprocess.CalledProcessError(rc, command, output)
            else:
                subprocess.run(shlex_split(command), check=True, env=env)
...
def run_vbs(s, sync=True, check=True, env=None):
    ...
    try:
        args = shlex_split(f'wscript {shlex_quote(fp)}')
        if sync:
...
def _shell_timeout_unix(command, timeout, env=None):
    ...
    with subprocess.Popen(shlex_split(command), env=env, preexec_fn=os.setsid) as process:
        try:
