...
from planqk.commons.parmeters import is_container_type


class GenerateOpenApi3JsonSchema(GenerateJsonSchema):
    def generate(self, model: CoreSchema, mode: JsonSchemaMode) -> dict:
        schema = super().generate(model, mode)
        self._replace_examples_props_with_example_props(schema)
        return schema

    def _replace_examples_props_with_example_props(self, schema: dict) -> None:
        if "properties" in schema:
            for prop in schema["properties"].values():
                if "examples" in prop:
                    examples = prop.pop("examples")
                    if isinstance(examples, list) and examples:
                        prop["example"] = examples[0]
                self._replace_examples_props_with_example_props(prop)
        # Also process other nested objects
        for key, value in schema.items():
            if isinstance(value, dict):
                self._replace_examples_props_with_example_props(value)
...
def generate_pydantic_schema(parameter_type) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    schema = parameter_type.model_json_schema(
        ...
        schema_generator=GenerateOpenApi3JsonSchema,
        mode="serialization"
    )
