...
from pypos3d.wftk.WFBasic import Point3d, TexCoord2f, C_FACE_TEXT, FEPSILON3
...
class MxFace: # implements Blockable
  # Enhancement: Used sloted class for performance aspect
  v0:int
  v1:int
  v2:int
  mark:int
  tag:int
  grpNo:int # Original groupe id
  matIdx:int # Preserve original material index
...
class MxStdModel(object):
  ...
  def add_face(self, v1, v2, v3, isProtected, matIdx, grpNo):
    # FIX: Check Triangle surface before adding it
    # Compute each edges length
    cp0,cp1,cp2 = self.vertices[v1], self.vertices[v2], self.vertices[v3]
    ...
    p = 0.5*(l0+l1+l2)
    s2 = p*(p-l0)*(p-l1)*(p-l2) # s2 is the Squared surface of the triangle
    if s2<FEPSILON3: # Rejected Triangle      
      logging.warning("Triangle[%d, %d, %d] rejected: too small (%g)", v1, v2, v3, s2)
      return C_FAIL
    else:
  ...
  def read(self, geom:'WaveGeom', protEdges=False, protMat:'string'=None, lstProtFaces = None):
    ...
    protMat: str, optional, default None
      Indicate to the algorithm to protect the faces with the given
      material name.
    ...
    if hasTexture:
      ...
      try:
        for grp in geom.getGroups():
          ...
          tvertIdxTbl = [-1] * len(grp.vertIdx) # grp.tvertIdx
          
          # I must admit that this kind of Vertex --> Texture mapping is FALSE!!
          for faceno, fao in enumerate(grp.matIdx):
            if fao.fAttr & C_FACE_TEXT:
              si = grp.getFaceStartIdx(faceno)
              ei = grp.getFaceLastIdx(faceno)
              tvertIdxTbl[si:ei] = fao.tvertIdx
