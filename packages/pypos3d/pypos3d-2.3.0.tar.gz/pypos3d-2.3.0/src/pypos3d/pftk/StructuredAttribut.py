...
class ShaderTree(StructuredAttribut):
  ...
  #					file NO_MAP
  #					}
  def CreateNode(self, poserkNodeTypeCode, inpBaseName=None, pos=None, inputsCollapsed=False, showPreview=False): # kNodeTypeCodeBLENDER or kNodeTypeCodeIMAGEMAP
    n = Node()
    ...
    if inpBaseName:
      ...
      n._name  = ('Blender_' if poserkNodeTypeCode==PoserConst.kNodeTypeCodeBLENDER else 'Image_Map_') + str(no)
  ...
  def RemoveNodeNonRec(self, n):
    ''' Delete a node from the tree. Without linkage check nor reccursive delete. '''
    r = self.deleteAttribut(n.getName())
    if not r:
      logging.warning("Node:%s not found in ShaderTree",n.getName())
    
  def __RecRemoveNode(self, n, shNodeSet):
    ''' Internal Recursively Delete a node from the tree. With valueParm linkage '''
    # For each child node
    if not n: return 
    
    lsrc = n.getLstAttr()
    if lsrc:
      for ni in [ noi for noi in lsrc if isinstance(noi, NodeInput)]:
        innode = ni.getInNode(self)
        # Store potential Attached shaderNodes
        for parmS in [ ni.getParmR(), ni.getParmG(), ni.getParmB() ]:
          if parmS and isinstance(parmS, str):
            shNodeSet.add(RemoveQuotes(parmS))

        self.__RecRemoveNode(innode, shNodeSet)
  ...
  def RemoveNode(self, n):
    ''' Recursively Delete a node from the tree. With valueParm linkage.
    Return:
      set of shaderNodes Names to delete
    '''
    resSet = set()
    self.__RecRemoveNode(n, resSet)
    return resSet
...
class PoserMaterial(StructuredAttribut):
  ...
  def getShaderNodes(self):
    ''' Compute the list of ShaderNodes names of a PoserMaterials.
    
    Returns:
    --------
      a set of shadernodes names
    '''
    usedShnNames = set()
    
    for node in self.getLstNodes():
      for ni in [ attr for attr in node.getLstAttr() if attr.getPoserType()==PoserToken.E_nodeInput ]:
        usedShnNames |= { RemoveQuotes(ni.getParmR()), RemoveQuotes(ni.getParmG()), RemoveQuotes(ni.getParmB()) }  

    return usedShnNames


  @classmethod
  def getShaderNodesFromList(cls, lstMat):
    ''' Compute the list of ShaderNodes names from a list of PoserMaterials.
...
class Keys(StructuredAttribut):
  ...
  #
  def getKeyList(self):
    return [ ks for ks in sorted(self._hshKeys.values(), key=lambda k:k._noFrame) ]
...
class CustomData(StructuredAttribut):
  ...
  def read(self, st):
    while True:
      ...
      if code== PoserFileParser.TT_WORD:
        if cn=="data":
          # FIX: split generate errors when the string data contains spaces
          pg = rw.find('"')
          sdata = rw[pg:]
          tv = rw[:pg-1].split()
          # FIX: 20250309 customdata may have no string data
          if len(tv)==3:
            self.set(tv[0], RemoveQuotes(sdata), int(tv[1]), int(tv[2]))
          else:
            logging.warning("Line[%s] - empty customdata:%s",st.lineno(), tv[0])
