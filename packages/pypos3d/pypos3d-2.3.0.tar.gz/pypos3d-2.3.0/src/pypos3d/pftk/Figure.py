...
class Figure():
  ...
  def __init__(self, pf, idx=0, frf=None):
    ...
    self._conformScale = None   # Optional attribute found in a Poser 6 file (RebyHair)
    self._conformMorphs = None  # Optional attribute found in a Poser 6 file (RebyHair)
  ...
  def getProps(self):
    ''' Return a filtered list of actors on 'Prop' descendant '''
  ...
  def findActor(self, actorName, withIndex=True):
    ...
    if withIndex:
      ...
      backoff = 1+len(str(self._bodyIdx))
      return next( (a for a in self.getActors() if actorName == a.getName()[:-backoff]), None)
...
#     return _res
  ...
  def importChannels(self, lstChanDest:list, poserRootDir, hideComputed=False):
    ...
    ChannelDescriptors must be likned to target actors.

    Old method entry kept for backward compatilbility.
    
    The real code of the method has been moved to PoserFile (to fix a small design issue)
    ...
    return self._pf.importChannels(lstChanDest, poserRootDir, hideComputed)
  ...
  #
  def cleanAllChannel(self, pattern, force=False):
    ''' Clean (delete) all channels with a name that starts with the pattern.
        All matching channels are deleted when force=True, 
        else only channels without any descendant are deleted.
    '''
  ...
  def cleanShaderNode(self):
    ''' Clean (delete) all shader nodes channels that are not referenced by any material input. 
    
    Manage also Props and their custom Materials
    TODO: Create the same at PoserFile level and manage UNIVERSE attached props or Lights
    
    Returns
    -------
    int :
      Ret Code is C_OK if nodes have been found and deleted, C_FAIL if none
    '''
    
    body = self.getRootActor()
    
    # Retrieve all Figure ShaderNodes
    shnNames = { shn.getName() for shn in [ p for p in body.getChannels() if p.getPoserType()==PoserToken.E_shaderNodeParm ] }

    # Compute list of Referenced shadernodes in Figure's materials
    usedShnNames = PoserMaterial.getShaderNodesFromList(self.getLstMaterial())

    # Scan Props with custom Materials
    for pp in [ p for p in self.getProps() if p.getCustomMaterial() ]:
      usedShnNames |= PoserMaterial.getShaderNodesFromList( [ m for m in pp.getLstAttr() if (m.getPoserType()==PoserToken.E_material) ] )

    logging.info(f"Figure [{self.getPrintName()}]: Found ShaderNodes {len(shnNames)}")

    shnNames -= usedShnNames
    
    # Filter the set of shadernodes names before deletion
    lstShn = [ shn for shn in [ p for p in body.getChannels() if p.getPoserType()==PoserToken.E_shaderNodeParm and p.getName() in shnNames ] ]

    logging.info(f"Figure [{self.getPrintName()}]: Cleaning {len(lstShn)} ShaderNodes ")
    
    for gt in lstShn:
      logging.info(f"Figure [{self.getPrintName()}] Removing Channel {gt.getName()} cleared in {body.getName()}")
      body.deleteChannel(gt)
      
    return C_OK if lstShn else C_FAIL
    ...
    while (code!=PoserFileParser.TT_EOF) and (code!=PoserFileParser.TT_RIGHTBRACKET):
      if code==PoserFileParser.TT_WORD:
        try:
          ...
          elif vc==PoserToken.E_conformScale:
            self._conformScale = (val[0] == "1")
          elif vc==PoserToken.E_conformMorphs:
            self._conformMorphs = (val[0] == "1")
  ...
  def writeData(self, fw, pfx):
    ...
    WBoolLine(fw, nextPfx, "conformScale", self._conformScale)    
    WBoolLine(fw, nextPfx, "conformMorphs", self._conformMorphs)
