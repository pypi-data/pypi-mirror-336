...
# -------------------------------------------
# Axis Combinations deisgnation
C_AXIS_NONE         = 0x0000000
C_AXIS_X            = 0x0000001
C_AXIS_Y            = 0x0000010
C_AXIS_Z            = 0x0000100
C_AXIS_XY           = 0x0010000 | C_AXIS_X | C_AXIS_Y
...
def readPoint(s):
  ''' Convert a string into a Point3d instance.
  The string shall contain three comma separated floats.
  This function may raise exceptions
  '''
  s = s.strip()
  return Point3d(eval(s)) if s else Point3d() 
  
def readVector(s):
  ''' Convert a string into a Vector3d instance.
  The string shall contain three comma separated floats.
  This function may raise exceptions
  '''
  s = s.strip()
  return Vector3d(eval(s)) if s else Vector3d()
  
def interpolate(x0, x1, x, t0, t1):
  ''' Linear interpolation in 'x' between (x0, t0) and (x1, t1).
  Return
  ------
  float : the interpolated value in x
  '''
  depl = x1 - x0
...
class TexCoord2f():
  ...
  @classmethod
  def faceOrientation(cls, tabTxCoord, resTab=None):
    ''' Compute the sign of the normal to 2 following texture vectors.
    FIXME: To optimize
    
    Stop on the first negative value.
        (Oy)
    ...
    Return 
    ------
    float : the cross product of the first negative value else returns 1.0
    '''
    if not tabTxCoord or len(tabTxCoord)<3:
      return 1.0
    
    txp = tabTxCoord[-1]
    txc = tabTxCoord[0]            
             
    eux, euy = txc.x - txp.x, txc.y - txp.y
    n = math.sqrt(eux*eux+euy*euy)
    if n>0.0:
      eux /= n
      euy /= n
    ...
    for i,txc in enumerate(tabTxCoord):
        txn = tabTxCoord[ (i+1) % nbpt ]
        
        # Compute new vector coordinates
        evx = txn.x - txc.x
        evy = txn.y - txc.y
        n = math.sqrt(evx*evx+evy*evy)
        if n>0.0:
          evx /= n
          evy /= n
        pseudoSign = eux*evy - euy*evx
        
        if resTab==None:
          # Without result table Stop on first issue
          if pseudoSign<0.0:
            return pseudoSign
        else:
          resTab.append(pseudoSign)
    ...
    return 1.0 if resTab==None else min(resTab)

  
  @classmethod
  def transform(cls, texTab, scaleX=1.0, scaleY=1.0, \
                     transX=0.0, transY=0.0, rotate=0.0):
    ''' Perform a linear transform on a texture list and returns the boundaries
    '''
      # Transform
    txmin, tymin = sys.float_info.max, sys.float_info.max
    txmax, tymax = -txmin, -tymin
    
    if math.fabs(rotate)>FEPSILON:
      doRotate = True
      c = math.cos(rotate)
      s = math.sin(rotate)
    else:      
    ...
    for t in  texTab:
      ...
      # Compute min,max in the [0,0] frame
      if x > txmax:
        txmax = x
      if x < txmin:
        txmin = x
  ...
  @classmethod
  def adaptScale(cls, texTab, transX, transY, txmin, txmax, tymin, tymax, adaptScale = C_AXIS_NONE):
    ''' Adapt scale so that the UV map enters [0 .. 1]x[0 .. 1] 
    Does perform any change when scales are null
    '''
    # Scale in [0 .. 1]x[0 .. 1]
    if adaptScale:
      # Protect from null ranges
      dpos = txmax-transX
      fx = (1.0 - transX)/dpos if math.fabs(dpos)>FEPSILON else sys.float_info.max
      
      dneg = transX-txmin
      fx = min(fx, (transX)/dneg if math.fabs(dneg)>FEPSILON else sys.float_info.max )
      
      dpos = tymax-transY
      fy = (1.0 - transY)/dpos if math.fabs(dpos)>FEPSILON else sys.float_info.max
      
      dneg = transY-tymin
      fy = min(fy, (transY)/dneg if math.fabs(dneg)>FEPSILON else sys.float_info.max )
      
      # Compute an homogeneous scale
      if adaptScale & C_AXIS_XY:
        f = min(fx,fy)
        fx,fy = f,f
        
      if math.fabs(fx)<FEPSILON or math.fabs(fy)<FEPSILON:
        # Null scale found ==> Consider translating
        return C_FAIL

      for t in texTab:      
        t.x = transX + (t.x - transX) * fx
        t.y = transY + (t.y - transY) * fy
...
class Edge: # (NamedTuple):
  ...
  def __init__(self, p0, p1, idx0=-1, idx1=-1, hasTexture=False):
    self.p0 = p0
    self.p1 = p1
    self.idx0 = idx0
    self.idx1 = idx1
...
def get2DProjectedPoint(p1, p2, p):
  '''
  Compute the projected point p' of point p on a line defined by 2 points v1,v2
  '''
  ...
  # For the point to project on line extention connecting p1 and p2
  k = ((p.x - p1.x) * (p2.x - p1.x) + (p.y - p1.y) * (p2.y - p1.y)) / l2

  # pp = p1 + t * (p2 - p1)
  pp = Point3d( p1.x + k * v.x, p1.y + k * v.y, 0.0)
  return pp, k
...
def CreateLoop(lstEdges):
  ...
  EdgeStock = [ [ IndexAdd(coordList, edge.p0), IndexAdd(coordList, edge.p1) ] for edge in lstEdges ]      
