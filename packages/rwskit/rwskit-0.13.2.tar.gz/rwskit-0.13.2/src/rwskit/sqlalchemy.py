...
Walkable = DeclarativeBase | Mapper
WalkPredicate = Callable[[DeclarativeBase, DeclarativeBase], bool]
...
class AlchemyEngine:
    ...
    @require(
        ...
        and issubclass(base_model, DeclarativeBase),
    )
    def __init__(
        self,
        engine_or_config: AsyncEngine | Engine | DatabaseConnectionConfig,
    ...
    def is_async(self) -> bool:
        """Returns ``True`` if the underlying engine is asynchronous."""
        return self._is_async(self._engine)
    ...
    def sync_engine(self) -> Engine:
        ...
        return engine.sync_engine if self._is_async(engine) else cast(Engine, engine)
    ...
    def async_engine(self) -> AsyncEngine:
        ...
        """
        if self._is_async(self._engine):
            return self._engine
    ...
    async def async_session_scope(
        ...
        """
        if not self._is_async(self._engine) or self._async_session_factory is None:
            raise ValueError("Async session scope is not supported.")
    ...
    @cache
    def _is_async(self, engine: Engine | AsyncEngine) -> TypeGuard[AsyncEngine]:
        """Returns true if the engine supports async mode."""
        return engine is not None and isinstance(engine, AsyncEngine)
    ...
    def _configure_sync_session_factory(self) -> sessionmaker:
...
def base_mapped_column(
    *args: Any, dto: bool = True, nk: bool = False, **kwargs: Any
) -> MappedColumn[Any]:
    """Extends :func:`sa.orm.mapped_column` to add kwargs for ``dto`` and ``nk``.
    ...
    info = kwargs.pop("info", {}) | {"dto": dto, "nk": nk}
...
class BaseModel(
    ...
    * ``to_dict`` and ``from_dict`` currently only consider mapped properties.
    ...
    # TODO: The hasher should be moved to the wrapper class
    ...
    #     # Note: You can only define a custom hash function on a dataclass if
    #     # `eq=False` and `unsafe_hash=False`.
    #     # TODO I Do want to hash on the natural key, but the natural key should not
    #      be defined using the ``hash``
    #     #      attribute.
    #     return self.hasher.hash(self.natural_key)
    #
    # def __eq__(self, other: Any) -> bool:
    #     # TODO: I don't think I want to implement it like this.
    ...
    @classmethod
    def from_dict(cls: Type[Self], data: dict) -> BaseModel:
        """
    ...
    @classmethod
    def from_dto(cls: Type[BaseModel], dto: pydantic.BaseModel) -> BaseModel:
    ...
    def validate_mapper(cls):
        ...
        def _validate_natural_keys(model_class: Type[BaseModel]):
            ...
            attrs = model_class.natural_key_columns()
        ...
        def _validate_dto_configuration(model_class: Type[BaseModel]):
            ...
            natural_key = [k for k in model_class.natural_key_attribute_names()]
        ...
        def _add_natural_key_index(model_class: Type[BaseModel]):
            ...
            # TODO Verify this is correct. Using the column properties directly
            #      from `model_class.natural_key_columns` appears to create
            #      the proper index, but pylance complains that it is the
            #      wrong type.
            columns = [p.columns[0] for p in model_class.natural_key_columns()]
    ...
    def _from_dict_update_relationships(
        ...
        for f, r in pairs:
            ...
            try:
                ...
                log.debug(f"data does not have a key for: {name}")
                pass  # TODO check if we are getting here by mistake
            else:
...
class SqlBinaryExpression(YamlConfig):
    ...
    def __call__(
        self, model_or_table: Type[B] | sa.Table | FromClause
    ) -> sa.BinaryExpression:
...
class Repository:
    ...
    def upsert(
        ...
        ..warning::
            This does not update many-to-many associations.
    ...
    @require(
        lambda filter_by: all(
            isinstance(f, SqlBinaryExpression)
            for f in filter_by  # noqa
        )
    )
    @require(
        lambda order_by: all(
            isinstance(o, SqlOrderExpression)
            for o in order_by  # noqa
        )
    )
    ...
    def _normalize_data(
        ...
        if not is_iterable(data):
            data = cast(Iterable[BaseModel], [data])

        # The above check ensures that 'data' is iterable, but pylance
        # can't figure that out.
        return cast(Iterable[BaseModel], data)

    @contextmanager
    def _get_sync_session(self, session: Optional[Session]) -> Generator[Session]:
        if session is None:
    ...
    def _group_as_dict(
        ...
        def add_to_result(node: BaseModel):
            ...
            attrs = [str(c.key) for c in node.table_columns()]
            value: dict[str, Any] = {attr: getattr(node, attr, None) for attr in attrs}
    ...
    def _filter_duplicate_values(
        ...
        tmp = {
            model: {frozenset(r.items()) for r in rows} for model, rows in data.items()
        }
    ...
    def _sort_by_insertion_order(
        ...
        return {
            k: v
            for k, v in sorted(
                data.items(), key=lambda e: order_lookup[cast(Table, e[0].__table__)]
            )
        }
    ...
    def _make_upsert_statement(
        ...
        def _get_insert_function() -> Callable[..., PostgresInsert | SqliteInsert]:
            if self.engine.dialect == "postgresql":
                ...
                raise ValueError(
                    f"The database dialect '{self.engine.dialect}' does not support "
                    "'upserts'"
                )
