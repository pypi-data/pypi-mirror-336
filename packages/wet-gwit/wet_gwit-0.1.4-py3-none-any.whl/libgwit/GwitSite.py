...
class GwitSite:
    ...
    def fetch_unique_remote(self, repo: Repo, remote:str, remote_name:str):
        ...
        origin_branch_ref = f"remotes/{remote_name}/{self.branch_ref}"
        try:
    ...
    def fetch_repo(self, repo: Repo, remotes:list, branch:str = '', rewrite:int = 0) -> None:
        ...
        self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, threads count: {len(threads)}, remote_names: {remote_names}")
        ...
        if self.branch_ref in repo.heads:
            heads.append(repo.commit(self.branch_ref))
            ...
            if len(heads) == 0:
                ...
                for key, known_head in enumerate(heads):
                    if known_head in head.iter_parents():
                        ...
                        continue
                    elif not head in heads:
                        heads.append(head)

        self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, heads count: {heads}")
        ...
        if len(heads) == 1:
            ...
            most_recent = {
                ...
                'datetime': current_commit.committed_datetime if current_commit else 0
            }
            for head in heads:
                if head.committed_datetime > most_recent['datetime']:
                    most_recent = {'head': head, 'datetime': head.authored_datetime}
            self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, current: {current_commit}, most_recent: {most_recent}, rewrite: {rewrite}")
            if current_commit != most_recent['head'] and current_commit in most_recent['head'].iter_parents():
                self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, no rewrite")
                ...
                if rewrite == 1:
                    self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, archive")
                ...
                # rewrite = 1 or 2
                self.logger.debug(f"{__name__}::{inspect.currentframe().f_code.co_name}, archive or rewrite")
                repo.git.checkout(most_recent['head'].hexsha)
                if self.branch_ref in repo.heads:
                    repo.heads[self.branch_ref].delete(repo, self.branch_ref, force=True)
                repo.create_head(self.branch_ref, most_recent['head'].hexsha)
