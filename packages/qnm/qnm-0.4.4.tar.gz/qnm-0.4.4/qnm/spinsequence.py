...
class KerrSpinSeq(object):
    ...
    def do_find_sequence(self):
        ...
        # This flag is set to true if we are trying to rerun because
        # we found Re[\omega] < 0. We will only try this once. If it
        # still comes back as negative, continue?
        om_re_neg_rerun = False
        ...
        while _a <= self.a_max:
            ...
            # if (np.real(result) < 0):
            #     if (not om_re_neg_rerun):
            #         om_re_neg_rerun = True
            #         omega_guess = -np.conj(result)
            #         A0          = np.conj(self.solver.A)
            #         print("neg, retry with om={}, A={}".format(omega_guess,A0))
            #         continue
            #     else:
            #         print("neg, don't retry")
            #         om_re_neg_rerun = False
            #         # give up, don't try again
