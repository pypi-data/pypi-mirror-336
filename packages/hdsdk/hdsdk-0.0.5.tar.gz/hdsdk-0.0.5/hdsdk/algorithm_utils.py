from hdsdk.conf.common_config import UNPROCESSABLE_ENTITY, DNY_HIGH, DNY_LOW, ALERT_LOW_1, ALERT_LOW_2, ALERT_HIGH_1, \
    ALERT_HIGH_2, PW_STABILITY_FACTOR, PW_STABILITY_LIMIT_VALUE, DEFAULT_FALSE_PROP
from hdsdk.utils.data_util import get_history_data_by_group, get_threshold_config, get_pw_point
...
from hdsdk.exceptions import GroupNumError, GroupDataError, PowerDataError, StableOperationError, \
    ThresholdError, ThresholdDataError, ThresholdZeroError, PointsError, ParamZeroError
...
class AlgorithmUtils:
    ...
    def update_data(self, request_data):
        ...
        self.algorithm_fitting = int(request_data.get('algorithm_fitting', 0))
        self.algorithms = request_data.get('algorithms', [])
        self.desc = request_data.get('desc', '')
        self.groups = request_data.get('groups', [])
        self.name = request_data.get('name', "")
        self.param = request_data.get('param', [])
        self.probability = request_data.get('probability', 0.0)
        self.judge_type = request_data.get('type', "")
        self.unit_num = request_data.get("unitNum", "")
        self.threshold = request_data.get('threshold', [])
        self.data = request_data.get('data', {})
        self.working_power = request_data.get("working_power", [])
        self.work_data = request_data.get("work_data", {})
        #
        # 更新输出结果
    ...
    def check_power_data(self, pw_data):
        if pw_data is None or len(pw_data) == 0:
