...
# @File    : api_client.py
import requests
from hdsdk.conf.url_config import BACKEND_CLIENT_URL, BACKEND_API_PATH, EXT_API_CLIENT_URL, EXT_API_BASE_PATH
...
class APIClient:
    ...
    def __init__(self, org_domain='cvxa3663', account='admin', password='Admin@123'):
    ...
    def login(self):
        endpoint = BACKEND_CLIENT_URL + BACKEND_API_PATH
        data = {
    ...
    def getExtApi(self, id='', url='', param=None):
        if param is None:
            param = {}
        endpoint = EXT_API_CLIENT_URL + EXT_API_BASE_PATH
        if id == '':
