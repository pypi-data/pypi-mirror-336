"""
Textual TUI application for SLURM job monitoring
"""
import subprocess
import shutil
import os
...
class SlurmInfo:
    ...
    def get_slurm_jobs() -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cmd = ["squeue", f"--format=%i{DELIMITER}%j{DELIMITER}%u{DELIMITER}%P{DELIMITER}%T{DELIMITER}%M{DELIMITER}%l{DELIMITER}%D{DELIMITER}%r{DELIMITER}%S{DELIMITER}%p", "--noheader"]
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if len(parts) >= 11:
                    job_id, name, user, partition, state, time, time_limit, nodes, reason, start_time, priority = parts[:11]
                else:
                    continue
    ...
    def get_partition_details(partition_name: str) -> Dict[str, str]:
        ...
        partition_name = partition_name.removesuffix("*")
...
class SlurmNotAvailableBanner(Static):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.update("[b]SLURM IS NOT INSTALLED[/b]\n\nThe 'squeue' command was not found in your PATH.\nPlease install SLURM to view job information.")
...
class JobDetailsPanel(Static):
    ...
    def update_job_details(self, job_id: str):
        ...
        if not details:
            self.update(f"[b][u]Job Details for {job_id}[/u][/b]\n\n[i]No details available[/i]")
            return
        ...
        important_keys = ["JobId", "JobName", "UserId", "Partition", "State", "RunTime", "TimeLimit", 
                        "NumNodes", "NumCPUs", "Reason", "Priority", "Command", "WorkDir"]
...
class SlurmJobsView(Container):
    ...
    def refresh_jobs(self) -> None:
        ...
        current_user_jobs = sorted([job for job in raw_jobs if job["user"] == self.current_username], key=lambda j: j["id"])
        sorted_jobs.extend(current_user_jobs)
        
        # Other users' running jobs
        other_running_jobs = sorted([job for job in raw_jobs if job["user"] != self.current_username and job["state"] == "RUNNING"], key=lambda j: j["id"])
        sorted_jobs.extend(other_running_jobs)
        
        # Other users' pending jobs
        other_pending_jobs = sorted([job for job in raw_jobs if job["user"] != self.current_username and job["state"] != "RUNNING"], key=lambda j: j["id"])
        ...
        for i, job in enumerate(self.jobs):
            ...
            row = [f"[{style}]{job[key]:{align}{width}}[/{style}]" for key, [_, _, width, align] in self.columns.items() if width>0]

            self.table.add_row(*row, key=row_key)
        
        self.status_message.update(f"Last updated: {self.last_updated.strftime('%Y-%m-%d %H:%M:%S')} ({len(self.jobs)} jobs found)")
        
        # Select the first row if there are jobs
        if self.jobs and self.table.row_count > 0 :
            try:
                self.table.cursor_coordinate = Coordinate([job["id"] for job in self.jobs].index(self.cursor_job_id), 0)
            except Exception as e:
                self.table.cursor_coordinate = Coordinate(0, 0)
            self.details_panel.update_job_details(str(self.jobs[self.table.cursor_row]["id"]))
...
class HardwareDetailsPanel(Static):
    ...
    def update_partition_details(self, partition_name: str):
        ...
        if not partition_name or (self.current_resource_id == partition_name and self.current_resource_type == "partition"):
            return
        ...
        if not details:
            self.update(f"[b][u]Partition Details for {partition_name}[/u][/b]\n\n[i]No details available[/i]")
            return

        # Start with the header
        content = f"[b][u]Partition Details for {partition_name}[/u][/b]\n\n"
        
        # Add important details first in a more structured way
        important_keys = ["PartitionName", "Nodes", "State", "MaxTime", "DefaultTime", 
                         "MaxNodes", "MinNodes", "Priority", "DefaultQOS", "PreemptMode"]
    ...
    def update_node_details(self, node_name: str):
        ...
        if not node_name or (self.current_resource_id == node_name and self.current_resource_type == "node"):
            return
        ...
        if not details:
            self.update(f"[b][u]Node Details for {node_name}[/u][/b]\n\n[i]No details available[/i]")
            return
        ...
        # Add important details first in a more structured way
        important_keys = ["NodeName", "Partitions", "State", "CPUs", "CPULoad", 
                         "RealMemory", "AllocMemory", "FreeMem", "Sockets", "Cores", 
                         "ThreadsPerCore", "Gres", "ActiveFeatures", "AvailableFeatures"]
...
class HardwareConfigView(Container):
    ...
    def refresh_hardware_info(self) -> None:
        ...
        self.refresh_nodes()
        self.last_updated = datetime.now()
        self.status_message.update(f"Last updated: {self.last_updated.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def refresh_partitions(self) -> None:
        ...
        try:
            ...
            cmd = ["sinfo", f"--format=%P{DELIMITER}%a{DELIMITER}%D{DELIMITER}%T{DELIMITER}%c{DELIMITER}%l{DELIMITER}%m", "--noheader"]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
                    ...
                    if self.cursor_partition_name:
                        ...
                        for i, partition in enumerate(partitions):
                            if partition["name"] == self.cursor_partition_name:
                                self.partition_table.cursor_coordinate = Coordinate(i, 0)
                                break
                except Exception:
                    pass
            ...
            self.partition_table.clear()
            self.partition_table.add_row("Error retrieving partition data", str(e), "", "", "", "", "")
    ...
    def refresh_nodes(self) -> None:
        ...
        try:
            ...
            cmd = ["sinfo", "--Node", f"--format=%N{DELIMITER}%P{DELIMITER}%t{DELIMITER}%c{DELIMITER}%O{DELIMITER}%m{DELIMITER}%T", "--noheader"]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            ...
            self.node_table.clear()
            self.node_table.add_row("Error retrieving node data", str(e), "", "", "", "", "")
...
class JobSubmitterView(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with form_scroller:
            ...
            yield Select([("null", "CPU"), ("null2", "GPU")], id="partition", allow_blank=False)
            ...
            yield Input(placeholder=f"{os.getcwd()}", id="working_dir", value=f"{os.getcwd()}")
            
            # Job script
            yield Static("Job Command/Script:")
            yield Input(placeholder="Command or script to run", id="job_command", value="")
            
            # Output file
            yield Static("Standard Output File:")
            yield Input(placeholder="slurm-%j.out", id="output_file", value="slurm-%j.out")
            
            # Error file
            yield Static("Standard Error File:")
            yield Input(placeholder="slurm-%j.err", id="error_file", value="slurm-%j.err")
    ...
    def on_mount(self) -> None:
        ...
        self.status_message.update("Configure your job parameters and click Submit Job when ready.")
    ...
    def load_partitions(self) -> None:
        ...
        try:
            ...
            for line in result.stdout.strip().split('\n'):
                if line:
                    partitions.append((line.strip(), line.strip()))
            ...
            if partitions:
                ...
                self.status_message.update(f"Loaded {len(partitions)} partitions successfully")
            else:
    ...
    def submit_job(self) -> None:
        ...
        try:
            # Get form values
            job_name = self.query_one("#job_name", expect_type=Input).value or "slurm_job"
            partition = self.query_one("#partition", expect_type=Select).value
            nodes = self.query_one("#nodes", expect_type=Input).value or "1"
            cpus_per_task = self.query_one("#cpus_per_task", expect_type=Input).value or "1"
            mem_per_cpu = self.query_one("#mem_per_cpu", expect_type=Input).value or "1024"
            time_limit = self.query_one("#time_limit", expect_type=Input).value or "01:00:00"
            working_dir = self.query_one("#working_dir", expect_type=Input).value or os.getcwd()
            job_command = self.query_one("#job_command", expect_type=Input).value
            output_file = self.query_one("#output_file", expect_type=Input).value or "slurm-%j.out"
            error_file = self.query_one("#error_file", expect_type=Input).value or "slurm-%j.err"
            ...
            temp_script = self._create_job_script(
                job_name, partition, nodes, cpus_per_task, 
                mem_per_cpu, time_limit, working_dir, 
                job_command, output_file, error_file
            )
            ...
            self.submit_status.update(f"[green]Job submitted successfully! Job ID: {job_id}[/green]")
    ...
    def _create_job_script(self, job_name, partition, nodes, cpus_per_task, 
                          mem_per_cpu, time_limit, working_dir, 
                          job_command, output_file, error_file):
...
class JobMonitorView(Container):
    ...
    def start_monitoring(self, job_id: str) -> None:
        ...
        if not job_details:
            self.status_message.update(f"[red]Error: Could not retrieve details for job {job_id}[/red]")
            return
        ...
        # Update status
        self.status_message.update(f"Monitoring job {job_id}\nStdOut: {stdout_file}\nStdErr: {stderr_file}")
    ...
    def refresh_logs(self) -> None:
        ...
        if job_state in ["COMPLETED", "FAILED", "CANCELLED", "TIMEOUT"]:
            self.status_message.update(f"Job {self.current_job_id} has ended with state: {job_state}")
        else:
            self.status_message.update(f"Monitoring job {self.current_job_id} (State: {job_state})")
    ...
    def _refresh_stdout(self) -> None:
        ...
        try:
            if not os.path.exists(self.stdout_path):
                self.stdout_content.update(f"[yellow]File not found: {self.stdout_path}[/yellow]")
                return
    ...
    def _refresh_stderr(self) -> None:
        ...
        try:
            if not os.path.exists(self.stderr_path):
                self.stderr_content.update(f"[yellow]File not found: {self.stderr_path}[/yellow]")
                return
...
class SlurmPilotApp(App):
    ...
    def _show_view(self, view_id: str) -> None:
        ...
        # Hide all views
        for view_name in [self.VIEW_JOBS, self.VIEW_HARDWARE, self.VIEW_SUBMITTER, self.VIEW_MONITOR]:
            try:
                ...
                if view_name == view_id:
                    ...
                    if view_name == self.VIEW_SUBMITTER:
                        submitter_view = self.query_one(f"#{self.VIEW_SUBMITTER}", expect_type=JobSubmitterView)
    ...
    def action_refresh(self) -> None:
        ...
        if self.current_view == self.VIEW_JOBS:
            ...
            hardware_view = self.query_one(f"#{self.VIEW_HARDWARE}", expect_type=HardwareConfigView)
    ...
    def action_go_back(self) -> None:
        ...
        if self.current_view == self.VIEW_MONITOR:
            monitor_view = self.query_one(f"#{self.VIEW_MONITOR}", expect_type=JobMonitorView)
        ...
        if self.current_view == self.VIEW_JOBS:
            ...
            if jobs_view.jobs and jobs_view.table.cursor_row >= 0:
                ...
                try:
                    ...
                    jobs_view.status_message.update(f"Successfully cancelled job {job_id}")
                    
                    # Refresh the job list to update the UI
                    jobs_view.refresh_jobs()
                except subprocess.CalledProcessError as e:
                    # Handle command execution error
                    jobs_view.status_message.update(f"[red]Error cancelling job {job_id}: {e.stderr}[/red]")
                except Exception as e:
