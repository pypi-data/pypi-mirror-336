...
class CostEntryData(SettingsModel):
    ...
    def data_validation(cls, values):
        if values.entry_type in [CommandCostEntryTypes.static_cost, CommandCostEntryTypes.pending_static_cost,
        ...
        elif values.entry_type in [CommandCostEntryTypes.rate_change, CommandCostEntryTypes.rate_validation,
                                   CommandCostEntryTypes.partial_rate_change]:
            values.value = str(values.value)
        if values.entry_type not in [CommandCostEntryTypes.payment, CommandCostEntryTypes.cancel_remaining_costs,
                                     CommandCostEntryTypes.payoff_costs]:
            if values.group is None:
                raise ValueError("group must be set")
        elif values.entry_type in [CommandCostEntryTypes.cancel_remaining_costs, CommandCostEntryTypes.payoff_costs]:
            if values.group is not None:
                raise ValueError("group must NOT be set")
        elif values.entry_type == CommandCostEntryTypes.free_timerange:
