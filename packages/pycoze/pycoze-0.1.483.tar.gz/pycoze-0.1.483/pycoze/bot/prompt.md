...
def write_or_overwrite_file(path: str, content: str) -> None:
    ...
    Parameters:
        ...
        When the content contains markdown code format triple backticks ```, they MUST be escaped as @`@`@`@.
...
def replace_part_of_a_file(path: str, diff: str) -> None:
    ...
    6. When the REPLACE content contains markdown code format triple backticks ```, they MUST be escaped as @`@`@`@.
