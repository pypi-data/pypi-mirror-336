...
class WriteFileTool(Tool):
    ...
    def execute(self) -> str:
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(self.params["content"].replace("@`@`@`@", "```").replace("@\"@\"@\"@", "\"\"\"").replace("@'@'@'@", "'''"))
...
class ReplaceInFileTool(Tool):
    ...
    def execute(self) -> str:
        ...
        for diff_block in diff_content.split("<<<<<<< SEARCH")[1:]:
            ...
            search = search.strip().replace("@`@`@`@", "```").replace("@\"@\"@\"@", "\"\"\"").replace("@'@'@'@", "'''")
            replace = replace.split(">>>>>>> REPLACE")[0].strip().replace("@`@`@`@", "```").replace("@\"@\"@\"@", "\"\"\"").replace("@'@'@'@", "'''")
