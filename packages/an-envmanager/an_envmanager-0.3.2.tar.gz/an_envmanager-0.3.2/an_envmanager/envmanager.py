# -*- coding: utf-8 -*-
"""ENVM_20250315_01.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1758Qglr_Nne9IeOcVAZgdGSV_fYSD_-l
...
2025/03/15 0.3.0 get_env_varで、文字配列、数値配列を読み込めるようにした
2025/03/15 0.3.1 get_env_varで、辞書型に対応した
2025/03/25 0.3.2 依存関係修正
```
"""
...
class EnvManager:
    ...
    def __init__(self, env_files=None):
        ...
        if git_env_path.exists():
            ...
            raise Exception(f"{git_env_path} が見つかりません。")
            sys.exit( 1 )
    ...
    def load_all_envs(self):
        ...
        for env_file in self.env_files:
            ...
            if env_path.exists():
                ...
                raise Exception(f"{env_file} が見つかりません。")
    ...
    def get_env_var(self, var_name, type="string"):
        ...
        - "int": 整数
        - "float": 浮動小数点数
        - "bool": 真偽値 ("true"/"false" と記述)
        - "string_array": 文字列のリスト
        - "int_array": 整数のリスト
        - "float_array": 浮動小数点数のリスト
        - "dict": 辞書
        """
        allowed_types = ["string", "int", "float", "bool", "string_array", "int_array", "float_array", "dict"]
        ...
        elif type in ["string_array", "int_array", "float_array", "dict"]:
            try:
                parsed = ast.literal_eval(value)
            except Exception as e:
                raise ValueError(f"Unable to parse value for '{var_name}': {value}. Error: {e}")

            if type == "dict":
                if not isinstance(parsed, dict):
                    raise ValueError(f"Value for '{var_name}' is not a dict.")
                return parsed
            else:
...
if __name__ == "__main__":
    ...
    print( envmanager.get_env_var( "Ven_venv_names", type = "string_array"))
    print( envmanager.get_env_var( "Ven_venv_dict", type = "dict") )
    # Ven_venv_dict = ast.literal_eval( ven_tag )
