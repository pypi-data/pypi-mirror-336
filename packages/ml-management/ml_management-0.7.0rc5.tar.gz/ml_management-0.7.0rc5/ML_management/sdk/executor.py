...
def pagination_executor(
    ...
    base_query = op.pagination_executor(
        limit=limit,
        offset=offset,
        filter_settings=ObjectFilterSettings(name=name, tag=TagFilterSettings(key=tag_key, value=tag_value)),
    ).list_executor
    base_query.name()
    base_query.tags()
    base_query.description()
    ...
    base_query = (
        ...
        .pagination_executor_version(
            ...
            filter_settings=ObjectVersionFilterSettings(
                version=version, tag=TagFilterSettings(key=tag_key, value=tag_value)
            ),
        )
