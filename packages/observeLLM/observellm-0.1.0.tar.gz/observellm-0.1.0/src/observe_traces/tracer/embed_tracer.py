...
EMBEDDING_PROVIDER_CONFIGS = {
    "pinecone": {
        ...
        "embeddings_extractor": lambda data: [item["values"] for item in data.get("data", [])]
    },
    "cohere": {
        "token_parser": parse_cohere_tokens,
        "price_calculator": calculate_cohere_price,
        "embeddings_extractor": lambda data: data.get("embeddings", {}).get("float", [])
    },
    "jina": {
        "token_parser": parse_jina_tokens,
        "price_calculator": calculate_jina_price,
        "embeddings_extractor": lambda data: [item["embedding"] for item in data.get("data", [])]
    },
    "voyageai": {
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    span_id = await _LangfuseService.create_span_for_embedding(trace_id = trace_id, span_data = span_data, name=f"{provider.capitalize()} Embeddings Generation")
                    # logger.info(f"Generation created with ID: {span_id}")
                
                except Exception as e:
                    raise f"Error while creating generation for trace: {trace_id}"
...
# Example usage in your embedding service class:
"""
from app.services.embedding_tracing import embedding_tracing

class EmbeddingService:
    @embedding_tracing(provider="pinecone")
    async def pinecone_dense_embeddings(self, model_name, dimension, inputs, input_type):
        # Your existing implementation
        ...
        
    @embedding_tracing(provider="cohere")
    async def cohere_dense_embeddings(self, model_name, texts, input_type):
        # Your existing implementation
        ...
        
    @embedding_tracing(provider="jina")
    async def jina_dense_embeddings(self, model_name, dimension, inputs, input_type):
        # Your existing implementation
        ...
        
    @embedding_tracing(provider="voyageai")
    async def voyageai_dense_embeddings(self, model_name, dimension, inputs, input_type):
        # Your existing implementation
        ...
