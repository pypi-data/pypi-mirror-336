...
def plsqlx(query, params={}, con=None, retries=0, wait_sec=5, batch_size=50000, debug=False):
    """
    ...
    Optional parameters:
      ...
      - wait_sec: wait time between retries.
    ...
    driver_type_codes = [str(desc[1]) for desc in description]  # converting type codes to strings
    column_names = [desc[0] for desc in description]
    
    # Build Arrow schema using our mapping.
    arrow_fields = []
...
def find_duplicates_pl(df, columns=None):
    if columns is not None:
        duplicates = (
        sor_transfers.group_by(columns)
        .agg(pl.len().alias("count"))
        .filter(pl.col("count") > 1)
        )
    else:
        duplicates = (
        sor_transfers.group_by(list(df.columns))
        .agg(pl.len().alias("count"))
        .filter(pl.col("count") > 1)
        )
