...
class Bot:
    ...
    def reply_message(self, chat_id: Union[int, str] = None, text: str = None, mode: str = "Markdown", disable_web_page_preview: bool = None, disable_notification: bool = None, reply_to_message_id: int = None, reply_markup: Union[dict, Markup] = None) -> Optional['Message']:
        ...
        :param photo: Путь к файлу фото, URL или байты.
    ...
    def get_profile_photos(self, user_id: int = None, offset: int = None, limit: int = None) -> Optional['UserProfilePhotos']:
        ...
        :param user_id: Идентификатор пользователя (обязательный, int)
        :type user_id: int
        :param offset: Смещение для получения фотографий (опциональный, int
        :type offset: int
        :param limit: Максимальное количество фотографий для получения (опциональный, int, по умолчанию 100, максимум 100)
        :type limit: int
...
class Ai:
    ...
    def get_models(self, stype: str = "text", models: bool = False) -> Union[List, Dict, bool]:
        ...
        :param stype: Тип моделей ('text' или 'image')
        :type stype: str
        :param models: Если True, возвращает только список имен моделей (для text)
        :type models: bool
        :return: Список моделей или отформатированные данные, False при ошибке
    ...
    def generate_otvet(self, prompt: str = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False, system: str = '', private: bool = True) -> str:
    ...
    def generate_chat_otvet(self, messages: list = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False) -> str:
        '''Генерация ответа с чатом, поддержка Vision'''
        try:
            if messages is None:
    ...
    def generate_voice_otvet(self, prompt: str = None, filename: str = "ozv", voice: str = 'nova') -> bool:
        '''Генерация аудио, доступные голоса: alloy, echo, fable, onyx, nova, shimmer, coral, verse, ballad, ash, sage, amuch, dan'''
        try:
            if prompt is None:
                raise TypeError("Забыли указать prompt")
    ...
    def generate_image(self, prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = False, model: str = 'flux', save: bool = False, filename: str = 'gen_img') -> Union[str, bool]:
