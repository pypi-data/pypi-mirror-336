...
class Bot:
    ...
    def reply_media_group(self, chat_id: Union[int, str] = None, media: list = None, disable_notification: bool = None) -> Optional[list]:
        '''Отправляет несколько обьекто media сразу'''
        method = 'sendMediaGroup'
        if chat_id is None:
            raise ValueError("chat_id не должен быть None")
        elif media is None or not isinstance(media, list):
            raise ValueError("media должен быть непустым списком состоящим из объектов InputMedia")
        elif len(media) > 10:
            raise ValueError("Нельзя отправлять более 10 объектов в одном сообщении (ограничение Telegram)")
        media_payload = [item.to_dict() for item in media]
        params = {
            'chat_id': chat_id,
    ...
    def reply_invoice(self, chat_id: Union[int, str] = None, title: str = None, description: str = None, payload: str = None,
        ...
        '''Отправляет счет на оплату (инвойс)'''
        method = 'sendInvoice'
    ...
    def reply_pre_checkout_query(self, pre_checkout_query_id: str = None, ok: bool = None,
                                  error_message: str = None) -> bool:
        '''Отвечает на запрос предчекаута (pre-checkout)'''
    ...
    def get_user_profile_photos(self, user_id: int = None, offset: int = None, limit: int = None, photo_index: int = None) -> Optional[str]:
...
#Редакт чего-то
    ...
    def reply_media_group(self, chat_id: Union[int, str] = None, media: list = None, disable_notification: bool = None) -> Optional[list]:
        '''Отправляет несколько объектов media сразу'''
        method = 'sendMediaGroup'
        files = {}
        if chat_id is None:
            raise ValueError("chat_id не должен быть None")
        if media is None or not isinstance(media, list) or len(media) == 0:
...
#Регистрация хэндлеров
    ...
    def _handle_inline_query(self, inline_query_data: dict = None) -> None:
        ...
        for handler in self.handlers.get('inline_query', []):
            # Если не указан фильтр или текст запроса содержит заданную строку:
...
#Блок - Нейросети
def chat_gpt(prompt: str = None) -> str:
...
def get_models(stype="text", models=False):
    '''Получение списка актуальных text/image моделей'''
    if stype not in ["image", "text"]:
        raise ValueError("type должен быть 'image' или 'text'")
    else:
        if stype == "image":
            ...
            if models:
                ...
                return format_models(models_data)
...
def format_models(models):
    ...
    def format_model(model):
        ...
        censored = "Да" if model.get('censored', False) else "Нет"
        vision = "Да" if model.get('vision', False) else "Нет"
        return f"• `{name}`: {description} | тип: {type_model} | цензура: {censored} | виденье: {vision}"
    formatted_models = [format_model(model) for model in models]
    return '\n'.join(formatted_models)

def generate_otvet(prompt: str = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False, system: str = '', private: bool = True):
    '''Генерация ответа на prompt, по умолчанию используется модель gpt-4o, чтобы посмотреть модели, используйте функцию get_models()'''
    try:
        if prompt is None:
            raise TypeError("Забыли указать prompt")
        elif seed is None:
            seed = random.randint(0, 1000)
        payload = {"model": model, "json": jsonMode, "system": system, "private": private, "seed": seed}
        response = requests.get(f"https://text.pollinations.ai/{prompt}", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
...
def generate_chat_otvet(messages: list = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False):
    '''Генерация ответа с чатом, поддержка моделей с виденьем, по умолчанию используется модель gpt-4o, чтобы посмотреть модели, используйте функцию get_models()'''
    try:
        if messages is None:
            raise TypeError("Забыли указать messages")
        elif seed is None:
            seed = random.randint(0, 1000)
        payload = {"messages": messages, "model": model, "jsonMode": jsonMode, "seed": seed}
        response = requests.post(f"https://text.pollinations.ai/", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        response.raise_for_status()
        api_response = response.content.decode('utf-8')
        return api_response
    except Exception as e:
...
def generate_voice_otvet(prompt: str = None, filename: str = "ozv", voice: str = 'nova'):
    '''Голоса: alloy, echo, fable, onyx, nova, shimmer, coral, verse, ballad, ash, sage, amuch, dan'''
    try:
        if prompt is None:
            raise TypeError("Забыли указать prompt")
        response = requests.get(f"https://text.pollinations.ai/{prompt}?model=openai-audio&voice={voice}", headers={"Content-Type": "audio/mpeg"}, timeout=60.0)
        response.raise_for_status()
...
def generate_image(prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = False, model: str = 'flux', save: bool = False, filename: str = 'gen_img'):
    '''Доступные модели: turbo - Stable Diffusion 1.5 | flux - Stable Diffusion 2.1'''
    if prompt is None:
        raise ValueError("Забыли указать prompt")
    elif seed is None:
        seed = random.randint(0, 1000)
    if save:
        response = requests.get(f"https://pollinations.ai/p/{prompt}?width={width}&height={height}&seed={seed}&enhance={enhance}&private={private}&nologo={nologo}&model={model}")
        response.raise_for_status()
        with open(f'{filename}.jpg', 'wb') as file:
            file.write(response.content)
        return True
    else:
        return f"https://pollinations.ai/p/{prompt}?width={width}&height={height}&seed={seed}&enhance={enhance}&private={private}&nologo={nologo}&model={model}"