...
class MutationService:
    ...
    def extract_and_validate_transcript(self, input_hgvs: str) -> str:
        """
        Extracts the transcript part from the input HGVS string and validates
        that it is either "NM_004992.4" or "NM_001110792.2".

        Parameters:
        input_hgvs (str): The full HGVS mutation string (e.g., "NM_004992.4:c.916C>T").

        Returns:
        str: The validated transcript part.

        Raises:
        ValueError: If the transcript part is not one of the allowed values.
        """
        try:
            transcript = input_hgvs.split(":")[0]
        except Exception as e:
            raise ValueError(f"Invalid HGVS format: {input_hgvs}") from e
        ...
        if transcript not in allowed_transcripts:
            raise ValueError(f"Transcript '{transcript}' is not allowed. Expected one of {allowed_transcripts}.")
        return transcript
    ...
    def get_gene_mutation(self, input_hgvs: str) -> GeneMutation:
        ...
        try:
            ...
            input_transcript = self.extract_and_validate_transcript(input_hgvs)
            initial_response = self.variantvalidator_adapter.normalize_mutation(
                variant_description=input_hgvs,
                select_transcripts=input_transcript
            )
            ...
            if not genomic_coordinate:
                raise Exception("Genomic coordinate not found in the initial VariantValidator response.")
