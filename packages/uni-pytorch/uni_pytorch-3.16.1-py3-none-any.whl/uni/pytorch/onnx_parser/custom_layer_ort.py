...
# Actual ort implementation from sony-custom-layers requires all its torch dependencies, which increases the docker
...
@onnx_op(op_type="Sony::FasterRCNNBoxDecode",
...
def dummy_nms_ort(boxes, scores, score_threshold, iou_threshold, max_detections):
    ...
    return (np.zeros((batch, max_detections, 4)), np.zeros(
        (batch, max_detections)), np.zeros((batch, max_detections), dtype=np.int32), np.zeros((batch, 1),
                                                                                              dtype=np.int32))


@onnx_op(op_type="Sony::MultiClassNMSWithIndices",
         inputs=[PyCustomOpDef.dt_float, PyCustomOpDef.dt_float],
         outputs=[
...
def dummy_nms_with_indices_ort(boxes, scores, score_threshold, iou_threshold, max_detections):
    ...
    return (np.zeros((batch, max_detections, 4)), np.zeros(
        (batch, max_detections)), np.zeros(
            (batch, max_detections), dtype=np.int32), np.zeros((batch, max_detections),
