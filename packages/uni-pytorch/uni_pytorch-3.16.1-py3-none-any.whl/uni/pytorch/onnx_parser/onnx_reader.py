...
class OnnxReader(BaseReader):
    ...
    def __init__(self, fw_model: Union[str, onnx.ModelProto]):
        ...
        self._tensor_shapes: Dict[str, Optional[tuple]] = \
    ...
    def validate_model_input(self):
        ...
        if len(self.model.graph.input) != 1:
            raise UnsupportedModelInputError(ModelErrorInputMessages.MULTI_INPUT_ERR)
        input_shape = self.tensors[self.model.graph.input[0].name].shape
        if len(input_shape) != 4:
            raise UnsupportedModelInputError(ModelErrorInputMessages.DIMENSION_ERR)
        if input_shape[0] not in [1, None]:
            raise UnsupportedModelInputError(ModelErrorInputMessages.BATCH_SIZE_ERR)
        if None in input_shape[1:]:
            raise UnsupportedModelInputError(ModelErrorInputMessages.DYNAMIC_INPUT_ERR)
    ...
    def _build_nodes_and_connectivity(self):
        ...
        for n in self.model.graph.node:
            ...
            for i, tid in enumerate(node.output):
                ...
                self.tensors.add_source(tensor_id=tid, node_name=node.name, index=i, shape=self._tensor_shapes.get(tid))
    ...
    def _create_placeholder(self, graph_input: onnx.ValueInfoProto, ordinal: int) -> OnnxMetaNode:
        ...
        tensor_name, shape = OnnxReaderHelper.parse_tensor_type(graph_input)
        ...
        ph_node = OnnxMetaNode(name=name,
                               ...
                               history=history)
        self.tensors.add_source(tensor_id=tensor_name, node_name=ph_node.name, index=0, shape=shape)
        return ph_node

    def _create_output(self, graph_output: onnx.ValueInfoProto, ordinal: int) -> OnnxMetaNode:
        """ create 'fake' OnnxMetaNode for Output """
        tensor_id, shape = OnnxReaderHelper.parse_tensor_type(graph_output)
        ...
        out_node = OnnxMetaNode(name=name,
                                ...
                                history=history)
        self.tensors.add_dest(tensor_id=tensor_id, node_name=out_node.name, index=0, shape=shape)
        return out_node
