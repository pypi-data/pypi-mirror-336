...
class OnnxSimplifier:
    ...
    def _remove_transpose_after_mctq_const(self):
        nodes_to_delete, new_consts = self._find_nodes_transpose_after_mctq_const()
        self._apply_on_onnx(nodes_to_delete, new_consts)
    ...
    def _find_nodes_transpose_after_mctq_const(self):
        ...
        for quant_node in nodes:
            ...
            if not transpose_nodes:
                continue
            assert len(out_nodes) == len(transpose_nodes) == 1, "Only one transpose node is expected"
            transpose_node = transpose_nodes[0]
            weight_node: OnnxMetaNode = self.meta_graph.get_ordered_in_nodes(quant_node, data=True)[0]

            old_value = weight_node.get_const_data()
            axes = transpose_node.attr["perm"]
            new_value = np.transpose(old_value, axes=axes)
            new_weight_onnx = self._create_onnx_constant(weight_node.name, new_value, [quant_node.input[0]])
            new_consts.append(new_weight_onnx)
            ...
            nodes_to_delete[weight_node.name] = weight_node
            nodes_to_delete[transpose_node.name] = transpose_node
        return nodes_to_delete, new_consts
    ...
    def _apply_on_onnx(self, nodes_to_delete: dict, constant_to_add: list, new_edges: Optional[list] = None):
...
def main(args=None):
    ...
    if args.output:
        onnx.save(onnx_sim.onnx_model, args.output)
