...
class NnirNode(abc.ABC):
    ...
    def error_not_network_output(self, out_idx, is_last_node):
        msg = f'Output {out_idx} of {self.op} followed by another layer is invalid.'
        if is_last_node:
            msg += f' {self.op} should be the last layer in the network.'
        return msg
    ...
    def validate_node_output_is_network_output(self, idx_list: Optional[List[int]] = None, return_error: bool = False):
        ...
        from uni.common.core.nnir_graph.nnir_nodes import DummyOutput, Output
        ...
        for out_idx in idx_list:
            if any(not isinstance(out_node, (Output, DummyOutput)) for out_node in out_nodes[out_idx]):
                errors.append(
