...
def get_api_client():
    ...
    if not config:
        raise click.ClickException(
            f"설정 파일({get_config_path()})을 찾을 수 없습니다. 'uvx deeply-vault login -s <서버 URL>' 명령어로 로그인해주세요.")

    if not config.get("server"):
        raise click.ClickException(
            "서버 URL이 설정되지 않았습니다. 'uvx deeply-vault login -s <서버 URL>' 명령어로 로그인해주세요.")

    if not token:
        raise click.ClickException(
            "로그인이 필요합니다. 'uvx deeply-vault login' 명령어로 로그인해주세요.")

    if not config.get("vault"):
        raise click.ClickException(
            "볼트가 선택되지 않았습니다. 'uvx deeply-vault vault select <볼트 이름>' 명령어로 볼트를 선택해주세요.")
