...
class AmpliconProcessor:
    ...
    def _align_sequence_to_reference(self, sequence: str, ref_seq: str) -> str:
        ...
        try:
            # Optimize aligner settings to prevent overflow
            self.aligner.mode = 'global'
            self.aligner.match_score = 2
            self.aligner.mismatch_score = -3  # Increased penalty
            self.aligner.open_gap_score = -4  # Increased penalty
            self.aligner.extend_gap_score = -1.5  # Increased penalty
            self.aligner.target_internal_open_gap_score = -4
            self.aligner.target_internal_extend_gap_score = -1.5
            self.aligner.target_left_open_gap_score = -4
            self.aligner.target_left_extend_gap_score = -1.5
            self.aligner.target_right_open_gap_score = -4
            self.aligner.target_right_extend_gap_score = -1.5
            self.aligner.query_internal_open_gap_score = -4
            self.aligner.query_internal_extend_gap_score = -1.5
            self.aligner.query_left_open_gap_score = -4
            self.aligner.query_left_extend_gap_score = -1.5
            self.aligner.query_right_open_gap_score = -4
            self.aligner.query_right_extend_gap_score = -1.5
            
            # Limit the number of alignments to prevent overflow
            self.aligner.max_alignments = 1
            ...
            try:
                ...
                if "number of optimal alignments is larger than" in str(e):
                    # Fall back to local alignment with stricter settings
                    self.aligner.mode = 'local'
                    self.aligner.mismatch_score = -4
                    ...
                    try:
                        ...
                        logger.warning("Alignment failed even with fallback method, returning original sequence")
                        return sequence
                else:
                    raise
    ...
    def _analyze_amplicons(self,
        ...
        for haplotype, count in sorted(filtered_haplotypes.items(), key=lambda x: x[1], reverse=True):
            ...
            # This gives a more accurate representation of the haplotype abundance
            frequency = (count / total_reads) * 100
            
            # Count mutations (lowercase bases indicate mutations, '-' indicates indels)
    ...
    def _get_mutation_positions(self, haplotype: str, ref_seq: str, ref_name: str) -> List[Dict[str, str]]:
        ...
        while hap_pos < len(haplotype):
            if haplotype[hap_pos] == '-':  # Deletion
                ...
                mutations.append({
                    'position': pos + 1,  # Convert to 1-based
                    'ref': ref_seq[pos].upper(),
                    'alt': haplotype[hap_pos].upper(),
                    'type': 'substitution'
                })
                pos += 1
                hap_pos += 1
