...
class Blipper:
    def __init__(
        ...
        woocommerce_site_url=None,
        woocommerce_username=None,
        woocommerce_password=None,
        google_access_token=None
    ):
        self.blipper_api_key = blipper_api_key
        self.anthropic_api_key = anthropic_api_key
        self.base_url = config.blipper_url
        self.model = (model,)
        self.headers = {
            ...
            "atenea-woocommerce-site-url":woocommerce_site_url,
            "atenea-woocommerce-username":woocommerce_username,
            "atenea-woocommerce-password": woocommerce_password,
            "model": model,
        }
    ...
    def response_template(self, input_data: dict, func_name: str):
        if self.authenticated:
            response = requests.post(
                config.blipper_url + func_name, json=input_data, headers=self.headers
            )
    ...
    def response_files(self, input_data: dict, files: dict, func_name: str):
        if self.authenticated:
            response = requests.post(
                config.blipper_url + func_name, files=files, data=input_data, headers=self.headers
            )
            logger.info(f"response: {response}")
            if self.verbose:
                resp_json = response.json()
    ...
    def getBlipperStatus(self, apikey):
        data = {"key": apikey}
        response = requests.post(config.BLIPPER_AUTH_URL, json=data)
    ...
    def getAnthropicStatus(self, apikey):
        ...
        response = requests.post(config.ANTHROPIC_AUTH_URL, json=data, headers=headers)
        return response.ok
    ...
    def getAllFiles(self):
        if self.authenticated:
            ...
            response = requests.get(config.blipper_url + "getAllFiles", headers=headers)
    ...
    def list_agents(self):
        if self.authenticated:
            response = requests.get(
                config.blipper_url + "list-agents", headers=self.headers
            )
            return response.json()
        else:
            print_invalid_api_key()
    ...
    def WooconnercePublishProduct(
        ...
        sku: str | None = None,
        categories: list[Union[int, str]] | None = None,
        ...
        func_name = "woocommerce/publish-product"
        result = self.response_template(input_data=data, func_name=func_name)
        return result
    

    def WooconnerceModifyProduct(
        self,
        product_identifier: Union[int, str],
        lookup_by: str,
        name: str | None = None,
        ...
        func_name = "woocommerce/modify-product"
        result = self.response_template(input_data=data, func_name=func_name)
        return result
    

    def WooconnercePublishBlog(self, title: str, content: str, status: Literal["publish", "draft"]):
        if self.authenticated:
            data = {
                "title": title,
                "content": content,
        ...
        func_name = "woocommerce/publish-blog"
        result = self.response_template(input_data=data, func_name=func_name)
        return result


    def GoogleCalendarListCalendars(self):
        response = requests.get(
            config.blipper_url + f"google/calendars/?access_token={self.google_access_token}", headers=self.headers
        )
        logger.info(f"response: {response}")
        return response.json()["response"]

    def GoogleCalendarCreateEvent(
        self,
        description: str,
        start_datetime: str,
        end_datetime: str
        ...
        response = requests.post(
            config.blipper_url + f"google/events/?access_token={self.google_access_token}", json=event_data, headers=self.headers
        )
        logger.info(f"response: {response}")
        return response.json()["response"]
    
    def GoogleCalendarModifyEvent(
        self,
        description: str,
        start_datetime: str,
        end_datetime: str
    ...
    def GoogleCalendarFindEvent(self, event_id: str):
        response = requests.get(
            config.blipper_url + f"google/events/{event_id}/?access_token={self.google_access_token}", headers=self.headers
        )
        logger.info(f"response: {response}")
        return response.json()["response"]
    
    def GoogleCalendarDeleteEvent(self, event_id: str):
    ...
    def GoogleCalendarFreeBusy(self, time_min: str, time_max: str):
        data = {
            "time_min": time_min,
            "time_max": time_max
        }
        response = requests.post(
            config.blipper_url + f"google/freebusy/?access_token={self.google_access_token}", json=data, headers=self.headers
        )
        logger.info(f"response: {response}")
        return response.json()["response"]
    
    def extractTextFromFile(self, localfile: BufferedReader, file_id: str | None = None) -> None:
        if self.authenticated:
            file = {"localfile": (localfile.name, localfile.read(), Path(localfile.name).suffix)}
            response = requests.post(
                self.base_url + "extractText",
                files=file,
    ...
    def GoogleCalendarGenerateToken(self):
        import webbrowser
        print(f"Abrir la siguiente página en el navegador para generar el token: https://blipper.epystemic.com/google/login/")
        webbrowser.open("https://blipper.epystemic.com/google/login/")
