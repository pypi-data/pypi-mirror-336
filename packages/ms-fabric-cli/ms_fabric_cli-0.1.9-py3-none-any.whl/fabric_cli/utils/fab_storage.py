...
def _write_to_local(file_path: str, data: Any, export: Optional[bool] = True) -> None:
...
def _write_to_lakehouse(
    file_path: str, data: Any, export: Optional[bool] = True
) -> None:
    processed_data, is_json = _validate_json(data)
    if is_json:
    ...
    file_path = file_path.replace(
        ".Workspace", "", 1).replace(".workspace", "", 1)
    args = Namespace()
...
def _write_to_sjd(
    ...
    try:
        ...
        raise FabricCLIError(
            f"Cannot access '{file_path}'. No such file or directory")
