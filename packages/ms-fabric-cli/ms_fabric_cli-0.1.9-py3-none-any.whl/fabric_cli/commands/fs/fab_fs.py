...
def mv_command(args: Namespace) -> None:
    ...
    _check_command_line_support(
        args.fab_mode, from_context, (args.force or args.all))
...
def export_command(args: Namespace) -> None:
    context = handle_context.get_command_context(args.path)
    _check_command_line_support(
        args.fab_mode, context, (args.force or args.all))
    ...
    if len(args.from_path) == len(args.to_path) == 1:
        ...
        for i in range(1, len(combined_paths)):
            ...
            try:
                ...
                if all(
                    isinstance(context, OneLakeItem) or isinstance(
                        context, Item)
...
def _print_context() -> None:
    ...
    full_name = context.get_name()
    name = full_name.split(".")[0]
    type_ = "." + full_name.split(".")[1] if "." in full_name else ""
    id_ = context.get_id() if context.get_id() else ""
...
def _check_command_line_support(mode: str, context: Context, force: bool) -> None:
...
def _search_capacity_id(capacity_name: str) -> str | None:
    ...
    for sub in subscriptions:
        ...
        capacity = next(
            (item for item in capacities if item["name"].lower(
            ) == cp_name.lower()),
            None,
        )
        if capacity:
            rg_name = capacity["id"].split("/")[4]
            # utils_ui.print_grey(
            #     f"Found Capacity '{cp_name}' in Subscription '{sub_name}' with id '{sub_id}' and Resource Group '{rg_name}'"
            # )
            return capacity["id"]
...
def _find_capacity(virtual_ws_item: VirtualWorkspaceItem, capacities: list) -> dict:
    for capacity in capacities:
        if capacity["id"] == virtual_ws_item.get_id():
            return capacity
    raise FabricCLIError(
        f"Capacity {virtual_ws_item.get_name()} not found",
        fab_constant.ERROR_NOT_FOUND,
    )
...
def check_fabric_capacity(virtual_ws_item: VirtualWorkspaceItem) -> None:
    ...
    if fab_response.status_code in {200, 201}:
        ...
        # Chek that the capacity sku matches the expression F\d+
        if not re.match(r"F\d+", _capacity.get("sku", "Unknown")):
            raise FabricCLIError(
                f"Capacity {virtual_ws_item.get_name()} is not a F SKU. Only Fabric capacities are supported",
