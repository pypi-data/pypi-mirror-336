...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Sequential(
            NN_FCBNRL_MM(hidden_size, cluster_dim, use_RL=False),
        )
...
class DMTEVT_model(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cluster_dim is None:
            self.cluster_dim = T_hidden_size
        else:
        ...
        self.vis = self.InitNetworkMLP(
            NS=[self.cluster_dim*num_use_moe, vis_dim],
            last_relu=False, use_DO=False)
    ...
    def InitNetworkMLP(self, NS, last_relu=True, use_DO=True, use_BN=True, use_RL=True):
        ...
        for i in range(len(NS) - 1):
            ...
            if i == len(NS) - 2 and not last_relu:
                layers.append(NN_FCBNRL_MM(NS[i], NS[i + 1], use_RL=False, use_DO=False, use_BN=False))
            else:
                layers.append(NN_FCBNRL_MM(NS[i], NS[i + 1], use_RL=False, use_DO=False, use_BN=False))
    ...
    def forward(self, x, tau=100.0):
        ...
        if len(x.shape) == 2:
            ...
            if len(x.shape) == 2:
                ...
                # print('self.mask.device:', self.mask.device)
                ...
                x_masked = torch.einsum('bik,bk->bik', self.mask, x)
                x_masked = x_masked.reshape((batch_size, self.hparams.num_use_moe, num_channel, num_x, num_y))
        else:
        ...
        # Reshape outputs
        lat_high_dim = lat_higt_dim_out.reshape((batch_size, -1))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.use_high_manifold_loss:
            loss_emb += self.LossManifold(
        ...
        # Compute total loss
        loss_all = loss_emb + orthogonal_loss * 0.1
