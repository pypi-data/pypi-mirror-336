...
class Aliceblue:
        ...
        if len(filter_contract) == 0:
            ...
            for i in range(len(filter_contract)):
                if filter_contract['Token'][i] not in token:
                    ...
                    inst.append(Instrument(filter_contract['Exch'][i], filter_contract['Token'][i],
                                           filter_contract['Symbol'][i], filter_contract['Trading Symbol'][i],
                                           filter_contract['Expiry Date'][i], filter_contract['Lot Size'][i]))
            if len(inst) == 1:
                return inst[0]
            else:
                return inst
    ...
    def search_instruments(self, exchange, symbol):
        ...
        if scrip_response == []:
            ...
            for i in range(len(scrip_response)):
                ...
                inst.append(Instrument(scrip_response[i]['exch'], scrip_response[i]['token'],
                                       scrip_response[i]['formattedInsName'], scrip_response[i]['symbol'],
                                       scrip_response[i]['expiry'], scrip_response[i]['lotSize']))
            return inst
    ...
    def basket_margin(self, basketmargins):
        basket_data = []
        for basket_list in basketmargins:
            basket_data.append({
                "exchange": basket_list['exchange'],
                "tradingSymbol": basket_list['tradingSymbol'],
                "price": basket_list['price'],
                "qty": basket_list['qty'],
                "product": basket_list['product'],
                "priceType": basket_list['priceType'],
                "triggerPrice": basket_list['triggerPrice'],
                "transType": basket_list['transType']
            })
        ...
        if response['stat'] == 'Ok':
            ...
            for i in range(len(total_holdings)):
                data = {
                    ...
                    "trading_symbol": total_holdings[i]['Bsetsym'] if total_holdings[i]['ExchSeg1'] == 'BSE' else
                    total_holdings[i]['Nsetsym'],
