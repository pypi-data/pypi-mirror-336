...
class iPlugin:
    ...
    # MAX_RECORDS = 10 # TODO: agp: REMOVE HACK
        ...
        if (n := self.parent.remain_tasks()) < self.MAX_QUEUE_TASK:
            if swarm.uid:
                ...
                for _task in milk(universe):
                    ...
                    # reschedule the task even further
            ...
            for key in sort_key:
                ...
                if func := TYPES_MAP.get(value.__class__):
                    candidates = {
                        ...
                        if k != key and func(sample[k]) == value
                    }
                    if candidates:
                        ...
                        # # using same sort_key as maybe source and target
...
class PutPlugin(iPlugin):
    ...
    async def handle(self, stream: List[Dict], context: Dict):
        ...
        for idx, data in enumerate(stream):
            ...
            # if not (idx % 100):
            #     await self.bot.mutex.update(p=idx/L)
