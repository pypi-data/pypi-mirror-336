...
class Client(object):
    ...
    def create_training_task(self,
        ...
        try:
            ...
            if framework is None:
                raise TencentCloudSDKException(message='missing parameter {}'.format("framework"))
            ...
            if framework.Name == "CUSTOM":
                ...
                if framework.UserName is not None and framework.Passwd is not None:   
                    ...
                    req.ImageInfo.ImageSecret.KeyId = self._platform_public_key_info.KeyId
            else:
            ...
            if req.TrainingMode == "SPARK":
                ...
                if head_resource.InstanceNum != 1:
                    raise TencentCloudSDKException(message='RAY task head only support one instance')
            ...
            if restart_limit != 0:
                if restart_limit > 10:
                    raise TencentCloudSDKException(message='restart_limit should be less than 10')
    ...
    def _encrypt(self, text):
        if text is None:
            return None
        if self._platform_public_key_info == None:
            self._init_public_key()
        from .encrypt import rsa_encrypt
        byte_text = text.encode('utf-8')
        if len(byte_text) <= MAX_CHUNK_SIZE:
            return rsa_encrypt(self._platform_public_key_info.PublicKeyPem, byte_text).decode("utf-8")
        encrypted_results = []
        start_index = 0
        while start_index < len(byte_text):
            ...
            encrypted_chunk = rsa_encrypt(self._platform_public_key_info.PublicKeyPem, chunk)
            if encrypted_chunk:
                encrypted_results.append(encrypted_chunk.decode('utf-8'))
            else:
                raise TencentCloudSDKException(message="rsa encrypt error, encrypted_chunk is None")
