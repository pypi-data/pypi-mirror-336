...
# {
        ...
        for i, member_params in enumerate(class_params_spec):
            if i == choose_rem:
                ...
                if rem_member in member_params:
                    index_to_remove = np.where(member_params == rem_member)[0]
                    if len(index_to_remove) > 0 and len(member_params) > 1:  # Check if index exists and size will remain >= 1
    ...
    # {
        ...
        #ROB I turned this off, it's working as one would expect, as well, we only care about memebership
        #if new_param not in solution['isvars']:

        # THIS    solution['isvars']  = list(set(solution['isvars']+[str(new_param)]))
        # NOT THIS   solution['isvars'] = sorted(set(solution['isvars']).union(new_param))
    # }
    ...
    def perturb_add_member_paramfeature(self, solution):
        ...
        if len(candidates) > 0:
            if self.param.LCR is not None:
                all_vars = self.param.LCR.classes.get('latent_class_1', {}).get('mem_vars', None)
                if all_vars is None:
            ...
            while retries < max_retries:
                ...
                if self.param.LCR is not None:
                    ...
                    if all_vars is None:
                        raise ValueError(f"all_vars is None for latent_class_{pick+1}. Check LCR configuration.")

                # Ensure member_params_spec[pick] is not None
                if member_params_spec[pick] is None:
                    raise ValueError(f"member_params_spec[{pick}] is None. Check your solution['member_params_spec'].")
