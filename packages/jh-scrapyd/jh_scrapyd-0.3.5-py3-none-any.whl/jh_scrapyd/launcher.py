...
from jh_scrapyd import __version__
from jh_scrapyd.interfaces import IEnvironment, IJobStorage, IPoller
from jh_scrapyd.jh.common import kill_process
...
class Launcher(Service):
    ...
    def __init__(self, config, app):
        ...
        self.runner = config.get("runner", "jh_scrapyd.runner")
        self.app = app
    ...
    def startService(self):
        ...
        log.info(
            "jh_scrapyd {version} started: max_proc={max_proc!r}, runner={runner!r}",
            version=__version__,
    ...
    def restart(self, process):
        """重启指定的进程"""
        for p_index, p in list(self.processes.items()):
            if p.pid != process.pid:
                continue

            try:
                # 终止进程
                if not kill_process(int(process.pid)):
                    log.error(f"Process {process.pid} could not be killed.")
                    break

                # 结束旧进程并重新拉起新的进程
                self._process_finished("", p_index)

                log.info(f"Process {process.pid} successfully restarted.")
                break  # 找到目标进程后退出循环

            except (OSError, KeyError) as e:
                log.error(f"Failed to restart process {process.pid}: {e}")
                break

        else:  # 如果循环结束未找到匹配的进程
            log.error(f"Process {process.pid} not found in the process pool.")
