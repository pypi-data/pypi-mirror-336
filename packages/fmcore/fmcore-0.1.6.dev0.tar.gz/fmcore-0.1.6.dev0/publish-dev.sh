...
# For macOS BSD sed, we need to use a different approach
# Replace dynamic versioning with static version
sed -i '' "s/dynamic = \[\"version\"\]/version = \"$DEV_VERSION\"/" pyproject.toml

# Remove the hatch.version section (using a temp file approach for macOS)
awk '!/\[tool.hatch.version\]/ && !/source = "vcs"/ && !/local = false/' pyproject.toml > pyproject.tmp && mv pyproject.tmp pyproject.toml
