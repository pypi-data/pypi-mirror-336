...
def run_docker_container(image: str, port_mapping: bool = True, interactive_bash: bool = False, additional_args: List[str] = None, attach: bool = False) -> None:
    """
...
def test_environment_connection(server_address: str, num_steps: int = 5) -> None:
    """
    Test the connection to a NeoDynamics environment server by calling the client's main function.
    ...
    """
    try:
        # Import the client module's main function
...
def stop_container(image: str) -> None:
    """
    Stop a Docker container by image name.

    Args:
        image: The image name of the container to stop
    """
    try:
        ...
        print(f"Error stopping container: {str(e)}", file=sys.stderr)
        sys.exit(1)
...
def build_run(
    ...
    try:
        ...
        # Start container in background (detached mode is now default)
        run_docker_container(image, port_mapping, False, additional_args=None, attach=False)
...
def build_run_test(
    ...
    """
    try:
        image = build_run(path, name, tag, port_mapping, verbose, context)

        # Step 3: Test the connection
        print("\n=== STEP 3: Testing Connection ===")

        # Test the connection
        test_environment_connection(server_address, num_steps)
...
def main():
    ...
    # Test command
    test_parser = subparsers.add_parser("test", help="Test connection to a NeoDynamics environment server")
    ...
    if args.command == "build":
        ...
        test_environment_connection(args.address, args.steps)
