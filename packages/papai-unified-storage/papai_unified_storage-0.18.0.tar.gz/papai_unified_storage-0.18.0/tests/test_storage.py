...
from papai_unified_storage.utils import joinpath
...
def test_dataframe_read_parquet(storage: Storage):
    import pandas as pd
...
def test_dataframe_write_parquet(storage: Storage):
    import pandas as pd
    ...
    with TemporaryDirectory() as d:
        storage.write_dataframe_to_parquet(f"{d}/test.parquet", df)
...
def test_disable_debug_logging(storage: Storage):
    def assert_never_called(msg: str):
        assert False

    storage.log_debug_fn = assert_never_called
    with storage.disable_debug_logging():
        storage.log_debug_fn("This should not be called")
