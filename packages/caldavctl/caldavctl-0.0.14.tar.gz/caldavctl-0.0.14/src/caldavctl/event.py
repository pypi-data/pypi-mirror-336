...
from caldavctl.utils import ESCAPE_SEQUENCES, calmonth, edit_text_with_editor, to_datetime
...
@click.option('-t', '--today',
              ...
              callback=exclusive_option, help='Today\'s events',)
@click.option('-w', '--week',
              default=None, metavar='<offset>', type=int,
              callback=exclusive_option, help='Week events')
...
def list_events(context, today, week, dtstart, dtend, day, show_description, show_uid, show_timezone, template_file):
    ...
    The date range can be specified in several ways. The options are self
    explanatory except the --week option. This option takes an integer
    argument, if we use '0' the time range will be the current week. If we use
    '1' the time range will correspond to next week, likewise, '-1' corresponds
    to last week or '-2' to two weeks ago.
    ...
    with dav.caldav_calendar(server, calendar_id) as calendar:
        ...
        for event in events:
            ...
            start_date = ev.get('dtstart').dt
            if isinstance(start_date, datetime):
                start_date = start_date.astimezone(tz)
            end_date = ev.get('dtend').dt
            if isinstance(end_date, datetime):
                end_date = end_date.astimezone(tz)
            event_context = {
                'start_date': start_date.strftime('%Y-%m-%d %H:%M') if isinstance(start_date, datetime) else f'{start_date} (day)',
                'start_date_tz': start_date.tzinfo if isinstance(start_date, datetime) else None,
                'end_date': end_date.strftime('%Y-%m-%d %H:%M') if isinstance(end_date, datetime) else f'{end_date} (day)',
                'end_date_tz': end_date.tzinfo if isinstance(end_date, datetime) else None,
