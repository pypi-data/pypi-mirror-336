...
_trace_id: ContextVar[Optional[str]] = ContextVar("contrast_trace_id", default=None)
...
class ReportingClient(threading.Thread):
    ...
    def new_observability_span(self, span: dict) -> None:
        if (trace_id := _trace_id.get()) is None:
            ...
            return
        logger.debug(
            "observed span (note - currently has no effect)",
            span=span,
            trace_id=trace_id,
        )

    def update_observability_metric(self, metric: dict) -> None:
        logger.debug(
            "updated observability metric (note - currently has no effect)",
            metric=metric,
        )
