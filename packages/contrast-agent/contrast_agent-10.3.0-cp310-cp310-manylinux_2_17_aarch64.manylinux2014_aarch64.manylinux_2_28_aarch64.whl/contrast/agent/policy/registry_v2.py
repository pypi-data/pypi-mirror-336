...
def assess_cmd_exec_pre(context: RequestContext, instance, args, kwargs) -> None:
    pass


def assess_cmd_exec_post(
    context: RequestContext, instance, args, kwargs, result
) -> None:
    pass
...
def assess_cmd_exec_builder(event_dict: EventDict) -> EventHandler:
    def assess_cmd_exec_handler(
        context: RequestContext, instance, args, kwargs
    ) -> Generator:
        assess_cmd_exec_pre(context, instance, args, kwargs)
        result = yield
        assess_cmd_exec_post(context, instance, args, kwargs, result)

    return assess_cmd_exec_handler


def observe_handler_builder(event_dict: EventDict) -> EventHandler:
    def observe_handler(context: RequestContext, instance, args, kwargs) -> Generator:
        # TODO: PYT-3659 This is currently set up as a single, generic event handler for
        # all of observability. It's possible that we'll need to break this up into more
        # specific handlers for different observe events, but we might also be able to
        # achieve this with just slightly different EventDicts passed to this builder.
        #
        # Determining when to write an entierly new builder vs just using different args
        # in the event dict is a judgement call.
        _ = yield
...
def protect_handler_builder(event_dict: EventDict) -> EventHandler:
    def protect_handler(context: RequestContext, instance, args, kwargs) -> Generator:
        _ = yield
    ...
    def generic_contrast_wrapper(wrapped, instance, args, kwargs):
        ...
        with scope.contrast_scope():
            ...
            for handler in event_handlers:
                gen = handler(context, instance, args, kwargs)
                try:
                    next(gen)
