...
from kojen.kojentypes import Interface, Message
...
class TestFeatures(unittest.TestCase):
    ...
    def test_PurgeMessageHeaders(self):
        m1 = Message("m1", 1)
        m1.AddType("name", "type")
        m2 = Message("m2", 2)
        m2.AddType("name", "type")
    ...
    def test_extract_TAG_and_A_and_B_multiple_same(self):
    ...
    def test_extract_TAG_and_A_and_B_multiple_different(self):
        a = "XXX::blabla<<<something=1>>> !@#!@$ <<<else=2>>>"
        res_a = extractTagAndAandB(a, "<<<something>>>")
        res_b = extractTagAndAandB(a, "<<<else>>>")
        self.assertEqual(res_a[0], "<<<something=1>>>", "Wrong tag")
        self.assertEqual(res_a[1], "1", "Wrong default")
