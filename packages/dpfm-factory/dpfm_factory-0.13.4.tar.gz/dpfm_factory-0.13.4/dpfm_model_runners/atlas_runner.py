...
def robust_atlas_inference(data: bytes, input_img_size: int, target_img_size: int = 224, read_timeout_sec: int = 180, max_retries: int = 3) -> (str, int):
    global apigee_auth
...
class AtlasLoader:
    ...
    def get_image_embedding(self, image, processor=None, model=None, device=None):
        ...
        rgb_image_bytes = Image.fromarray(rgb_array).tobytes()
