...
class AssistantModel:
    ...
    def chat_completions(
        ...
        if model is None:
            ...
            raise ValueError(
                "Invalid model. Valid options are `gpt-4o` and `claude-3-5-sonnet`"
            )
    ...
    def _chat_completions_single(
        ...
        chat_request = (
            ChatCompletionsRequest(
                messages=messages, filter=filter, model=model)
            if filter
    ...
    def chat(
        ...
        context_options: Union[ContextOptions, Dict, None] = None,
        ...
        :param context_options: Option to control the context snippents sent to the LLM.
        ...
        if stream:
            return self._chat_streaming(messages=messages, model=model, filter=filter, include_highlights=include_highlights, context_options=context_options)
        else:
            return self._chat_single(messages=messages, model=model, filter=filter, json_response=json_response, include_highlights=include_highlights, context_options=context_options)
    ...
    def _chat_streaming(
        ...
        if context_options is not None:
            ...
            if context_options.snippet_size is not None:
                options["snippet_size"] = context_options.snippet_size
            if options:
                content["context_options"] = vars(ContextOptionsModel(**options))
