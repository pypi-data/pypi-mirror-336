            ...
            if file_url is not None:
                ...
                if resp.ok:
                    ...
                    if splitter is not None:
                        ...
                        for chunk in chunks:
                            ctext += chunk.text
                            sentences.append(Sentence(start=start, end=len(ctext), metadata=chunk.metadata))
                            start = len(ctext)
                        assert ctext == text
                    doc = Document(identifier=source.filename, title=title, text=text, sentences=sentences,
