...
ENV = {
    ...
    # "GOPATH": os.environ["GOPATH"], FIXME: it might actually be requried
...
class GenerateCdxgenSbom(Pipeline):
    ...
    def run_cdxgen_scan(self):
        ...
        cwd = self.project.codebase_path
        command = [
            CDXGEN_BIN,
            cwd,
            "--spec-version",
        ...
        logger.info(f"[*] codebase_path: {self.project.codebase_path}")
        env = get_env(self.project.codebase_path)
    ...
    def clear_repo(self):
        ...
        """
        logger.info(f"[*] Running clear_repo on {self.project.codebase_path}")
        if self.project.codebase_path.exists():
            subprocess_run(["rm", "-rf", self.project.codebase_path])
