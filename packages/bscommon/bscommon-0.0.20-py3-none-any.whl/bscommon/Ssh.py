...
def sshRunAction(action,configPath:str,configName:str=None,args:list=None):
	def actionT(config,args):
		Com.ssh(action,config.ip,config.port,config.username,config.password,config)
...
def sshManageRun(nameOrAction,configName:str=None):
	if type(nameOrAction) is str:
		sshRun(nameOrAction,mainDir+'configs'+os.sep+'managenodes'+os.sep,configName)
	else:
		sshRunAction(nameOrAction,mainDir+'configs'+os.sep+'managenodes'+os.sep,configName)

# 工作节点：连接ssh并执行python脚本
# nameOrAction-模块名/函数; 模块名-main为入口函数，如: utils.com 或 com; 函数-外部定义
def sshWorkRun(nameOrAction,configName:str=None):
	if type(nameOrAction) is str:
		sshRun(nameOrAction,mainDir+'configs'+os.sep+'worknodes'+os.sep,configName)
	else:
		sshRunAction(nameOrAction,mainDir+'configs'+os.sep+'worknodes'+os.sep,configName)

# 主机节点（即普通节点）：连接ssh并执行python脚本
# nameOrAction-模块名/函数; 模块名-main为入口函数，如: utils.com 或 com; 函数-外部定义
def sshHostRun(nameOrAction,configName:str=None):
	if type(nameOrAction) is str:
		sshRun(nameOrAction,mainDir+'configs'+os.sep+'hosts'+os.sep,configName)
	else:
		sshRunAction(nameOrAction,mainDir+'configs'+os.sep+'hosts'+os.sep,configName)
