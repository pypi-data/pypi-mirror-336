...
from ligandmpnn.utils.data_utils import (
    alphabet,
    element_dict_rev,
    featurize,
    get_score,
...
from ligandmpnn.utils.model_utils import ProteinMPNN
from ligandmpnn.utils.sc_utils import Packer, pack_side_chains
...
def get_args():
    ...
    argparser.add_argument(
        "--pdb_path", type=str, help="Path to the input PDB."
    )
    argparser.add_argument(
        "--pdb_path_multi",
        type=str,
    ...
    argparser.add_argument(
        "--checkpoint_path_sc",
        type=str,
        default="ligandmpnn_sc_v_32_002_16.pt",
        help="Path to model weights.",
    )
...
def main() -> None:
    ...
    """
    args = get_args()
    if args.pdb_path_multi == None and args.pdb_path == None:
        print("You must specify either --pdb_path or --pdb_path_multi", flush=True)
        sys.exit()
