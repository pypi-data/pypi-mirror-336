...
def _make_preparing_callback(method_name, message_cls):
    def callback(self: "BaseHandler", client: ws_base.WebSocketClientBase, command: dict):
        method = getattr(self, method_name)
...
class BaseHandler(HandlerInterface):
    ...
    _CMD_CALLBACK_DICT = {
        ...
        # 直播准备 | 结束直播
        "PREPARING": _make_preparing_callback("_on_preparing", web_models.PreparingMessage),
    ...
    def _on_preparing(self, client: ws_base.WebSocketClientBase, message: web_models.PreparingMessage):
        """
        直播间关闭
        """
