...
from vajra._native.utils.zmq_helper import recv_step_outputs, send_step_inputs
...
class BaseLLMReplicaController(BaseReplicaController):
    ...
    def _bind_zmq_socket(self, socket: ZmqSocket, port: int):
        for attempt in range(MAX_ZMQ_RETRIES):
            try:
                ...
                if attempt < MAX_ZMQ_RETRIES - 1:
                    logger.info(
                        f"Failed to bind socket, retrying in {ZMQ_RETRY_DELAY} seconds..."
                    )
                    time.sleep(ZMQ_RETRY_DELAY)
                else:
                    raise Exception(
                        f"Failed to bind socket after {MAX_ZMQ_RETRIES} attempts"
                    ) from e

    def _init_zmq_sockets(self) -> None:
        self.zmq_context = ZmqContext()
        self.enqueue_socket = ZmqSocket(self.zmq_context, ZmqConstants.PUB)
        self._bind_zmq_socket(self.enqueue_socket, self.comm_info.enqueue_socket_port)
        self.output_socket = ZmqSocket(self.zmq_context, ZmqConstants.PULL)
    ...
    def step(self) -> None:
        ...
        send_step_inputs(self.enqueue_socket, step_inputs)
        self.metrics_store.on_schedule(
            self.replica_id, scheduler_output, start_time, end_time
        )

        all_sampler_outputs: List[SamplerOutputs] = []
        for _ in range(self.config.parallel_config.kv_parallel_size):
            step_outputs = recv_step_outputs(self.output_socket)
        ...
        for output in all_request_outputs:
