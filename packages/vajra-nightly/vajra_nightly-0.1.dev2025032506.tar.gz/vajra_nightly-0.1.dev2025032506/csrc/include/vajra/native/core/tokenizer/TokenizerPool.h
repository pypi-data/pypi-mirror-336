...
#include "native/utils/BlockingQueue.h"

namespace vajra {

struct TokenizerPoolInput {
  TokenizerPoolInput(const std::string& seq_id_param,
                     const float arrival_time_param,
  ...
  const float arrival_time;
  const std::string prompt;
  const SamplingParams sampling_params;
};

struct TokenizerPoolOutput {
  TokenizerPoolOutput(const std::string& seq_id_param,
                      const float arrival_time_param,
                      const std::string& prompt_param,
...
class TokenizerPool {
 public:
  ...
  std::optional<TokenizerPoolOutput> GetOutput();
 ...
 private:
  ...
  BlockingQueue<TokenizerPoolInput> input_queue_;
  BlockingQueue<TokenizerPoolOutput> output_queue_;
