...
constexpr int PROFILE_LAYER_ID = 1;
...
class WorkerMetricsStore : public BaseMetricsStore {
 public:
  ...
  bool IsOperationEnabled(MetricType metric_type, std::optional<int> rank,
                          std::optional<int> layer_id);

  // Metrics recording methods
  void OnBatchStageStart(const SchedulerOutputPtr& scheduler_output);

  void OnBatchStageEnd(const SequenceMetadataVector& seq_metadata_list,
                       int tensor_parallel_rank, int pipeline_parallel_rank,
                       int kv_parallel_rank, float start_time, float end_time);
