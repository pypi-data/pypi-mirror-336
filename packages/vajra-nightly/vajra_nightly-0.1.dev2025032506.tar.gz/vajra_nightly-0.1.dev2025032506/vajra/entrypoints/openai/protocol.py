...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    def to_sampling_params(self) -> SamplingParams:
        ...
        assert self.temperature is not None, "Temperature cannot be None"
        assert self.top_p is not None, "Top_p cannot be None"
        assert self.max_tokens is not None, "Max_tokens cannot be None"
        assert self.top_k is not None, "Top_k cannot be -1"
        assert self.ignore_eos is not None, "Ignore_eos cannot be None"

        assert not self.stop, "Stop string is not supported in Vajra yet."
