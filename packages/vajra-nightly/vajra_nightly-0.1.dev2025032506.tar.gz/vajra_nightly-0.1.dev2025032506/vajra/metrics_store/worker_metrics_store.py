...
from vajra.datatypes import SchedulerOutput, SequenceMetadata  # type: ignore
...
class WorkerMetricsStore(BaseMetricsStore):
    ...
    def reset_native_handle(self) -> WorkerMetricsStoreC:
        self._native_handle = WorkerMetricsStoreC(
            self.config, self.metrics, self.model_num_layers
        )
        return self._native_handle
    ...
    def is_operation_enabled(
        ...
        **kwargs,
        ...
        return self.native_handle.is_operation_enabled(metric_type, rank, layer_id)
