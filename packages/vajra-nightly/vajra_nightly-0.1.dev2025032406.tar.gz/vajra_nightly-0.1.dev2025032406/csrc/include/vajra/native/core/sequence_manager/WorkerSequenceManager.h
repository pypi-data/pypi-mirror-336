...
class WorkerSequenceManager : public BaseSequenceManager {
 public:
  ...
  void OnStepCompleted(const std::vector<SequenceScheduleMetadataPtr>&
                       ...
                       const SamplerOutputs& sampler_outputs) override;

  std::pair<Sequences, Sequences> OnSchedule(
      SchedulerOutputPtr scheduler_output) override;

  std::tuple<Sequences, Sequences, std::vector<SequenceMetadataPtr>>
  OnScheduleWorker(const SchedulerOutputPtr scheduler_output);
 ...
 private:
  void UpdateSeqNumProcessedTokens(
  ...
  std::unordered_map<std::string, std::size_t> seq_num_processed_tokens_;
