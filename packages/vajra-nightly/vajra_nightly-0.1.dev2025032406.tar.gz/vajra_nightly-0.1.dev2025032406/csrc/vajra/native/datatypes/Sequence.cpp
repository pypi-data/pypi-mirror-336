...
Sequence::Sequence(const std::string seq_id_param,
                   ...
                   const double arrival_time_param,
...
void Sequence::UpdatePromptTokensProcessed(std::size_t num_tokens /*[in]*/) {
  ...
  ASSERT_VALID_RUNTIME(num_tokens > 0,
                       ...
                       num_tokens);
  std::size_t num_processed = prompt_tokens_processed_ + num_tokens;
  ASSERT_VALID_RUNTIME(
      num_processed <= prompt_token_ids_->size(),
      "number of processed prompt tokens (from {} to {}) exceeds the "
      "number of prompt "
      "tokens {}",
      prompt_tokens_processed_, num_processed, prompt_token_ids_->size());

  prompt_tokens_processed_ = num_processed;
  if (prompt_tokens_processed_ == prompt_token_ids_->size()) {
    ASSERT_VALID_RUNTIME(
...
void Sequence::UpdatePromptTokensStageProcessed(
  ...
  std::size_t num_processed = prompt_tokens_stage_processed_ + num_tokens;
  ASSERT_VALID_RUNTIME(
      num_processed <= prompt_token_ids_->size(),
      "number of processed stage prompt tokens (from {} to {}) exceeds the "
      "number of prompt "
      "tokens {}",
      prompt_tokens_stage_processed_, num_processed, prompt_token_ids_->size());

  prompt_tokens_stage_processed_ = num_processed;
  if (prompt_tokens_stage_processed_ == prompt_token_ids_->size()) {
...
void Sequence::Reset() {
  ...
  prompt_tokens_stage_processed_ = 0;
...
void Sequence::AppendTokensToBlocks(const TokenIds& token_ids /*[in]*/) {
  ...
  while (cursor < token_ids.size()) {
    ...
    last_block->AppendTokens(std::vector<std::size_t>(
