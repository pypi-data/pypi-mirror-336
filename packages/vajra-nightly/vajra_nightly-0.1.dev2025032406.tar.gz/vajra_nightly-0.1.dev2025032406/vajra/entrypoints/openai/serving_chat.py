...
from vajra.engine.async_inference_engine import AsyncInferenceEngineWrapper
...
from vajra.entrypoints.openai.serving_engine import OpenAIServing
from vajra.logger import init_logger
from vajra.utils import random_uuid
...
class OpenAIServingChat(OpenAIServing):
    ...
    def __init__(
        self,
        engine_wrapper: AsyncInferenceEngineWrapper,
    ...
    def _parse_chat_message_content(
        ...
        # TODO(Amey, Varun, Mayank): Can we make this more type-safe?
        return self._parse_chat_message_content_parts(role, content)  # type: ignore
    ...
    async def create_chat_completion(
        ...
        try:
            ...
            # TODO(Amey, Varun, Mayank): Does this work correctly?
        ...
        assert isinstance(prompt, str), "Currently we only support string prompt"

        result_generator = self.engine_wrapper.generate(
            request_id,
            prompt,
            sampling_params,
        )
        ...
        if request.stream:
            ...
            try:
                ...
                # TODO: Use a vajra-specific Validation Error
                return self.create_error_response(str(e))
    ...
    async def chat_completion_full_generator(
        ...
        async for res in result_generator:
            if raw_request is not None and await raw_request.is_disconnected():
                ...
                await self.engine_wrapper.abort(request_id)
            ...
            message = ChatMessage(
                ...
                tool_calls=[
                    ToolCall(
                        function=FunctionCall(
                            name=request.tool_choice.function.name,
                            arguments=final_res.text,
                        )
                    )
                ],
            )
        elif not request.tool_choice or request.tool_choice == "none":
            message = ChatMessage(role=role, content=final_res.text)
        else:
            raise NotImplementedError(f"Unknown tool choice: {request.tool_choice}")

        choice_data = ChatCompletionResponseChoice(
            message=message, finish_reason=final_res.finish_reason
        )
