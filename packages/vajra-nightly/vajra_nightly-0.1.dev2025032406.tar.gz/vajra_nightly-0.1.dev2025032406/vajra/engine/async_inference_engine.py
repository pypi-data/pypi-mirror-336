...
class _AsyncInferenceEngine:
    ...
    def get_model_config(self) -> "ModelConfig":
        # Access model config through controller for now
    ...
    async def get_outputs_async(self) -> List[RequestOutput]:
        """
        Simple wrapper around the synchronous `get_outputs` method
        """
        return await asyncio.get_event_loop().run_in_executor(
            None, self.engine.get_outputs
        )  # type: ignore


class AsyncInferenceEngineWrapper:
    ...
    def __init__(self, engine: _AsyncInferenceEngine, verbose: bool = False) -> None:
        self.engine = engine
    ...
    @classmethod
    def get_wrapper_from_engine_config(
        cls, config: InferenceEngineConfig, verbose: bool = False
    ) -> "AsyncInferenceEngineWrapper":
        """Creates an LLM engine wrapper from the system configuration."""
        # Create InferenceEngine instead of directly using controller
        engine = InferenceEngine(config)
        return cls(_AsyncInferenceEngine(engine), verbose=verbose)
    ...
    async def engine_step(self) -> bool:
        ...
        for new_request in new_requests:
            # Add the request into the Vajra engine's waiting queue.
            try:
                self.engine.add_request(**new_request)
            except ValueError as e:
                # TODO: use a Vajra specific error for failed validation
        ...
        request_outputs = await self.engine.get_outputs_async()
    ...
    async def generate(
        ...
        Details:
            - If the engine is not running, start the background loop,
              ...
              :meth:`~vajra.engine.async_llm_engine.AsyncLLMEngine.engine_step`
