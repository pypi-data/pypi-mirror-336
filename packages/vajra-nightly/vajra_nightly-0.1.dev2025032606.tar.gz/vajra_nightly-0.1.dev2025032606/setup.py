...
class BuildProtoCommand(build_py):
    """Custom command to compile .proto files to Python during build."""
    
    def run(self):
        # Check if protoc is available
        if which("protoc") is None:
            sys.stderr.write("Error: 'protoc' is not found. Please install Protocol Buffers compiler.\n")
            sys.exit(1)
        
        # Check if protoc version is >= 3.19, this is to ensure compatibility with protobuf 5.x
        protoc_version = subprocess.check_output(["protoc", "--version"]).decode().split()[1]
        if parse(protoc_version) < parse("3.19.0"):
            sys.stderr.write(
                "Error: 'protoc' version must be >= 3.19.0. Please upgrade.\n"
        ...
        output_dir = os.path.join(ROOT_DIR, "vajra", "datatypes", "generated")
        ...
        if not proto_files:
            sys.stderr.write("Warning: No .proto files found in {}\n".format(proto_dir))
        
        for proto_file in proto_files:
            proto_file_basename = os.path.basename(proto_file)
            sys.stdout.write("Compiling {} to Python...\n".format(proto_file_basename))
            ...
            try:
                ...
                sys.stderr.write("Error: Failed to compile {}: {}\n".format(proto_file_basename, e))
                sys.exit(1)
...
setup(
    ...
    cmdclass={"build_ext": cmake_build_ext, "build_py": BuildProtoCommand},
