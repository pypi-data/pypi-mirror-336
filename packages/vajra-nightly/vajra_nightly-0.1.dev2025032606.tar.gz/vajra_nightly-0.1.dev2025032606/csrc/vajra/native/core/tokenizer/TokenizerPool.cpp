...
void TokenizerPool::Loop() {
  ...
  while (true) {
    auto input = input_queue_.Dequeue();
    if (!input) break;  // Shutdown signal received

    auto token_ids = tokenizer.Encode(input->prompt);
    output_queue_.Enqueue(TokenizerPoolOutput(
...
void TokenizerPool::Shutdown() {
  ...
  input_queue_.Pause();   // Unblock worker threads
  output_queue_.Pause();  // Unblock threads waiting on output
