...
void ChromeTracer::Put(const SequenceMetadataVector& seq_metadata_list,
                       std::size_t replica_id, std::size_t tensor_parallel_rank,
  ...
  event["pid"] = fmt::format("{}-kvp{}", replica_id, kv_parallel_rank);
...
void ChromeTracer::PutSchedulerEvent(
    std::size_t replica_id, std::size_t scheduler_id,
  ...
  event["ts"] = start_time * 1e6;
  event["dur"] = (end_time - start_time) * 1e6;
  event["pid"] = std::to_string(replica_id);
  event["tid"] = "Scheduler";
