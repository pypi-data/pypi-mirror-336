...
std::vector<float> GetTemperatures(const Sequences& seqs /*[in]*/) {
  std::vector<float> temperatures;
  temperatures.reserve(seqs.size());

  for (const auto& seq : seqs) {
    float temperature = seq->sampling_params.temperature;
...
std::pair<std::vector<float>, std::vector<int>> GetTopPTopK(
    const Sequences& seqs /*[in]*/, int vocab_size /*[in]*/) {
...
//==============================================================================
Sampler::Sampler(const torch::Tensor& embedding /*[in]*/,
                 int vocab_size /*[in]*/,
...
SamplerOutputs Sampler::Forward(
  ...
  auto [top_ps, top_ks] = GetTopPTopK(seqs, vocab_size_);
  ...
  for (size_t seq_idx = 0; seq_idx < seq_metadata_list.size(); ++seq_idx) {
    ...
    outputs.emplace_back(std::make_shared<SamplerOutput>(
        ...
        std::vector<size_t>{sample_results_accessor[seq_idx]}));
  }
