...
namespace vajra {
//==============================================================================
constexpr std::size_t FLASHINFER_WORKSPACE_SIZE = 128 * 1024 * 1024;
...
class FlashinferAttentionWrapper {
 public:
  ...
  std::size_t GetNumQTokens() const {
  ...
  std::size_t GetNumQTokensWithoutValidation() const { return num_q_tokens_; }
  bool GetIsNoOp() const { return is_no_op_; }
  bool GetShouldSaveKVCache() const { return should_save_kv_cache_; }
  bool GetIsMetadataInitialized() const { return metadata_initialized_; }
  std::optional<torch::Tensor> GetSlotMappingTensor() const {
 ...
 private:
  ...
  std::vector<std::vector<std::size_t>> kvp_seqs_group_ids_;
