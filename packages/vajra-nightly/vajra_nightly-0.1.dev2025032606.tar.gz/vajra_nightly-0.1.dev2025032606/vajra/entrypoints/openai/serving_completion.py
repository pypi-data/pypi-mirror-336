...
from vajra.utils import merge_async_iterators, random_uuid
...
class OpenAIServingCompletion(OpenAIServing):
    ...
    def __init__(self, engine_wrapper: AsyncInferenceEngineWrapper, model_config: ModelConfig,
                 served_model_names: List[str],):
        super().__init__(engine_wrapper=engine_wrapper,
    ...
    async def create_completion(self, request: CompletionRequest,
        ...
        try:
            ...
            for i, prompt in enumerate(prompts):
                ...
                generator = self.engine_wrapper.generate(
                    f"{request_id}-{i}",
                    prompt,
                    sampling_params,
                )
        ...
        try:
            async for i, res in result_generator:
                if await raw_request.is_disconnected():
                    ...
                    await self.engine_wrapper.abort(f"{request_id}-{i}")
    ...
    def request_output_to_completion_response(
        self,
        final_res_batch: List[Optional[RequestOutput]],
        request: CompletionRequest,
        request_id: str,
        created_time: int,
        model_name: str,
