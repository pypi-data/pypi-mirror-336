        ...
        if is_live:
            ...
            if cur_bar_local_time_str not in self.cache:
                ...
                if cur_bar_local_time.second == 0:
                    ...
                    if len(self.cache) == 0:
                        start_time = cur_bar_local_time - timedelta(seconds=cur_bar_local_time.second) - timedelta(minutes=2)
                        end_time = cur_bar_local_time - timedelta(seconds=cur_bar_local_time.second)
                        self.batch_fetch(start_time, end_time)
            else:
                if self.p.debug:
