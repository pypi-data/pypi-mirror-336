...
class ExtxyzLoader(DataLoader):
    """Implementation of a data loader that loads data from extxyz format via
    the ``ase`` library."""
    ...
    def load(
        ...
        _cfg = dataset_config.data_loader

        # TRAIN
        train_systems = self._load_chemical_systems(
            _cfg.train_dataset_paths, _cfg.train_num_to_load
        )

        # VALIDATION
        valid_systems = self._load_chemical_systems(
            _cfg.valid_dataset_paths, _cfg.valid_num_to_load
        )

        # TEST
        test_systems = self._load_chemical_systems(
            _cfg.test_dataset_paths, _cfg.test_num_to_load
        )
