...
from mlip_jax.models.nequip.nequip_helpers import (
    MLP,
    get_nonlinearity_by_name,
    prod,
    tp_path_exists,
)
...
class NequIPNet(nn.Module):  # TODO this is almost identical to MACENet maybe merge?
    ...
    def __call__(
        ...
        nequip_kwargs = dict(self.nequip_kwargs)
        # gate = parse_activation(mace_kwargs["gate"]) #TODO convert nonlinearities
        # mace_kwargs["gate"] = gate #TODO convert nonlinearities

        nequip = NequIP(**nequip_kwargs)

        node_energies = nequip(vectors, node_z, senders, receivers)

        node_energies = self.std * node_energies + self.mean

        if self.learnable_atomic_energies:  # TODO tests this works with nequip
            num_species = nequip_kwargs["num_species"]
            ...
            atomic_energies_ = jnp.asarray(
                self.atomic_energies
            )  # TODO: this is not in the original NequIP, test with and without
...
class NequIP(nn.Module):
    ...
    def __call__(
        ...
        node_attr = e3nn.IrrepsArray(embedding_irreps, identity[node_z])
        node_feats = Linear(irreps_out=e3nn.Irreps(hidden_irreps))(node_attr)

        # Edges Embedding
        if not (hasattr(vectors, "irreps") and hasattr(vectors, "array")):
            vectors_irreps = e3nn.IrrepsArray("1o", vectors)

        scalar_dr_edge = safe_norm(vectors_irreps.array, axis=-1)

        edge_sh = e3nn.spherical_harmonics(self.sh_irreps, vectors, normalize=True)
...
class NequIPLayer(nn.Module):
    """NequIP Convolution. Adapted from Google DeepMind materials discovery:
    ...
    def __call__(
        ...
        edge_sh: util.Array,
        senders: util.Array,
        receivers: util.Array,
        edge_embedded: util.Array,
    ) -> e3nn.IrrepsArray:
        ...
        node_feats = e3nn.index_add(receivers, edge_feats, out_dim=node_feats.shape[0])
