...
from mlip_jax.simulation.configs.jax_md_config import JaxMDSimulationConfig
from mlip_jax.simulation.enums import SimulationType
from mlip_jax.simulation.exceptions import SimulationIsNotInitializedError
from mlip_jax.simulation.jax_md.helpers import (
...
from mlip_jax.simulation.jax_md.states import (
    EpisodeLog,
...
class JaxMDSimulationEngine(SimulationEngine):
    ...
    def __init__(self):
        ...
        Calls constructor of parent class and initializes several members to ``None``
        that will be filled later during the simulation initialization phase.
        """
        super().__init__()
        self._displacement_fun = None
        self._shift_fun = None
        self._neighbor_fun = None
        self._pure_simulation_step_fun = None
        self._internal_state = None
        self._config = None
        self._steps_per_episode = None
    ...
    def init(
        ...
        model_calculate_fun = self._get_model_calculate_fun(graph, model, model_params)
    ...
    def _reallocate_neighbors(self) -> None:
        logging.info("Neighbor lists require reallocation...")
        ...
        logging.info("Reallocation of neighbor lists completed.")
    ...
    def _get_model_calculate_fun(
        ...
        def calc_func(
            ...
            model_apply_func: ModelPredictorFun,
            params: ModelParameters,
        ) -> np.ndarray:
            batched_graph = batch_graph_with_one_dummy(
                system_state, positions, base_graph
            )
            model_output = model_apply_func(params, batched_graph)
            output_forces = jnp.delete(model_output["forces"], -1, axis=0)
            return output_forces * KCAL_PER_MOL_PER_ELECTRON_VOLT

        return functools.partial(
            calc_func,
            base_graph=graph,
            model_apply_func=model,
            params=model_params,
        )
    ...
    def _update_state(
        ...
        log_frequency = self._config.log_frequency

        def _concat(current: np.ndarray, new: np.ndarray) -> np.ndarray:
            if current is None:
                return new[::log_frequency]
            return np.concatenate([current, new[::log_frequency]], axis=0)
