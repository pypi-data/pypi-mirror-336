...
class GraphDataManager:
    ...
    def __init__(
        ...
        if len(keep_graphs) != len(self.graphs):
            ...
            logging.warning(
                f"Discarded {len(self.graphs) - len(keep_graphs)} "
                f"graphs due to size constraints."
            )
        ...
        if self.should_shuffle:
            ...
            logging.info("Shuffling data now...")
    ...
    def subset(self, i: slice | int | list | float):
        """Constructs and returns a new graph data manager containing a subset of
        ...
        Returns:
            A new graph data manager containing only a subset of the graphs of the
            current one.
        """
        graphs = self.graphs
        ...
        logging.info("Constructing subset with %s graphs...", len(graphs))
        subset_manager = GraphDataManager(
            graphs=graphs,
        ...
        logging.info("...and with %s batches.", len(subset_manager))
