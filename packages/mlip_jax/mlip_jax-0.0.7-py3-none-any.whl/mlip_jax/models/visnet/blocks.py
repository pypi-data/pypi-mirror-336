...
def parse_activation_fn(activation: str) -> Callable[[chex.Array], chex.Array]:
    act_class_mapping = {
    ...
    if activation in act_class_mapping:
        return act_class_mapping[activation]
    else:
...
class CosineCutoff(nn.Module):
    ...
    @nn.compact
    def __call__(self, distances: chex.Array) -> chex.Array:
...
class ExpNormalSmearing(nn.Module):
    ...
    def __call__(self, dist: chex.Array) -> chex.Array:
...
def parse_rbf_fn(rbf_type: str) -> Callable:
    # Mapping of RBF class names to their Flax classes
    rbf_class_mapping = {"gauss": GaussianSmearing, "expnorm": ExpNormalSmearing}
    return rbf_class_mapping[rbf_type]


class Sphere(nn.Module):
    degree: int = 2

    def __call__(self, edge_vec: chex.Array) -> chex.Array:
    ...
    def _spherical_harmonics(
        self, x: chex.Array, y: chex.Array, z: chex.Array
    ) -> chex.Array:
...
class NeighborEmbedding(nn.Module):
    ...
    def setup(self):
        self.embedding = nn.Embed(self.num_species, self.hidden_channels)
...
class GatedEquivariantBlock(nn.Module):
    ...
    def setup(self):
        if self.intermediate_channels is None:
            intermediate_channels = self.hidden_channels
        ...
        self.update_net = nn.Sequential(
            [
                ...
                parse_activation_fn(self.activation),
                nn.Dense(
                    self.out_channels * 2,
                    kernel_init=initializers.xavier_uniform(),
                    bias_init=initializers.zeros_init(),
