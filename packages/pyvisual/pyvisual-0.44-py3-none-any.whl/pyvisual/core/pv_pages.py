...
class PvPages(QStackedWidget):
    ...
    def create_page(self, name: str = None, bg_color: str = None) -> int:
        """
        Creates a new page as a plain QWidget, adds it to the stack, and returns its index.
        Optionally, specify an object name and a background color (CSS color string, e.g. "#FFCCCC").
        """
        page = QWidget()
    ...
    def set_current_page(self, index: int):
        ...
        """
        if 0 <= index < self.count():
            if self.animation_duration > 0 and self.currentIndex() != index:
                ...
                # When finished, set the current index to the new page.
