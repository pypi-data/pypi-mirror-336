...
class ApplianceClient:
    ...
    def send_tensor(
        ...
        while True:
            try:
                response = None
                for response in send_tensor_callback(
    ...
    def infer(
        ...
        resp = None
        for resp in self.stub.InferenceStream(request):
    ...
    def carry_over_from_ptr(self, request: CarryOverFromPTRRequest) -> None:
        """Carryover tensor from persistent tensor repository to this session."""
        logger.debug("Sending carry_over_from_repo request")
...
def fw_user_serialize(
    ...
    try:
        return dill.dumps(obj, recurse=recurse).hex()
    except Exception as e:
        if recurse:
            logger.debug(
            ...
            try:
                return fw_user_serialize(obj, name, from_usr, recurse=False)
            except Exception as e:
                raise e from None
