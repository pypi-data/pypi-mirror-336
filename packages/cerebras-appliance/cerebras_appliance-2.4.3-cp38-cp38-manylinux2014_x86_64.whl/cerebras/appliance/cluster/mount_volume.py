...
class ClusterVolumeManager:
    ...
    def __init__(self, volumes: VolumeList):
        ...
        for volume in volumes.items:
            ...
            # Internal cluster volumes not meant to be used by client
            if any(
                volume.meta.labels.get(label, None) == "true"
                for label in ["cached-compile", "workdir-logs", "allow-venv"]
            ):
                self._internal.add(cluster_mountpoint)
                continue
    ...
    def unavailable_mountpoints(self) -> List[str]:
        return sorted(
            self._unknown | set(self._mismatching.keys()) | self._internal
        )
    ...
    def get_mountpoint_for_path(self, path: str) -> str:
        ...
        for cluster_mountpoint in sorted(
            ...
            if _is_same_path(
                ...
                elif cluster_mountpoint in self._internal:
                    raise ValueError(
                        f"Path {path} is on an internal cluster volume {cluster_mountpoint} "
                        f"and not available for user mounting."
                    )
