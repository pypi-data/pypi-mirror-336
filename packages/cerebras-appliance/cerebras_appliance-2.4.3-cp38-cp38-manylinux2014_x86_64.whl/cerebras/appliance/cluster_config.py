...
class _PositiveInt(Descriptor):
    ...
    def sanitize(self, value: int) -> DebugArgs.DebugMGR.JobPriority:
        if not value:
            return self.default
...
class ClusterConfig:
    ...
    Args:
        ...
        max_wgt_servers: Maximum number of weight servers to support run.
        max_act_per_csx: Maximum number of activation servers per system.
    ...
    max_wgt_servers: int = _PositiveInt(default=24)
    max_act_per_csx: int = _PositiveInt(default=60)
    ...
    def __post_init__(self):
        ...
        # In Release 2.2, we introduced a workflow id for all jobs launched from
        # a single user-side invocation. Jobs that are queued inherit the job
        # priority of most recent running job if any in the same workflow.
    ...
    def job_timer(self) -> Optional[JobTimer]:
        ...
        if self.job_time_sec is not None and self.job_time_sec > 0:
            return self.get_job_timer(self.job_time_sec)
        return None

    @lru_cache(maxsize=1)
    def get_job_timer(  # pylint: disable=no-self-use
        self, job_time_sec
    ) -> JobTimer:
        """Returns a cached JobTimer object."""
        return JobTimer(job_time_sec)
