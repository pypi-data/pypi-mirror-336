...
def Logwrap(f, logger):
    ...
    def wrapper(*args, **kwargs):
        ...
        if (depth is not None and depth == 1) or (len(logger.running_stack) == 1):
            stacks = inspect.stack()
            for stack in stacks:
                if "site-packages" not in stack.filename:
                    fndata.update({
                        "stack": {
                    ...
                    break
        # 已记录，清理追加的参数
        for key in [k for k in kwargs if k.startswith("uwa")]:
            kwargs.pop(key)
        try:
            res = f(*args, **kwargs)
        except TargetNotFoundError as e:
            # 对于 TargetNotFoundError 继续运行 
            raise e
