...
class TaskWelfare(TaskLogic):
    ...
    def task_打开福利中心(self):
        ...
        if "福利中心界面" in self.interface_info:
            self.node_current = "task_每日签到"
            return "task_finish"

        elif "主界面" in self.interface_info:
            self.find_data_from_keys_list_click(["resource/images_info/other/奖励图标.bmp"],self.image_data)
        else:
    ...
    def task_每日签到(self):
        ...
        if "每日签到" in self.node_list or self.node_counter>=5:#说明已领取
            ...
            self.find_data_from_keys_list_click(["resource/images_info/other/奖励中心.png"], self.image_data)
            return "task_finish"
            ...
            if scope_list:
                ...
                if max_element[0]<900: #说明不是最后一列,点击后一格的位置
                    self.mouse_left_click(max_element[0]+54,max_element[1]+19,delay_time=1)
    ...
    def task_成长奖励(self):
        ...
        if "成长奖励" in self.node_list or self.node_counter >= 10:
            ...
            if "成长奖励" not in self.node_list:
                self.find_data_from_keys_list_click(["成长奖励"], self.word_handle_data)
    ...
    def task_details(self):
        ...
        if not self.node_flag:
            self.queue_message({"interval":30,"word": {11: {"enable": False}}})
            self.node_flag=True
