...
def _construct_chain_deployment(chain_deployment: dict) -> Deployment:
    ...
    return Deployment(
        ...
        endpoint_url=construct_endpoint_url(workspace_url, endpoint_name),
        # ToDo (ML-42293): Remove the fallback to rag_app_url once the backend is updated
        review_app_url=chain_deployment.get(
            "review_app_url", chain_deployment.get("rag_app_url", None)
        ),
    )
