            ...
            try:
                monitor = _create_monitor_internal(
                    ...
                    monitoring_config=MonitoringConfig(
                        sample=0.1,
                        paused=True,
                        metrics=[],  # Must be explicitly set to empty list to avoid default metrics server side
                    ),
                )
                _logger.info(
...
def deploy(
    ...
    **kwargs,
    ...
    # Create a review app v2 for the deployment.
    try:
        deployment_info = _create_review_app_and_update_url(deployment_info)
    except Exception:
        # If we fail to create a review app V2, we will use the old review app URL.
        _logger.warning(
            f"Failed to create a stable review app URL for serving endpoint '{deployment_info.endpoint_name}'. "
            "Falling back to the version specific URL.",
            exc_info=True,
        )
...
def _create_review_app_and_update_url(deployment_info: Deployment) -> Deployment:
    """Create a review app v2 for the deployment."""
    client = mlflow.MlflowClient()
    model_info = client.get_model_version(
        deployment_info.model_name, deployment_info.model_version
    )
    ...
    my_review_app.add_agent(
        agent_name=deployment_info.model_name,
        model_serving_endpoint=deployment_info.endpoint_name,
    )
    # Update the review app URL.
    deployment_info.review_app_url = my_review_app.url + "/chat"
    return deployment_info
