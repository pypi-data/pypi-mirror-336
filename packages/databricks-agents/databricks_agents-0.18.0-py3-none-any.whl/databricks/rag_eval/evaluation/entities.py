...
def get_null_mlflow_feedback() -> Feedback:
    """Gets null feedback used as an indicator for the type of
    an assessment that is erroring. Mlflow since v0.21.0 requires
    assessments with an error to also have a feedback or expectation. This
    null feedback serves to fulfill that requirement.

    Returns:
        Feedback: null feedback
    """
    # TODO(ML-51639): remove this method once MLflow supports error-only feedback values
...
class PerRequestAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment_v2(
        ...
        """
        # TODO(ML-51637): Deprecate usage of MLflow v2 assessments
    ...
    def to_mlflow_assessment_v3(self, trace_id: str, span_id: Optional[str]):
        """
        ...
        source = mlflow_entities.AssessmentSource(
            ...
            source_id=self.assessment_source.source_id or self.assessment_name,
        )
        if self.rating.error_message is not None:
            value_kwarg = {
                ...
                "feedback": get_null_mlflow_feedback(),
            }
        else:
            value = self.rating.categorical_value or self.rating.double_value
            value_kwarg = {"feedback": Feedback(value=value)}
        return mlflow_entities.Assessment(
            ...
            source=source,
            rationale=self.rating.rationale,
            # These two fields, plus assessment_id, are created service-side.
            create_time_ms=0,
...
class PerChunkAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment_v3(
        self, trace_id: str, span_id: Optional[str]
        ...
        WARNING: Has some issues with per-chunk judges, because they generate near-identical looking
        assessments, only differing in their metadata.

        MLFlow does not currently allow multiple assessments with identical trace_id, span_id, source_id,
        and source_type.

        The CHUNK_INDEX_KEY naming is also not yet finalized in MLFlow.
        """
        assessments = []
        for position, rating in self.positional_rating.items():
            ...
            assessments.append(
                mlflow_entities.Assessment(
                    ...
                    source=source,
                    rationale=rating.rationale,
                    # TODO(ML-51116): Test whether these assessments are properly associated
                    # with the chunk span_ids.
...
class MetricResult:
    ...
    def to_mlflow_assessment_v2(self) -> Optional[mlflow_eval.Assessment]:
        """
        ...
        # Note that we ignore assessment v3 here, as this code will be deprecated soon anyways.
        return None
    ...
    def to_mlflow_assessment_v3(
        ...
        :return: MLflow Assessment object
        """
        # Metrics come in two flavors: pure python code and LLM judges.
        # The latter is because we allow custom metrics, which look like
        # @metric def my_metric(...) -> Union[float,bool,int, mlflow_eval.Assessment].
        # The mlflow_eval.Assessment type is an escape hatch that allows rich return values
        # including error messages and rationales. It's poorly named and really just have
        # been called "custom assessments" and handled as an AssessmentResult, but it's a MetricResult instead.
        if isinstance(self.metric_value, mlflow_eval.Assessment):
            if self.metric_value.source is not None:
                source_type = {
                ...
                }[self.metric_value.source.source_type]
                source = mlflow_entities.AssessmentSource(
                    source_type=source_type,
                    source_id=self.metric_value.source.source_id
                    or self.metric_full_name,
                )
            else:
                source = mlflow_entities.AssessmentSource(
                    source_type=mlflow_entities.AssessmentSourceType.LLM_JUDGE,
                    source_id=self.metric_full_name,
                )

            if self.metric_value.error_code is not None:
                feedback = get_null_mlflow_feedback()
                rationale = None
                error_assessment = mlflow_entities.AssessmentError(
                    error_code=self.metric_value.error_code,
                    error_message=self.metric_value.error_message,
                )
            else:
                feedback = Feedback(self.metric_value.value)
                rationale = self.metric_value.rationale
                error_assessment = None
        elif isinstance(self.metric_value, mlflow_entities.Assessment):
            assessment = self.metric_value
            ...
            feedback = Feedback(self.metric_value)
            rationale = None
            error_assessment = None
        ...
        return mlflow_entities.Assessment(
            trace_id=trace_id,
            span_id=span_id,
            name=self.metric_full_name,
            source=source,
            feedback=feedback,
            error=error_assessment,
            rationale=rationale,
            # These two fields, plus assessment_id, are created service-side.
            create_time_ms=0,
            last_update_time_ms=0,
...
class EvalResult:
    ...
    overall_assessment: Optional[Rating] = None
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        trace_id = self.eval_item.trace.info.request_id
        root_span = _get_root_span(self.eval_item.trace)
        span_id = root_span.span_id if root_span is not None else None

        for metric in self.metric_results:
            converted_metric_results.append(
                metric.to_mlflow_assessment_v3(trace_id, span_id)
            )
        ...
        for assessment in self.assessment_results:
            if isinstance(assessment, PerRequestAssessmentResult):
                converted_assessment_results.append(
                    assessment.to_mlflow_assessment_v3(trace_id, span_id)
                )
            elif isinstance(assessment, PerChunkAssessmentResult):
                # TODO(ML-51638): We're slapping all these assessments it all on the root span for now,
                # but a per-chunk assessment should really have more targeted spans.
                # In order to do this, we have to thread through some span_id information from
                # the original chunk judge.
        ...
        for expected_col_name in _EXPECTATION_FIELDS:
            expected_value = eval_item_dict.get(expected_col_name)
            if expected_value is None:
                continue

            # Expectation values can only hold primitives or a list of primitives. As such,
            # we need to convert objects such as retrieved documents to a JSON string.
            processed_expected_value = expected_value
            if isinstance(expected_value, list) and not all(
                isinstance(value, str) for value in expected_value
            ):
                processed_expected_value = [
                    _expectation_obj_to_json_str(value)
                    for value in expected_value
                    if value is not None
                ]
            elif isinstance(expected_value, dict):
                processed_expected_value = _expectation_obj_to_json_str(expected_value)
            ...
            converted_expectations.append(
                mlflow_entities.Assessment(
                    ...
                    expectation=Expectation(value=processed_expected_value),
                    # These two fields, plus assessment_id, are created service-side.
                    create_time_ms=0,
                    last_update_time_ms=0,
                )
...
# There's already a copy of this function in ./traces.py but this avoids a circular dep
