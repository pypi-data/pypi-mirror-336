...
RAG_EVAL_RATE_LIMIT_QUOTA = _EnvironmentVariable(
    "RAG_EVAL_RATE_LIMIT_QUOTA", float, 8.0
)
...
AGENT_EVAL_ENABLE_MULTI_TURN_EVALUATION = _BooleanEnvironmentVariable(
    "AGENT_EVAL_ENABLE_MULTI_TURN_EVALUATION", True
)

AGENT_EVAL_TRACE_SERVER_ENABLED = _BooleanEnvironmentVariable(
    "AGENT_EVAL_TRACE_SERVER_ENABLED", False
)

AGENT_EVAL_LOG_TO_MLFLOW_ENABLED = _BooleanEnvironmentVariable(
    "AGENT_EVAL_LOG_TO_MLFLOW_ENABLED", False
)
...
RAG_EVAL_LLM_JUDGE_MAX_JUDGE_ERROR_RETRIES = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_MAX_JUDGE_ERROR_RETRIES", int, 5
)

# Backoff factor in seconds.
RAG_EVAL_LLM_JUDGE_BACKOFF_FACTOR = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_BACKOFF_FACTOR", float, 5
)

# Jitter in seconds to add to the backoff factor.
RAG_EVAL_LLM_JUDGE_BACKOFF_JITTER = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_BACKOFF_JITTER", float, 20
)

RAG_EVAL_LLM_JUDGE_WAIT_EXP_BASE = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_WAIT_MULTIPLIER", float, 1.3
)

RAG_EVAL_LLM_JUDGE_MAX_WAIT = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_MAX_WAIT", float, 240
)

RAG_EVAL_LLM_JUDGE_MIN_WAIT = _EnvironmentVariable(
    "RAG_EVAL_LLM_JUDGE_MIN_WAIT", float, 30
)

# ================ Retry Configurations for Other Judge Service Endpoints ================

# Maximum number of retries.
AGENT_EVAL_JUDGE_SERVICE_ERROR_RETRIES = _EnvironmentVariable(
    "AGENT_EVAL_JUDGE_SERVICE_ERROR_RETRIES", int, 10
)

# Backoff factor in seconds.
AGENT_EVAL_JUDGE_SERVICE_BACKOFF_FACTOR = _EnvironmentVariable(
    "AGENT_EVAL_JUDGE_SERVICE_BACKOFF_FACTOR", float, 0
)

# Jitter in seconds to add to the backoff factor.
AGENT_EVAL_JUDGE_SERVICE_JITTER = _EnvironmentVariable(
    "AGENT_EVAL_JUDGE_SERVICE_JITTER", float, 5
)
...
AGENT_EVAL_MAX_NUM_GUIDELINES = _EnvironmentVariable(
    "AGENT_EVAL_MAX_NUM_GUIDELINES", int, 20
)
