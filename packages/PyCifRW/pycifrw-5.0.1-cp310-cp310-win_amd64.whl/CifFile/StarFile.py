...
class LoopBlock(object):
    ...
    def __getitem__(self,dataname):
        ...
        if dataname in self.keys():
            ...
            raise KeyError('{} not in loop block'.format(dataname))
    ...
    def recursive_iter(self,dict_so_far={},coord=[]):
        ...
        for dimup in range(0,self.dimension):  #look higher in the tree
            if len(drill_values)>0:            #this block has values
                ...
                raise StarError("Malformed loop packet {}".format(repr( top_items[0] )))
...
class StarBlock(object):
    ...
    the enclosing block collection (e.g. CIF or STAR file object)""")
        try:
           ...
           if self.dictionary:
               ...
               print('Set {} to derived value {}'.format(key, repr(new_value)))
               return new_value
           else:
               raise KeyError('No such item: {}'.format(key))
        ...
        print('{}: is_value {} provide_value {} value {}'.format(key,repr( is_value ),repr( self.provide_value ),repr( rawitem )))
        if is_value:
                ...
                if do_calculate:
                   ...
                   try:
                       ...
                       try:
                           ...
                           print("Could not turn {} into a value:".format(repr(s)))
                           return rawitem
                   else:
                       print('Set {} to derived value {}'.format(key, repr( new_value )))
                       return new_value
    ...
    def AddItem(self,key,value,precheck=False):
        ...
        if not isinstance(key,(unicode,str)):
             raise TypeError('Star datanames are strings only (got {})'.format(repr( key )))
        ...
        if key in self:
             if not self.overwrite:
                 raise StarError( 'Duplicated item name: {}'.format(key))
    ...
    def AddLoopItem(self,incomingdata,precheck=False,maxlength=-1):
        ...
        if isinstance(incomingdata[0],(tuple,list)):
           ...
           self.CreateLoop(incomingdata[0])
             ...
             raise TypeError('Star datanames are strings only (got {})'.format(repr( incomingdata[0] )))
        else:
    ...
    def check_data_name(self,dataname,maxlength=-1):
        ...
        if self.characterset=='ascii':
            ...
            if len ([a for a in dataname if ord(a) > 0x10000 and (ord(a) & 0xE == 0xE)]) > 0:
                print('{} fails'.format(dataname))
                for a in dataname: print('{}'.format(ord(a)),end="")
                print()
    ...
    def check_item_value(self,item):
        ...
        def check_one (it):
            if isinstance(it,unicode):
                ...
                if not me:
                    print("Fail value check: {}".format(it))
                    raise StarError('Bad character in {}'.format(it))
                else:
                    if me.span() != (0,len(it)):
                        print("Fail value check, match only {}-{} in string {}".format(me.span()[0],me.span()[1],repr( it )))
    ...
    #                                                                         
    # Note that all string data should be Unicode.  To maintain compatibility 
    # for Python 2 we apply Unicode to any string data.                       
    #                                                                         
    #                                                                         
    ...
    def GetFullItemValue(self,itemname):
        ...
        try:
            ...
            raise KeyError('Itemname {} not in datablock'.format(itemname))
    ...
    def CreateLoop(self,datanames,order=-1,length_check=True):
           ...
           if length_check:
               ...
               if len(listed_values) == len(datanames):
                   ...
                   if len(len_set)>1:
                       raise ValueError('Request to loop datanames {} with different lengths: {}'.format(repr( datanames ),repr( len_set )))
    ...
    def AddLoopName(self,oldname, newname):
        ...
        if loop_no < 0:
            raise KeyError('{} not in loop'.format(oldname))
    ...
    def GetKeyedPacket(self,keyname,keyvalue,no_case=False):
        ...
        if len(one_pack)!=1:
            raise ValueError("Bad packet key {} = {}: returned {} packets".format(keyname,keyvalue,len(one_pack)))
        print("Keyed packet: {}".format(one_pack[0]))
        return one_pack[0]

    #  The current version of DDLm allows compound keys.  We implement a routine to return
    # a single packet corresponding to the values of the specified datanames. 
    #                                                                         
    #                                                                         
    ...
    def GetCompoundKeyedPacket(self,keydict):
        ...
        if len(my_loop)!=1:
            raise ValueError("Bad packet keys {}: returned {} packets".format(repr(keydict),len(my_loop)))
        print("Compound keyed packet: {}".format(my_loop[0]))
        return my_loop[0]

    # Plain single key. This is the older routine where we assume that        
    # we only have a single key per category.  We still have to put the       
    # single key into a list as the __getattr__ method of the StarPacket      
    # will assume that it has been passed a list of keys.                     
    #                                                                         
    #                                                                         
    ...
    def GetKeyedSemanticPacket(self,keyvalue,cat_id):
        ...
        if not hasattr(p,"key"):
            raise ValueError("No key found for {}, packet is {}".format(cat_id,str(p)))
    ...
    # to store the key names and values, so that the __getattr__ method       
    # of the packet can properly derive the needed non-key values.            
    #                                                                         
    ...
    #                                                                         
    # The final DDLm specification allowed compound keys for categories. When 
    # combined with child categories, this means that a child key may be absent
    # but its parent key may be present and is considered equivalent.         
    #                                                                         
    #                                                                         
    ...
    def set_grammar(self,new_grammar):
        ...
        if new_grammar == '2.0':
            ...
            raise StarError('Request to set unknown grammar {}'.format(new_grammar))
    ...
    # The [[finish_at]] and [[start_from]] arguments cause output to stop/start when one of the
    # datanames in the arguments is found.                                    
    #                                                                         
        ...
        if start_from != '':
            if start_from in requested_order:
                ...
                if cand_pos < len(self.output_order):
                    print('Output starts from {}, requested {}'.format(self.output_order[cand_pos],start_from))
        ...
        if finish_at != '':
            if finish_at in requested_order:
                ...
                if len(loop_order)>0:
                    cand_pos = min(cand_pos,self.output_order.index(loop_order[0]))
                if cand_pos < len(self.output_order):
                    print('Output finishes before {}, requested before {}'.format(self.output_order[cand_pos],finish_at))
                    self.output_order = self.output_order[:cand_pos]
                else:
        ...
        if mode == 'strict':
           for key in new_block.keys():
               if key in self and key not in match_att:
                  raise StarError( "Identical keys {} in strict merge mode".format(key))
           ...
           print('Overlay mode, current overwrite is {}'.format(self.overwrite))
    ...
    def assign_dictionary(self,dic):
        if not dic.diclang=="DDLm":
            print("Warning: ignoring dictionary {}".format(dic.my_uri))
            return
        self.dictionary = dic
...
class StarPacket(list):
    ...
    def __getattr__(self,att_name):
        ...
        if att_name in ('cif_dictionary','fulldata','key'):
            raise AttributeError('Programming error: can only assign value of {}'.format(att_name))
...
"""
    ...
    def set_grammar(self,new_grammar):
        ...
        if new_grammar not in ['1.1','1.0','2.0','STAR2']:
            raise StarError('Unrecognised output grammar {}'.format(new_grammar))
        ...
        if parent is None:
           ...
           if parent.lower() in self.lower_keys:
              if self.scoping == 'instance':
               ...
               print('Warning:Parent block {} does not exist for child {}'.format(parent,newblockname))
    ...
    # Making a Block Collection from a set of our own block names.  This is used
    # in merging, where we must merge with a Block Collection. Any pointers to
    # parent blocks that are not in the list become None, ie. become top      
    # level blocks.  We use our own child table to find links between the     
    # supplied block names and ourself. ::                                    
    #                                                                         
    #                                                                         
    ...
    # frames.  Instead, save frames relationships are held in a separate table,
    # which we look up when outputting.                                       
    #                                                                         
    ...
    # flag an error.  We do not read in the ddl specifications themselves,    
    ...
    # is looped in DDL2-style dictionaries.  We cannot match on a complete match
    ...
    # specified in [[parent]]. In previous versions this was text, but due to the
    # inability to specify to future callers that the name has been changed, [[parent]]
    # is now itself a datablock.                                              
    #                                                                         
    # As for [[NewBlock]], we allow duplicate save frame names in the precise situation
    # where one of the blocks is a top-level block.                           
    #                                                                         
    # The [[drop_att]] attribute allows a particular datablock attribute to be used
    # to determine if datablocks are semantically identical.                  
    #                                                                         
    #                                                                         
    ...
    def merge_fast(self,new_bc,parent=None):
        ...
        if parent is not None:
            ...
            if len(parent_name)==0 or len(parent_name)>1:
                raise StarError("Unable to find unique parent block name: have {}".format(str(parent_name)))
    ...
    def merge(self,new_bc,mode=None,parent=None,single_block=[],
        ...
        for key in new_keys:        #run over incoming blocknames
            ...
            if not basekey in self or mode=="replace":
                ...
                if mode=="strict":
                    raise StarError( "In strict merge mode: block {} in old and block {} in new files".format(basekey,key))
                elif mode=="overlay":
                    # print "Merging block %s with %s" % (basekey,key)
                    self[basekey].merge(new_bc[key],mode,match_att=match_att)
                else:
                    raise StarError( "Merge called with unknown mode {}".format(mode))
    ...
    def __setattr__(self,attr_name,newval):
        if attr_name == 'scoping':
            if newval not in ('dictionary','instance'):
                raise StarError("Star file may only have 'dictionary' or 'instance' scoping, not {}".format(newval))
    ...
    def get_parent(self,blockname):
        ...
        try:
            ...
            raise StarError('no parent for {}'.format(blockname))
        try:
           second = next(possibles)
        except StopIteration:
           return first[1].parent
        raise StarError('More than one parent for {}'.format(blockname))
    ...
    def set_parent(self,parentname,childname):
        ...
        if parentname.lower() not in self.lower_keys:
            raise KeyError('Parent block {} does not exist'.format(parentname))
        if childname.lower() not in self.lower_keys:
            raise KeyError('Child block {} does not exist'.format(childname))
    ...
    def WriteOut(self,comment='',wraplength=80,maxoutlength=0,blockorder=None,saves_after=None):
        ...
        for blockref,blockname in top_block_names:
            ...
            if self.grammar == 'STAR2':  #nested save frames
                ...
                raise StarError('Grammar {} is not recognised for output'.format(self.grammar))
        ...
        outstring.close()
        if len(all_names)>0:
            print('WARNING: following blocks not output: {}'.format(repr(all_names)))
        else:
...
class CIFStringIO(StringIO):
    ...
    def write(self,outstring,canbreak=False,mustbreak=False,do_tab=True,newindent=False,unindent=False,
        ...
        if delimiter:
            if len(outstring)>1:
                raise ValueError('Delimiter {} is longer than one character'.format(repr( outstring )))
...
class StarDerivationError(Exception):
    ...
    def __str__(self):
        return "Derivation of {} failed, None returned".format(self.fail_name)
...
def ReadStarWithError(filename,prepared = None, maxlength=-1,
    ...
    * `permissive` allows non UTF8 encodings (currently only latin1) in the input file. These are a
    ...
    import CifFile.YappsStarParser_1_0 as Y10
    import CifFile.YappsStarParser_2_0 as Y20
    import CifFile.YappsStarParser_STAR2 as YST
    if prepared is None:
        prepared = StarFile()
    ...
    if maxlength > 0:
        ...
        if toolong:
            ...
            raise StarError( 'Line {} contains more than {} characters'.format(pos+1,maxlength))
...
def remove_line_prefix(instring):
    ...
    if prefix_match is not None:
        ...
        print('Found prefix {}'.format(prefix_text))
...
def process_template(template_file):
    ...
    for item in template_as_cif.item_order:  #order of input
        if not isinstance(item,int):    #not nested
            ...
            loop_regex = "(^[ \t]*(?P<loop>loop_)[ \t\n\r]+(?P<name>" + testname + ")([ \t\n\r]+_[\\S]+){" + str(total_items - 1) + "}[ \t]*$(?P<packet>(.(?!_loop|_[\\S]+))*))"
            ...
            for loopname in testnames:
                ...
                while thismatch.start('all') > next_end:
                    try:
                        ...
                        print('next end {}'.format(next_end))
                    except StopIteration:
                        break
                print('Start {}, last_end {}'.format(thismatch.start('all'),last_end))
                ...
                print('{}: {}'.format(loopname,repr( hint_dict )))
                form_hints.append(hint_dict)
    return form_hints
