...
#
# charpos and context are provided so that we can emit a sensible
# error message.
...
class StarParser(yappsrt.Parser):
    ...
    def data(self, currentblock, _parent=None):
        ...
        if _token == 'LBLOCK':
            ...
            makeloop(currentblock,top_loop,_context)
        else: # == 'data_name'
    ...
    def save_frame(self, prepared, _parent=None):
        ...
        savehead = save_heading[5:];savebc = StarFile(characterset='unicode');newname = savebc.NewBlock(savehead,prepared.blocktype(overwrite=False));stored_block = savebc[newname]
        while self._peek('save_end', 'LBLOCK', 'data_name', 'save_heading', 'END', 'data_heading') in ['LBLOCK', 'data_name']:
