...
def get_markdown_one(data_obj, no_title=None, is_error=None):
    """
    获得 markdown 的内容信息
    data_obj =
    {
    ...
    if isinstance(content, list) or isinstance(content, set):
        for o_c in content:
            if isinstance(o_c, dict) or isinstance(o_c, tuple):
                for o_k in o_c:
                    html_list.extend(get_markdown_content(key=o_k, value=o_c[o_k], error=error, stripe=stripe))
                    stripe = not stripe
            else:
                html_list.extend(get_markdown_content(key=o_c, error=error, stripe=stripe))
                stripe = not stripe
    elif isinstance(content, dict) or isinstance(content, tuple):
        for o_k in content:
            html_list.extend(get_markdown_content(key=o_k, value=content[o_k], error=error, stripe=stripe))
            stripe = not stripe
    else:
        html_list.extend(get_markdown_content(key=str(content), error=error, stripe=stripe))
    return ''.join(get_markdown_title(title=title, body=html_list, error=error))


# markdown 中的 content 代码
def get_markdown_content(key=None, value=None, error=False, stripe=False):
...
def get_markdown_title(title=None, body=[], error=False):
    type_color = 'warning' if error else 'info'
