    ...
    try:
        if db_conn is None:
            if save_to_thread:
                ...
                if not hasattr(__THREAD_LOCAL_DB_DATA, 'mysql_conn'):
                    __THREAD_LOCAL_DB_DATA.mysql_conn = {}
                if thread_key not in __THREAD_LOCAL_DB_DATA.mysql_conn:
                    __THREAD_LOCAL_DB_DATA.mysql_conn[thread_key] = get_connect_from_config(db_config, database=database)
                db_conn = __THREAD_LOCAL_DB_DATA.mysql_conn[thread_key]
            else:
...
def get_data_from_doris(sql='', db_config='doris'):
    if not hasattr(__THREAD_LOCAL_DB_DATA, 'conn_doris'):
        __THREAD_LOCAL_DB_DATA.conn_doris = get_doris_conn(db_config)
    conn_doris = __THREAD_LOCAL_DB_DATA.conn_doris
