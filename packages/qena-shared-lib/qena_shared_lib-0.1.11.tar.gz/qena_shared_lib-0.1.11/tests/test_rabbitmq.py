...
from qena_shared_lib.rabbitmq._pool import ChannelPool
from qena_shared_lib.utils import AsyncEventLoopMixin


@fixture(scope="module")
def rabbitmq() -> Generator[RabbitMqContainer, None, None]:
    rabbitmq_container = RabbitMqContainer("rabbitmq:4.0.3-management")

    rabbitmq_container.with_name("test_rabbitmq")
...
async def test_rabbitmq_connection_manager(
    rabbitmq: RabbitMqContainer, logstash: BaseLogstashSender
) -> None:
...
async def test_rabbitmq_connection_manager_wrong_listeners(
    ...
    rabbitmq_connector = RabbitMqManager(logstash=logstash)
...
async def test_rabbitmq_connection_management_wrong_inner_listener(
    ...
    rabbit_mq_manager = RabbitMqManager(logstash=logstash)
...
@mark.asyncio
async def test_rabbitmq_exception_handler_exception(
    logstash: BaseLogstashSender,
) -> None:
    rabbit_mq_manager = RabbitMqManager(logstash=logstash)

    class NoneExceptionClass:
        pass

    with raises(TypeError) as exception_info:
        _ = rabbit_mq_manager.exception_handler(NoneExceptionClass)  # type: ignore

    exception_value = exception_info.value

    assert str(exception_value).startswith(
        "exception should be of type `Exception`, got"
    )

    with raises(TypeError) as exception_info:
        _ = rabbit_mq_manager.exception_handler(Exception)("none callable")  # type: ignore

    exception_value = exception_info.value

    assert str(exception_value).startswith("handler not a callable, got")

    with raises(TypeError) as exception_info:

        @rabbit_mq_manager.exception_handler(Exception)  # type: ignore
        def exception_handler(listener: str, exception: Exception) -> None:
            pass

        exception_value = exception_info.value

        assert str(exception_value).startswith("parameter `listener` at `1`")

    with raises(TypeError) as exception_info:

        @rabbit_mq_manager.exception_handler(Exception)  # type: ignore
        def exception_handler(
            listener: ListenerContext, exception: str
        ) -> None:
            pass

        exception_value = exception_info.value

        assert str(exception_value).startswith("parameter `exception` at `2`")

    with raises(ValueError) as exception_info:

        @rabbit_mq_manager.exception_handler(Exception)  # type: ignore
        def exception_handler(
            listener: ListenerContext, exception: Exception, dependency: str
        ) -> None:
            pass
        ...
        assert str(exception_value).startswith("parameter `exception` at `2`")
        assert str(exception_value).startswith(
            "handlers cannot contain parameters other than `Annotated[type, DependsOn(type)]`"
        )


@mark.asyncio
async def test_rabbitmq_wrong_service(logstash: BaseLogstashSender) -> None:
...
async def test_simultaneous_rabbitmq_rpc_client_call(
    ...
    def rpc_call_done(call: str, task: Task) -> None:
        e = task.exception()
...
async def test_rabbitmq_rpc_worker_exception(
    ...
    container.register(
        BaseLogstashSender, instance=TestLogstashSender("test_service")
    )
...
async def test_rabbitmq_consumer_exception_handler(
    ...
    @rabbitmq_connector.exception_handler(ValueError)
    def handle_value_error_exception(
        context: ListenerContext, error: ValueError
    ) -> None:
...
async def test_rabbitmq_rpc_worker_exception_handler(
    ...
    @rabbitmq_connector.exception_handler(ValueError)
    async def handle_value_error_exception(
        context: ListenerContext, exception: ValueError
    ) -> None:
        await sleep(0)
        rpc_worker_value_error_handler_future.set_exception(exception)

    @rabbitmq_connector.exception_handler(RabbitMQServiceException)
    def handle_rabbit_mq_exception(
        context: ListenerContext, exception: RabbitMQServiceException
    ) -> None:
        rpc_worker_rabbitmq_exception_handler_future.set_exception(exception)
...
async def test_rabbitmq_validation_exception_handler(
    ...
    container = Container()
    mock_logstash = MockLogstashSender(service_name="test")

    container.register(
        service=BaseLogstashSender,
        scope=Scope.singleton,
        instance=mock_logstash,
    )
    container.register(service=LoggerProvider, scope=Scope.singleton)
...
async def test_rabbitmq_service_exception_handler(
    ...
    service_exception_handler_queue: Queue[LogstashLogRecord] = Queue()
    ...
    assert isinstance(log, LogstashLogRecord)
...
async def test_rabbitmq_consumer_all_exception_handler(
    ...
    @rabbitmq_connector.exception_handler(Exception)
    def handle_all_other_exception(
        _: ListenerContext, exception: Exception
    ) -> None:
...
async def test_rabbitmq_consumer_exception_handler_precedence(
    ...
    @rabbitmq_connector.exception_handler(ValueError)
    def handle_value_error_exception(
        _: ListenerContext, exception: ValueError
    ) -> None:
        consumer_exception_handler_future.set_result((ValueError, exception))
...
async def test_rabbitmq_consumer_dependency_exception(
    ...
    @rabbitmq_connector.exception_handler(Exception)
    def listener_exception_handler(
        _: ListenerContext, exception: Exception
    ) -> None:
...
async def test_rabbitmq_listener_exception_handler_dependency(
    ...
    def listener_exception_handler(
        _: ListenerContext,
        exception: Exception,
        consumer_dependency: Annotated[
            ConsumerDependency, DependsOn(ConsumerDependency)
        ],
    ) -> None:
        consumer_dependency.value = (consumer_dependency.value or 0) + 10
        consumer_exception_future.set_exception(exception)
...
async def test_rabbitmq_listener_exception_handler_parameter(
    ...
    @rabbitmq_connector.exception_handler(Exception)
    def listener_exception_handler(
        context: ListenerContext, exception: Exception
    ) -> None:
...
async def test_rabbitmq_service(
    ...
    class TestRabbitmqService(AbstractRabbitMQService, AsyncEventLoopMixin):
        async def initialize(
            ...
            test_rabbitmq_service_future = self.loop.create_future()
