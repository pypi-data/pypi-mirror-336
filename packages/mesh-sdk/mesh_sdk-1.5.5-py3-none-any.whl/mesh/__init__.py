...
def init(api_key: Optional[str] = None, **kwargs) -> None:
    """
    Initialize the Mesh SDK with configuration options.
    
    This function allows you to set the API key and other configuration options
    before making any API calls. It provides a convenient way to configure
    the SDK programmatically instead of using environment variables.
    
    Args:
        api_key: The Mesh API key (typically starting with 'mesh_pk_')
        **kwargs: Additional configuration options for the SDK
    
    Examples:
        # Initialize with an API key
        mesh.init(api_key="mesh_pk_yourkey123456789")
        
        # Then use any top-level functions
    ...
    if api_key:
        os.environ["MESH_API_KEY"] = api_key
        # Reset client so it will be recreated with the new API key
        _client = None
    
    # Process any other configuration options
    for key, value in kwargs.items():
        # Set in environment with MESH_ prefix
        env_key = f"MESH_{key.upper()}"
