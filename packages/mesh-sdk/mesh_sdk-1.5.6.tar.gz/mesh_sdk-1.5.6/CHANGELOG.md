...
### Fixed
- Improved API key authentication for headless environments by skipping the validation endpoint
- Modified client to bypass `/auth/validate` endpoint when using API keys, which fixes authentication in server environments
- Enhanced compatibility with various server configurations, particularly those running on Render
- Fixed authentication errors in headless environments while maintaining backward compatibility

## [1.5.5] - 2025-03-24

### Fixed
- Fixed authentication errors by updating all endpoint URL functions in config.py
- Replaced all instances of get_config() with get_all_config() in endpoint URL functions
- Added fallback default API URL to ensure endpoint generation never fails
- Improved reliability of API key validation in various environments
