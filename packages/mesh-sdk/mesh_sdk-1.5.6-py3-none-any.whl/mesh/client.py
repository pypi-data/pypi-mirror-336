...
class MeshClient:
    ...
    def _validate_token(self) -> bool:
        ...
        try:
            ...
            if api_key and api_key.startswith("mesh_"):
                ...
                # Skip validation endpoint call for API keys as the endpoint may not properly handle them
                # while the actual API endpoints do authenticate correctly with the API key.
                # Just store the API key for use in actual API calls
                self._api_key = api_key
                self.logger.debug("API key accepted based on format (mesh_*) - skipping validation endpoint call")
                return True
    ...
    def _get_headers(self, additional_headers=None) -> Dict[str, str]:
        ...
        try:
            ...
            api_key = get_api_key()
            if api_key and api_key.startswith("mesh_"):
                # Send API key in x-api-key header for older servers
                headers["x-api-key"] = api_key
                # Also maintain the Authorization header format for newer servers and backward compatibility
                headers["Authorization"] = f"ApiKey {api_key}"
                logger.debug(f"Using API key authentication (dual-header approach)")
                return headers
        except Exception as e:
