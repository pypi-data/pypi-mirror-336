...
from relationalai.early_access.dsl.core.types.standard import Boolean, Hash, Integer, Decimal, Float, Double, BigInteger, String
...
class Compiler:
    def __init__(self):
        # TODO: Find IR analogs for BigInteger, Date and DateTime types
        self.types: dict[Type, ir.Type] = {
                ...
                Double: types.Number,
                String: types.String,
                BigInteger: f.scalar_type("Int64"),
                Symbol: types.Symbol,
                Any: types.Any
        }
    ...
    def _parse_relation_name(name: str, producer: Callable[[str, Any], Union[ir.Field, ir.Var]]) \
        ...
        if ':' in name:
            parts = name.split(':')
            var_list = [producer(f":{part}", Symbol) for part in parts[1:]]
    ...
    def to_relation(self, ctx: CompilerContext, item:Atom, name:str) -> ir.Relation:
        ...
        if entity_id not in self.relations:
            if name in builtins.builtin_relations_by_name:
                ...
                self.relations[entity_id] = f.relation(name, fields)
