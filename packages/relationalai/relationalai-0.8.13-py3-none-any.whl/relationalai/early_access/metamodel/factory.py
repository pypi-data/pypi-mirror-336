...
def scalar_type(name: str):
    return ir.ScalarType(name)
...
# property helpers
def relation(name: str, fields: list[ir.Field], requires: OrderedSet[ir.Capability]=ordered_set()):
    return ir.Relation(name, tuple(fields), requires.frozen())
...
def logical(body: PySequence[ir.Task], hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
    return ir.Logical(engine, tuple(hoisted), tuple(body))

def sequence(tasks: PySequence[ir.Task], hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
    return ir.Sequence(engine, tuple(hoisted), tuple(tasks))

def union(tasks: PySequence[ir.Task], hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
    return ir.Union(engine, tuple(hoisted), tuple(tasks))

def match(tasks: PySequence[ir.Task], hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
    return ir.Match(engine, tuple(hoisted), tuple(tasks))

def until(check: ir.Task, body: ir.Task, hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
    return ir.Until(engine, tuple(hoisted), check, body)

def wait(check: ir.Task, hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
...
def else_(var: ir.Var, value: ir.Value) -> ir.Else:
    return Else(var, value)

def Else(var: ir.Var, value: ir.Value) -> ir.Else:
    return ir.Else(var, value)
...
def loop(iter: ir.Var, body: ir.Task, hoisted: PySequence[ir.VarOrElse]=[], engine: Optional[ir.Engine]=None):
