from relationalai.early_access.builder import Fragment
from relationalai.early_access.builder.builder import Compiler as BuilderCompiler
from relationalai.early_access.rel import Compiler as RelCompiler
from relationalai.early_access.metamodel import factory as f
from relationalai.early_access.metamodel import ir
from relationalai.early_access.devmode import Compiler as DevModeCompiler
...
class CompilationManager:
        ...
        try:
            ...
            for i in range(max_frames_to_check):
                ...
                if (not filename.startswith('<') and
                    ...
                    # Check whether this file imports from relationalai.early_access.builder
                    try:
                        with open(filename, 'r') as f:
                            content = f.read()
                            if ('from relationalai.early_access.builder' in content or
                                'import relationalai.early_access.builder' in content):
    ...
    def ir(self):
        if not self.fragment._model:
            raise ValueError("Fragment must be part of a model to compile to IR")
        builder_compiler = BuilderCompiler()
        rule_tasks = [
            builder_compiler.compile_fragment(rule, effects_only=True)
            for rule in self.fragment._model.rules
        ]
        fragment_task = builder_compiler.compile_fragment(self.fragment)
    ...
    def devmode(self, ir_model=None):
        if ir_model is None:
            ir_model = self.ir()
        devmode_compiler = DevModeCompiler()
        sql = devmode_compiler.compile(ir_model)
        return sql
