...
STATIC_DEPENDENCIES = [
    ('graphlib', '04cd9c07-16aa-4433-bdd7-adfdfaee317d', version_range('0.1.45', '0.2.0', '0.1.45 - 0.1.*')),
...
#
def check_static_dependencies(response:DataFrame, platform: str, app_name: str, engine_name: str, model_name:str, warn_on_packages = False):
...
def check_package_manager(response:DataFrame, platform: str, app_name: str, engine_name: str, model_name:str, warn_on_packages = False):
    results = response.results
    ...
    update_packages = _compare_dependencies(
        PACKAGE_DEPENDENCIES,
        STATIC_DEPENDENCIES,
        _extract_library_versions(results),
        _extract_std_version(results),
...
# expected package_deps and static_deps. Raise warnings when necessary and return whether
# we should make the package manager update the package versions.
def _compare_dependencies(package_deps, static_deps, lock, std, platform: str, app_name: str, engine_name: str, model_name:str, warn_on_packages = False):
    ...
    if not lock and not std:
        return warn(InvalidVersionKind.SchemaOutOfDate, static_deps)
