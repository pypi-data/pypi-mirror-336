...
class ResourcesBase(ABC):
    ...
    @abstractmethod
    def resume_engine(self, name: str) -> dict:
        pass
...
class Client():
    def __init__(
        ...
        if not dry_run:
            if self.use_graph_index:
                try:
                    self.resources.auto_create_engine_async(config.get("engine", None))
    ...
    def _manage_packages(self):
        ...
        try:
            ...
            # query the current state of the registry and packages in the database
            if enable_full_package_manager:
                res, raw = self._timed_query("query_package_manager", dependencies.generate_query_package_manager(), abort_on_error=False)
                update_registry, update_project = dependencies.check_package_manager(raw, platform, app_name, engine_name, database_name)

                # it may be necessary to update (refresh) the registry
                if update_registry:
                    self._timed_query("update_registry", dependencies.generate_update_registry(), readonly=False, abort_on_error=False)

                # it may be necessary to update the packages
                if update_project:
                    self._timed_query("update_packages", dependencies.generate_update_packages(), readonly=False, abort_on_error=False)
            else:
                res, raw = self._timed_query("query_version_check", dependencies.generate_query_version_check(), abort_on_error=False)
                dependencies.check_static_dependencies(raw, platform, app_name, engine_name, database_name, warn_on_packages=True)
    ...
    def _query(self, code:str, task:m.Task|None, end_span, readonly=True, inputs={}, nowait_durable=None, headers={}, abort_on_error=True):
        ...
        try:
            ...
            if "engine is suspended" in f"{e}":
                ...
                self.resources.resume_engine(engine_name)
