...
from relationalai.tools.constants import SNOWFLAKE_AUTHS, USE_GRAPH_INDEX, WAIT_FOR_STREAM_SYNC
from .. import std
...
class Resources(ResourcesBase):
    ...
    def resume_engine_async(self, name:str):
        self._exec(f"call {APP_NAME}.api.resume_engine_async('{name}');")
        return {}
    ...
    def poll_use_index(
         self, app_name: str, sources: Iterable[str], model: str, engine_name: str, program_span_id: str | None
    ):
        ...
        with debugging.span("check if ERP is running"):
    ...
    def get_export_code(self, params: ExportParams):
        ...
        if use_graph_index:
            ...
            source_relations_for_non_graph_index_path = [
                f"'{self._get_relation_name(source)}'" for source in self.sources
            ]
        python_code = process_jinja_template(
            ...
            py_inputs=py_inputs,
            py_outs=py_outs,
            source_references=", ".join(source_obj_references),
        ...
        sql_code = textwrap.dedent(f"""
            CREATE OR REPLACE PROCEDURE {params.func_name}({sql_inputs}{sql_inputs and ',' or ''} {destination_input} engine STRING DEFAULT NULL)
    ...
    def create_import_stream(self, source:ImportSource, model:str, rate = 1, options: dict|None = None):
        ...
        relation_name = self._get_relation_name(object)
    ...
    def _check_source_updates(self, sources: Iterable[str]):
        ...
        for db, schema, table in (source.split(".") for source in sources):
            partitioned_sources[db.upper()][schema.upper()].append(table.upper())
        ...
        query = (
            " UNION ALL ".join(
                ...
                RIGHT JOIN (
                    ...
                    WHERE TABLE_CATALOG = $${db}$$ AND ({" OR ".join(
                        f"(TABLE_SCHEMA = $${schema}$$ AND TABLE_NAME IN ({','.join(f'$${table}$$' for table in tables)}))"
        ...
        source_types: dict[str, SourceInfo] = {
            source.upper(): cast(SourceInfo, {"type": None, "state": ""})
            for source in sources
        }
        for row in self._exec(query):
            # Unlike SHOW OBJECTS, querying the info schema directly calls them BASE TABLE
            source_types[row["FQN"].upper()] = {
                "type": "TABLE" if row["KIND"] == "BASE TABLE" else row["KIND"],
                "state": row["STATE"],
            }
    ...
    def _get_relation_name(self, db_table_name: str) -> str:
        return db_table_name.replace(".", "_").lower()
    ...
    def schema_info(self, database:str, schema:str, tables:Iterable[str]):
        ...
        columns = self._exec(textwrap.dedent(f"""
            begin
                SHOW COLUMNS IN SCHEMA {database}.{schema};
        ...
        if columns:
            for row in columns:
                results[row.table_name.lower()]["columns"][row.column_name] = row.data_type
        return results
...
class SnowflakeClient(Client):
    def create_database(self, isolated=True, nowait_durable=True):
        ...
        with debugging.span("poll_use_index", sources=self.resources.sources, model=model, engine=engine_name):
            self.poll_use_index(app_name, self.resources.sources, model, engine_name, program_span_id)
    ...
    def poll_use_index(
        ...
        return self.resources.poll_use_index(
            app_name, sources, model, engine_name, program_span_id
        )
