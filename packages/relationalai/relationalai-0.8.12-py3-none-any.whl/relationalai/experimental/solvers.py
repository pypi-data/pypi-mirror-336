...
class SolverModel:
    ...
    def solve(self, solver: str | Solver, log_to_console=True, **kwargs):
        self.is_solved = False
        options = kwargs
        options["version"] = 1
        ...
        # Support for solving on Azure.
        # TODO (dba) Remove this once we drop Azure support.
        if isinstance(solver, str):
            if self.graph._config.get("host", None) != "azure-latest.relationalai.com":
            ...
            self.graph.exec_raw(textwrap.dedent(f"""
                ...
                @no_diagnostics(:EXPERIMENTAL)
                def {scope}model_string {{ rel_primitive_solverlib_model_string[{{
                                (:variable, _solver_unwrap[{scope}variables]);
                                (:variable_name, _solver_unwrap[{variable_name_string}]);
                                (:expression_name, _solver_unwrap[{component_name_string}]);
                                {scope}specialized_components;
...
# --------------------------------------------------
#
# TODO (dba) In the future we will have a unified engine API for both
#            kgcore, solvers, and GNN. Until then we define our own
#            solver specific API in this module. The `worker` API will
#            then also be renamed to `engine`.
...
class Provider:
    ...
    def create_solver(
        ...
        self.resources._exec(
            f"CALL {APP_NAME}.experimental.create_worker('{ENGINE_TYPE_SOLVER}', '{name}', '{size}', {engine_config});"
        )
    ...
    def create_solver_async(
        ...
        if auto_suspend_mins is not None:
            engine_config["auto_suspend_mins"] = auto_suspend_mins
        self.resources._exec(
            f"CALL {APP_NAME}.experimental.create_worker_async('{ENGINE_TYPE_SOLVER}', '{name}', '{size}', {engine_config});"
        )

    def delete_solver(self, name: str):
        self.resources._exec(
            f"CALL {APP_NAME}.experimental.delete_worker('{ENGINE_TYPE_SOLVER}', '{name}');"
        )

    def resume_solver_async(self, name: str):
        self.resources._exec(
            f"CALL {APP_NAME}.experimental.resume_worker_async('{ENGINE_TYPE_SOLVER}', '{name}');"
        )

    def get_solver(self, name: str):
        results = self.resources._exec(
            f"CALL {APP_NAME}.experimental.get_worker('{ENGINE_TYPE_SOLVER}', '{name}');"
        )
    ...
    def list_solvers(self, state: str | None = None):
        ...
        statement = f"SELECT NAME,ID,SIZE,STATUS,CREATED_BY,CREATED_ON,UPDATED_ON,SETTINGS FROM {APP_NAME}.experimental.workers {where_clause};"
    ...
    def list_jobs(self, state=None, limit=None):
        ...
        results = self.resources._exec(
            f"SELECT ID,STATE,CREATED_BY,CREATED_ON,FINISHED_AT,DURATION,PAYLOAD,WORKER_NAME FROM {APP_NAME}.experimental.jobs where type='{ENGINE_TYPE_SOLVER}' {state_clause} ORDER BY created_on DESC {limit_clause};"
        )
