...
class __AuxBoxGymObsSpace:
    ...
    def __init__(
        ...
        self._dict_properties = {
            ...
            "gen_p": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=0.0, dtype=dt_float)
                - tol_redisp
                - extra_for_losses,
                ob_sp_cls.gen_pmax + tol_redisp + extra_for_losses,
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            "gen_q": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=np.inf, dtype=dt_float),
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            ...
            "gen_margin_up": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=0.0, dtype=dt_float),
                1.0 * ob_sp_cls.gen_max_ramp_up,
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            "gen_margin_down": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=0.0, dtype=dt_float),
                1.0 * ob_sp_cls.gen_max_ramp_down,
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            "gen_theta": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=-180., dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=180., dtype=dt_float),
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            "load_p": (
                np.full(shape=(ob_sp_cls.n_load,), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_load,), fill_value=+np.inf, dtype=dt_float),
                (ob_sp_cls.n_load,),
                dt_float,
            ),
            ...
            "load_v": (
                np.full(shape=(ob_sp_cls.n_load,), fill_value=0.0, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_load,), fill_value=np.inf, dtype=dt_float),
                (ob_sp_cls.n_load,),
                dt_float,
            ),
            "load_theta": (
                np.full(shape=(ob_sp_cls.n_load,), fill_value=-180., dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_load,), fill_value=180., dtype=dt_float),
                (ob_sp_cls.n_load,),
                dt_float,
            ),
            "p_or": (
                np.full(shape=(ob_sp_cls.n_line,), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_line,), fill_value=np.inf, dtype=dt_float),
                (ob_sp_cls.n_line,),
                dt_float,
            ),
            ...
            "a_or": (
                np.full(shape=(ob_sp_cls.n_line,), fill_value=0.0, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_line,), fill_value=np.inf, dtype=dt_float),
                (ob_sp_cls.n_line,),
                dt_float,
            ),
            ...
            "theta_or": (
                np.full(shape=(ob_sp_cls.n_line,), fill_value=-180., dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_line,), fill_value=180., dtype=dt_float),
                (ob_sp_cls.n_line,),
                dt_float,
            ),
            ...
            "line_status": (
                np.full(shape=(ob_sp_cls.n_line,), fill_value=0, dtype=dt_int),
                np.full(shape=(ob_sp_cls.n_line,), fill_value=1, dtype=dt_int),
                (ob_sp_cls.n_line,),
                dt_int,
            ),
            "timestep_overflow": (
                np.full(
                    shape=(ob_sp_cls.n_line,), fill_value=np.iinfo(dt_int).min, dtype=dt_int
                ),
                np.full(
                    shape=(ob_sp_cls.n_line,), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (ob_sp_cls.n_line,),
                dt_int,
            ),
            "topo_vect": (
                np.full(shape=(ob_sp_cls.dim_topo,), fill_value=-1, dtype=dt_int),
                np.full(shape=(ob_sp_cls.dim_topo,), fill_value=ob_sp_cls.n_busbar_per_sub, dtype=dt_int),
                (ob_sp_cls.dim_topo,),
                dt_int,
            ),
            "time_before_cooldown_line": (
            ...
            "time_before_cooldown_sub": (
                np.full(shape=(ob_sp_cls.n_sub,), fill_value=0, dtype=dt_int),
                np.full(
                    shape=(ob_sp_cls.n_sub,), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (ob_sp_cls.n_sub,),
                dt_int,
            ),
            ...
            "storage_power_target": (
                -1.0 * ob_sp_cls.storage_max_p_prod,
                1.0 * ob_sp_cls.storage_max_p_absorb,
                (ob_sp_cls.n_storage,),
                dt_float,
            ),
            "storage_power": (
            ...
            "storage_theta": (
                np.full(shape=(ob_sp_cls.n_storage,), fill_value=-180., dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_storage,), fill_value=180., dtype=dt_float),
                (ob_sp_cls.n_storage,),
                dt_float,
            ),
            "curtailment": (
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=0.0, dtype=dt_float),
                np.full(shape=(ob_sp_cls.n_gen,), fill_value=1.0, dtype=dt_float),
                (ob_sp_cls.n_gen,),
                dt_float,
            ),
            "curtailment_limit": (
            ...
            "last_alarm": (
                np.full(shape=(ob_sp_cls.dim_alarms,), fill_value=-1, dtype=dt_int),
                np.full(
                    shape=(ob_sp_cls.dim_alarms,),
                    fill_value=np.iinfo(dt_int).max,
                    dtype=dt_int,
                ),
                (ob_sp_cls.dim_alarms,),
                dt_int,
            ),
            "attention_budget": (
            ...
            # alert stuff
            "active_alert": (
                np.full(shape=(ob_sp_cls.dim_alerts,), fill_value=False, dtype=dt_bool),
                np.full(shape=(ob_sp_cls.dim_alerts,), fill_value=True, dtype=dt_bool),
                (ob_sp_cls.dim_alerts,),
                dt_bool,
            ),
            "time_since_last_alert": (
                np.full(shape=(ob_sp_cls.dim_alerts,), fill_value=-1, dtype=dt_int),
                np.full(shape=(ob_sp_cls.dim_alerts,), fill_value=np.iinfo(dt_int).max, dtype=dt_int),
                (ob_sp_cls.dim_alerts,),
                dt_int,
            ),
            "alert_duration": (
            ...
            "total_number_of_alert": (
                np.full(shape=(1 if ob_sp_cls.dim_alerts else 0,), fill_value=-1, dtype=dt_int),
                np.full(shape=(1 if ob_sp_cls.dim_alerts else 0,), fill_value=np.iinfo(dt_int).max, dtype=dt_int),
                (1 if ob_sp_cls.dim_alerts else 0,),
                dt_int,
            ),
            "time_since_last_attack": (
                np.full(shape=(ob_sp_cls.dim_alerts,), fill_value=-1, dtype=dt_int),
