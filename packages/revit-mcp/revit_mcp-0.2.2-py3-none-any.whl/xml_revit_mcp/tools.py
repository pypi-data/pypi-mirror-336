...
def find_elements(ctx: Context, method: str = "FindElements", params: List[dict[str, object]] = None) -> str:
    ...
    参数:
        ...
        params (List[Dict[str, Union[int, str, bool]]]): 查询条件列表，每个字典包含:
            ...
            - categoryName (str, optional): BuiltInCategory或者Category.Name（可能受语言版本影响）
    ...
    示例:
        >>> # 按类别ID查找
        >>> response = find_elements(ctx, params=[{"categoryId": -2000011, "isInstance": Flase}])
    ...
    try:
        ...
        for param in params:
            ...
            if "categoryId" not in param:
                raise ValueError("每个参数必须包含categoryId字段")

            # 验证categoryId
            if "categoryId" in param:
                validated_param["categoryId"] = str(param["categoryId"])
