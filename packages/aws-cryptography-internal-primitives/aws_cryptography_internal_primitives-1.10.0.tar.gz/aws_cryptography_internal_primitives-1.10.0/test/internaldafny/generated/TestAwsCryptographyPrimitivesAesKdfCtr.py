...
class default__:
    ...
    def AesKdfCtrTests():
        ...
        d_3_valueOrError0_ = AesKdfCtr.default__.AesKdfCtrStream(d_1_nonce_, d_0_key_, 44)
        if not(not((d_3_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestAesKdfCtr.dfy(18,18): " + _dafny.string_of(d_3_valueOrError0_))
        d_4_result_: _dafny.Seq
        d_4_result_ = (d_3_valueOrError0_).Extract()
        if not((len(d_4_result_)) == (44)):
            raise _dafny.HaltException("test/TestAesKdfCtr.dfy(19,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        if not((d_2_goal_) == (d_4_result_)):
        ...
        if not(not((d_5_valueOrError1_).IsFailure())):
            raise _dafny.HaltException("test/TestAesKdfCtr.dfy(25,14): " + _dafny.string_of(d_5_valueOrError1_))
        d_4_result_ = (d_5_valueOrError1_).Extract()
        if not((len(d_4_result_)) == (16)):
        ...
        if not(not((d_6_valueOrError2_).IsFailure())):
            raise _dafny.HaltException("test/TestAesKdfCtr.dfy(32,14): " + _dafny.string_of(d_6_valueOrError2_))
        d_4_result_ = (d_6_valueOrError2_).Extract()
        if not((len(d_4_result_)) == (64)):
