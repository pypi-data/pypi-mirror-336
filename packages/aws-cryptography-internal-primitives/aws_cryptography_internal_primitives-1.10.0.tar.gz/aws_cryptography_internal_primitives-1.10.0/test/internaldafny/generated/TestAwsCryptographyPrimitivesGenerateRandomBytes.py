...
class default__:
    ...
    def BasicGenerateRandomBytes():
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestGenerateRandomBytes.dfy(11,18): " + _dafny.string_of(d_0_valueOrError0_))
        d_1_client_: AtomicPrimitives.AtomicPrimitivesClient
        d_1_client_ = (d_0_valueOrError0_).Extract()
        d_2_length_: int
        d_2_length_ = 5
        d_3_input_: AwsCryptographyPrimitivesTypes.GenerateRandomBytesInput
        d_3_input_ = AwsCryptographyPrimitivesTypes.GenerateRandomBytesInput_GenerateRandomBytesInput(d_2_length_)
        d_4_output_: Wrappers.Result
        out1_: Wrappers.Result
        out1_ = (d_1_client_).GenerateRandomBytes(d_3_input_)
        d_4_output_ = out1_
        d_5_valueOrError1_: Wrappers.Result = Wrappers.Result.default(_dafny.Seq)()
        d_5_valueOrError1_ = d_4_output_
        if not(not((d_5_valueOrError1_).IsFailure())):
            raise _dafny.HaltException("test/TestGenerateRandomBytes.dfy(20,17): " + _dafny.string_of(d_5_valueOrError1_))
        d_6_value_: _dafny.Seq
        d_6_value_ = (d_5_valueOrError1_).Extract()
        if not((len(d_6_value_)) == (d_2_length_)):
