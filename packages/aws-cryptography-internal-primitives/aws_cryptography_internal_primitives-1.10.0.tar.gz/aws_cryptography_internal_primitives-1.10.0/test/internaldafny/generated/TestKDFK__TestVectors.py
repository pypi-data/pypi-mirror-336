...
class default__:
    ...
    def ExpectInternalTestVectors():
        hi0_ = len(default__.rawTestVectors)
        for d_0_i_ in range(0, hi0_):
            default__.ExpectRawDeriveTestVector((default__.rawTestVectors)[d_0_i_])
        hi1_ = len(default__.testVectors)
        for d_1_i_ in range(0, hi1_):
            default__.ExpectTestVector((default__.testVectors)[d_1_i_])
    ...
    def ExpectRawDeriveTestVector(vector):
        ...
        d_4_L_ = let_tmp_rhs0_.L
        d_5_OKM_ = let_tmp_rhs0_.OKM
        _dafny.print(_dafny.string_of((d_0_name_) + (_dafny.Seq("\n"))))
        if not((((((len(d_2_IKM_)) == (32)) or ((len(d_2_IKM_)) == (48))) and ((d_4_L_) > (0))) and (((4) + (len(d_3_info_))) < (StandardLibrary_UInt.default__.INT32__MAX__LIMIT))) and (((d_1_hash_) == (AwsCryptographyPrimitivesTypes.DigestAlgorithm_SHA__256())) or ((d_1_hash_) == (AwsCryptographyPrimitivesTypes.DigestAlgorithm_SHA__384())))):
            raise _dafny.HaltException("test/TestKDF_TestVectors.dfy(364,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        if not((((d_4_L_) + (Digest.default__.Length(AwsCryptographyPrimitivesTypes.DigestAlgorithm_SHA__256()))) < ((StandardLibrary_UInt.default__.INT32__MAX__LIMIT) - (1))) and (((d_4_L_) + (Digest.default__.Length(AwsCryptographyPrimitivesTypes.DigestAlgorithm_SHA__384()))) < ((StandardLibrary_UInt.default__.INT32__MAX__LIMIT) - (1)))):
            raise _dafny.HaltException("test/TestKDF_TestVectors.dfy(365,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        TestKDF.default__.KdfRawDeriveTest(d_2_IKM_, d_3_info_, d_4_L_, d_5_OKM_, d_1_hash_)
    ...
    def ExpectTestVector(vector):
        ...
        d_2_IKM_ = let_tmp_rhs0_.IKM
        d_3_info_ = let_tmp_rhs0_.info
        d_4_purpose_ = let_tmp_rhs0_.purpose
        d_5_L_ = let_tmp_rhs0_.L
        d_6_OKM_ = let_tmp_rhs0_.OKM
        _dafny.print(_dafny.string_of((d_0_name_) + (_dafny.Seq("\n"))))
        TestKDF.default__.KdfTest(AwsCryptographyPrimitivesTypes.KdfCtrInput_KdfCtrInput(d_1_hash_, d_2_IKM_, d_5_L_, Wrappers.Option_Some(d_4_purpose_), Wrappers.Option_Some(d_3_info_)), d_6_OKM_)
