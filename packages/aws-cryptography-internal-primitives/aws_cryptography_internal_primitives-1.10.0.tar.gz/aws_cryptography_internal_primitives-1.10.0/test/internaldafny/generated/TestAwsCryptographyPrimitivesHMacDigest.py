...
class default__:
    ...
    def DigestTests():
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestHMAC.dfy(14,18): " + _dafny.string_of(d_0_valueOrError0_))
        d_1_client_: AtomicPrimitives.AtomicPrimitivesClient
        d_1_client_ = (d_0_valueOrError0_).Extract()
        default__.HmacSHA__256(d_1_client_)
        default__.HmacSHA__384(d_1_client_)
        default__.HmacSHA__512(d_1_client_)
    ...
    def HmacSHA__256(client):
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestHMAC.dfy(26,13): " + _dafny.string_of(d_0_valueOrError0_))
        d_1___v0_: tuple
    ...
    def HmacSHA__384(client):
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestHMAC.dfy(47,13): " + _dafny.string_of(d_0_valueOrError0_))
        d_1___v1_: tuple
    ...
    def HmacSHA__512(client):
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("test/TestHMAC.dfy(69,13): " + _dafny.string_of(d_0_valueOrError0_))
        d_1___v2_: tuple
        d_1___v2_ = (d_0_valueOrError0_).Extract()

    @staticmethod
    def BasicHMacTest(client, input, expectedDigest):
        d_0_valueOrError0_ = (client).HMac(input)
        if (d_0_valueOrError0_).IsFailure():
            return (d_0_valueOrError0_).PropagateFailure()
        elif True:
            d_1_output_ = (d_0_valueOrError0_).Extract()
            d_2_valueOrError1_ = Wrappers.default__.Need((len(d_1_output_)) == (Digest.default__.Length((input).digestAlgorithm)), AwsCryptographyPrimitivesTypes.Error_AwsCryptographicPrimitivesError(_dafny.Seq("Error")))
            if (d_2_valueOrError1_).IsFailure():
                return (d_2_valueOrError1_).PropagateFailure()
            elif True:
                d_3_valueOrError2_ = Wrappers.default__.Need((d_1_output_) == (expectedDigest), AwsCryptographyPrimitivesTypes.Error_AwsCryptographicPrimitivesError(_dafny.Seq("Error")))
