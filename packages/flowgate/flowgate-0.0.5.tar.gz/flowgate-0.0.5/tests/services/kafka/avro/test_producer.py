...
from flowgate.services.kafka.avro import producer
...
def confluent_avro_producer():
    ...
    with ExitStack() as stack:
        stack.enter_context(
            patch(
                "flowgate.services.kafka.avro.producer.ConfluentAvroProducer.__init__",
                MagicMock,
            )
        )
        stack.enter_context(
            patch(
                "flowgate.services.kafka.avro.producer.ConfluentAvroProducer.produce",
                producer,
            )
        )
        yield producer
...
@pytest.fixture
@patch("flowgate.services.kafka.avro.producer.AvroProducer._close", MagicMock())
