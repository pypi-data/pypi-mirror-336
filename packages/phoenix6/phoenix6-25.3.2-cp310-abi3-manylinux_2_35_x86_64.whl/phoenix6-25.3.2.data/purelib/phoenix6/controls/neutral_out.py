...
class NeutralOut:
    """
    Request neutral output of actuator. The applied brake type is determined by the
    NeutralMode configuration.
    ...
    def _send_request(self, network: str, device_hash: int) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlNeutralOut(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, self.use_timesync))

    
    def with_use_timesync(self, new_use_timesync: bool) -> 'NeutralOut':
        """
    ...
    def with_update_freq_hz(self, new_update_freq_hz: hertz) -> 'NeutralOut':
        """
