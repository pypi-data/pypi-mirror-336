...
def validate_documents(files):
    ...
    for field, contents in files.items():
        if not file_is_open_document_format(contents):
            errors[field] = file_is_open_document_format.__name__
...
def validate_documents_from_question(files, section):
    errors = {}
    ...
    for field, contents in files.items():
        ...
        validator = FILE_TYPE_TO_VALIDATION.get(question.file_settings['file_type'])
...
def upload_declaration_document(
    uploader,
    upload_type,
    documents_url,
    service,
    ...
    file_path = generate_file_name_for_declaration(
        service['frameworkSlug'],
        upload_type,
        service['supplierId'],
        filename_suffix,
...
def upload_declaration_documents(
    ...
    files = filter_empty_files(files)
    errors = validate_documents_from_question(files, section)
    ...
    for field, contents in files.items():
        ...
        url = upload_declaration_document(
            uploader,
            upload_type,
            documents_url,
            {
...
def generate_file_name(framework_slug, upload_type, supplier_id, service_id, field, filename, suffix=None):
    ...
    ID_TO_FILE_NAME_SUFFIX = {
        ...
        'technicalAbilityCertificateOneURL': 'technical-ability-certificate-1',
        'technicalAbilityCertificateTwoURL': 'technical-ability-certificate-2',
        'carbonReductionCopyUpload': 'carbon-reduction-copy-upload',
    }
...
def generate_file_name_for_declaration(
    framework_slug,
    upload_type,
    supplier_id,
    filename_suffix,
