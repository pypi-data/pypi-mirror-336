...
cdef class SIFTImageDistance(Distance):
    # Attributs de classe
    cdef object _match_threshold
    cdef object _sift
    cdef object __match_features
    cdef object __filter_matches
    cdef object _type
    cdef object __extract_features
    "
    ...
    def _match_features(self, desc1: cv2.Mat, desc2: cv2.Mat) -> List[List[cv2.DMatch]]:
        '
    ...
    def compute(self, image1: cv2.Mat, image2: cv2.Mat) -> Optional[MatchResult]:
        '
        ...
        return MatchResult(distance=avg_distance, num_matches=len(good_matches), match_ratio=match_ratio)
    ...
    def _match_features(self):
        return self.__match_features
    @_match_features.setter
    def _match_features(self, value):
        self.__match_features = value
    @property
    def type(self):
        return self._type
    @type.setter
    ...
    @_extract_features.setter
    def _extract_features(self, value):
        self.__extract_features = value
    @property
    def _filter_matches(self):
        return self.__filter_matches
    @_filter_matches.setter
    def _filter_matches(self, value):
        self.__filter_matches = value
    """
    A class that computes the distance between two images using the SIFT algorithm.
    This implementation uses OpenCV's SIFT implementation and provides a measure
    of similarity between images that is invariant to scale, rotation, and translation.
...
cdef class SURFImageDistance(Distance):
    # Attributs de classe
    cdef object _match_threshold
    cdef object _surf
    cdef object __match_features
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__()
        self._type = 'image'
        self._surf = cv2.xfeatures2d.SURF_create(hessianThreshold=hessian_threshold, extended=extended)
        self._match_threshold = match_threshold
    ...
    def _extract_features(self, image: cv2.Mat) -> Tuple[(List[cv2.KeyPoint], cv2.Mat)]:
        ...
        Returns:
            Tuple containing:
                ...
                - Matrix of descriptors
        '
        (keypoints, descriptors) = self._surf.detectAndCompute(image, None)
        return (keypoints, descriptors)
    ...
    def _filter_matches(self, matches: List[List[cv2.DMatch]], kp1: List[cv2.KeyPoint], kp2: List[cv2.KeyPoint]) -> Tuple[(List[cv2.DMatch], float)]:
        '
    ...
    def compute(self, image1: cv2.Mat, image2: cv2.Mat) -> Optional[SURFMatchResult]:
        '
        ...
        try:
            ...
            total_keypoints = min(len(kp1), len(kp2))
            match_ratio = (len(good_matches) / total_keypoints)
            return SURFMatchResult(distance=avg_distance, num_matches=len(good_matches), match_ratio=match_ratio, avg_strength=avg_strength)
...
cdef class ORBImageDistance(Distance):
    # Attributs de classe
    cdef object _match_threshold
    cdef object __match_features
    cdef object __filter_matches
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__()
        self._type = 'image'
        self._orb = cv2.ORB_create(nfeatures=n_features, scaleFactor=scale_factor, nlevels=n_levels, fastThreshold=fast_threshold)
        self._match_threshold = match_threshold
    ...
    def compute(self, image1: cv2.Mat, image2: cv2.Mat) -> Optional[ORBMatchResult]:
        '
        ...
        try:
            ...
            total_keypoints = min(len(kp1), len(kp2))
            match_ratio = (len(good_matches) / total_keypoints)
            return ORBMatchResult(distance=avg_distance, num_matches=len(good_matches), match_ratio=match_ratio, avg_response=avg_response)
...
cdef class ChamferDistance(Distance):
    # Attributs de classe
    cdef object _bidirectional
    cdef object __find_nearest_distance
    cdef object _edge_kernel_size
    cdef object __extract_edge_points
    cdef object _type
    cdef object __compute_directional_distance
    cdef object _edge_threshold
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._edge_kernel_size = edge_kernel_size
    ...
    def _extract_edge_points(self, image: cv2.Mat) -> Set[Tuple[(int, int)]]:
        ...
        '
        edges = cv2.Canny(image, self._edge_threshold, (self._edge_threshold * 2), self._edge_kernel_size)
        points = set()
    ...
    def _find_nearest_distance(self, point: Tuple[(int, int)], target_points: Set[Tuple[(int, int)]]) -> float:
        '
    ...
    def _compute_directional_distance(self, source_points: Set[Tuple[(int, int)]], target_points: Set[Tuple[(int, int)]]) -> Tuple[(float, int)]:
        '
    ...
    def compute(self, image1: cv2.Mat, image2: cv2.Mat) -> ChamferResult:
        '
        ...
        return ChamferResult(distance=chamfer_distance, forward_distance=forward_dist, backward_distance=backward_dist, num_points_img1=num_points1, num_points_img2=num_points2)
    ...
    @property
    def _find_nearest_distance(self):
        return self.__find_nearest_distance
    @_find_nearest_distance.setter
    def _find_nearest_distance(self, value):
        self.__find_nearest_distance = value
    @property
    def edge_kernel_size(self):
        return self._edge_kernel_size
    @edge_kernel_size.setter
    def edge_kernel_size(self, value):
        self._edge_kernel_size = value
    @property
    def _extract_edge_points(self):
        return self.__extract_edge_points
    @_extract_edge_points.setter
    def _extract_edge_points(self, value):
        self.__extract_edge_points = value
    @property
    def type(self):
        return self._type
    @type.setter
    def type(self, value):
        self._type = value
    @property
    def _compute_directional_distance(self):
        return self.__compute_directional_distance
    @_compute_directional_distance.setter
    def _compute_directional_distance(self, value):
        self.__compute_directional_distance = value
    @property
...
cdef class PerceptualDistance(Distance):
    # Attributs de classe
    cdef object __compute_local_statistics
    cdef object __compute_contrast_difference
    cdef object _weights
    cdef object __compute_brightness_difference
    cdef object _block_size
    cdef object __compute_structure_difference
    cdef object _type
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._weights = {'contrast': contrast_weight, 'structure': structure_weight, 'brightness': brightness_weight}

    def _compute_local_statistics(self, image: cv2.Mat) -> Tuple[(cv2.Mat, cv2.Mat)]:
        '
        ...
        '
        mean = cv2.blur(image.astype(float), (self._block_size, self._block_size))
        squared = cv2.blur((image.astype(float) ** 2), (self._block_size, self._block_size))
        variance = (squared - (mean ** 2))
        std = cv2.sqrt(cv2.max(variance, 0))
        return (mean, std)

    def _compute_contrast_difference(self, std1: cv2.Mat, std2: cv2.Mat) -> float:
        '
    ...
    def _compute_structure_difference(self, image1: cv2.Mat, image2: cv2.Mat) -> float:
        '
    ...
    def _compute_brightness_difference(self, mean1: cv2.Mat, mean2: cv2.Mat) -> float:
        '
    ...
    def compute(self, image1: cv2.Mat, image2: cv2.Mat) -> PerceptualResult:
        '
        ...
        total_distance = (((self._weights['contrast'] * contrast_score) + (self._weights['structure'] * structure_score)) + (self._weights['brightness'] * brightness_score))
        details = {'local_contrast_variation': float(cv2.meanStdDev((std1 - std2))[0][0]), 'brightness_distribution': float(cv2.compareHist(cv2.calcHist([image1], [0], None, [256], [0, 256]), cv2.calcHist([image2], [0], None, [256], [0, 256]), cv2.HISTCMP_BHATTACHARYYA))}
        return PerceptualResult(distance=total_distance, contrast_score=contrast_score, structure_score=structure_score, brightness_score=brightness_score, details=details)


    # Propriétés pour accéder aux attributs depuis Python
    @property
    def _compute_local_statistics(self):
        return self.__compute_local_statistics
    @_compute_local_statistics.setter
    def _compute_local_statistics(self, value):
        self.__compute_local_statistics = value
    @property
    def weights(self):
    ...
    def weights(self, value):
        self._weights = value
    @property
    def _compute_contrast_difference(self):
        return self.__compute_contrast_difference
    @_compute_contrast_difference.setter
    def _compute_contrast_difference(self, value):
        self.__compute_contrast_difference = value
    @property
    def _compute_brightness_difference(self):
        return self.__compute_brightness_difference
    @_compute_brightness_difference.setter
    def _compute_brightness_difference(self, value):
        self.__compute_brightness_difference = value
    @property
    def block_size(self):
    ...
    @property
    def _compute_structure_difference(self):
        return self.__compute_structure_difference
    @_compute_structure_difference.setter
    def _compute_structure_difference(self, value):
        self.__compute_structure_difference = value
    @property
    def type(self):
        return self._type
    @type.setter
...
cdef class LPIPS(Distance):
    # Attributs de classe
    cdef object __extract_patches
    cdef object __compute_patch_features
    cdef object __weights
    cdef object _stdev
    cdef object __initialize_weights
    cdef object _patch_size
    cdef object _type
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__weights = self._initialize_weights()
    ...
    def _extract_patches(self, image: List[List[List[float]]]) -> List[List[List[List[float]]]]:
        ...
        width = len(image[0])
        patches = []
        for i in range(0, ((height - self._patch_size) + 1), self._patch_size):
            row_patches = []
            for j in range(0, ((width - self._patch_size) + 1), self._patch_size):
                patch = []
    ...
    def _compute_patch_features(self, patch: List[List[List[float]]]) -> List[float]:
        ...
        color_mean = [statistics.mean(r_values), statistics.mean(g_values), statistics.mean(b_values)]
        color_std = [self.stdev(r_values), self.stdev(g_values), self.stdev(b_values)]
    ...
    def compute(self, image1: List[List[List[float]]], image2: List[List[List[float]]]) -> float:
        '
        ...
        for i in range(len(patches1)):
            for j in range(len(patches1[0])):
                ...
                patch_distance = 0.0
                for (f1, f2, w) in zip(features1, features2, self.__weights):
    ...
    def __str__(self) -> str:
        ...
        '
        return f'LPIPS Calculator (patch_size={self._patch_size})'


    # Propriétés pour accéder aux attributs depuis Python
    @property
    def _extract_patches(self):
        return self.__extract_patches
    @_extract_patches.setter
    def _extract_patches(self, value):
        self.__extract_patches = value
    @property
    def _compute_patch_features(self):
        return self.__compute_patch_features
    @_compute_patch_features.setter
    def _compute_patch_features(self, value):
        self.__compute_patch_features = value
    @property
    def _weights(self):
    ...
    def _initialize_weights(self):
        return self.__initialize_weights
    @_initialize_weights.setter
    def _initialize_weights(self, value):
        self.__initialize_weights = value
    @property
    def patch_size(self):
...
cdef class BRISQUE(Distance):
    # Attributs de classe
    cdef object __extract_patches
    cdef object _gaussian_window
    cdef object __to_luminance
    cdef object _patch_size
    cdef object __compute_mscn_coefficients
    cdef object _type
    cdef object __compute_aggd_features
    cdef object __create_gaussian_window
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._type = 'image'
        self._patch_size = patch_size
        self._gaussian_window = self._create_gaussian_window()
    ...
    def _to_luminance(self, image: List[List[List[float]]]) -> List[List[float]]:
        ...
        for i in range(height):
            for j in range(width):
                (r, g, b) = image[i][j]
                luminance[i][j] = (((0.2989 * r) + (0.587 * g)) + (0.114 * b))
        return luminance

    def _extract_patches(self, image: List[List[float]]) -> List[List[List[float]]]:
        '
    ...
    def _compute_mscn_coefficients(self, patch: List[List[float]]) -> List[List[float]]:
        '
        ...
        for i in range(self._patch_size):
            for j in range(self._patch_size):
                local_mean += (patch[i][j] * self._gaussian_window[i][j])
        local_var = 0.0
        for i in range(self._patch_size):
            for j in range(self._patch_size):
                diff = (patch[i][j] - local_mean)
                local_var += ((diff * diff) * self._gaussian_window[i][j])
        local_std = (sqrt(local_var) + 1e-07)
        mscn = [([0.0] * self._patch_size) for _ in range(self._patch_size)]
        for i in range(self._patch_size):
            for j in range(self._patch_size):
                mscn[i][j] = ((patch[i][j] - local_mean) / local_std)
        return mscn

    def _compute_aggd_features(self, coeffs: List[List[float]]) -> List[float]:
        '
    ...
    def compute(self, image: List[List[List[float]]]) -> float:
        ...
        height = len(image)
        width = len(image[0])
        if ((height < self._patch_size) or (width < self._patch_size)):
            raise ValueError(f'Image dimensions must be at least {self._patch_size}x{self._patch_size}')
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: Description of the calculator
        '
        return f'BRISQUE Calculator (patch_size={self._patch_size})'
    ...
    def gaussian_window(self):
        return self._gaussian_window
    @gaussian_window.setter
    def gaussian_window(self, value):
        self._gaussian_window = value
    @property
    def _to_luminance(self):
        return self.__to_luminance
    @_to_luminance.setter
    def _to_luminance(self, value):
        self.__to_luminance = value
    @property
    def patch_size(self):
        return self._patch_size
    @patch_size.setter
    ...
    def _compute_mscn_coefficients(self):
        return self.__compute_mscn_coefficients
    @_compute_mscn_coefficients.setter
    def _compute_mscn_coefficients(self, value):
        self.__compute_mscn_coefficients = value
    @property
    def type(self):
        return self._type
    @type.setter
    ...
    def _compute_aggd_features(self):
        return self.__compute_aggd_features
    @_compute_aggd_features.setter
    def _compute_aggd_features(self, value):
        self.__compute_aggd_features = value
    @property
    def _create_gaussian_window(self):
        return self.__create_gaussian_window
    @_create_gaussian_window.setter
    def _create_gaussian_window(self, value):
        self.__create_gaussian_window = value
    """
    A pure Python implementation of BRISQUE (Blind/Referenceless Image Spatial Quality Evaluator).
    This is a simplified version that captures the core concepts without external dependencies.
...
cdef class VGG16Distance(Distance):
    # Attributs de classe
    cdef dict _filters
    cdef object __initialize_filters
    cdef object __max_pool
    cdef object _layer_configs
    cdef object _type
    cdef object __convolve
    cdef object __extract_features
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__()
        self._type = 'image'
        self._layer_configs = [(64, 3, 2), (128, 3, 2), (256, 3, 2), (512, 3, 2), (512, 3, 2)]
    ...
    def _initialize_filters(self) -> Dict[(int, List[List[List[List[float]]]])]:
        ...
        filters = {}
        current_depth = 3
        for (block_idx, (n_filters, filter_size, _)) in enumerate(self._layer_configs):
            block_filters = []
            for _ in range(n_filters):
                filter_weights = [[[([0.0] * current_depth) for _ in range(filter_size)] for _ in range(filter_size)] for _ in range(1)]
    ...
    def _convolve(self, input_data: List[List[List[float]]], filter_weights: List[List[List[List[float]]]]) -> List[List[List[float]]]:
        '
        ...
        output = [[([0.0] * n_filters) for _ in range(out_width)] for _ in range(out_height)]
                    ...
                    for i in range(filter_size):
                        for j in range(filter_size):
                            for d in range(depth):
                                value += (input_data[(h + i)][(w + j)][d] * filter_weights[f][0][i][j][d])
                    output[h][w][f] = max(0.0, value)
        return output

    def _max_pool(self, input_data: List[List[List[float]]], pool_size: int) -> List[List[List[float]]]:
        '
        ...
        output = [[([0.0] * depth) for _ in range(out_width)] for _ in range(out_height)]
        for h in range(out_height):
            for w in range(out_width):
                for d in range(depth):
                    pool_max = float('-inf')
                    for i in range(pool_size):
                        for j in range(pool_size):
                            pool_max = max(pool_max, input_data[((h * pool_size) + i)][((w * pool_size) + j)][d])
                    output[h][w][d] = pool_max
        return output

    def _extract_features(self, image: List[List[List[float]]]) -> List[List[List[List[float]]]]:
        '
        ...
        features = []
        current_input = image
        for (block_idx, (n_filters, filter_size, pool_size)) in enumerate(self._layer_configs):
            conv_output = self._convolve(current_input, self._filters[block_idx])
    ...
    def compute(self, image1: List[List[List[float]]], image2: List[List[List[float]]]) -> float:
        ...
        total_distance = 0.0
        layer_weights = [0.1, 0.1, 0.2, 0.3, 0.3]
    ...
    @property
    def _max_pool(self):
        return self.__max_pool
    @_max_pool.setter
    def _max_pool(self, value):
        self.__max_pool = value
    @property
    def _initialize_filters(self):
        return self.__initialize_filters
    @_initialize_filters.setter
    def _initialize_filters(self, value):
        self.__initialize_filters = value
    @property
    def layer_configs(self):
        return self._layer_configs
    @layer_configs.setter
    def layer_configs(self, value):
        self._layer_configs = value
    @property
    def type(self):
        return self._type
    @type.setter
    ...
    @property
    def _convolve(self):
        return self.__convolve
    @_convolve.setter
    def _convolve(self, value):
        self.__convolve = value
    @property
    def _extract_features(self):
        return self.__extract_features
    @_extract_features.setter
...
cdef class ResNetDistance(Distance):
    # Attributs de classe
    cdef object __apply_relu
    cdef dict _filters
    cdef object __initialize_filters
    cdef object __create_basic_filters
    cdef object __batch_norm
    cdef object __apply_residual_block
    cdef object _n_blocks
    cdef object _block_configs
    cdef object _type
    cdef object __convolve
    cdef object __add_features
    cdef object __extract_features
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._block_configs = [(64, 1), (128, 2), (256, 2), (512, 2)][:n_blocks]
        self._filters = self._initialize_filters()
    ...
    def _initialize_filters(self) -> Dict[(str, List[List[List[List[float]]]])]:
        ...
        filters = {}
        current_channels = 3
        for (block_idx, (channels, _)) in enumerate(self._block_configs):
            filters[f'block{block_idx}_conv1'] = self._create_basic_filters(current_channels, channels, 3)
            filters[f'block{block_idx}_conv2'] = self._create_basic_filters(channels, channels, 3)
            if (current_channels != channels):
                filters[f'block{block_idx}_shortcut'] = self._create_basic_filters(current_channels, channels, 1)
            current_channels = channels
        return filters

    def _create_basic_filters(self, in_channels: int, out_channels: int, kernel_size: int) -> List[List[List[List[float]]]]:
        '
        ...
        for _ in range(out_channels):
            filter_weights = [[[([0.0] * in_channels) for _ in range(kernel_size)] for _ in range(kernel_size)] for _ in range(1)]
    ...
    def _batch_norm(self, features: List[List[List[float]]], epsilon: float=1e-05) -> List[List[List[float]]]:
        '
        ...
        for c in range(channels):
            values = [features[h][w][c] for h in range(height) for w in range(width)]
            mean = statistics.mean(values)
            var = (statistics.variance(values) if (len(values) > 1) else 0)
            channel_stats.append((mean, var))
        normalized = [[([0.0] * channels) for _ in range(width)] for _ in range(height)]
        for h in range(height):
            for w in range(width):
                for c in range(channels):
                    (mean, var) = channel_stats[c]
                    normalized[h][w][c] = ((features[h][w][c] - mean) / sqrt((var + epsilon)))
        return normalized

    def _apply_residual_block(self, input_data: List[List[List[float]]], block_idx: int, reduction: int) -> List[List[List[float]]]:
        '
        ...
        Returns:
            List[List[List[float]]]: Output feature maps
        '
        conv1 = self._convolve(input_data, self._filters[f'block{block_idx}_conv1'], stride=reduction)
        bn1 = self._batch_norm(conv1)
        relu1 = self._apply_relu(bn1)
        conv2 = self._convolve(relu1, self._filters[f'block{block_idx}_conv2'], stride=1)
        bn2 = self._batch_norm(conv2)
        if ((reduction > 1) or (f'block{block_idx}_shortcut' in self._filters)):
            shortcut = self._convolve(input_data, self._filters[f'block{block_idx}_shortcut'], stride=reduction)
    ...
    def _convolve(self, input_data: List[List[List[float]]], filter_weights: List[List[List[List[float]]]], stride: int=1) -> List[List[List[float]]]:
        '
        ...
        output = [[([0.0] * out_channels) for _ in range(out_width)] for _ in range(out_height)]
        for h in range(out_height):
            for w in range(out_width):
                    ...
                    for i in range(kernel_size):
                        for j in range(kernel_size):
                            for ic in range(in_channels):
                                h_in = ((h * stride) + i)
                                w_in = ((w * stride) + j)
                                value += (input_data[h_in][w_in][ic] * filter_weights[oc][0][i][j][ic])
                    output[h][w][oc] = value
        return output

    def _apply_relu(self, features: List[List[List[float]]]) -> List[List[List[float]]]:
        '
        ...
        '
        return [[[max(0.0, val) for val in row] for row in layer] for layer in features]

    def _add_features(self, features1: List[List[List[float]]], features2: List[List[List[float]]]) -> List[List[List[float]]]:
        '
        ...
        return [[[(features1[h][w][c] + features2[h][w][c]) for c in range(channels)] for w in range(width)] for h in range(height)]
    ...
    def _extract_features(self, image: List[List[List[float]]]) -> List[List[List[List[float]]]]:
        ...
        features = []
        current_input = image
        for (block_idx, (_, reduction)) in enumerate(self._block_configs):
            block_output = self._apply_residual_block(current_input, block_idx, reduction)
    ...
    def compute(self, image1: List[List[List[float]]], image2: List[List[List[float]]]) -> float:
        ...
        features2 = self._extract_features(image2)
        total_distance = 0.0
        block_weights = [0.1, 0.2, 0.3, 0.4][:self._n_blocks]
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: Description of the calculator
        '
        return f'ResNet-Based Image Distance Calculator (n_blocks={self._n_blocks})'


    # Propriétés pour accéder aux attributs depuis Python
    @property
    def _apply_relu(self):
        return self.__apply_relu
    @_apply_relu.setter
    def _apply_relu(self, value):
        self.__apply_relu = value
    @property
    def filters(self):
        return self._filters
    @filters.setter
    ...
    def _create_basic_filters(self):
        return self.__create_basic_filters
    @_create_basic_filters.setter
    def _create_basic_filters(self, value):
        self.__create_basic_filters = value
    @property
    def _batch_norm(self):
        return self.__batch_norm
    @_batch_norm.setter
    def _batch_norm(self, value):
        self.__batch_norm = value
    @property
    def _apply_residual_block(self):
        return self.__apply_residual_block
    @_apply_residual_block.setter
    def _apply_residual_block(self, value):
        self.__apply_residual_block = value
    @property
    def n_blocks(self):
    ...
    @property
    def block_configs(self):
        return self._block_configs
    @block_configs.setter
    def block_configs(self, value):
        self._block_configs = value
    @property
    def type(self):
        return self._type
    @type.setter
    ...
    def _add_features(self):
        return self.__add_features
    @_add_features.setter
    def _add_features(self, value):
        self.__add_features = value
    @property
    def _extract_features(self):
        return self.__extract_features
    @_extract_features.setter
...
cdef class InceptionDistance(Distance):
    # Attributs de classe
    cdef object __create_filters
    cdef dict _filters
    cdef object __apply_inception_module
    cdef object __initialize_filters
    cdef object _n_modules
    cdef object __max_pool
    cdef object __concatenate_features
    cdef object _type
    cdef object __convolve
    cdef object _module_configs
    cdef object __extract_features
    '
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._filters = self._initialize_filters()
        self._module_configs = [(64, 96, 128, 16, 32, 32), (128, 128, 192, 32, 96, 64), (192, 192, 256, 48, 128, 96)][:n_modules]
    ...
    def _initialize_filters(self) -> Dict[(str, List[List[List[List[float]]]])]:
        ...
        filters = {}
        current_channels = 3
        for (module_idx, config) in enumerate(self._module_configs):
            filters[f'module{module_idx}_1x1'] = self._create_filters(current_channels, config[0], 1)
            filters[f'module{module_idx}_3x3_reduce'] = self._create_filters(current_channels, config[1], 1)
            filters[f'module{module_idx}_3x3'] = self._create_filters(config[1], config[2], 3)
            filters[f'module{module_idx}_5x5_reduce'] = self._create_filters(current_channels, config[3], 1)
            filters[f'module{module_idx}_5x5'] = self._create_filters(config[3], config[4], 5)
            filters[f'module{module_idx}_pool_proj'] = self._create_filters(current_channels, config[5], 1)
            current_channels = (((config[0] + config[2]) + config[4]) + config[5])
        return filters

    def _create_filters(self, in_channels: int, out_channels: int, kernel_size: int) -> List[List[List[List[float]]]]:
        '
        Create basic convolutional filters.
                    ...
                    for i in range(kernel_size):
                        for j in range(kernel_size):
                            for ic in range(in_channels):
                                ...
                                w_in = ((w * stride) + j)
                                value += (input_data[h_in][w_in][ic] * filter_weights[oc][0][i][j][ic])
                    output[h][w][oc] = max(0.0, value)
        return output

    def _max_pool(self, input_data: List[List[List[float]]], pool_size: int=3, stride: int=1) -> List[List[List[float]]]:
        '
        Apply max pooling operation.
        ...
        output = [[([0.0] * channels) for _ in range(out_width)] for _ in range(out_height)]
        for h in range(out_height):
            for w in range(out_width):
                for c in range(channels):
                    max_val = float('-inf')
    ...
    def _concatenate_features(self, feature_maps: List[List[List[List[float]]]]) -> List[List[List[float]]]:
        '
        ...
        output = [[([0.0] * total_channels) for _ in range(width)] for _ in range(height)]
        channel_offset = 0
    ...
    def _apply_inception_module(self, input_data: List[List[List[float]]], module_idx: int) -> List[List[List[float]]]:
        '
        ...
        Returns:
            List[List[List[float]]]: Output feature maps
        '
        path1x1 = self._convolve(input_data, self._filters[f'module{module_idx}_1x1'])
        path3x3 = self._convolve(input_data, self._filters[f'module{module_idx}_3x3_reduce'])
        path3x3 = self._convolve(path3x3, self._filters[f'module{module_idx}_3x3'])
        path5x5 = self._convolve(input_data, self._filters[f'module{module_idx}_5x5_reduce'])
        path5x5 = self._convolve(path5x5, self._filters[f'module{module_idx}_5x5'])
        pool_path = self._max_pool(input_data)
        pool_path = self._convolve(pool_path, self._filters[f'module{module_idx}_pool_proj'])
    ...
    def compute(self, image1: List[List[List[float]]], image2: List[List[List[float]]]) -> float:
        ...
        features2 = self._extract_features(image2)
        total_distance = 0.0
        module_weights = [0.2, 0.3, 0.5][:self._n_modules]
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: Description of the calculator
        '
        return f'Inception-Based Image Distance Calculator (n_modules={self._n_modules})'


    # Propriétés pour accéder aux attributs depuis Python
    @property
    def _create_filters(self):
        return self.__create_filters
    @_create_filters.setter
    def _create_filters(self, value):
        self.__create_filters = value
    @property
    def filters(self):
        return self._filters
    @filters.setter
    ...
    def _apply_inception_module(self):
        return self.__apply_inception_module
    @_apply_inception_module.setter
    def _apply_inception_module(self, value):
        self.__apply_inception_module = value
    @property
    def _initialize_filters(self):
        return self.__initialize_filters
    @_initialize_filters.setter
    ...
    @n_modules.setter
    def n_modules(self, value):
        self._n_modules = value
    @property
    def _max_pool(self):
        return self.__max_pool
    @_max_pool.setter
    ...
    def _concatenate_features(self):
        return self.__concatenate_features
    @_concatenate_features.setter
    def _concatenate_features(self, value):
        self.__concatenate_features = value
    @property
    def type(self):
        return self._type
    @type.setter
    ...
    def module_configs(self, value):
        self._module_configs = value
    @property
    def _extract_features(self):
        return self.__extract_features
    @_extract_features.setter
