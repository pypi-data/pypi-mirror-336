...
class SchemaForge:
    ...
    def generate_model(
        ...
        # Send request
        endpoint = "/api/v1/generate_model"
        url = f"{self.api_base}{endpoint}"
    ...
    def load_generated_model(self, response: ModelGenerationResponse) -> Type[BaseModel]:
        """
        Load a generated model from the response
        ...
        """
        try:
            return load_model_from_code(response.model_code)
        except Exception as e:
            raise SchemaForgeError(f"Failed to load generated model: {str(e)}")
