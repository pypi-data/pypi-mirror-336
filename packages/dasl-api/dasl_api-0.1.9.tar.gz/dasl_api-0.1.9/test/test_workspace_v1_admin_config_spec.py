# coding: utf-8

"""
    Antimatter Security Lakehouse Public API

    Interact with the Antimatter ASL API.

    The version of the OpenAPI document: 0.1.9
    Contact: support@antimatter.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from dasl_api.models.workspace_v1_admin_config_spec import WorkspaceV1AdminConfigSpec

class TestWorkspaceV1AdminConfigSpec(unittest.TestCase):
    """WorkspaceV1AdminConfigSpec unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> WorkspaceV1AdminConfigSpec:
        """Test WorkspaceV1AdminConfigSpec
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `WorkspaceV1AdminConfigSpec`
        """
        model = WorkspaceV1AdminConfigSpec()
        if include_optional:
            return WorkspaceV1AdminConfigSpec(
                auth = dasl_api.models.workspace_v1_admin_config_spec_auth.workspace_v1_AdminConfigSpec_auth(
                    host = '', 
                    service_principal = dasl_api.models.workspace_v1_admin_config_spec_auth_service_principal.workspace_v1_AdminConfigSpec_auth_servicePrincipal(
                        client_id = '', 
                        secret = '', ), 
                    app_client_id = dasl_api.models.workspace_v1_admin_config_spec_auth_app_client_id.workspace_v1_AdminConfigSpec_auth_appClientId(
                        client_id = '', ), )
            )
        else:
            return WorkspaceV1AdminConfigSpec(
                auth = dasl_api.models.workspace_v1_admin_config_spec_auth.workspace_v1_AdminConfigSpec_auth(
                    host = '', 
                    service_principal = dasl_api.models.workspace_v1_admin_config_spec_auth_service_principal.workspace_v1_AdminConfigSpec_auth_servicePrincipal(
                        client_id = '', 
                        secret = '', ), 
                    app_client_id = dasl_api.models.workspace_v1_admin_config_spec_auth_app_client_id.workspace_v1_AdminConfigSpec_auth_appClientId(
                        client_id = '', ), ),
        )
        """

    def testWorkspaceV1AdminConfigSpec(self):
        """Test WorkspaceV1AdminConfigSpec"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
