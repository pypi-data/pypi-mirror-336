...
export_resolve_topologies()
{
	...
	bp::scope().attr(resolve_topologies_function_name).attr("__doc__") =
			...
			"topological and rotation filename extensions) does not support reading (when filenames specified), "
			"or if *resolved_topologies* or *resolved_topological_sections* is a filename and it is not supported for writing\n"
