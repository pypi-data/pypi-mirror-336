...
GPlatesPropertyValues::GpmlFeatureSnapshotReference::transcribe_construct_data(
		GPlatesScribe::Scribe &scribe,
		GPlatesScribe::ConstructObject<GpmlFeatureSnapshotReference> &gpml_feature_snapshot_reference)
{
	if (scribe.is_saving())
	{
		scribe.save(TRANSCRIBE_SOURCE, gpml_feature_snapshot_reference->get_feature_id(), "feature_id");
		scribe.save(TRANSCRIBE_SOURCE, gpml_feature_snapshot_reference->get_revision_id(), "revision_id");
		scribe.save(TRANSCRIBE_SOURCE, gpml_feature_snapshot_reference->get_value_type(), "value_type");
	}
	else // loading
	{
		...
		GPlatesScribe::LoadRef<GPlatesModel::RevisionId> revision_id_ =
				scribe.load<GPlatesModel::RevisionId>(TRANSCRIBE_SOURCE, "revision_id");
		if (!revision_id_.is_valid())
		{
			return scribe.get_transcribe_result();
		}
		...
		gpml_feature_snapshot_reference.construct_object(feature_id_, revision_id_, value_type_);
	}
...
GPlatesScribe::TranscribeResult
GPlatesPropertyValues::GpmlFeatureSnapshotReference::transcribe(
		GPlatesScribe::Scribe &scribe,
		bool transcribed_construct_data)
{
	if (!transcribed_construct_data)
	{
		if (scribe.is_saving())
		{
			...
			scribe.save(TRANSCRIBE_SOURCE, get_revision_id(), "revision_id");
		}
		else // loading
		{
			GPlatesScribe::LoadRef<GPlatesModel::FeatureId> feature_id_ =
	...
	if (!scribe.transcribe_base<GPlatesModel::PropertyValue, GpmlFeatureSnapshotReference>(TRANSCRIBE_SOURCE))
	{
		return scribe.get_transcribe_result();
	}
