...
from nonebot.log import logger as logges
...
__plugin_meta__ = PluginMetadata(
    name="PicaJm",
    description="可以看pica和jm",
    type="application",
    homepage="https://github.com/CCYellowStar2/nonebot-plugin-PicaJm",
...
proxy = plugin_config.SYSTEM_PROXY
...
#填写pica账号
pica_account = plugin_config.pica_account
pica_password = plugin_config.pica_password
...
async def get_jm_from_id(bookid:str,album_id, ep:int = 1, name = ''):
    ...
    try:
        ...
        logges.info(name)
        for imge in cm:
            logges.info(f"当前正在下载:第{str(imge.index)}张: {imge.filename}")
            try:
                await download_img2(imge.img_url,cm_name,imge.filename,ep,imge.scramble_id)
            except:
                logges.info(f"第{str(imge.index)}张: {imge.filename} 下载失败")
                pass   
        isok=True
    except Exception as e:
    ...
    if not pic_result["code"] == 200:
        ...
        for x in range(1, pages_num+1):
            ...
            for y in range(len(docs)):
                ...
                logges.info(f"当前正在下载:第{x}页第{y}张:{ogname}")
                try:
                    await download_img(url,cm_name,ogname,ep)
                except:
                    logges.info(f"第{x}页第{y}张:{ogname}下载失败")
                    pass   
        isok=True
...
async def download_thumb(url, booktitle,ep:int=1):
    ...
    if not os.path.exists(comic_folder):
        os.mkdir(comic_folder)
    for i in range(3):
        logges.info(f"download_thumb --> {i}")
        try:
            async with aiohttp.ClientSession() as session:
                rs = await session.get(url=url, headers=headers, proxy=proxy, timeout=30)
                ...
                async with aiofiles.open(image_path, "wb") as download:
                    try:
                        ...
                        logges.info("pica资源获取失败:HTTP状态码:"+str(rs.status))
...
def make_zip(source_dir, output_filename):
    ...
    if not plugin_config.zip_ispwd:
        ...
        with pyzipper.AESZipFile(
            ...
            ps=str(plugin_config.zip_password)
            logges.info(ps)            
    ...
    logges.info(f"压缩包创建完成:位于{output_filename}")
...
async def favourite(bot: Bot, ev: Event, msg: Message = CommandArg()):
    if not pica_account or not pica_password:
        await bot.send(ev, "请去config配置文件设置你的哔咔账号密码")
        return    
    ...
    if isinstance(ev, PrivateMessageEvent):
        ...
        try:
            ...
            try:
                ...
                try:
                    await bot.upload_group_file(group_id=int(gid),file=zippath,name=name[:80]+"-"+str(ep)+".zip")      
                    if plugin_config.zip_ispwd:
                        ps = plugin_config.zip_password
...
async def get_pica_random(bot: Bot, ev: Event):
    ...
    if isinstance(ev, PrivateMessageEvent):
        ...
        try:
            ...
            await bot.send_group_forward_msg(group_id=ev.group_id, messages=output2)        
        try:
            await bot.upload_group_file(group_id=int(gid),file=zippath,name=title[:80]+"-1"+".zip")           
        except:
            await bot.send(ev, f"消息已响应, 但上传失败.请查看日志窗口")
    if plugin_config.zip_ispwd:
        ps = plugin_config.zip_password
