...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="""Report the offending libraries from a given project+version in a short format suitable for jira/slack notifications. Note blackduck connection depends on a .restconfig.json file which must be present in the current directory. It's format is: 
...
def cves(project_name,version_name):
    """
    ...
    bom = hub.get_vulnerable_bom_components(
        version, limit="999"
    )  
    df = pd.DataFrame(
        {
