...
""")
    ...
    parser.set_defaults(vulnerability_only=False)
    ...
    try:
        df = stats(args.project_name, args.version_name, args.operational,
                   args.cutoff, headers, tree,args.experimental)
    except NameError as e:
        eprint(e)
        sys.exit(os.EX_DATAERR)
    except Exception as e:
        if str(e) == "'bearerToken'":
            eprint("Invalid Authorization Token. Check contents of .restconfig.json. Try --help for more information")
        traceback.print_exc()      
...
def stats(
    ...
    try:
        # FIXME: refactor this to work nicely with pandas and cutoffs like the typical invocation
        if alternative_enpoint:
            bom = hub.get_vulnerable_bom_components(version, limit="999")#&filter=securityRisk%3Acritical&filter=securityRisk%3Ahigh&filter=securityRisk%3Amedium&filter=securityRisk%3Alow")
            #print(json.dumps(bom))
            df = pd.DataFrame({
                'Component': [], 'CVE': [], 'CVE NAME': [], 'CVE DESCRIPTION': [], 'SCORE': [], 'REMEDIATION': [], 'URL' : [], 'CWEID' : []
            })
            ...
            for component in bom['items']:
                ...
                df.loc[len(df)]=[compName,cve_description,cve_name,cve_description,score,remediation,cve_url,cweId]            
            json_data = json.dumps(df.to_dict('records'))
            print(json_data)
            return

        bom_components = hub.get_version_components(version, limit="999&filter=securityRisk%3Acritical&filter=securityRisk%3Ahigh&filter=securityRisk%3Amedium&filter=securityRisk%3Alow")
    except Exception as e:
        eprint(e)
    ...
    total_df = pd.DataFrame({
        'Project': [], 'Component': [], 'Version': [], 'URL': [],
        headers["critical"]: [],
        headers["high"]: [], headers["medium"]: [], headers["low"]: [],
    })
    ...
    for component in bom_components['items']:
        ...
        if component['componentType'] == 'SUB_PROJECT':
            total_df = pd.concat([total_df,
                                  stats(compName, compVersion, operational,
                                        cutoff, headers=headers,
