import { _ as X, aq as F, ar as Y, as as _, at as H, l as i, c as V, au as q, av as U, af as $, ak as z, ag as P, ae as K, aw as Q, ax as W, ay as Z } from "./mermaid.core-mH_T8-GP.js";
import { G as B } from "./graph-CgN_eKj8.js";
import { l as I } from "./layout-DlZf58hF.js";
import { i as x } from "./Index-XtzQEp5R.js";
import { c as L } from "./clone-BSRH2tc4.js";
import { m as A } from "./_basePickBy-CYTwJ2aT.js";
function p(e) {
  var t = {
    options: {
...
var f = /* @__PURE__ */ new Map(), b = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ X(() => {
  ...
  e !== o && c.push(e), i.warn("Copying (nodes) clusterId", e, "nodes", c), c.forEach((a) => {
    if (t.children(a).length > 0)
      ...
      const r = t.node(a);
      i.info("cp ", a, " to ", o, " with parent ", e), n.setNode(a, r), o !== t.parent(a) && (i.warn("Setting parent", a, t.parent(a)), n.setParent(a, t.parent(a))), e !== o && a !== e ? (i.debug("Setting parent", a, e), n.setParent(a, e)) : (i.info("In copy ", e, "root", o, "data", t.node(e), o), i.debug("Not Setting parent for node=", a, "cluster!==rootId", e !== o, "node!==clusterId", a !== e));
      const u = t.edges(a);
      i.debug("Copying Edges", u), u.forEach((l) => {
        ...
        try {
          se(l, o) ? (i.info("Copying as ", l.v, l.w, h, l.name), n.setEdge(l.v, l.w, h, l.name), i.info("newGraph edges ", n.edges(), n.edge(n.edges()[0]))) : i.info("Skipping copy of edge ", l.v, "-->", l.w, " rootId: ", o, " clusterId:", e);
        } catch (C) {
          i.error(C);
        }
      });
  ...
  const o = e.edges().filter((l) => l.v === t || l.w === t), c = e.edges().filter((l) => l.v === n || l.w === n), a = o.map((l) => ({
    v: l.v === t ? n : l.v,
    w: l.w === t ? t : l.w
  })), r = c.map((l) => ({
    v: l.v,
  ...
  e.nodes().forEach(function(n) {
    e.children(n).length > 0 && (i.warn("Cluster identified", n, " Replacement id in edges: ", D(n, e, n)), b.set(n, R(n, e)), f.set(n, {
      id: D(n, e, n),
  ...
  e.edges().forEach(function(n) {
    ...
    let c = n.v, a = n.w;
    if (i.warn("Fix XXX", f, "ids:", n.v, n.w, "Translating: ", f.get(n.v), " --- ", f.get(n.w)), f.get(n.v) || f.get(n.w)) {
  ...
  i.debug("Nodes = ", n, t);
  for (const r of n)
    if (i.debug("Extracting node", r, f, f.has(r) && !f.get(r).externalConnections, !e.parent(r), e.node(r), e.children("D"), " Depth ", t), !f.has(r))
      ...
      i.warn("Cluster ** ", r, " **not meeting the criteria !externalConnections:", !f.get(r).externalConnections, " no parent: ", !e.parent(r), " children ", e.children(r) && e.children(r).length > 0, e.children("D"), t), i.debug(f);
  ...
  await Promise.all(t.nodes().map(async function(d) {
    ...
    if (c !== void 0) {
      ...
      i.trace(`Setting data for parent cluster XXX
 Node.id = `, d, `
 data=`, w.height, `
Parent cluster`, c.height), t.setNode(c.id, w), t.parent(d) || (i.trace("Setting parent", d, c.id), t.setParent(d, c.id, w));
    }
    if (i.info("(Insert) Node XXX" + d + ": " + JSON.stringify(t.node(d))), s != null && s.clusterNode) {
      ...
      const N = await j(g, s.graph, n, o, t.node(d), a), S = N.elem;
      ...
      t.children(d).length > 0 ? (i.trace("Cluster - the non recursive path XBX", d, s.id, s, s.width, "Graph:", t), i.trace(D(s.id, t)), f.set(s.id, {
        id: D(s.id, t),
        node: s
      })) : (i.trace("Node - the non recursive path XAX", d, g, t.node(d), r), await $(g, t.node(d), {
        config: a,
        dir: r
      }));
    ...
    const d = t.edges().map(async function(s) {
      ...
      i.info("Edge " + s.v + " -> " + s.w + ": " + JSON.stringify(s)), i.info("Edge " + s.v + " -> " + s.w + ": ", s, " ", JSON.stringify(t.edge(s))), i.info("Fix", f, "ids:", s.v, s.w, "Translating: ", f.get(s.v), f.get(s.w)), await Z(C, w);
    });
    await Promise.all(d);
  ...
  let E = 0, {
    subGraphTitleTotalMargin: y
  } = z(a);
  return await Promise.all(oe(t).map(async function(d) {
    var w;
    const s = t.node(d);
    if (i.info("Position XBX => " + d + ": (" + s.x, "," + s.y, ") width: ", s.width, " height: ", s.height), s != null && s.clusterNode)
      s.y += y, i.info("A tainted cluster node XBX1", d, s.id, s.width, s.height, s.x, s.y, t.parent(d)), f.get(s.id).node = s, P(s);
    else if (t.children(d).length > 0) {
      i.info("A pure cluster node XBX1", d, s.id, s.x, s.y, s.width, s.height, t.parent(d)), s.height += y, t.node(s.parentId);
      ...
      s.y += y / 2, i.info("A regular node XBX1 - using the padding", s.id, "parent", s.parentId, s.width, s.height, s.x, s.y, "offsetY", s.offsetY, "parent", m, m == null ? void 0 : m.offsetY, s), P(s);
    }
  ...
  F(o, e.markers, e.type, e.diagramId), Y(), _(), H(), te(), e.nodes.forEach((g) => {
    n.setNode(g.id, {
      ...g
    }), g.parentId && n.setParent(g.id, g.parentId);
