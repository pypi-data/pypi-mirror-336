import { _ as S, l as z, c as at, H as Nt, U as Dt, F as tt, i as et, V as mt, W as Ot, X as At, d as It, aj as Ct, N as Rt } from "./mermaid.core-mH_T8-GP.js";
import { c as pt } from "./cytoscape.esm-BudeXNE2.js";
import { e as ot, g as xt } from "./Index-XtzQEp5R.js";
var dt = {
  exports: {}
}, it = {
  exports: {}
        ...
        function(m, y, r) {
          ...
          l.prototype.run = function() {
            ...
            var _ = function(M, G) {
              ...
              for (var M = function() {
                O.fit && O.cy.fit(O.eles, O.padding), D || (D = !0, N.cy.one("layoutready", O.ready), N.cy.trigger({
                  type: "layoutready",
                  layout: N
                }));
              ...
              if (F) {
                d.checkLayoutSuccess() && !d.isSubLayout && d.doPostLayout(), d.tilingPostLayout && d.tilingPostLayout(), d.isLayoutFinished = !0, N.options.eles.nodes().positions(_), M(), N.cy.one("layoutstop", N.options.stop), N.cy.trigger({
                  type: "layoutstop",
                  layout: N
                }), L && cancelAnimationFrame(L), D = !1;
                return;
              }
...
var st = function() {
  var I = /* @__PURE__ */ S(function(O, d, N, s) {
    ...
    performAction: /* @__PURE__ */ S(function(d, N, s, h, u, p, A) {
      ...
      switch (u) {
        ...
        case 15:
          ...
          break;
        case 16:
          h.getLogger().trace("Icon: ", p[C]), h.decorateNode({
            icon: p[C]
          });
          break;
        case 17:
        ...
        case 20:
          ...
          break;
        case 25:
          h.getLogger().trace("node found ..", p[C - 2]), this.$ = {
            id: p[C - 1],
            descr: p[C - 1],
            type: h.getType(p[C - 2], p[C])
          };
          break;
        case 26:
          this.$ = {
            id: p[C],
            descr: p[C],
            type: h.nodeType.DEFAULT
          };
          break;
        case 27:
          h.getLogger().trace("node found ..", p[C - 3]), this.$ = {
            id: p[C - 3],
            descr: p[C - 1],
            type: h.getType(p[C - 2], p[C])
          };
    ...
    table: [{
      ...
      19: t,
      22: e
    }), I(v, [2, 18]), I(v, [2, 19]), I(v, [2, 20]), I(v, [2, 21]), I(v, [2, 23]), I(v, [2, 24]), I(v, [2, 26], {
      19: [1, 30]
    }), {
      20: [1, 31]
    }, {
      ...
      7: c,
      11: l
    }), I(E, [2, 8]), I(E, [2, 9]), I(E, [2, 10]), I(v, [2, 15]), I(v, [2, 16]), I(v, [2, 17]), {
      20: [1, 35]
    }, {
      21: [1, 36]
    }, I(n, [2, 13], {
    ...
    parse: /* @__PURE__ */ S(function(d) {
      var N = this, s = [0], h = [], u = [null], p = [], A = this.table, C = "", R = 0, x = 0, _ = 2, U = 1, X = p.slice.call(arguments, 1), M = Object.create(this.lexer), G = {
        yy: {}
      };
...
Expecting ` + q.join(", ") + ", got '" + (this.terminals_[P] || P) + "'" : rt = "Parse error on line " + (R + 1) + ": Unexpected " + (P == U ? "end of input" : "'" + (this.terminals_[P] || P) + "'"), this.parseError(rt, {
        ...
        switch ($[0]) {
          ...
          case 2:
            ...
            }, Y && (Q._$.range = [p[p.length - (j || 1)].range[0], p[p.length - 1].range[1]]), K = this.performAction.apply(Q, [C, x, R, G.yy, $[1], u, p].concat(X)), typeof K < "u")
              return K;
...
var _t = st, Z = [], vt = 0, ht = {}, Ft = /* @__PURE__ */ S(() => {
  ...
  if (e)
    ...
    throw new Error('There can be only one root. No parent could be found for ("' + t.descr + '")');
}, "addNode"), W = {
  DEFAULT: 0,
  NO_BORDER: 0,
  ROUNDED_RECT: 1,
  ...
  w.append("path").attr("id", "node-" + f.id).attr("class", "node-bkg node-" + I.type2Str(f.type)).attr("d", `M0 ${f.height - 5} v${-f.height + 2 * 5} q0,-5 5,-5 h${f.width - 2 * 5} q5,0 5,5 v${f.height - 5} H0 Z`), w.append("line").attr("class", "node-line-" + m).attr("x1", 0).attr("y1", f.height).attr("x2", f.width).attr("y2", f.height);
  ...
  w.append("path").attr("id", "node-" + f.id).attr("class", "node-bkg node-" + I.type2Str(f.type)).attr("d", `M0 0 a${r},${r} 0 0,1 ${m * 0.25},${-1 * m * 0.1}
  ...
  w.append("path").attr("id", "node-" + f.id).attr("class", "node-bkg node-" + I.type2Str(f.type)).attr("d", `M0 0 a${r},${r} 1 0,0 ${m * 0.25},${-1 * y * 0.1}
...
function yt(I, w, f, m, y) {
  return I.insert("polygon", ":first-child").attr("points", m.map(function(r) {
    return r.x + "," + r.y;
  }).join(" ")).attr("transform", "translate(" + (y.width - w) / 2 + ", " + f + ")");
}
...
`);
  ...
  }, y), r || g.attr("dy", "1em").attr("alignment-baseline", "middle").attr("dominant-baseline", "middle").attr("text-anchor", "middle");
  ...
  if (f.height = v.height + n * 1.1 * 0.5 + f.padding, f.width = v.width + 2 * f.padding, f.icon)
    if (f.type === I.nodeType.CIRCLE)
      f.height += 50, f.width += 50, e.append("foreignObject").attr("height", "50px").attr("width", f.width).attr("style", "text-align: center;").append("div").attr("class", "icon-container").append("i").attr("class", "node-icon-" + t + " " + f.icon), g.attr("transform", "translate(" + f.width / 2 + ", " + (f.height / 2 - 1.5 * f.padding) + ")");
    else {
      f.width += 50;
      const c = f.height;
      f.height = Math.max(c, 60);
      const l = Math.abs(f.height - c);
      e.append("foreignObject").attr("width", "60px").attr("height", f.height).attr("style", "text-align: center;margin-top:" + l / 2 + "px;").append("div").attr("class", "icon-container").append("i").attr("class", "node-icon-" + t + " " + f.icon), g.attr("transform", "translate(" + (25 + f.width / 2) + ", " + (l / 2 + f.padding / 2) + ")");
    }
  else if (r) {
...
async function lt(I, w, f, m, y) {
  await ee(I, w, f, m, y), f.children && await Promise.all(f.children.map((r, t) => lt(I, w, r, m < 0 ? t : m, y)));
}
S(lt, "drawNodes");
function Et(I, w) {
  w.edges().map((f, m) => {
    ...
    if (f[0]._private.bodyBounds) {
      ...
      z.trace("Edge: ", m, y), I.insert("path").attr("d", `M ${r.startX},${r.startY} L ${r.midX},${r.midY} L${r.endX},${r.endY} `).attr("class", "edge section-edge-" + y.section + " edge-depth-" + y.depth);
    }
  });
}
S(Et, "drawEdges");
...
function Tt(I, w) {
  w.nodes().map((f, m) => {
    ...
    z.info("Id:", m, "Position: (", f.position().x, ", ", f.position().y, ")", y), r.attr("transform", `translate(${f.position().x - y.width / 2}, ${f.position().y - y.height / 2})`), r.attr("attr", `apa-${m})`);
  });
}
S(Tt, "positionNodes");
...
` + I);
  ...
  Et(i, g), Tt(y, g), Dt(void 0, e, ((a = t.mindmap) == null ? void 0 : a.padding) ?? tt.mindmap.padding, ((v = t.mindmap) == null ? void 0 : v.useMaxWidth) ?? tt.mindmap.useMaxWidth);
}, "draw"), ne = {
  draw: ie
}, se = /* @__PURE__ */ S((I) => {
  let w = "";
