import { s as W, a as P, S as N } from "./chunk-4IRHCMPZ-YQLWmVbz.js";
import { _ as u, c as t, d as H, l as S, e as C, k as z, Q as U, R as F, u as O } from "./mermaid.core-mH_T8-GP.js";
import { G as J } from "./graph-CgN_eKj8.js";
import { l as X } from "./layout-DlZf58hF.js";
var L = {}, D = /* @__PURE__ */ u((e, i) => {
...
}, "addTitleAndBox"), V = /* @__PURE__ */ u((e) => (e.append("circle").attr("class", "end-state-outer").attr("r", t().state.sizeUnit + t().state.miniPadding).attr("cx", t().state.padding + t().state.sizeUnit + t().state.miniPadding).attr("cy", t().state.padding + t().state.sizeUnit + t().state.miniPadding), e.append("circle").attr("class", "end-state-inner").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit + 2).attr("cy", t().state.padding + t().state.sizeUnit + 2)), "drawEndState"), tt = /* @__PURE__ */ u((e, i) => {
  ...
  const d = i.append("rect").attr("x", 0).attr("y", t().state.padding), o = i.append("g"), {
    textWidth: n,
    textHeight: g
  } = et(e, 0, 0, o);
  ...
  let a = "";
  if (t().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), x.attr("marker-end", "url(" + a + "#" + o(N.relationType.DEPENDENCY) + "End)"), d.title !== void 0) {
    const s = e.append("g").attr("class", "stateLabel"), {
      x: y,
      y: p
    } = O.calcLabelPosition(i.points), c = z.getRows(d.title);
    let l = 0;
    const B = [];
    let m = 0, k = 0;
  ...
  C(s, B, m, b.useMaxWidth), s.attr("viewBox", `${c.x - b.padding}  ${c.y - b.padding} ` + l + " " + B);
  ...
  for (const r of l) {
    ...
    if (f.doc) {
      ...
      h = G(f.doc, w, f.id, !o, n, g, x);
      {
        ...
        h.width = v.width, h.height = v.height + b.padding / 2, T[f.id] = {
  ...
  c.forEach(function(r) {
    B++, S.debug("Setting edge", r), a.setEdge(r.id1, r.id2, {
      relation: r,
      width: dt(r.title),
  ...
  a.nodes().forEach(function(r) {
    r !== void 0 && a.node(r) !== void 0 ? (S.warn("Node " + r + ": " + JSON.stringify(a.node(r))), n.select("#" + m.id + " #" + r).attr("transform", "translate(" + (a.node(r).x - a.node(r).width / 2) + "," + (a.node(r).y + (T[r] ? T[r].y : 0) - a.node(r).height / 2) + " )"), n.select("#" + m.id + " #" + r).attr("data-x-shift", a.node(r).x - a.node(r).width / 2), g.querySelectorAll("#" + m.id + " #" + r + " .divider").forEach((h) => {
