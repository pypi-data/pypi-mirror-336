import { p as Z } from "./chunk-IUKPXING-e0t97HZe.js";
import { I as F } from "./chunk-66XRIAFR-CHVC94M8.js";
import { _ as h, q as U, p as rr, s as er, g as tr, a as ar, b as nr, l as w, c as sr, d as or, u as cr, C as ir, y as dr, k as B, D as hr, E as lr, F as $r, G as fr } from "./mermaid.core-mH_T8-GP.js";
import { p as gr } from "./gitGraph-YCYPL57B-Ct4_k1FH.js";
var x = {
  NORMAL: 0,
  REVERSE: 1,
  HIGHLIGHT: 2,
...
var ur = /* @__PURE__ */ h(function(t) {
  ...
  if (a && i.records.commits.has(a)) {
    const d = new Error('Incorrect usage of "merge". Commit with id:' + a + " already exists, use different custom Id");
    throw d.hash = {
  ...
  if (!a || !i.records.commits.has(a)) {
    ...
    const f = {
      ...
      branch: i.records.currBranch,
      type: x.CHERRY_PICK,
      tags: s ? s.filter(Boolean) : [`cherry-pick:${c.id}${c.type === x.MERGE ? `|parent:${e}` : ""}`]
    };
...
var Cr = /* @__PURE__ */ h(function() {
  ...
  }).sort((r, a) => (r.order ?? 0) - (a.order ?? 0)).map(({
    name: r
  }) => ({
    name: r
  }));
  ...
  if (n === x.HIGHLIGHT)
    t.append("rect").attr("x", a.x - 10).attr("y", a.y - 10).attr("width", 20).attr("height", 20).attr("class", `commit ${r.id} commit-highlight${e % O} ${s}-outer`), t.append("rect").attr("x", a.x - 6).attr("y", a.y - 6).attr("width", 12).attr("height", 12).attr("class", `commit ${r.id} commit${e % O} ${s}-inner`);
    ...
    n === x.REVERSE && t.append("path").attr("d", `M ${a.x - 5},${a.y - 5}L${a.x + 5},${a.y + 5}M${a.x - 5},${a.y + 5}L${a.x + 5},${a.y - 5}`).attr("class", `commit ${s} ${r.id} commit${e % O}`);
  }
  ...
  if (r.type !== x.CHERRY_PICK && (r.customId && r.type === x.MERGE || r.type !== x.MERGE) && (b != null && b.showCommitLabel)) {
    ...
    if ($ && (c.attr("x", a.posWithOffset - $.width / 2 - L).attr("y", a.y + 13.5).attr("width", $.width + 2 * L).attr("height", $.height + 2 * L), u === "TB" || u === "BT" ? (c.attr("x", a.x - ($.width + 4 * k + 5)).attr("y", a.y - 12), o.attr("x", a.x - ($.width + 4 * k)).attr("y", a.y + $.height - 12)) : o.attr("x", a.posWithOffset - $.width / 2), b.rotateCommitLabel))
      if (u === "TB" || u === "BT")
        o.attr("transform", "rotate(-45, " + a.x + ", " + a.y + ")"), c.attr("transform", "rotate(-45, " + a.x + ", " + a.y + ")");
      else {
  ...
  if (r.tags.length > 0) {
    ...
    for (const {
      ...
      ${a.posWithOffset + c / 2 + k},${p - y - L}
      ${a.posWithOffset + c / 2 + k},${p + y + L}
      ${a.posWithOffset - c / 2 - k},${p + y + L}`), f.attr("cy", p).attr("cx", s - c / 2 + k / 2).attr("r", 1.5).attr("class", "tag-hole"), u === "TB" || u === "BT") {
        const m = s + d;
        ...
        ${a.x + R + c + 4},${m - y - 2}
        ${a.x + R + c + 4},${m + y + 2}
        ${a.x + R},${m + y + 2}`).attr("transform", "translate(12,12) rotate(45, " + a.x + "," + s + ")"), f.attr("cx", a.x + k / 2).attr("cy", m).attr("transform", "translate(12,12) rotate(45, " + a.x + "," + s + ")"), l.attr("x", a.x + 5).attr("y", m + 3).attr("transform", "translate(14,14) rotate(45, " + a.x + "," + s + ")");
      }
    }
  }
  ...
  u === "BT" && (o && Ar(l, r, n), l = l.reverse()), l.forEach((f) => {
    ...
    u === "TB" || u === "BT" ? E.set(g.id, {
      x: d.x,
      y: d.posWithOffset
    }) : E.set(g.id, {
      x: d.posWithOffset,
      y: d.y
    }), n = u === "BT" && o ? n + I : n + I + R, n > M && (M = n);
  });
  ...
  r.forEach((s, e) => {
    ...
    const y = l.getBBox();
    f.attr("class", "branchLabelBkg label" + n).attr("rx", 4).attr("ry", 4).attr("x", -y.width - 4 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)).attr("y", -y.height / 2 + 8).attr("width", y.width + 18).attr("height", y.height + 4), d.attr("transform", "translate(" + (-y.width - 14 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)) + ", " + (c - y.height / 2 - 1) + ")"), u === "TB" ? (f.attr("x", c - y.width / 2 - 10).attr("y", 0), d.attr("transform", "translate(" + (c - y.width / 2 - 5) + ", 0)")) : u === "BT" ? (f.attr("x", c - y.width / 2 - 10).attr("y", M), d.attr("transform", "translate(" + (c - y.width / 2 - 5) + ", " + M + ")")) : f.attr("transform", "translate(-19, " + (c - y.height / 2) + ")");
  });
}, "drawBranches"), te = /* @__PURE__ */ h(function(t, r, a, s, e) {
  return C.set(t, {
    pos: r,
    index: a
  }), r += 50 + (e ? 40 : 0) + (u === "TB" || u === "BT" ? s.width / 2 : 0), r;
...
`, "id:", r, a), !b)
  ...
  }), K(o, G, !1), b.showBranches && ee(o, c), re(o, G), K(o, G, !0), cr.insertTitle(o, "gitTitleText", b.titleTopMargin ?? 0, n.getDiagramTitle()), ir(void 0, o, b.diagramPadding, b.useMaxWidth);
}, "draw"), ne = {
  draw: ae
}, se = /* @__PURE__ */ h((t) => `
  .commit-id,
