...
def not_equal(left, right) -> Boolean:
    """
    Returns True if the left argument is not equal to the right argument,
    False otherwise. You may also use the `!=` operator to compare two
    values, though the result will be a Python type.

    [Specification](http://cql.hl7.org/09-b-cqlreference.html#not-equal)
    ...
    ```python
    not_equal(Integer(1), Integer(2))  # True
    not_equal(Integer(1), Integer(1))  # False
    ```
    """
