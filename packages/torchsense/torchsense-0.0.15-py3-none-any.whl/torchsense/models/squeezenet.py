...
class Fire(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, squzee_channel):
        ...
        self.squeeze = nn.Sequential(
            nn.Conv1d(in_channel, squzee_channel, 1),
            nn.BatchNorm1d(squzee_channel),
            nn.ReLU(inplace=True)
        )

        self.expand_1x1 = nn.Sequential(
            nn.Conv1d(squzee_channel, int(out_channel / 2), 1),
            nn.BatchNorm1d(int(out_channel / 2)),
            nn.ReLU(inplace=True)
        )

        self.expand_3x3 = nn.Sequential(
            nn.Conv1d(squzee_channel, int(out_channel / 2), 3, padding=1),
            nn.BatchNorm1d(int(out_channel / 2)),
            nn.ReLU(inplace=True)
        )
...
class SqueezeNet(nn.Module):
    ...
    def __init__(self, class_num=2):
        ...
        self.conv10 = nn.Conv1d(512, class_num, 1)
