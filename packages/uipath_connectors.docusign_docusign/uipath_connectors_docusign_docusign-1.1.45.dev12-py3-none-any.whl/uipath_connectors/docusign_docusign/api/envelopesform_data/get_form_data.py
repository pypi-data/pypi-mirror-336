from http import HTTPStatus
from typing import Any, Optional, Union

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response
from ... import errors

from ...models.default_error import DefaultError
from ...models.get_form_data_response import GetFormDataResponse


def _get_kwargs(
    envelope_id: str,
    *,
    fields: Optional[str] = None,
    order_by: Optional[str] = None,
    page_size: Optional[int] = None,
    next_page: Optional[str] = None,
) -> dict[str, Any]:
    params: dict[str, Any] = {}

    params["fields"] = fields

    params["orderBy"] = order_by

    params["pageSize"] = page_size

    params["nextPage"] = next_page

    params = {k: v for k, v in params.items() if v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/envelopes/{envelope_id}/form_data".format(
            envelope_id=envelope_id,
        ),
        "params": params,
    }

    return _kwargs


def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Optional[Union[DefaultError, GetFormDataResponse]]:
    if response.status_code == 200:
        response_200 = GetFormDataResponse.from_dict(response.json())

        return response_200
    if response.status_code == 400:
        response_400 = DefaultError.from_dict(response.json())

        return response_400
    if response.status_code == 401:
        response_401 = DefaultError.from_dict(response.json())

        return response_401
    if response.status_code == 403:
        response_403 = DefaultError.from_dict(response.json())

        return response_403
    if response.status_code == 404:
        response_404 = DefaultError.from_dict(response.json())

        return response_404
    if response.status_code == 405:
        response_405 = DefaultError.from_dict(response.json())

        return response_405
    if response.status_code == 406:
        response_406 = DefaultError.from_dict(response.json())

        return response_406
    if response.status_code == 409:
        response_409 = DefaultError.from_dict(response.json())

        return response_409
    if response.status_code == 415:
        response_415 = DefaultError.from_dict(response.json())

        return response_415
    if response.status_code == 500:
        response_500 = DefaultError.from_dict(response.json())

        return response_500
    if response.status_code == 402:
        response_402 = DefaultError.from_dict(response.json())

        return response_402
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[DefaultError, GetFormDataResponse]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    envelope_id_lookup: Any,
    envelope_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    fields: Optional[str] = None,
    order_by: Optional[str] = None,
    page_size: Optional[int] = None,
    next_page: Optional[str] = None,
) -> Response[Union[DefaultError, GetFormDataResponse]]:
    r"""Get Form Data

     Retrieve the envelope and tab data (also called form data) from any in-process, completed, or
    canceled envelope. Recipients who are also full administrators on an account can view form data for
    any envelopes that another user on the account has sent to them. To use this activity, the Sending
    Setting \"Allow sender to download form data\" must be enabled on the account.

    Args:
        envelope_id (str): Type the name or ID of the envelope. If the envelope is not found in
            the drop-down, you can first scroll the drop-down till the bottom to get all the available
            envelopes and then type the envelope or retrieve the envelope ID from the DocuSign
            application -> "Manage" tab -> Goto “Sent” or “Draft” under “Envelopes” -> Click on the
            respective envelope and retrieve the envelope ID from URL. For example, if the URL is
            “https://appdemo.docusign.com/documents/details/3ae67e54-f761-4a5b-a23e-b5e4835492cc”, the
            envelope ID is “3ae67e54-f761-4a5b-a23e-b5e4835492cc”
        fields (Optional[str]): Specify the fields to return in the response payload. Defaults to
            all standard fields
        order_by (Optional[str]): The field(s) to order by. For example to order by firstname
            ascending use &#39;firstname asc&#39;
        page_size (Optional[int]): The number of resources to return in a given page
        next_page (Optional[str]): The next page cursor, taken from the response header:
            `elements-next-page-token`

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[DefaultError, GetFormDataResponse]]
    """

    if not envelope_id and envelope_id_lookup:
        lookup_response_raw = client.get_httpx_client().request(
            method="get",
            url="/envelopes_dtl?from_date=2000-01-01&status=created,sent,completed",
        )
        lookup_response = lookup_response_raw.json()

        found_items = []
        for item in lookup_response:
            if (
                envelope_id_lookup in item["emailSubject"]
                or envelope_id_lookup in item["envelopeId"]
            ):
                found_items.append(item)

        if not found_items:
            raise ValueError(
                "No matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created,sent,completed"
            )
        if len(found_items) > 1:
            print(
                "Warning: Multiple matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created,sent,completed. Using the first match."
            )

        envelope_id = found_items[0]["envelopeId"]

    kwargs = _get_kwargs(
        envelope_id=envelope_id,
        fields=fields,
        order_by=order_by,
        page_size=page_size,
        next_page=next_page,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    envelope_id_lookup: Any,
    envelope_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    fields: Optional[str] = None,
    order_by: Optional[str] = None,
    page_size: Optional[int] = None,
    next_page: Optional[str] = None,
) -> Optional[Union[DefaultError, GetFormDataResponse]]:
    r"""Get Form Data

     Retrieve the envelope and tab data (also called form data) from any in-process, completed, or
    canceled envelope. Recipients who are also full administrators on an account can view form data for
    any envelopes that another user on the account has sent to them. To use this activity, the Sending
    Setting \"Allow sender to download form data\" must be enabled on the account.

    Args:
        envelope_id (str): Type the name or ID of the envelope. If the envelope is not found in
            the drop-down, you can first scroll the drop-down till the bottom to get all the available
            envelopes and then type the envelope or retrieve the envelope ID from the DocuSign
            application -> "Manage" tab -> Goto “Sent” or “Draft” under “Envelopes” -> Click on the
            respective envelope and retrieve the envelope ID from URL. For example, if the URL is
            “https://appdemo.docusign.com/documents/details/3ae67e54-f761-4a5b-a23e-b5e4835492cc”, the
            envelope ID is “3ae67e54-f761-4a5b-a23e-b5e4835492cc”
        fields (Optional[str]): Specify the fields to return in the response payload. Defaults to
            all standard fields
        order_by (Optional[str]): The field(s) to order by. For example to order by firstname
            ascending use &#39;firstname asc&#39;
        page_size (Optional[int]): The number of resources to return in a given page
        next_page (Optional[str]): The next page cursor, taken from the response header:
            `elements-next-page-token`

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[DefaultError, GetFormDataResponse]
    """

    return sync_detailed(
        envelope_id=envelope_id,
        envelope_id_lookup=envelope_id_lookup,
        client=client,
        fields=fields,
        order_by=order_by,
        page_size=page_size,
        next_page=next_page,
    ).parsed


async def asyncio_detailed(
    envelope_id_lookup: Any,
    envelope_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    fields: Optional[str] = None,
    order_by: Optional[str] = None,
    page_size: Optional[int] = None,
    next_page: Optional[str] = None,
) -> Response[Union[DefaultError, GetFormDataResponse]]:
    r"""Get Form Data

     Retrieve the envelope and tab data (also called form data) from any in-process, completed, or
    canceled envelope. Recipients who are also full administrators on an account can view form data for
    any envelopes that another user on the account has sent to them. To use this activity, the Sending
    Setting \"Allow sender to download form data\" must be enabled on the account.

    Args:
        envelope_id (str): Type the name or ID of the envelope. If the envelope is not found in
            the drop-down, you can first scroll the drop-down till the bottom to get all the available
            envelopes and then type the envelope or retrieve the envelope ID from the DocuSign
            application -> "Manage" tab -> Goto “Sent” or “Draft” under “Envelopes” -> Click on the
            respective envelope and retrieve the envelope ID from URL. For example, if the URL is
            “https://appdemo.docusign.com/documents/details/3ae67e54-f761-4a5b-a23e-b5e4835492cc”, the
            envelope ID is “3ae67e54-f761-4a5b-a23e-b5e4835492cc”
        fields (Optional[str]): Specify the fields to return in the response payload. Defaults to
            all standard fields
        order_by (Optional[str]): The field(s) to order by. For example to order by firstname
            ascending use &#39;firstname asc&#39;
        page_size (Optional[int]): The number of resources to return in a given page
        next_page (Optional[str]): The next page cursor, taken from the response header:
            `elements-next-page-token`

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[DefaultError, GetFormDataResponse]]
    """

    if not envelope_id and envelope_id_lookup:
        lookup_response_raw = await client.get_async_httpx_client().request(
            method="get",
            url="/envelopes_dtl?from_date=2000-01-01&status=created,sent,completed",
        )
        lookup_response = lookup_response_raw.json()

        found_items = []
        for item in lookup_response:
            if (
                envelope_id_lookup in item["emailSubject"]
                or envelope_id_lookup in item["envelopeId"]
            ):
                found_items.append(item)

        if not found_items:
            raise ValueError(
                "No matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created,sent,completed"
            )
        if len(found_items) > 1:
            print(
                "Warning: Multiple matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created,sent,completed. Using the first match."
            )

        envelope_id = found_items[0]["envelopeId"]

    kwargs = _get_kwargs(
        envelope_id=envelope_id,
        fields=fields,
        order_by=order_by,
        page_size=page_size,
        next_page=next_page,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    envelope_id_lookup: Any,
    envelope_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
    fields: Optional[str] = None,
    order_by: Optional[str] = None,
    page_size: Optional[int] = None,
    next_page: Optional[str] = None,
) -> Optional[Union[DefaultError, GetFormDataResponse]]:
    r"""Get Form Data

     Retrieve the envelope and tab data (also called form data) from any in-process, completed, or
    canceled envelope. Recipients who are also full administrators on an account can view form data for
    any envelopes that another user on the account has sent to them. To use this activity, the Sending
    Setting \"Allow sender to download form data\" must be enabled on the account.

    Args:
        envelope_id (str): Type the name or ID of the envelope. If the envelope is not found in
            the drop-down, you can first scroll the drop-down till the bottom to get all the available
            envelopes and then type the envelope or retrieve the envelope ID from the DocuSign
            application -> "Manage" tab -> Goto “Sent” or “Draft” under “Envelopes” -> Click on the
            respective envelope and retrieve the envelope ID from URL. For example, if the URL is
            “https://appdemo.docusign.com/documents/details/3ae67e54-f761-4a5b-a23e-b5e4835492cc”, the
            envelope ID is “3ae67e54-f761-4a5b-a23e-b5e4835492cc”
        fields (Optional[str]): Specify the fields to return in the response payload. Defaults to
            all standard fields
        order_by (Optional[str]): The field(s) to order by. For example to order by firstname
            ascending use &#39;firstname asc&#39;
        page_size (Optional[int]): The number of resources to return in a given page
        next_page (Optional[str]): The next page cursor, taken from the response header:
            `elements-next-page-token`

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[DefaultError, GetFormDataResponse]
    """

    return (
        await asyncio_detailed(
            envelope_id=envelope_id,
            envelope_id_lookup=envelope_id_lookup,
            client=client,
            fields=fields,
            order_by=order_by,
            page_size=page_size,
            next_page=next_page,
        )
    ).parsed
