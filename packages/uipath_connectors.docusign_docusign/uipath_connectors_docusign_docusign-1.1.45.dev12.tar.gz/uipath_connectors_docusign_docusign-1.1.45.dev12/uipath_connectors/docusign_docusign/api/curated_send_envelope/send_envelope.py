...
def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = SendEnvelopeResponse.from_dict(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[DefaultError, SendEnvelopeResponse]]:
    return Response(
    ...
    *,
    ...
    Args:
        envelope_id (str): Type the name or ID of the envelope. If the envelope is not found in
            ...
            application -> "Manage" tab -> Goto “Draft” under “Envelopes” -> Click on the respective
            envelope and retrieve the envelope ID from URL. For example, if the URL is
    ...
    if not envelope_id and envelope_id_lookup:
        lookup_response_raw = client.get_httpx_client().request(
            method="get", url="/envelopes_dtl?from_date=2000-01-01&status=created"
        )
        ...
        if not found_items:
            raise ValueError(
                "No matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created"
            )
        if len(found_items) > 1:
            print(
                "Warning: Multiple matches found for envelope_id_lookup in envelopes_dtl?from_date=2000-01-01&status=created. Using the first match."
            )
...
def sync(
    *,
    ...
    Returns:
        Union[DefaultError, SendEnvelopeResponse]
    """
