...
"""
        ...
        if os.name == 'nt':  # Windows
            # For completely hidden execution on Windows, use pythonw.exe instead of python.exe
            pythonw_exe = os.path.join(os.path.dirname(sys.executable), 'pythonw.exe')
            
            # Create a completely hidden process with no window
            import subprocess
            
            # Use CREATE_NO_WINDOW flag to ensure no window appears at all
            CREATE_NO_WINDOW = 0x08000000
            
            # Make sure the startupinfo is properly set to hide window
            startupinfo = subprocess.STARTUPINFO()
            startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            startupinfo.wShowWindow = 0  # SW_HIDE
            
            # Use pythonw.exe and the CREATE_NO_WINDOW flag together for complete hiding
            subprocess.Popen(
                [pythonw_exe, temp_script_path],
                creationflags=subprocess.CREATE_NEW_PROCESS_GROUP | subprocess.DETACHED_PROCESS | CREATE_NO_WINDOW,
