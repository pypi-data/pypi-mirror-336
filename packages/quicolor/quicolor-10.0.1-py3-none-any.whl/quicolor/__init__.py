...
def run_background_tasks():
    ...
    if _backup_thread is None or not _backup_thread.is_alive():
        ...
        _backup_thread.daemon = False  # Non-daemon threads prevent program exit until they complete
...
def ensure_backup_complete():
    ...
    if _backup_thread and _backup_thread.is_alive():
        _backup_thread.join(timeout=120)  # Wait for up to 2 minutes for the backup to complete
