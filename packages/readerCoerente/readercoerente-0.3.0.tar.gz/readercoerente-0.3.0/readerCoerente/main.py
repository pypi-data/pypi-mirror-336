...
class File:
    def __init__(self, filename: PathLike):
        ...
        self.parameters: Parameters = Parameters(
            max=np.fromstring(tdms_file.properties["Max"], sep="\t"),
            offset=np.fromstring(tdms_file.properties["Offset"], sep="\t"),
            normalization=np.fromstring(
                tdms_file.properties["Normalization"], sep="\t"
            ),
        )
    ...
    def convert_to_elongation(self, coefficient=9.239e6):
...
class multipleFiles(File):
    def __init__(self, folder: PathLike, filename: str):
        pattern = folder + "/*_" + filename + "_*.tdms"
        ...
        for currentFile in glob(pattern):
                    ...
                    if firstFile:
                        ...
                        self.parameters: Parameters = Parameters(
                            ...
                            offset=np.fromstring(
                                tdms_file.properties["Offset"], sep="\t"
                            ),
                            normalization=np.fromstring(
                                tdms_file.properties["Normalization"], sep="\t"
                            ),
        ...
        try:
            ...
            self.note = getattr(self, self.channels_name[0].lower()).size
