...
class TestFFT1D:
    ...
    def test_identity(self):
        ...
        for i in range(1, maxlen):
            assert_allclose(mkl_fft.ifft(mkl_fft.fft(x[0:i])), x[0:i],
                            atol=1e-12)
    ...
    def test_fft(self):
        ...
        assert_allclose(fft1(x) / np.sqrt(30),
                        mkl_fft.fft(x, norm="ortho"), atol=1e-6)
    ...
    def test_ifft(self, norm):
        x = random(30) + 1j * random(30)
        assert_allclose(
            x, mkl_fft.ifft(mkl_fft.fft(x, norm=norm), norm=norm),
            atol=1e-6)
    ...
    def test_fft2(self):
        ...
        assert_allclose(mkl_fft.fft(mkl_fft.fft(x, axis=1), axis=0),
                        mkl_fft.fft2(x), atol=1e-6)
        assert_allclose(mkl_fft.fft2(x),
                        mkl_fft.fft2(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.fft2(x) / np.sqrt(30 * 20),
                        mkl_fft.fft2(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.fft2(x) / (30. * 20.),
                        mkl_fft.fft2(x, norm="forward"), atol=1e-6)

    def test_ifft2(self):
        x = random((30, 20)) + 1j * random((30, 20))
        assert_allclose(mkl_fft.ifft(mkl_fft.ifft(x, axis=1), axis=0),
                        mkl_fft.ifft2(x), atol=1e-6)
        assert_allclose(mkl_fft.ifft2(x),
                        mkl_fft.ifft2(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.ifft2(x) * np.sqrt(30 * 20),
                        mkl_fft.ifft2(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.ifft2(x) * (30. * 20.),
                        mkl_fft.ifft2(x, norm="forward"), atol=1e-6)

    def test_fftn(self):
        x = random((30, 20, 10)) + 1j * random((30, 20, 10))
        assert_allclose(
            mkl_fft.fft(mkl_fft.fft(mkl_fft.fft(x, axis=2), axis=1), axis=0),
            mkl_fft.fftn(x), atol=1e-6)
        assert_allclose(mkl_fft.fftn(x),
                        mkl_fft.fftn(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.fftn(x) / np.sqrt(30 * 20 * 10),
                        mkl_fft.fftn(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.fftn(x) / (30. * 20. * 10.),
                        mkl_fft.fftn(x, norm="forward"), atol=1e-6)

    def test_ifftn(self):
        x = random((30, 20, 10)) + 1j * random((30, 20, 10))
        assert_allclose(
            mkl_fft.ifft(mkl_fft.ifft(mkl_fft.ifft(x, axis=2), axis=1), axis=0),
            mkl_fft.ifftn(x), atol=1e-6)
        assert_allclose(mkl_fft.ifftn(x),
                        mkl_fft.ifftn(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.ifftn(x) * np.sqrt(30 * 20 * 10),
                        mkl_fft.ifftn(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.ifftn(x) * (30. * 20. * 10.),
                        mkl_fft.ifftn(x, norm="forward"), atol=1e-6)
        ...
        for n in [x.size, 2 * x.size]:
            for norm in [None, 'backward', 'ortho', 'forward']:
                assert_allclose(
                    ...
                    mkl_fft.rfft(x, n=n, norm=norm), atol=1e-6)
            assert_allclose(
                mkl_fft.rfft(x, n=n),
                mkl_fft.rfft(x, n=n, norm="backward"), atol=1e-6)
            assert_allclose(
                mkl_fft.rfft(x, n=n) / np.sqrt(n),
                mkl_fft.rfft(x, n=n, norm="ortho"), atol=1e-6)
            assert_allclose(
                mkl_fft.rfft(x, n=n) / n,
                mkl_fft.rfft(x, n=n, norm="forward"), atol=1e-6)
    ...
    def test_irfft(self):
        ...
        assert_allclose(x, mkl_fft.irfft(mkl_fft.rfft(x, norm="backward"),
                        norm="backward"), atol=1e-6)
        assert_allclose(x, mkl_fft.irfft(mkl_fft.rfft(x, norm="ortho"),
                        norm="ortho"), atol=1e-6)
        assert_allclose(x, mkl_fft.irfft(mkl_fft.rfft(x, norm="forward"),
                        norm="forward"), atol=1e-6)

    def test_rfft2(self):
        x = random((30, 20))
        assert_allclose(mkl_fft.fft2(x)[:, :11], mkl_fft.rfft2(x), atol=1e-6)
        assert_allclose(mkl_fft.rfft2(x),
                        mkl_fft.rfft2(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.rfft2(x) / np.sqrt(30 * 20),
                        mkl_fft.rfft2(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.rfft2(x) / (30. * 20.),
                        mkl_fft.rfft2(x, norm="forward"), atol=1e-6)

    def test_irfft2(self):
        x = random((30, 20))
        assert_allclose(x, mkl_fft.irfft2(mkl_fft.rfft2(x)), atol=1e-6)
        assert_allclose(x, mkl_fft.irfft2(mkl_fft.rfft2(x, norm="backward"),
                        norm="backward"), atol=1e-6)
        assert_allclose(x, mkl_fft.irfft2(mkl_fft.rfft2(x, norm="ortho"),
                        norm="ortho"), atol=1e-6)
    ...
    def test_rfftn(self):
        ...
        assert_allclose(mkl_fft.rfftn(x),
                        mkl_fft.rfftn(x, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.rfftn(x) / np.sqrt(30 * 20 * 10),
                        mkl_fft.rfftn(x, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.rfftn(x) / (30. * 20. * 10.),
                        mkl_fft.rfftn(x, norm="forward"), atol=1e-6)
        # Regression test for gh-27159
        x = np.ones((2, 3))
        result = mkl_fft.rfftn(x, axes=(0, 0, 1), s=(10, 20, 40))
        assert result.shape == (10, 21)
        expected = mkl_fft.fft(mkl_fft.fft(mkl_fft.rfft(x, axis=1, n=40),
    ...
    def test_irfftn(self):
        ...
        assert_allclose(x, mkl_fft.irfftn(mkl_fft.rfftn(x, norm="backward"),
                        norm="backward"), atol=1e-6)
        assert_allclose(x, mkl_fft.irfftn(mkl_fft.rfftn(x, norm="ortho"),
                        norm="ortho"), atol=1e-6)
        assert_allclose(x, mkl_fft.irfftn(mkl_fft.rfftn(x, norm="forward"),
                        norm="forward"), atol=1e-6)
    ...
    def test_hfft(self):
        ...
        assert_allclose(mkl_fft.hfft(x_herm),
                        mkl_fft.hfft(x_herm, norm="backward"), atol=1e-6)
        assert_allclose(mkl_fft.hfft(x_herm) / np.sqrt(30),
                        mkl_fft.hfft(x_herm, norm="ortho"), atol=1e-6)
        assert_allclose(mkl_fft.hfft(x_herm) / 30.,
                        mkl_fft.hfft(x_herm, norm="forward"), atol=1e-6)
    ...
    def test_all_1d_norm_preserving(self):
        ...
        n = x.size * 2
        func_pairs = [(mkl_fft.fft, mkl_fft.ifft),
...
@pytest.mark.parametrize(
        "fft",
        [mkl_fft.fft, mkl_fft.fft2, mkl_fft.fftn,
         mkl_fft.ifft, mkl_fft.ifft2, mkl_fft.ifftn])
