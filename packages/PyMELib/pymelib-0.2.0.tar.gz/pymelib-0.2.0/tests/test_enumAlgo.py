...
def test_EnumMDS_recursive():
    ...
    assert set(EnumMDS(td, debug_flag=False)) == {frozenset({2}), frozenset({1, 3})}

    td = RootedDisjointBranchNiceTreeDecomposition(example_graph2, debug_flag=False, semi_dntd=False)
    create_factors(td)
    calculate_factors_for_mds_enum(td, td.get_root())
    assert set(EnumMDS(td, debug_flag=False)) == {frozenset({100, 102, 103}),
    ...
    create_factors(td)
    calculate_factors_for_mds_enum(td, td.get_root())
    assert set(EnumMDS(td, debug_flag=False)) == {frozenset({106}),
...
def test_EnumMDS_iterative():
    ...
    assert set(EnumMDS_iterative(td, debug_flag=False)) == {frozenset({2}), frozenset({1, 3})}

    td = RootedDisjointBranchNiceTreeDecomposition(example_graph2, debug_flag=False, semi_dntd=False)
    create_factors(td)
    calculate_factors_for_mds_enum_iterative(td)
    assert set(EnumMDS_iterative(td, debug_flag=False)) == {frozenset({100, 102, 103}),
                                                            frozenset({98, 99, 101, 103, 104}),
                                                            frozenset({98, 99, 101, 104, 105}),
                                                            frozenset({98, 99, 101, 102, 105}),
                                                            frozenset({98, 99, 101, 102, 103}),
    ...
    create_factors(td)
    calculate_factors_for_mds_enum_iterative(td)
    assert set(EnumMDS_iterative(td, debug_flag=False)) == {frozenset({106}),
                                                            frozenset({100, 102, 103}),
                                                            frozenset({98, 99, 101, 103, 104}),
                                                            frozenset({98, 99, 101, 104, 105}),
                                                            frozenset({98, 99, 101, 102, 105}),
