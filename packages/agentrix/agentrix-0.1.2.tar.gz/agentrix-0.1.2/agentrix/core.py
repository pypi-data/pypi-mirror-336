...
class Agent:
    ...
    def go(self, input_message, chat_history=None):
                ...
                for tool in self.tools:
                    if tool.name == tool_call.function.name:
                        ...
                        if not self.is_manager:
                            ...
                            # Generate natural language response using the tool result
                            final_response = self.client.chat.completions.create(
                                model=self.model_name,
                                messages=[
                                    {"role": "system", "content": self.system_prompt},
                                    {"role": "user", "content": input_message},
                                    {"role": "function", "name": tool.name, "content": str_result},
                                    {"role": "system", "content": "Use the tool's result to provide a natural, helpful response."}
                                ]
                            )
                            
                            nl_response = final_response.choices[0].message.content
                            if self.memory is not None:
                                self.memory.add_message("assistant", nl_response)
                            return nl_response
        ...
        return assistant_message or "I couldn't generate a response."
