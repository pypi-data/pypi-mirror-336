...
def SETTING_create_menu(data):  
    ...
    ai_menu.add_command(label="大模型辅助（逐行模式）", command=lambda:RWS_DeepSeekTool(ori_owercount_easyread.copy(),win))  
    ai_menu.add_command(label="大模型辅助（整表模式）", command=lambda:LLMTool(ori_owercount_easyread.copy(),win))  
...
def SETTING_edit_xml(file_path):
    ...
    # 设置弹出窗口大小和位置
    sw_treeQ = window.winfo_screenwidth()
    sh_treeQ = window.winfo_screenheight()
    ww_treeQ = 830
    wh_treeQ = 650  # 增加窗口高度以适应新的布局
    x_treeQ = (sw_treeQ - ww_treeQ) // 2
    y_treeQ = (sh_treeQ - wh_treeQ) // 2
    window.geometry(f"{ww_treeQ}x{wh_treeQ}+{x_treeQ}+{y_treeQ}")

    # 创建输入框
    tk.Label(window, text="Name").grid(row=0, column=0, padx=10, pady=10)
    tk.Label(window, text="URL").grid(row=0, column=1, padx=10, pady=10)
    tk.Label(window, text="Model").grid(row=0, column=2, padx=10, pady=10)
    tk.Label(window, text="API Key").grid(row=0, column=3, padx=10, pady=10)
    ...
    model_entry.grid(row=1, column=2, padx=10, pady=10)
    api_key_entry = tk.Entry(window)
    api_key_entry.grid(row=1, column=3, padx=10, pady=10)

    # 创建树形结构显示现有model
    tree_view = ttk.Treeview(window, columns=("name", "url", "model", "api_key"), show="headings")
    tree_view.heading("name", text="Name")
    tree_view.heading("url", text="URL")
    tree_view.heading("model", text="Model")
    tree_view.heading("api_key", text="API Key")
    tree_view.grid(row=2, column=0, columnspan=4, padx=10, pady=10, sticky="nsew")
    ...
    def add_model():
        ...
        if not name or not url or not model_name or not api_key:
            messagebox.showerror("错误", "所有字段不能为空！")
            return
        new_model = ET.SubElement(root, "model")
        new_model.set("name", name)
        new_url = ET.SubElement(new_model, "url")
        new_url.text = url
        new_model_name = ET.SubElement(new_model, "model")
        new_model_name.text = model_name
        new_api_key = ET.SubElement(new_model, "api_key")
        new_api_key.text = api_key
        tree_view.insert("", "end", values=(name, url, model_name, api_key))
        messagebox.showinfo("成功", "新增model成功！")
    ...
    def delete_model():
        ...
        if not selected_item:
            ...
            return
        name = tree_view.item(selected_item, "values")[0]
        for model in root.findall("model"):
            if model.get("name") == name:
                root.remove(model)
                tree_view.delete(selected_item)
                messagebox.showinfo("成功", "删除model成功！")
                return
        messagebox.showerror("错误", "未找到指定的model！")
    ...
    def modify_model():
        ...
        if not name or not url or not model_name or not api_key:
            ...
            return
        for model in root.findall("model"):
            if model.get("name") == tree_view.item(selected_item, "values")[0]:
                model.set("name", name)
                model.find("url").text = url
                model.find("model").text = model_name
                model.find("api_key").text = api_key
                tree_view.item(selected_item, values=(name, url, model_name, api_key))
                messagebox.showinfo("成功", "修改model成功！")
                return
        messagebox.showerror("错误", "未找到指定的model！")
    ...
    # 创建操作按钮
    button_frame = tk.Frame(window)  # 创建一个按钮容器
    button_frame.grid(row=3, column=0, columnspan=4, pady=10)

    add_button = tk.Button(button_frame, text="新增", command=add_model)
    add_button.grid(row=0, column=0, padx=10)
    delete_button = tk.Button(button_frame, text="删除", command=delete_model)
    delete_button.grid(row=0, column=1, padx=10)
    modify_button = tk.Button(button_frame, text="修改", command=modify_model)
    modify_button.grid(row=0, column=2, padx=10)
    save_button = tk.Button(button_frame, text="保存XML", command=lambda: tree.write(file_path, encoding="utf-8", xml_declaration=True))
    save_button.grid(row=0, column=3, padx=10)

    # 创建测试框
    tk.Label(window, text="提示词").grid(row=4, column=0, padx=10, pady=10, sticky="w")

    # 使用多行文本框 (tk.Text) 用于输入提示词
    prompt_text = tk.Text(window, height=5, width=50)
    prompt_text.grid(row=5, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")

    # 添加滚动条
    scrollbar = tk.Scrollbar(window, command=prompt_text.yview)  # 创建滚动条并关联到 prompt_text
    scrollbar.grid(row=5, column=3, padx=10, pady=10, sticky="ns")  # 放置滚动条在第3列
    prompt_text.insert(tk.END,'彩虹有多少种颜色？请以```json开头的json格式返回。')

    # 配置 prompt_text 使用滚动条
    prompt_text.config(yscrollcommand=scrollbar.set)

    # 创建测试按钮
    test_button = tk.Button(window, text="测试", command=test_model)
    test_button.grid(row=6, column=0, columnspan=4, pady=10)

    # 配置行和列的权重，使界面更具弹性
    window.grid_rowconfigure(5, weight=1)  # 让多行文本框所在的行可以扩展
    window.grid_columnconfigure(0, weight=1)  # 让列可以扩展
...
def PROGRAM_display_df_in_treeview(ori_owercount_easyread, methon, ori, page_size=100):    
    ...
    if methon=="psur" and  '报告编码' in ori_owercount_easyread.columns  and '器械故障表现' in ori_owercount_easyread.columns   and '-持有人-品名-证号列' in ori_owercount_easyread.columns:
        ...
        PROGRAM_create_button(framecanvas, "故障表现", lambda:PROGRAM_display_df_in_treeview(TOOLS_create_pivot_tool(ori_owercount_easyread,[['器械故障表现'], [], ['报告编码'], ['nunique'], {'伤害表现': 'SMALL_count_mode', '敏感词': 'count_ALL'}, []]).sort_values(by='报告编码', ascending=False).reset_index(drop=True),'psur',ori))       
        PROGRAM_create_button(framecanvas, "伤害表现", lambda:PROGRAM_display_df_in_treeview(TOOLS_create_pivot_tool(ori_owercount_easyread,[['伤害表现'], [], ['报告编码'], ['nunique'], {'器械故障表现': 'SMALL_count_mode', '敏感词': 'count_ALL','通用故障表现': 'count_ALL'}, []]).sort_values(by='报告编码', ascending=False).reset_index(drop=True),'psur',ori))       
