...
from tracer.phoenix_tracing import tracing_service
from config.context_config import get_trace_context
...
def calculate_togetherai_price(
    model_name: str, tokens: int
) -> Dict[str, float]:
    pricing = {
        "togethercomputer/m2-bert-80M-8k-retrieval": 0.05,
        "togethercomputer/m2-bert-80M-32k-retrieval": 0.05,
        # Add more models as needed
    }
...
def parse_cohere_tokens(response_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    return {
        "tokens": billed_units.get("tokens", 0),
    }
...
def parse_togetherai_tokens(response_data: Dict[str, Any]) -> Dict[str, int]:
    # Extract usage data from together.ai response
    if "usage" in response_data:
        usage = response_data.get("usage", {})
        return {
...
EMBEDDING_PROVIDER_CONFIGS = {
    ...
    "togetherai": {
        "token_parser": parse_togetherai_tokens,
        "price_calculator": calculate_togetherai_price,
        "embeddings_extractor": lambda data: data.get("embeddings", []),
    },
}
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                tracing_service.add_embedding_span(
                    span_name="EmbeddingGeneration", data=trace_data
                )
                ...
                tracing_service.add_error_span(
                    span_name="Error.Embedding.Operation",
