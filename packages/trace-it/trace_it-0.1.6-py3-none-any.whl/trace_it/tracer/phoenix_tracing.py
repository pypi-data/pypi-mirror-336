...
from trace_it.config.context_config import TRACE_CONTEXT, TRACER_CONTEXT
...
# Replace the existing lines at the end of the file
# tracer = TRACER_CONTEXT.get()
# tracing_service = TracingService(tracer)

# With this lazy initialization class
class LazyTracingService:
    _service = None
    _last_tracer = None
    
    def __getattr__(self, name):
        # Check if tracer has changed
        current_tracer = TRACER_CONTEXT.get()
        if current_tracer != self._last_tracer:
            self._last_tracer = current_tracer
            if current_tracer is None:
                self._service = DummyTracingService()
            else:
                self._service = TracingService(current_tracer)
        
        # Forward the method call to the actual service
        return getattr(self._service, name)

# Optional: Create a dummy service for graceful degradation
class DummyTracingService:
    def __getattr__(self, name):
        def noop(*args, **kwargs):
            print(f"Warning: Tracing operation '{name}' called before tracer was initialized")
        return noop

# Create a singleton instance
tracing_service = LazyTracingService()
