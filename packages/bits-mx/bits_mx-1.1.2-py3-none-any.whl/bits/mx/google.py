...
class Google(object):
    ...
    def get_people_transports(self):
        ...
        for username in sorted(self.update.people):
            email = f"{username}@broadinstitute.org"
            ...
            if "future_hire" in self.update.people[username] and self.update.people[username]['future_hire'] == 1:
                continue
            # skip if there is no corresponding Google account
            if email not in self.update.google_users:
                continue
            # skip if the user is suspended in Google
            elif self.update.google_users[email]["suspended"]:
                continue
            # skip if the user is archived in Google
            elif self.update.google_users[email]["archived"]:
                continue
            # add broadinstitute.org transport
            transports.append(f"{email}\tsmtp:[{self.transportdest}]")
            # add broadinstitute.com transport
            transports.append(f"{username}@broadinstitute.com\tsmtp:[{self.transportdest}]")
        return transports
