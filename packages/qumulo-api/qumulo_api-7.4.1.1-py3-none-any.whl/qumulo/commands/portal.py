...
def pretty_portal_state(state: str) -> str:
...
def format_hub_portals(hubs: List[HubPortal], as_json: bool) -> str:
    ...
    columns = ['ID', 'State', 'Hub Root', 'Spoke Host', 'Spoke Name', 'Spoke Type']
...
def format_spoke_portals(spokes: List[SpokePortal], as_json: bool) -> str:
    ...
    columns = ['ID', 'State', 'Type', 'Spoke Root', 'Hub Host', 'Hub Root ID', 'Hub Portal ID']
    rows = []
    ...
    for spoke in spokes:
        ...
        row = [
            spoke.id,
            pretty_portal_state(spoke.state),
...
class ListPortals(qumulo.lib.opts.Subcommand):
    ...
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        ...
        rows = [[hub.id, pretty_portal_state(hub.state), 'Hub', hub.root_path] for hub in hubs]

        rows.extend(
            [
                [spoke.id, pretty_portal_state(spoke.state), 'Spoke', spoke.spoke_root_path]
                for spoke in spokes
            ]
        )
