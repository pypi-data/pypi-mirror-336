...
def community_detection_ray(method, current_destfile, current_resolution, layers):
    binary_path = Path(importlib.resources.files("BioFusion")) / "bin" / method
    os.chmod(binary_path, 0o755)  # Ensure it's executable
    # Convert to absolute path
    binary_path = str(binary_path.resolve())
    system_order = f"{binary_path} -o {current_destfile} -p {current_resolution} {layers} > /dev/null"
    subprocess.run(system_order, shell=True, capture_output=True, text=True)
...
def communities_detection(input_layers: list[str] = None,
    ...
    tasks = []
    with tqdm(total=len(resolution_gamma_step), desc="Community Detection Progress") as pbar:
        ...
        if tasks:
            ray.get(tasks)
            pbar.update(len(tasks))  # Update progress bar for remaining tasks
