...
from . import file_folder


class FTPClient:
    def __init__(self, server: str, port: int, username: str, password: str, timeout: int = 20, is_sftp: bool = None):
        """
    ...
    def _connect(self):
        """
        Context manager that yields an open connection (SFTP or FTP) based on the protocol.
        """
        if self.is_sftp:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(self.server, username=self.username, password=self.password, port=self.port, timeout=self.timeout)
            sftp = ssh.open_sftp()
    ...
    def content(self, file_path: str) -> dict:
        """
    ...
    def get_last_modified(self, file_path: str) -> dict:
        """
    ...
    def get_folder_list(self, folder_path: str = '') -> list:
        """
    ...
    def upload_file(self, local_path: str, file_path: str, confirm: bool = True) -> bool:
        """
    ...
    def download_file(self, local_path: str, file_path: str, make_directory: bool = True, remove_file: bool = True) -> bool:
        """
