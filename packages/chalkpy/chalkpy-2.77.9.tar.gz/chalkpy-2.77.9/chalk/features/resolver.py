...
RESOLVER_FUNCTION_CAPTURE_LIMIT = 100


@dataclasses.dataclass(frozen=True)
class FunctionCapturedGlobalFeatureClass:
    """
...
@dataclasses.dataclass(frozen=True)
class FunctionCapturedGlobalBuiltin:
    """
...
@dataclasses.dataclass(frozen=True)
class FunctionCapturedGlobalEnum:
    module: str
    name: str
...
@dataclasses.dataclass(frozen=True)
class FunctionCapturedGlobalModule:
    name: str


@dataclasses.dataclass(frozen=True)
class FunctionCapturedGlobalFunction:
    """
...
def parse_extract_function_object_captured_globals(
    ...
    for global_var, global_value in fn_closure_vars.globals.items():
        ...
        elif isinstance(global_value, ModuleType):
            module_name = global_value.__name__

            if (
                global_value is math
                or global_value is re
                or global_value is datetime_module
                or global_value is base64
                or global_value is hashlib
                or global_value is json
                or global_value is random
            ):
                function_captured_globals[global_var] = FunctionCapturedGlobalModule(name=module_name)
            elif global_value.__name__ == "numpy":
                try:
                    import numpy as np

                    if global_value is np:
                        function_captured_globals[global_var] = FunctionCapturedGlobalModule(name=module_name)
                except ImportError:
                    pass
            elif global_value.__name__ == "pandas":
                try:
                    import pandas as pd

                    if global_value is pd:
                        function_captured_globals[global_var] = FunctionCapturedGlobalModule(name=module_name)
                except ImportError:
                    pass
...
def _parse_stream_resolver_output_features(
    ...
    *,
    ...
    for feature in return_annotation.features:
        ...
        elif namespace != feature.namespace:
            error_builder.add_diagnostic(
                message=(
