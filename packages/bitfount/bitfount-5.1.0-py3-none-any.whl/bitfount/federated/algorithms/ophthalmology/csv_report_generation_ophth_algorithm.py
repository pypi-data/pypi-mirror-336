...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.trial_name is not None:
            self.file_name = (
                f"{trial_name}-prescreening-patients-"
    ...
    def _add_filtering_to_csv_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.filter is not None:
            ...
            for i, col_filter in enumerate(self.filter):
                ...
                try:
                    df = _add_filtering_to_df(df, col_filter, self.rename_columns)
    ...
    def run(
        ...
        if self.original_cols is not None and not all(
            ...
            print(f"Original cols before: {self.original_cols}")
            print(f"aux_results_df columns: {aux_results_df.columns.tolist()}")
            print(f"self.aux_cols: {self.aux_cols}")
            ...
            print(f"Original cols after: {self.original_cols}")
        ...
        for df in test_data_dfs:
            ...
            this_csv_path = append_dataframe_to_csv(
    ...
    def _get_csv_path(self, task_id: Optional[str]) -> Path:
        ...
        if self.trial_name is not None:
            self.file_name = (
                f"{self.trial_name}-prescreening-patients-"
                f"{datetime.today().strftime('%Y-%m-%d')}"
            )
        else:
            self.file_name = "results"
