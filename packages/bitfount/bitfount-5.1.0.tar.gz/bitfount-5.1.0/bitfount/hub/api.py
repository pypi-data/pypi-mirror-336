...
class NextGenEnterpriseAPI:
    ...
    def get_patient_info(
        ...
        if conditions is not None or procedures is not None:
            return PatientInfo(
    ...
    def get_conditions_information(
        ...
        """
        resp = self.session.get(f"{self.url}/persons/{patient_id}/chart/diagnoses")
        resp.raise_for_status()

        resp_json: NextGenEnterpriseDiagnosesJSON = resp.json()
        items: list[NextGenEnterpriseConditionsEntryJSON] = pydash.get(
            resp_json, "items"
        )
        return items
    ...
    def get_procedures_information(
        ...
        Returns:
            A list of JSON objects containing information on procedures administered
            to the patient. If there was no such list, return None.
        """
        resp = self.session.get(f"{self.url}/persons/{patient_id}/chart/procedures")
        resp.raise_for_status()

        resp_json: NextGenEnterpriseProceduresJSON = resp.json()
        items: list[NextGenEnterpriseProceduresEntryJSON] = pydash.get(
            resp_json, "items"
        )
        return items
