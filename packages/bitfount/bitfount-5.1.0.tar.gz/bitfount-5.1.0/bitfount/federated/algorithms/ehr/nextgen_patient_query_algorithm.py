...
from bitfount.hub.authentication_flow import BearerAuthSession, BitfountSession
...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
    ...
    ) -> dict[PatientDetails, PatientCodeDetails]:
        ...
        Returns:
            Tuple of (matching patients, failed patients) where `failed_patients` is
            those that we were unable to get enough information to establish whether
            they are a match or not. This is distinct from those who simply did not
            match.
        """
        ...
        bearer_token = smart_auth.get_access_token()

        # Initialize API clients
        nextgen_session = BearerAuthSession(bearer_token.token)
        ...
        patient_query_results: dict[PatientDetails, PatientCodeDetails] = {}
        ...
        for patient in patients:
            ...
            if patient_info is None:
                _logger.warning(
                    "Unable to retrieve/parse a patient's treatment information;"
                    " skipping."
                )
                continue

            # Unpack tuple
            conditions, procedures = patient_info
            ...
            patient_query_results[patient] = PatientCodeDetails(
        ...
        # PatientCodeDetails entry
        for missed_patient in (p for p in patients if p not in patient_query_results):
            patient_query_results[missed_patient] = PatientCodeDetails(
                icd10_codes={
    ...
    def merge_results_with_dataframe(
        query_results: dict[PatientDetails, PatientCodeDetails],
        df: pd.DataFrame,
        bitfount_patient_id_column: str = _BITFOUNT_PATIENT_ID_KEY,
        ...
        Args:
            query_results: Dictionary mapping PatientDetails to their code statuses.
        ...
        Returns:
            DataFrame with additional columns for each code's status.
        ...
        for patient, code_details in query_results.items():
