...
def hlsvdpro(data, nsv_sought, m=None, sparse=False):
    ...
    uk = u[:, :k].copy()  			# Jia, compatible with Numpy 2.0
...
def create_hlsvd_fids(result, npts, dwell, sum_results=False, convert=True):
    ...
    fids = np.zeros((len(freqs), npts), dtype=complex) # Jia. np.complex is deprecated in Numpy 1.20 
    ...
    if sum_results: 
        result = np.sum(fids, axis=0)
        return result
    else:
        return fids
    # return result  #Jia I believe this is wrong
