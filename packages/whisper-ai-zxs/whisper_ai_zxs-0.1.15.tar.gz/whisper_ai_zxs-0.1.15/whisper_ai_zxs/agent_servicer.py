...
class AgentServicer:
    ...
    def listening_user(self): 
        ...
        for user in user_list:
            new_chat_list = self.call("get_new_chats", user, self.user_reply_time_dic.get(user)["time"] if user in self.user_reply_time_dic else "")
            if (len(new_chat_list) > 0) :
                if self.user_reply_time_dic[user]["continue_times"] < 10 :
                ...
                if user in self.user_reply_time_dic:
                    continue_times = self.user_reply_time_dic[user]["continue_time"]
                ...
                self.user_reply_time_dic[user] = {
                                                ...
                                                "real_time" : self.user_reply_time_dic.get(user)["real_time"] if user in self.user_reply_time_dic else datetime(2020, 1, 1),
                                                "continue_times" : 0
                                            } 
        ...
        for user in list(self.user_reply_time_dic.keys()):  # 先复制 key 的列表
            if (datetime.now() - self.user_reply_time_dic[user]["real_time"]).total_seconds() / 60 > 10:
                ...
                del self.user_reply_time_dic[user]  # 安全删除
    ...
    def on_error(self, e): 
        ...
        logging.error(f"{self._kf_name}循环出现异常{e}！")
        WhisperTools_Qywx.send_to_error_robot(f"{self._kf_name}循环出现异常：第{self._error_count}次。({e})")
        if (self._error_count > 3):
            result = self.call("activate", self.get_kf_name())
            logging.error(f"强制激活的结果：{result}")
            if (result == True) :
