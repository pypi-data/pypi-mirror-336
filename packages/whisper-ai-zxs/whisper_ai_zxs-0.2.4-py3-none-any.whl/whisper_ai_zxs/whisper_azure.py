...
class WhisperAzure:
    ...
    def reply(self, fromUserID, msgList):
        ...
        # 打印结果
        logger_whisper.info(f"assistant_id : {assistant_id}")
        ...
        if my_thread:
            logger_whisper.info(f"找到老的沟通记录。{my_thread.id}")
            newchatFlag = False
            ...
            if len(runs.data) > 0 :
                ...
                if run.status != "completed" and run.status != "expired" and run.status != "failed":
                    ...
                    logger_whisper.info(f'新创建沟通线程。{my_assistant.id},{my_thread.id}')  
                    newchatFlag = True
        else:
            my_thread = client.beta.threads.create()
            logger_whisper.info(f'新创建沟通线程。{my_assistant.id},{my_thread.id}')  
        ...
        for item in msgList:
            ...
            if item["type"] == "text":
                ...
                logger_whisper.info("Data URL:", data_url)
                ...
                logger_whisper.info("Vision Assistant Message: " + response.choices[0].message.content)
    ...
    def add_history(self, fromUserID, msgList):
        ...
        # 打印结果
        logger_whisper.info("assistant id: {assistant_id}")
        my_assistant = client.beta.assistants.retrieve(assistant_id)
        ...
        # 检查是否存在有效的ThreadID
        query = f"SELECT `ThreadID` FROM `openai_user_list` WHERE `UserID` = '{self._registry.get_shop_name()}-{fromUserID}'"
        result = myDB.query(query)
        ...
        logger_whisper.info(f"开始插入AI对话记录：{content}")
        # print(content)
        thread_message = client.beta.threads.messages.create(
            my_thread.id,
            role="assistant",
        ...
        logger_whisper.info(thread_message)
        pass
