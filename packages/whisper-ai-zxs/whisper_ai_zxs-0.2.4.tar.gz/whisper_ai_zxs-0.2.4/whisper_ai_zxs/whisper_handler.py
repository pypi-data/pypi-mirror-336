...
class EventHandler(AssistantEventHandler):
  ...
  def on_event(self, event):
    ...
    if event.event == 'thread.run.requires_action':
      ...
      if EventHandler.shared_fail_times < 2 :
        ...
        logger_whisper.warning("thread.run.failed 三次后，删除用户threadID，并转接给人工客服。")
        myDB = WhisperDB()
  ...
  def handle_requires_action(self, data, run_id):
    ...
    for tool in data.required_action.submit_tool_outputs.tool_calls:
      logger_whisper.info("收到openai调用："+tool.function.name)
      ...
      try:
        ...
        logger_whisper.warning(f"函数处理错误！{e}，做第二次尝试！")
        try:
          result = self.process_function(tool.function.name, tool.function.arguments)
        except Exception as e:
          logger_whisper.warning(f"第二次还是错误！{e}")
  ...
  def process_function(self, name, arguments):
    ...
    if name == "transfer_to_customer_care":
      ...
      logger_whisper.warning("还不支持multi_tool_use，请单独调用各个函数！")
