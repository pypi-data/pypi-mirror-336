...
class ShapeV2Cracker(BaseCracker):
    ...
    def request(self):
        ...
        if not self.wanda_args.get("pkey"):
            ...
            if self.wanda_args["script_url"]:
                ...
                if not self.wanda_args.get("script_content"):
                    try:
                        script = self.session.get(self.wanda_args["script_url"], headers=headers).text
        ...
        self.wanda_args["cookies"] = {
            **self.cookies,
            **{
                k: v for k, v in self.session.get_cookies(url=self.href).items()
            }
        }
...
class ShapeV1Cracker(BaseCracker):
    ...
    def request(self):
        ...
        self.wanda_args["cookies"] = {
            **self.cookies,
            **{
                k: v for k, v in self.session.get_cookies(self.href).items()
            }
        }
