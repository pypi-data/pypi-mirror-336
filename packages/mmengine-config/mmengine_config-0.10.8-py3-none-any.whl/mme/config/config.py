...
@overload
def _lazy2string(cfg_dict: T, dict_type: Optional[Type] = None) -> T:
    """"""


@overload
def _lazy2string(cfg_dict: U, dict_type: Optional[Type] = None) -> U:
    """"""


@overload
def _lazy2string(
    cfg_dict: LazyAttr | LazyObject, dict_type: Optional[Type] = None
) -> str:
    """"""
...
class ConfigDict(Dict):
    ...
    def values(self):
        ...
        mappings = {}
        for key, value in super().items():
            mappings[key] = self.build_lazy(value)
...
def add_args(
    parser: ArgumentParser, cfg: Mapping[str, Any], prefix: str = ""
) -> ArgumentParser:
...
class Config:
    ...
    Args:
        ...
        filename (str or Path, optional): Name of config file.
            Defaults to None.
        format_python_code (bool): Whether to format Python code by black.
            Defaults to True.
    ...
    def fromfile(
        ...
        filename = str(filename) if isinstance(filename, Path) else filename
        if (
            lazy_import is False
            or lazy_import is None
            and not Config._is_lazy_import(filename)
        ):
            ...
            cfg = Config(
                cfg_dict, filename=filename, format_python_code=format_python_code
            )
    ...
    def _get_base_modules(nodes: list) -> list:
        ...
        for idx, node in enumerate(nodes):
            ...
            if (
                ...
                raise ConfigParsingError(
                    "Only `read_base` context manager can be used in the " "config"
                )
    ...
    def _file2dict(
        ...
        assert osp.exists(filename) and osp.isfile(filename)
    ...
    def _parse_lazy_import(filename: str) -> Tuple[ConfigDict, set]:
        ...
        with open(filename, encoding="utf-8") as f:
            ...
            for base_module in base_modules:
                ...
                match_result = re.match(r"\.*", base_module)
                if match_result is not None and match_result.group():
                    level = len(match_result.group())
                    # Relative import
                    ...
                    if len(module_list) == 1:
                        ...
                        module_path = (
                            f"{osp.join(root_path, *module_list[1:])}.py"  # noqa: E501
                        )
            ...
            transform = ImportTransformer(
                global_dict=global_dict, base_dict=base_dict, filename=filename
            )
            ...
            code = compile(modified_code, filename, mode="exec")
    ...
    def _get_base_files(filename: str) -> list:
        ...
        if file_format == ".py":
            ...
            raise ConfigParsingError(
                f"The config type should be py, but got {file_format}"
            )
    ...
    def auto_argparser(description=None):
        ...
        add_args(parser, cfg)  # type: ignore
        return parser, cfg
    ...
    @property
    def pretty_text(self) -> str:  # noqa: C901
        ...
        if self._format_python_code:
            try:
                ...
                style = black.Mode()
                text = black.format_str(text, mode=style)
            except Exception as e:
                raise SyntaxError(
                    "Failed to format the config file, please "
    ...
    def _is_lazy_import(filename: str) -> bool:
        ...
        for node in ast.walk(parsed_codes):
            ...
            if isinstance(node, ast.With):
                expr = node.items[0].context_expr
                if not isinstance(expr, ast.Call) or not expr.func.id == "read_base":
                    raise ConfigParsingError(
                        "Only `read_base` context manager can be used in the " "config"
                    )
                return True
