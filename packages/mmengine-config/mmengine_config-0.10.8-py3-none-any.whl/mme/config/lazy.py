...
def is_seq_of(
    seq: Any, expected_type: Union[Type, tuple], seq_type: Optional[Type] = None
) -> bool:
    ...
    def build(self) -> Any:
        ...
        if isinstance(self._module, str):
            try:
                ...
                raise type(e)(
                    f"Failed to import {self._module} " f"in {self.location} for {e}"
                )
            ...
            try:
                ...
                raise type(e)(
                    f"Failed to import {self.module} " f"in {self.location} for {e}"
                )
...
class LazyAttr:
    ...
    def __init__(
        self, name: str, source: Union["LazyObject", "LazyAttr"], location=None
    ):
        self.name = name
    ...
    def __call__(self, *args, **kwargs: Any) -> Any:
        ...
        raise RuntimeError(f"can't initialize {type(obj)}")
    ...
    def build(self) -> Any:
        ...
        try:
            ...
            raise ImportError(
                f"Failed to import {self.module}.{self.name} in " f"{self.location}"
            )
