...
def test_fuzz_merge_inner_join():
    ...
    right = pd.DataFrame({"id_right": [3, 4, 5, 6], "name_right": [
                         "baz", "bear", "fool", "food"]})

    result = fuzz_merge(left, right, left_on="name_left",
...
def test_fuzz_merge_multiple_columns_not_supported():
    ...
    try:
        fuzz_merge(left, right, left_on=[
                   "name_left", "id_left"], right_on="name_right")
    except NotImplementedError as e:
