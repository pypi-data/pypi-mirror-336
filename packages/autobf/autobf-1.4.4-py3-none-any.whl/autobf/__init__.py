...
def click(x=None,y=None,repeat=1,delay=0):
    '''
...
def jumpSlide(key,j_repeat,s_repeat):
    ...
    跳躍滑行
    '''
    bf.press('space',j_repeat)
    bf.slide(key,s_repeat)
...
def instinctOn(p1=(932,902),p2=(985,910)):
    '''
    ...
    L,T=p1
    R,B=p2
    ps=findColorsYX((124, 216, 239),30,img[T:B,L:R])
    if len(ps)<10:press('e')
...
def getMission(mission=1,baseX=1400,baseY=510,dy=50):
    '''
    取得任務
    mission=1,2,3...
    baseX,basY :任務1所在座標
...
def buyRandomFruit():
    ...
    print(Xc,Yc)
    time.sleep(1)
    check=bf.checkColorByXY(1150,505,color=(255, 255, 255))
    if check:
        print('OK')
        bf.row(1150,505)
        bf.click(1150,505)
        ...
        bf.click(1150,505)
        time.sleep(1)
        check=bf.checkColorByXY(573,579,color=(255, 255, 255))
        if check:
            print('時間未到')
            return False
        bf.click(1150,505)
...
def storeFruit():
    ...
    check=bf.checkColorByXY(1173,683,color=(255, 255, 255))
    if check:
        bf.row(1173,683);
        bf.click()
        putDownFruit()        
