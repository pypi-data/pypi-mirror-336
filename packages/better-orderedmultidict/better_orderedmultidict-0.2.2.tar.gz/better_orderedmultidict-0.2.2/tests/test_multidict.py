...
from better_orderedmultidict._multidict import MultiDict
...
class TestMultiDict(TestCase):
	...
	@property
	def list_inits(self) -> list[list[tuple[Any, Any]]]:
		return [
	...
	@property
	def dict_inits(self) -> list[dict]:
		return [
			{}, {1: 1}, {1: 1, 2: 2}, {None: None}, {False: False}, {None: 1, 1: None, 3: 5},
		]

	@property
	def list_updates(self) -> list[list[tuple[Any, Any]]]:
		return [
	...
	@property
	def dict_updates(self) -> list[dict]:
		return [
			{}, {7: 7}, {7: 7, 8: 8, 9: 9}, {None: None}, {1: 1, 2: 2}, {None: 1, 1: None, 3: 5},
		]
	@property
	def kw_updates(self) -> list[dict]:
		return [
			{}, {'1': 1}, {'1': 1, '2': 2}, {'sup': 'innit?', 'Knuth': None}, {'aa': 'aa'},
		]
	@property
	def nonkeys(self) -> list:
		return [_unique, 'asdfasdosduf', 'oaisfiapsn', 'ioausopdaui']
	@property
	def new_values(self) -> list:
		return [1, 2, 3, None, 'a', 'omnibus', object()]
	@property
	def new_values_lists(self) -> list[list]:
		return [
			[],
	...
	def get_original_omds(self):
		...
		return [
			*((items, MultiDict(items)) for items in items_lists),
	...
	def test_init(self):
		...
		for init in self.list_inits + self.dict_inits:
			omd = self.OMD(init)
			self.assertCountEqual(list(omd.items()), items_list(init))

		# init by OrderedMultiDict
		for init in self.list_inits:
			omd1 = self.OMD(init)
			omd2 = self.OMD(omd1)
			self.assertCountEqual(list(omd1.items()), list(omd2.items()))
			omd2.add(77, 3210)
		...
		for init in inits:
			...
			self.assertCountEqual(list(omd.items()), items_list(init) + kwargs)

		omd = self.OMD(**dict(kwargs))
		self.assertCountEqual(list(omd.items()), kwargs)
	...
	def test_update(self):
		...
		for update in self.list_updates + self.dict_updates:
			...
			for items, omd in self.get_original_omds():
				...
				self.assertCountEqual(list(omd.items()), filtered_items + new_items)
		...
		for items, omd in self.get_original_omds():
			...
			filtered_items = [item for item in items if item[0] not in update_keys]
			self.assertCountEqual(list(omd.items()), filtered_items + kwargs)
	...
	def test_expand(self):
		...
		for update in self.list_updates + self.dict_updates:
			for items, omd in self.get_original_omds():
				omd.extend(update)
				self.assertCountEqual(list(omd.items()), items + items_list(update))
		...
		for update in updates:
			for items, omd in self.get_original_omds():
				...
				self.assertCountEqual(list(omd.items()), items + items_list(update) + kwargs)

		for items, omd in self.get_original_omds():
			omd.extend(**dict(kwargs))
			self.assertCountEqual(list(omd.items()), items + kwargs)
	...
	def test_copy(self):
		for init in self.list_inits + self.dict_inits:
			...
			self.assertCountEqual(list(omd1.items()), list(omd2.items()))
			self.assertCountEqual(list(omd1.items()), list(omd3.items()))
			self.assertEqual(omd1._map, omd3._map)
			omd2.add(2, 2)
			self.assertNotEqual(list(omd1.items()), list(omd2.items()))
			self.assertCountEqual(list(omd1.items()), items_list(init))
			self.assertCountEqual(list(omd2.items()), items_list(init) + [(2, 2)])
			omd1.add(77, 3210)
			self.assertCountEqual(list(omd1.items()), items_list(init) + [(77, 3210)])
			self.assertCountEqual(list(omd2.items()), items_list(init) + [(2, 2)])
	...
	def test_getall(self):
		for init in self.list_inits:
			omd = self.OMD(init)
			for key in omd.unique_keys():
				self.assertCountEqual(omd.getall(key), [v for k, v in init if k == key])
			for nonkey in self.nonkeys:
	...
	def test_setall(self):
		for new_values in self.new_values_lists:
			for init in self.list_inits:
				omd = self.OMD(init)
				for key in list(omd.unique_keys()):
	...
	def test_add(self):
		for init in self.list_inits:
			...
			for key in omd.unique_keys():
				...
				self.assertCountEqual(omd[key], "bat mobil")
				self.assertCountEqual(omd.getall(key), old_values + ["bat mobil"])
				self.assertCountEqual(list(omd.values()), all_old_values + ["bat mobil"])
			omd = self.OMD(init)  # re-init the omd, so we have a fresh one
			for nonkey in self.nonkeys:
				all_old_values = list(omd.values())
				omd.add(nonkey, "bat mobil")
				self.assertCountEqual(omd.getall(nonkey), ["bat mobil"])
				self.assertCountEqual(list(omd.values()), all_old_values + ["bat mobil"])
		...
		for new_values in self.new_values_lists:
			for init in self.list_inits:
				...
				for key in omd.unique_keys():
					...
					self.assertCountEqual(omd.getall(key), old_values + new_values)
					self.assertCountEqual(list(omd.values()), all_old_values + new_values)
				omd = self.OMD(init)  # re-init the omd, so we have a fresh one
				for nonkey in self.nonkeys:
					all_old_values = list(omd.values())
					omd.addall(nonkey, new_values)
					self.assertCountEqual(omd.getall(nonkey), new_values)
					self.assertCountEqual(list(omd.values()), all_old_values + new_values)
		...
		for new_values in self.new_values_lists + self.new_values:
			for init in self.list_inits:
				...
				for key in list(omd.unique_keys()):
					...
					old_values = list(omd.getall(key))
					self.assertCountEqual(omd.popall(key, default=new_values), old_values)
					self.assertCountEqual(list(omd.keys()), [k for k in all_keys if k != key])
				omd = self.OMD(init)  # re-init the omd, so we have a fresh one
				for nonkey in self.nonkeys:
					all_keys = list(omd.keys())
					self.assertEqual(omd.popall(nonkey, default=new_values), new_values)
					self.assertCountEqual(list(omd.keys()), all_keys)
	...
	def test_popall_without_default(self):
		for init in self.list_inits:
			...
			for key in list(omd.unique_keys()):
				all_keys = list(omd.keys())
				old_values = list(omd.getall(key))
				self.assertCountEqual(omd.popall(key), old_values)
				self.assertCountEqual(list(omd.keys()), [k for k in all_keys if k != key])
			omd = self.OMD(init)  # re-init the omd, so we have a fresh one
			for nonkey in self.nonkeys:
				all_keys = list(omd.keys())
		...
		for new_value in self.new_values:
			for init in self.list_inits:
				...
				if init:
					...
					self.assertCountEqual(list(omd.items()), init[1:])
				else:
	...
	def test_poplastitem_with_default(self):
		for new_value in self.new_values:
			for init in self.list_inits:
				omd = self.OMD(init)
				if init:
					last_key = next(reversed(dict.fromkeys(k for k, v in init)))
					last_item = next(item for item in reversed(init) if item[0] == last_key)
					init.remove(last_item)
					self.assertEqual(omd.poplastitem(), last_item)
					self.assertCountEqual(list(omd.items()), init)
				else:
	...
	def _test_pop(self, pop_func: Callable[[MultiDict, Any], Any], popped_idx: int, remaining_slice: slice, *, default: Any = _SENTINEL):
		for init in self.list_inits:
			omd = self.OMD(init)
			for key in list(omd.unique_keys()):
				old_values = list(omd.getall(key))
				self.assertEqual(pop_func(omd, key), old_values[popped_idx])
				self.assertCountEqual(omd.getall(key), old_values[remaining_slice])
			...
			for nonkey in self.nonkeys:
				...
				self.assertCountEqual(list(omd.values()), all_old_values)
	...
	def test_keys(self):
		for init in self.list_inits:
			omd = self.OMD(init)
			self.assertCountEqual(list(omd.keys()), [item[0] for item in init])
	...
	def test_values(self):
		for init in self.list_inits:
			omd = self.OMD(init)
			self.assertCountEqual(list(omd.values()), [item[1] for item in init])
