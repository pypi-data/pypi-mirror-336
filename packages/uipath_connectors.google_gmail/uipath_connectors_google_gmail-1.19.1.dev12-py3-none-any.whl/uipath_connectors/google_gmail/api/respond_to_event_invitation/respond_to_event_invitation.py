...
from ...models.respond_to_event_invitation_request import (
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "patch",
        "url": "/RespondToEventInvitation/{id}".format(
            id=id,
        ),
        "params": params,
    }
...
def sync_detailed(
    ...
    *,
    ...
    """Respond To Event Invitation (Calendar Event)
     ...
     Response to an calendar event invitation

    Args:
        id (str): The event's invitation to be replied
        calendar_id (Optional[str]): The calendar in which respective event invitation is present
        send_notification (Optional[bool]): Notify organizer of the response Default: False.
        apply_on_series (Optional[bool]): Update all occurances of the event with same reply
            Default: False.
        body (RespondToEventInvitationRequest):
    ...
    if not calendar_id and calendar_id_lookup:
        ...
        if not found_items:
            raise ValueError(
                "No matches found for calendar_id_lookup in CuratedCalendarList"
            )
        if len(found_items) > 1:
            print(
                "Warning: Multiple matches found for calendar_id_lookup in CuratedCalendarList. Using the first match."
            )
