...
class OpenAIMetricsCalculator:
    ...
    def _try_get_model(self, inputs, output):
        if IS_LEGACY_OPENAI:
            ...
            if isinstance(output, dict):
                ...
                model = None
                for chunk in output:
                    if hasattr(chunk, "model") and len(chunk.model) > 0:
                        model = chunk.model
                        break
            if not model:
                model = inputs.get("model")
        ...
        try:
            ...
            if isinstance(output, list):
                if IS_LEGACY_OPENAI:
                    ...
                    # Calculate tokens from collected content
                    metrics["completion_tokens"] = len(enc.encode(content)) if content else 0
            else:
    ...
    def _get_encoding_for_chat_api(self, model):
        try:
            ...
            # Default to cl100k_base for OpenAI models and deepseek models
            enc = tiktoken.get_encoding("cl100k_base")

        # Set tokens per message based on model
        if model == "gpt-35-turbo-0301":
            tokens_per_message = 4
            tokens_per_name = -1
        elif any(m in model.lower() for m in ["gpt-35-turbo", "gpt-3.5-turbo", "gpt-4"]):
            tokens_per_message = 3
            tokens_per_name = 1
        elif "deepseek" in model.lower():
            # Deepseek models follow similar pattern to GPT-3.5/4
            tokens_per_message = 3
            tokens_per_name = 1
        else:
            # For unknown models, use GPT-3.5/4 defaults but log a warning
            print(f"Warning: Using default token calculation for unknown model {model}")
            tokens_per_message = 3
            tokens_per_name = 1
        ...
        try:
            ...
            if isinstance(output, list):
                if IS_LEGACY_OPENAI:
                    ...
                    # Collect all text from streaming chunks
                    content = ""
                    for chunk in output:
