...
class Hwp:
    ...
    def register_regedit(self):
        ...
        try:
            ...
            location = [i.split(": ")[1] for i in subprocess.check_output(['pip', 'show', 'pyhwpx'], stderr=subprocess.DEVNULL).decode().split("\r\n") if i.startswith("Location: ")][0]
            location = os.path.join(location, "pyhwpx")
        ...
        if not os.path.exists(os.path.join(location, "FilePathCheckerModule.dll")):
            ...
            print("pyinstaller 하위경로에서 보안모듈을 찾는 중..")
            ...
            if location == "":
                print("위 경로에서 보안모듈을 찾을 수 없습니다.")
                # pyhwpx가 설치되어 있지 않은 PC에서는,
                # 공식사이트에서 다운을 받게 하자.
                from zipfile import ZipFile
                print("https://github.com/hancom-io에서 다운로드를 시도합니다.")
                try:
