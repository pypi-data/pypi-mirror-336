# coding: utf-8

"""
    ai/h2o/eval_studio/v1/insight.proto

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: version not set
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from eval_studio_client.api.models.v1_list_dashboards_response import V1ListDashboardsResponse

class TestV1ListDashboardsResponse(unittest.TestCase):
    """V1ListDashboardsResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1ListDashboardsResponse:
        """Test V1ListDashboardsResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1ListDashboardsResponse`
        """
        model = V1ListDashboardsResponse()
        if include_optional:
            return V1ListDashboardsResponse(
                dashboards = [
                    eval_studio_client.api.models.v1_dashboard.v1Dashboard(
                        name = '', 
                        create_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        creator = '', 
                        update_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        updater = '', 
                        delete_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        deleter = '', 
                        display_name = '', 
                        description = '', 
                        status = 'DASHBOARD_STATUS_UNSPECIFIED', 
                        leaderboards = [
                            ''
                            ], 
                        create_operation = '', 
                        demo = True, )
                    ]
            )
        else:
            return V1ListDashboardsResponse(
        )
        """

    def testV1ListDashboardsResponse(self):
        """Test V1ListDashboardsResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
