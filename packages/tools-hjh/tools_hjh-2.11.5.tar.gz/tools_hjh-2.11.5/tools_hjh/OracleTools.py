...
class OracleTools:
    ...
    def get_table_metadata(ora_conn, username, table, partition=True):
        ...
        for col_ in cols_:
            ...
            if col_[3] is not None:
                data_default = col_[3].strip()
            else:
                data_default = None
            
            if col_[4] is not None:
                comments = col_[4].strip()
            else:
                comments = None
            
            if col_[5] is not None:
                virtual = col_[5]
            else:
                virtual = None
        ...
        sql = '''
            ...
            order by t.index_name, t2.column_position
        '''
        ...
        for row in rows:
            ...
            if idx_name not in idx_temp_dict:
                cols = [(idx_col_name, idx_col_sort)]
                idx_temp_dict[idx_name] = [cols, idx_type, idx_owner, partitioned, locality]
            else:
                idx_temp_dict[idx_name][0].append((idx_col_name, idx_col_sort))
                
        for k, v in idx_temp_dict.items():
            mess_map['indexes'].append({'name':k, 'columns':v[0], 'type':v[1], 'owner':v[2], 'partitioned':v[3], 'locality':v[4]})
    ...
    def desc(ora_conn, username, table, simple_mode=True, no_fk=False):
        ...
        for rs in rss:
            ...
            if rs['type'] == 'BITMAP':
                ...
                idx_type = ''
            if simple_mode:
                ss = 'index: (' + cols.replace(' ASC', '').replace(',', ', ') + ')' + idx_type
            else:
                ss = 'index: ' + name + ' (' + cols.replace(' ASC', '').replace(',', ', ') + ')' + idx_type
            mess = mess + ss + '\n'    
    ...
    def get_table_ddl(dba_conn, username, table, no_fk=False, partition=True):
        ...
        for idx in metadata['indexes']:
            ...
            cols_str = cols_str[:-2].replace(' ASC', '')
            
            locality = idx['locality']
            if locality == None:
                locality = ''
            owner = idx['owner']
            index_str = index_str + 'create ' + type_ + ' index ' + owner + '."' + name + '" on ' + username + '."' + table + '" (' + cols_str + ') ' + locality
    ...
    def get_table_ddl_pg(dba_conn, username, table, forever_number_to_numeric=True, to_lower=True):
        ...
        if to_lower:
            for index in metadata['indexes']:
                ...
                for idx_col in index['columns']:
                    col_name = idx_col[0]
                    for tab_col in metadata['columns']:
                        if '"' + tab_col['name'] + '"' in col_name:
                            col_name = col_name.replace('"' + tab_col['name'] + '"', '"' + tab_col['name'].lower() + '"')
                    idx_col_list.append((col_name, idx_col[1]))
                index['columns'] = idx_col_list
        ...
        for idx in metadata['indexes']:
            ...
            for col_mess in idx['columns']:
                ...
                if 'case' in col and 'when' in col and 'then' in col:
                    col = '(' + col + ')'
                # 索引字段是||连接
                if '||' in col:
                    col = '(' + col + ')'
                ...
                if 'trunc(' in col:
                    ...
                    col = col.split('trunc')[0] + "date_trunc('day', " + fun_content + col.split(fun_content)[1]
                if 'TRUNC(' in col:
                    fun_content = Tools.get_fun_content(col, 'TRUNC')
                    col = col.split('TRUNC')[0] + "date_trunc('day', " + fun_content + col.split(fun_content)[1]
                # 索引字段是 to_number() 函数
                if 'to_number(' in col:
                    ...
                    col = col.split('to_number')[0] + '(' + fun_content + '::numeric ' + col.split(fun_content)[1]
                if 'TO_NUMBER(' in col:
                    fun_content = Tools.get_fun_content(col, 'TO_NUMBER')
                    col = col.split('TO_NUMBER')[0] + '(' + fun_content + '::numeric ' + col.split(fun_content)[1]
                ...
                if 'to_char(' in col and ',' not in col:
                    ...
                    col = col.split('to_char')[0] + '(' + fun_content + '::varchar ' + col.split(fun_content)[1]
                if 'TO_CHAR(' in col and ',' not in col:
                    fun_content = Tools.get_fun_content(col, 'TO_CHAR')
                    col = col.split('TO_CHAR')[0] + '(' + fun_content + '::varchar ' + col.split(fun_content)[1]
                # 索引字段是 sys_op_c2c() 函数
                if 'sys_op_c2c(' in col:
                    ...
                    col = col.split('sys_op_c2c')[0] + fun_content + col.split(fun_content + ')')[1]
                if 'SYS_OP_C2C(' in col:
                    fun_content = Tools.get_fun_content(col, 'SYS_OP_C2C')
                    col = col.split('SYS_OP_C2C')[0] + fun_content + col.split(fun_content + ')')[1]

                idx_col_list.append((col, col_mess[1]))
            
            if type_ == 'UNIQUE' and metadata['partition'] != None:
                u_idx_col_list = idx_col_list.copy()
                for col in pk_add_cols_list:
                    if (col, 'ASC') not in u_idx_col_list and ('"' + col + '"', 'ASC') not in u_idx_col_list:
                        u_idx_col_list.append((col, 'ASC'))
                cols_list = u_idx_col_list
            else:
                cols_list = idx_col_list
            ...
            cols = ''
            for col_mess in cols_list:
                    cols = cols + col_mess[0] + ' ' + col_mess[1] + ', '
            cols = cols[:-2].replace(' ASC', '')
        ...
        for constraint in metadata['constraints']:
            ...
            if type_ == 'primary_key' or type_ == 'unique':
                ...
                my_cols = my_cols.replace(', ', ',').replace(',', '", "')
            ...
            if type_ == 'primary_key' and metadata['partition'] == None:
                ...
                for col in r_cols.split(','):
                    ...
                    r_cols_ = r_cols_ + '"' + col + '", '
                r_cols_ = r_cols_[:-2]
                k_str = k_str + 'alter table ' + username + '."' + table + '" add constraint "' + name + '" foreign key ("' + my_cols + '") references ' + r_table + ' (' + r_cols_ + ')'
                delete_rule = constraint['delete_rule']
                if delete_rule == 'CASCADE':
                    k_str = k_str + ' on delete cascade'
                elif delete_rule == 'SET NULL':
