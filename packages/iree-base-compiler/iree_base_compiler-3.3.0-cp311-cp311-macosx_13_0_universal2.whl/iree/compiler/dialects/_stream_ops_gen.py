...
class AsyncFuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, tied_operands=None, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_732')) else
      _ods_ir.AttrBuilder.get('anonymous_732')(function_type, context=_ods_context))
    if tied_operands is not None: attributes["tied_operands"] = (tied_operands if (
        isinstance(tied_operands, _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('Util_TiedOpStorageAttr')) else
          _ods_ir.AttrBuilder.get('Util_TiedOpStorageAttr')(tied_operands, context=_ods_context))
...
def resource_alloca(result, result_timepoint, storage_size, *, await_timepoint=None, affinity=None, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return ResourceAllocaOp(result=result, result_timepoint=result_timepoint, storage_size=storage_size, await_timepoint=await_timepoint, affinity=affinity, loc=loc, ip=ip).results
...
def resource_dealloca(operand, operand_size, *, await_timepoint=None, affinity=None, loc=None, ip=None) -> _ods_ir.Value:
  return ResourceDeallocaOp(operand=operand, operand_size=operand_size, await_timepoint=await_timepoint, affinity=affinity, loc=loc, ip=ip).result
...
def tensor_import(result, source, result_encoding, result_encoding_dims, result_size, *, affinity=None, loc=None, ip=None) -> _ods_ir.Value:
  return TensorImportOp(result=result, source=source, result_encoding=result_encoding, result_encoding_dims=result_encoding_dims, result_size=result_size, affinity=affinity, loc=loc, ip=ip).result
