...
def dispatch_tensor_load(result, source, source_dims, offsets, sizes, strides, static_offsets, static_sizes, static_strides, *, loc=None, ip=None) -> _ods_ir.Value:
...
def dispatch_workgroup_count_from_dag_root(operands_, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return DispatchWorkgroupCountFromDagRootOp(operands_=operands_, loc=loc, ip=ip).results
...
def dispatch_workgroup_count_from_slice(operands_, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return DispatchWorkgroupCountFromSliceOp(operands_=operands_, loc=loc, ip=ip).results
...
def dispatch_workload_ordinal(operand, ordinal, *, loc=None, ip=None) -> _ods_ir.Value:
  return DispatchWorkloadOrdinalOp(operand=operand, ordinal=ordinal, loc=loc, ip=ip).result
...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, tied_operands=None, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_654')) else
      _ods_ir.AttrBuilder.get('anonymous_654')(function_type, context=_ods_context))
    if tied_operands is not None: attributes["tied_operands"] = (tied_operands if (
        isinstance(tied_operands, _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('Util_TiedOpStorageAttr')) else
          _ods_ir.AttrBuilder.get('Util_TiedOpStorageAttr')(tied_operands, context=_ods_context))
