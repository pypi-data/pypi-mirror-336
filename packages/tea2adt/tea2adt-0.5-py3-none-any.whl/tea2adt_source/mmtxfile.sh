...
########
echo "*************************************"
echo "*** tea2adt transmitter, input from:"
echo "*** ${PIPE_FILE_OUT}"
echo "*************************************"
        ...
        do
            ...
            if [[ ${chunk_nr} -eq ${nr_splitted_files} ]] ; then
                if [[ ${PREAMBLE} == "" && ${TRAILER} == "" ]] ; then
                    ...
                    echo "${seq_tx_ascii}${seq_rx_ascii}[file_name]${file_name}[file_end]$(<${f} )" | source gpgappend.src
                    if [ "${TRAILER}" != "" ] ; then
                ...
                if [[ ${PREAMBLE} == "" && ${TRAILER} == "" ]] ; then
                    ...
                    echo -n ${PREAMBLE} > ${MSGFILE}
                    echo "${seq_tx_ascii}${seq_rx_ascii}[file_name]${file_name}[file]$(<${f} )" | source gpgappend.src
                    if [ "${TRAILER}" != "" ] ; then
                        echo ${TRAILER} >> ${MSGFILE}
                    fi
                fi
