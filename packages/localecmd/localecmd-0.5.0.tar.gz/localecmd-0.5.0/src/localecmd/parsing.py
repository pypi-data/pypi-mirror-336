...
import shlex
import logging
from typing import Any, Callable, List, Union, Iterable, get_origin, get_args, cast
from babel.numbers import parse_decimal, parse_number

from localecmd.localisation import _, f_, d_, get_language
...
def _convert_onetyped_arg(val: Union[str, list[str]], typ: Callable[[str], Any]):
    """
...
def convert_arg(val: Union[str, list[str]], typ: type):
    """
    Convert string to certain type
    ...
    if get_origin(typ) is Union:
        ...
        types = [
            d_(n.__name__) if hasattr(n, '__name__') else d_(str(n))
            for n in get_args(typ)
        ]
...
def convert_args(func: Function, *args, **kwargs) -> tuple[Iterable, dict]:
    ...
    for arg, val in ba.arguments.items():
        try:
            param = func.signature.parameters[arg]

            if param.kind == param.VAR_POSITIONAL:
                ba.arguments[arg] = tuple(
                    [convert_arg(v, param.annotation) for v in val]
                )
            elif param.kind == param.VAR_KEYWORD:
                ba.arguments[arg] = {
                    k: convert_arg(v, param.annotation) for k, v in val.items()
                }
            else:
                ba.arguments[arg] = convert_arg(val, param.annotation)
        except TypeError as e:
            msg = _("Raised while converting argument '{arg}' of function '{fname}'.")
            e.add_note(msg.format(arg=arg, fname=func.translated_name))
            raise
...
def line_to_args(
    line: str,
) -> tuple[str, list[str], dict[str, Union[str, list[str]]], str]:
    ...
    if kwi:
        args = [word for word in words[0 : kwi[0]]]
...
) -> Union[tuple[Function, Iterable[str], dict[str, Union[str, list[str]]]], None]:
    """
