...
from localecmd.localisation import _, N_, setup_translations, language_list
from localecmd.module import Module
from localecmd.parsing import parse_line, convert_args
...
class CLI:
    ...
    def runcmd(self, line: str, *, localed_input: bool = True) -> None:
        ...
        try:
            ...
            print(_("Error while executing:"), *e.args, repr(e))
...
def change_cli_language(language: str, fallback: bool = False) -> None:
    ...
    if full_lc is None:
        if fallback:
            ...
            msg = N_(
                "Could not switch to language code {language}. "
                "Check that all files are in {path}."
            )
