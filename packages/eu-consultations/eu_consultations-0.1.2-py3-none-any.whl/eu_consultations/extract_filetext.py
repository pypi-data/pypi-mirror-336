...
from eu_consultations.consultation_data import Consultation, DoclingDump
...
def download_consultation_files(
    consultation_data: List[Consultation], output_folder: str | os.PathLike
) -> List[Consultation]:
    """download all files referenced in consultation data and return data with path to files appended

    Args:
        consultation_data: List of Consultation dataclass objects (probably created with eu_consultation_scrape.scrape.async_scrape)
        output_folder: Path to folder to store files in

    Returns:
        List of Consultation dataclass objects with downloaded_filepath attribute set on attachments
    """
    # process all pdfs in a scrape
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    for consultation in tqdm(consultation_data, desc="Consultations"):
...
def extract_text_from_attachments(
    consultation_data_with_attachments: List[Consultation],
    ...
    Args:
        consultation_data_with_attachments: List of Consultation dataclass objects with references to downloaded files set (probably created with download_consultation_files)
    ...
    for consultation in tqdm(consultation_data_with_attachments, desc="Consultations"):
        for feedback in tqdm(
            consultation.feedback,
            desc=f"Feedback for consultation {consultation.id} - {consultation.short_title}",
        ):
            for attachment in feedback.attachments:
                ...
                if stream_out_folder is not None:
                    ...
                    srsly.write_json(
                        os.path.join(
                            docling_out_folder, f"{str(attachment.document_id)}.json"
                        ),
                        docling_dict,
                    )
        if stream_out_folder is not None:
            save_to_json(
                ...
                filename=f"{str(consultation.id)}.json",
            )
    return consultation_data_with_attachments
