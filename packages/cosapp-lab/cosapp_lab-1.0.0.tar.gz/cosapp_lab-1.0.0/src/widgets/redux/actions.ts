...
export function dashboardUpdateComputedResult_(
  ...
  const computedResult =
    action.data.variable ?? state.dashboardState.computedResult;
  const recorderData =
    action.data.recorder ?? state.dashboardState.recorderData;
  const driverData = action.data.driver ?? state.dashboardState.driverData;
  return {
    ...state,
    dashboardState: {
      ...state.dashboardState,
