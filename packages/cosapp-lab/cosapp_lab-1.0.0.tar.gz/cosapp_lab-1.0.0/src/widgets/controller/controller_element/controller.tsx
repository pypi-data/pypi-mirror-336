...
interface AppProps {
  ...
  updateParameterController: (variableName: string, value: number) => void;
}
...
const mapDispatchToProps = (dispatch: (f: any) => void) => {
  return {
    ...
    updateParameterController: (variableName: string, value: number) =>
      dispatch(ReduxAction.dashboardUpdateController(variableName, value))
  };
};
  ...
  on_msg = (data: { type: string; payload: { [key: string]: any } }) => {
    ...
    switch (type) {
      case 'Controller::update_signal': {
        Object.keys(this.props.selectedVariable ?? {}).forEach(key => {
          if (payload?.computed_data?.[key]) {
            this.props.updateParameterController(
              key,
              payload.computed_data[key][1]
            );
          }
        });
        const logMsg: string =
