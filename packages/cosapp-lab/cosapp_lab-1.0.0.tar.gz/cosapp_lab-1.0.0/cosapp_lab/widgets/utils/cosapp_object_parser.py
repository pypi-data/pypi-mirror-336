...
class CosappObjectParser(CosappParser):
    ...
    def set_up_system(self, system: System) -> None:
        self.system_name = system.name
        self._system = ref(system)
        try:
            data = self.extract_system_data(system)
            self.system_dict = replicate_dict_structure(data)
        except:
            self.system_dict = {}

        self._discover_children(system)
        self._get_data_from_system()
        for sys_name in self.children_list:
            self._discover_driver(sys_name)

    @classmethod
    def extract_system_data(cls, system: System) -> dict:
        """Extract system structure and state into a dictionary."""
        data = {}
        if system.__class__ is System:
            data["__class__"] = "System"  # Trick to allow container `System` without special type
        else:
            data["__class__"] = f"{system.__module__}.{system.__class__.__qualname__}"

        if (properties := system.properties):
            data["properties"] = properties

        inputs = {}
        for port in system.inputs.values():
            inputs.update(cls.extract_port_data(port))
        if inputs:
            data["inputs"] = inputs
        ...
        if system.children:
            data["subsystems"] = {
                name: cls.extract_system_data(child)[name]
                for name, child in system.children.items()
            }
            data["exec_order"] = list(system.exec_order)

        return {system.name: data}

    @classmethod
    def extract_port_data(cls, port: BasePort):
        portname = port.name
        return dict(
    ...
    def get_children_var_input(
        ...
        """Get the variables of a port which is not a sink of any connection.
        ...
        for sys_name in self.children_list:
            for driver_name in self.children_drive.get(sys_name, []):
                if driver_name != "None":
                    ...
                    if isinstance(selected_driver, AbstractTimeDriver):
    ...
    def get_data_from_recorder(
        ...
        (AbstractTimeDriver or not), the content of return with be different.
        The tuple returned has the following structure:
        Tuple[str, List, List ]:
            If driver is AbstractTimeDriver:
        ...
        if isinstance(current_driver, AbstractTimeDriver):
