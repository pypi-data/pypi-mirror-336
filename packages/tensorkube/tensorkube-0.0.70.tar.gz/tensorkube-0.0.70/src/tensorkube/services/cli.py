...
from tensorkube.constants import REGION, Events, DEFAULT_NAMESPACE, get_cluster_name, CliColors, \
    get_cfn_base_stack_name, TeardownType
from tensorkube.constants import get_base_login_url
from tensorkube.helpers import sanitise_assumed_role_arn, track_event, sanitise_name
...
from tensorkube.services.aws_service import get_aws_account_id, get_aws_user_arn, \
    get_aws_user_principal_arn, get_session_region, check_and_install_aws_cli, get_existing_bucket_name
...
from tensorkube.services.eks_service import update_eks_kubeconfig, give_eks_cluster_access
...
def configure(vpc_public_subnets, vpc_private_subnets, legacy, test):
    ...
    if vpc_public_subnets and vpc_private_subnets:
        if not legacy:
            info_text = click.style("Configuring Tensorkube with existing VPCs is not supported in this version of configure. Please use the --legacy flag to use the legacy configuration.", fg=CliColors.WARNING.value)
            click.echo(info_text)

            end_time = time.time() * 1000
            track_event(Events.CONFIGURE_END.value,
                        {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "vpc_not_supported"})
            return
    ...
    has_permissions = has_configure_permissions()
    if not has_permissions:
        end_time = time.time() * 1000
        click.echo("Insufficient AWS permissions to run the configure command. Please contact your AWS admin.")
        track_event(Events.CONFIGURE_END.value,
                {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "insufficient_permissions"})
        return

    click.echo("All permissions are in place. Starting configure...")
    if legacy:
        try:
            success = legacy_configure(vpc_public_subnets, vpc_private_subnets)
            if not success:
                return
            ...
            #TODO: add exception handling for ctrl+c
            click.echo(f"Error while configuring Tensorfuse: {e}")
            end_time = time.time() * 1000
            track_event(Events.CONFIGURE_END.value,
                        {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "error"})
            return
    else:
        try:
            ...
            status = get_stack_status_from_stack_name(get_cfn_base_stack_name())
            if status != CfnStackStatus.CREATE_COMPLETE:
                click.echo(f"Error while configuring Tensorfuse. Please check the cloudformation stack [{get_cfn_base_stack_name()}] for more details.")
                end_time = time.time() * 1000
                track_event(Events.CONFIGURE_ERROR.value,
                            {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "error"})
                return
            ...
            click.echo("Error while configuring Tensorfuse. {}".format(e))
            end_time = time.time() * 1000
            track_event(Events.CONFIGURE_END.value,
                        {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "error"})
            return
...
def deploy(gpus, gpu_type, cpu, memory, min_scale, max_scale, env, github_actions, secret, efs, concurrency, config_file, domain_name, port):
    ...
    config.set_if_not_exist('gpus', gpus).set_if_not_exist('gpu_type', gpu_type).set_if_not_exist('cpu',
        ...
        'github_actions', github_actions).set_if_not_exist('secret', secret).set_if_not_exist('domain_name', domain_name).set_if_not_exist('efs', efs).set_if_not_exist('concurrency', concurrency).set_if_not_exist('port', port)
    ...
    deploy_knative_service(env=config.get('env'), gpus=config.get('gpus'),
                           ...
                           concurrency= config.get('concurrency'), pod_status=pod_status, cwd=cwd,
                           sanitised_project_name=sanitised_project_name, image_url=image_url, secrets=depl_secrets,
                           ...
                           readiness = config.get('readiness'), port = config.get('port'))
