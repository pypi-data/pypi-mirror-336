...
class DirectoryComparer:
    ...
    def generate_text_report(self, results: Dict = None) -> str:
        ...
        report = [
            ...
            "=" * 80,
            f"Directory 1: {self.dir1}",
            f"Directory 2: {self.dir2}",
            ...
            f"Missing files (in dir1 but not in dir2): {len(results['missing_files'])}",
            f"Extra files (in dir2 but not in dir1): {len(results['extra_files'])}",
        ...
        if results["missing_files"]:
            report.extend(["-" * 80, "Missing files (in dir1 but not in dir2):", "-" * 80])
            for file in sorted(results["missing_files"]):
                report.append(f"  {file}")

        # Add list of extra files
        if results["extra_files"]:
            report.extend(["-" * 80, "Extra files (in dir2 but not in dir1):", "-" * 80])
    ...
    def generate_html_report(self, results: Dict = None) -> str:
        ...
        <body>
            <div class="container">
                ...
                <div class="details">
                    ...
                    {% if missing_files %}
                    <div class="section">
                        <h2>Missing Files (in dir1 but not in dir2)</h2>
                    ...
                    {% if extra_files %}
                    <div class="section">
                        <h2>Extra Files (in dir2 but not in dir1)</h2>
                ...
                <div class="footer">
                    <p>Generated by <a href="{{ repo_url }}" target="_blank">{{ repo_name }}</a> v{{ version }}</p>
                </div>
            </div>
        </body>
        </html>
        ...
        template_data = {
            ...
            "repo_url": "https://github.com/ethan-li/hpfc",
            "version": __version__
        }
