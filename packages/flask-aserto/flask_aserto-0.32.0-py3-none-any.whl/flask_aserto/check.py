...
def build_resource_context_mapper(opts: CheckOptions) -> ResourceMapper:
    def resource() -> ResourceContext:
        ...
        obj.object_id = (
            opts.object_id_mapper() if opts.object_id_mapper is not None else obj.object_id
        )

        relation = opts.relation_mapper() if opts.relation_mapper is not None else opts.relation
    ...
    def _build_policy_path_mapper(self) -> StringMapper:
        def mapper() -> str:
            ...
            if self._options.policy_path_mapper is not None:
                policy_path = self._options.policy_path_mapper()
            if policy_path == "":
                policy_path = "check"
