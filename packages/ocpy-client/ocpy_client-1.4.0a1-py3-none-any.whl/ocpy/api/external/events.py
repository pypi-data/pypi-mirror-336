...
class Event:
    ...
    def delete(self, **kwargs):
        ...
        if res.ok:
            return "ok"
        raise OcPyRequestException(
            "Could not delete event (" + self.get_identifier() + ")! (" + res.text + ")"
        )
...
def create_test_event(ev_api):
    ...
    return ev_api.create_event(test_acl, test_meta, processing_info, test_video_file)
...
def main():
    ...
    print(evs)
    print(len(evs))
    acl = api.get_events(limit=1, offset=0, with_publications=True, with_metadata=True)[
        0
    ].get_acl()

    print(
        api.get_events(limit=1, offset=0, with_publications=True, with_metadata=True)[
            0
        ].get_acl()
    )
    try:
