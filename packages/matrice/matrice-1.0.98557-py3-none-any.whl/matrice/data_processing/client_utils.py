...
ANNOTATION_EXTENSIONS = [".json", ".txt", ".xml", ".ndjson", ".yaml", ".csv"]
SAMPLES_EXTENSIONS = [
    # Image formats
...
def get_youtube_bb_partitions(video_files):
    ...
    # Group files by video identifier
    video_groups = {}
    ...
    # Sort each video group by frame number
    for video_id in video_groups:
        video_groups[video_id].sort(key=lambda x: x[1])  # Sort by frame number
    ...
    for video_id, frames in video_groups.items():
        ...
        # If adding this video's frames exceeds max partition size, start a new partition
        ...
        # Add the whole video frames to the current partition
        current_partition[video_id] = [file_path for file_path, _, _ in frames]
        current_size += total_video_size

    # Add any remaining files in the last partition
    if current_partition:
        partitions.append(
            create_partition(current_partition, current_size, partition_num)
        )

    logging.debug(f"Created {len(partitions)} video partitions")
    return partitions
...
def get_cloud_file_path(dataset_id, dataset_version, base_dataset_path, file_path, include_version_in_cloud_path=False):
    # Convert to absolute paths to avoid misalignment issues
    ...
    for attempt in range(max_attempts):
        try:
            with open(local_path, "rb") as f:
                response = requests.put(presigned_url, data=f, allow_redirects=True)
