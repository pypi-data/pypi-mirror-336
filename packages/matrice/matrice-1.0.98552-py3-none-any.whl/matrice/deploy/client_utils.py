...
class FastAPIClientUtils():
    ...
    def inference(self, client, input_data):
        try:
            logging.debug(f"Making inference request to {client['url']}")
            resp = self.http_client.post(
                    url=client["url"], files={"input": input_data}
                ).json()
            if "result" in resp:
    ...
    async def async_inference(self, client, input_data):
        try:
...
class TritonClientUtils():
    ...
    async def async_inference(self, client, input_data): # TODO: test it
        try:
