...
from matrice.data_processing.data_prep import dataset_items_producer
from matrice.data_processing.client import (
    add_batch_presigned_upload_urls,
    upload_batch_files,
)
from matrice.data_processing.server_utils import download_file, generate_short_uuid
...
from matrice.data_processing.image_augmentations import get_augmentation_compose
    ...
    for dataset_item in batch_dataset_items:
        ...
        for augmentation_fn in augmentation_fns:
            ...
            augmented_dataset_item['augmentation_result'] = augmentation_fn(
                image=image,
                bboxes=bboxes,
                class_labels=[None for _ in range(len(bboxes))] # Placeholder labels
            )
...
def save_augmented_images(batch_dataset_items, dataset_id):
    for item in batch_dataset_items:
        ...
        cv2.imwrite(new_filename, augmentation_result['image'])
...
def download_images(dataset_items: List[Dict]) -> List[Dict]:
    ...
    for dataset_item in dataset_items:
        try:
            download_file(dataset_item["fileLocation"], dataset_item["filename"])
...
def batch_insert_dataset_items(batch, dataset_id, rpc):
    logging.debug(f"batch to insert: {batch}")
    payload = {
        "datasetId": dataset_id,
        "datasetItems": batch,
    }
    resp = rpc.post("/v2/dataset/insert_dataset_items", payload=payload) # TODO: Get a live api from the BE for this
    logging.info(f"Response from insert: {resp}")
    return batch
...
def calculate_partition_stats(dataset_items: List[Dict]) -> List[Dict]:
    ...
    for item in dataset_items:
        ...
        partition_stats[item["partitionNumber"]]["diskSizeMB"] += os.path.getsize(item["local_file_path"]) / (1024 * 1024)
...
class DataAugmentation:
    ...
    def start_processing(self) -> None:
        ...
        update_partitions_numbers(self.rpc, self.dataset_id, new_dataset_items, partition_key="partitionNumber")
        create_partition_stats(
            rpc=self.rpc,
            dataset_id=self.dataset_id,
        ...
        batch_insert_dataset_items(new_dataset_items, self.dataset_id, self.rpc)
