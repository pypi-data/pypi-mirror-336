...
def download_video_samples(image_details: Dict[str, Any], rpc: Any, bucket_alias: str="", account_number: str="") -> Dict[str, Any]:
    """Download sample and update sample details.
...
def batch_download_samples(
    ...
    return [
        download_video_samples(image_details, rpc, bucket_alias, account_number) for image_details in batch_image_details
    ]
    ...
    while retry_count < attempts:
        try:
            ...
            for dataset_item in batch_image_details:
                frame_annotations = {}
                annotations = dataset_item.get('annotation', {})
                    ...
                    for ann in anns:
                        frame_annotations[frame_id].append({
                            ...
                            "bbox": [
                                ann['bbox'][0],  # xmin
                                ann['bbox'][1],  # ymin
                                ann['bbox'][2] + ann['bbox'][0],  # xmax
                                ann['bbox'][3] + ann['bbox'][1]   # ymax
                            ],
                ...
                item = {
                    "datasetItemId": str(dataset_item.get("datasetItemId")),
                    "version": str(version),
                    "splitType": str(dataset_item.get("splitType")),
                    "frameWiseAnnotations": frame_annotations,
                    "height": dataset_item.get("height"),
                    "width": dataset_item.get("width"),
                    "area": dataset_item.get("area"),
                    "fps": dataset_item.get("fps")
                }
                items.append(item)
