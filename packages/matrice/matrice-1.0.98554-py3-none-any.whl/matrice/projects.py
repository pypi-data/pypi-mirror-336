...
class Projects:
    ...
    def _create_dataset(
        ...
        """
        assert (
            source_url or dataset_path
        ), "Either source_url or dataset_path must be provided"
        ...
        if source_type == "lu":
            if not is_dataset_compressed:
                skip_annotation_pipeline = (
                    ...
                    or (input_type == "labelbox" and not source_type == "url")
                    or (input_type == "labelbox_classification" and not source_type == "url")
                )
                partition_stats = get_partition_status(
                    dataset_path,
                    skip_annotation_pipeline
                )
            else:
                source_url = upload_compressed_dataset(self.session.rpc, dataset_path)
        elif source_type == "url":
            try:
                dataset_size, err, msg = get_dataset_size_in_mb_from_url(self.session, source_url, self.project_id)
                if err:
                    dataset_size = 0
        ...
        resp = self.rpc.post(path=path, headers=headers, payload=create_dataset_request)
        ...
        if resp["success"]:
            ...
            if source_type == "lu" and not is_dataset_compressed:
                ...
                print("Dataset items uploaded successfully")

            return Dataset(self.session, dataset_id=dataset_id)
        else:
            raise Exception(f"Dataset creation failed: {resp.get('message', 'Unknown error')}")
    ...
    def import_cloud_dataset(
        ...
        Example
        -------
        >>> dataset = project.import_cloud_dataset("MyCloudDataset", "http://source.url", "AWS", "image")
        >>> print(f"Dataset created: {dataset}")
        """
        return self._create_dataset(
            dataset_name=dataset_name,
    ...
    def add_models_for_training(self,
                                model_train_configs,
                                primary_metric,
                                dataset_id = None,
                                dataset_name = None,
                                ...
                                compute_alias=""): # TODO_experiment: Test
        """
        ...
        if not (dataset_id and dataset_name):
            dataset = Dataset(session=self.session, dataset_id=dataset_id, dataset_name=dataset_name)
    ...
    def _create_deployment(
        ...
        Example
        -------
        >>> deployment, action = project._create_deployment("Deployment1", "model123", auto_scale=True)
        >>> if action:
    ...
    def get_trained_models(self):
        ...
        resp = self.rpc.get(path=path)
        return handle_response(
            resp, "Model trains fetched successfully", "Could not fetch model trains"
        )
