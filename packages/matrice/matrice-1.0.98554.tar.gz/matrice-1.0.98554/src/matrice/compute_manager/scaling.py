...
class Scaling:
    ...
    def __init__(self, session, instance_id=None):
        assert instance_id is not None, "Instance id not set for this instance. Cannot perform the operation for job-scheduler without instance id"
    ...
    def get_docker_hub_credentials(self):
        ...
        resp = self.rpc.get(path=path)
        return self.handle_response(
            resp, "Docker credentials fetched successfully", "Could not fetch the docker credentials"
        )
    
    def get_open_port(self):
        logging.info("Getting open port")
        try:
            port_start, port_end = 8200, 9000 # TODO: Get from API
            ...
            log_error("scaling.py", "get_open_port", e)
            logging.error(f"Error getting open port: {str(e)}")
            return None
    ...
    def add_account_compute_instance(self, account_number, alias, service_provider, instance_type, shut_down_time, lease_type, launch_duration):
        ...
        return self.handle_response(
            resp, "Compute instance added successfully", "Could not add the compute instance"
        )
    ...
    def stop_account_compute(self, account_number, alias):
        ...
        return self.handle_response(
            resp, "Compute instance stopped successfully", "Could not stop the compute instance"
        )
    ...
    def restart_account_compute(self, account_number, alias):
        ...
        resp = self.rpc.put(path=path)
        return self.handle_response(
            resp, "Compute instance restarted successfully", "Could not restart the compute instance"
        )
    ...
    def delete_account_compute(self, account_number, alias):
        ...
        return self.handle_response(
            resp, "Compute instance deleted successfully", "Could not delete the compute instance"
        )
