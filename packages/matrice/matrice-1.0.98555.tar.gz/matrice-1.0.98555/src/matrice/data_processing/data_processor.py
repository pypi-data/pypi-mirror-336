...
from matrice.data_processing.server_utils import get_number_of_dataset_batches
...
class DataProcessor:
    ...
    def update_status(self, stepCode, status, status_description, sample_count=None):
        try:
    ...
    def get_server_processing_pipeline(self) -> Pipeline:
        if self.input_type in self.PROCESSING_PIPELINES:
            ...
            logging.info(f"Processing {self.input_type} dataset")
            server_processing_pipeline = self.VIDEO_PROCESSING_PIPELINES[
                self.input_type
            ](
                self.rpc,
                self.dataset_id,
    ...
    def start_processing(self):
        if "labelbox" in self.input_type:
            ...
            dataset_path = handle_source_url_dataset_download(
                self.source_URL,
                self.input_type,
                self.dataset_id,
                self.rpc,
        ...
        self.update_status(
            ...
            sample_count=get_number_of_dataset_batches(
                self.rpc, self.dataset_id, self.target_version
            ),
        )
