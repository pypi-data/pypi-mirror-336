...
class AnthropicAPI(VendorBase):
    ...
    async def _hit_api_async(
        ...
        **vendor_params: Dict[str, Any],
        ...
        api_params = {
            ...
            "messages": messages[1:],
            "model": model,
            "max_tokens": lm_config.get("max_tokens", 8192 * 2),  # Default to 16384
            ...
            if "thinking" in create_sig.parameters and "claude-3-7" in model:
                if reasoning_effort in ["high", "medium"]:
                    ...
                    api_params["max_tokens"] = max(
                        api_params["max_tokens"], budget + 8192
                    )
    ...
    def _prepare_api_params(self, messages, **kwargs):
        # Ensure max_tokens is at least thinking budget + 8192
        if "max_tokens" not in kwargs:
            kwargs["max_tokens"] = 8192 * 2  # Default to 2x context window

        if "thinking" in kwargs and kwargs.get("thinking", {}).get("budget_tokens"):
            thinking_budget = kwargs["thinking"]["budget_tokens"]
            kwargs["max_tokens"] = max(kwargs["max_tokens"], thinking_budget + 8192)

        api_params = {
            "messages": formatted_messages,
            "model": self.model_name,
            "max_tokens": kwargs.get("max_tokens", 8192 * 2),
            # ... rest of params ...
        }
