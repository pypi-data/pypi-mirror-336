...
def ups_scalers(jwt_token, creation_id, enhance, duration):
    url = "https://service.vidu.com/vidu/v1/tasks"
    headers = {
        "Connection": "keep-alive",
        "X-Request-Id": "1ebd3240-8c96-4d74-9bae-26414a2cd419",
...
def download_ups_video(url, save_path, video_name, jwt_token, task_id, task_up_id):
    try:
        ...
        eliminar_tarea(task_up_id, jwt_token)
...
def get_credits_from_api(jwt_token, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos, escalar):
    ...
    try:
        ...
        if creditos <= credits:
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, escalar)
            ...
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token1, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, escalar)
            print("Generate Image to Video...")
            print("Credits:", credits)
...
def ups_proceso_tarea(jwt_token, task_id, task_up_id):
            ...
            if data.get("state") == "success":
                ...
                for creation in creations:
                    if creation.get("type") == "video" and "uri" in creation:
                        ...
                        download_ups_video(video_url, save_directory, video_filename, jwt_token, task_id, task_up_id)
...
def procesoo_tarea(jwt_token, task_id, escalar):
                ...
                for creation in creations:
                    if creation.get("type") == "video" and "uri" in creation:
                        ...
                        if escalar:
                            ...
                            enhance = False
                            duration = 4
                            task_up_id = ups_scalers(jwt_token, creation_id, enhance, duration)
                            ups_proceso_tarea(jwt_token, task_id, task_up_id)
                        else:
...
def enviar_tarea(jwt_token, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, escalar):
    url = "https://service.vidu.com/vidu/v1/tasks"

    headers = {
        "Host": "service.vidu.com",
        "Connection": "keep-alive",
        "X-Request-Id": "8f3cc549-2178-4078-96cc-9a24cfe1d3ec",
        "sec-ch-ua-platform": "\"Windows\"",
        "Accept-Language": "en",
        "x-recaptcha-token": "Task_Submit",
        "sec-ch-ua": "\"Not A(Brand\";v=\"8\", \"Chromium\";v=\"132\", \"Google Chrome\";v=\"132\"",
    ...
    try:
        ...
        if 'message' in response_data and 'insufficient credits' in response_data['message']:
            ...
            API_KEY = os.environ.get("JWT_TOKEN")
            jwt_token1=API_KEY
            enviar_tarea(jwt_token1, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, escalar)
            ...
            if task_id:
                result = procesoo_tarea(jwt_token, task_id, escalar)
...
def text_to_video_15_20(content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos, escalar):
    ...
    if API_KEY:
        ...
        jwt_token=API_KEY
        # Llamar a la función y mostrar el resultado
        credits = get_credits_from_api(jwt_token, content, enhance, request_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos, escalar)
