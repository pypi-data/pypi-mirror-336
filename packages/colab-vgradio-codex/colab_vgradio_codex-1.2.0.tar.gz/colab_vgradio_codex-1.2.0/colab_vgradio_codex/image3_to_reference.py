...
def get_credits_from_api(jwt_token, content,
    image1_id,
    image2_id,
    image3_id,
    x1, y1,
    x2, y2,
    x3, y3,
    enhance, task_type, style,
    duration, resolution, movement_amplitude, aspect_ratio,
    sample_count, schedule_mode, model_version, creditos):
    ...
    try:
        ...
        if creditos <= credits:
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token,
                content,
                image1_id,
                image2_id,
                image3_id,
                ...
                enhance, task_type, style,
                duration, resolution, movement_amplitude, aspect_ratio,
                sample_count, schedule_mode, model_version
            )
            ...
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token1,
                content,
                image1_id,
                image2_id,
                image3_id,
...
def enviar_tarea(jwt_token, content, image1_id, image2_id, image3_id,
                 x1, y1, x2, y2, x3, y3, enhance, task_type, style,
                 duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version):
    ...
    data = {
        "input": {
            "prompts": [
                ...
                {
                    "type": "image",
                    "content": f"ssupload:?id={image1_id}",
                    "src_imgs": [f"ssupload:?id={image1_id}"],
                    "selected_region": {"top_left": {"x": 0, "y": 0}, "bottom_right": {"x": x1, "y": y1}}
                },
                {
                    "type": "image",
                    "content": f"ssupload:?id={image2_id}",
                    "src_imgs": [f"ssupload:?id={image2_id}"],
                    "selected_region": {"top_left": {"x": 0, "y": 0}, "bottom_right": {"x": x2, "y": y2}}
                },
                {
                    "type": "image",
                    "content": f"ssupload:?id={image3_id}",
                    "src_imgs": [f"ssupload:?id={image3_id}"],
                    "selected_region": {"top_left": {"x": 0, "y": 0}, "bottom_right": {"x": x3, "y": y3}}
                }
            ],
            "enhance": enhance
        },
    ...
    try:
        ...
        if 'message' in response_data and 'insufficient credits' in response_data['message']:
            ...
            API_KEY = os.environ.get("JWT_TOKEN")
            jwt_token1=API_KEY
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token1, content, image1_id, image2_id, image3_id,
                 x1, y1, x2, y2, x3, y3, enhance, task_type, style,
                 duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version)
            os.environ["TASK_ID"] = task_id
            os.environ["VIDEO_URL"] = video_url
...
def img3_to_reference(content,
    ...
    if API_KEY:
        ...
        jwt_token=API_KEY
        # Llamar a la función y mostrar el resultado
        credits = get_credits_from_api(jwt_token,
            content,
            image1_id,
            image2_id,
            image3_id,
