...
def get_credits_from_api(jwt_token, content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos):
    ...
    try:
        ...
        if creditos <= credits:
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token, content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version)
            ...
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token1, content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version)
            os.environ["TASK_ID"] = task_id
            os.environ["VIDEO_URL"] = video_url
            os.environ["ADDITIONAL_ID"] = additional_id
            print("Generate Image to Video...")
...
def enviar_tarea(jwt_token, content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version):
    ...
    data = {
        "input": {
            "prompts": [
                ...
                {"type": "image", "content": f"ssupload:?id={image_id}", "src_imgs": [f"ssupload:?id={image_id}"]}
            ],
            "enhance": enhance
        },
        "type": task_type,
...
def img_to_video_15_20(content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos):
    ...
    if API_KEY:
        ...
        jwt_token=API_KEY
        # Llamar a la función y mostrar el resultado
        credits = get_credits_from_api(jwt_token, content, image_id, enhance, task_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos)
