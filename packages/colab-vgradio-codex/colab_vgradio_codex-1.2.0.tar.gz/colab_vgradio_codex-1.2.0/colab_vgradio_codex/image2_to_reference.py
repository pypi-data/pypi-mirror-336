#@title 3 imagen to vide referencia 2.0 .15 multi procesos
...
def get_credits_from_api(jwt_token, content, image_id_1, image_id_2, x2, y2, image_id_3, image_id_4, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos):
    ...
    try:
        ...
        if creditos <= credits:
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token, content, image_id_1, image_id_2, x2, y2, image_id_3, image_id_4, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version)
            ...
            print("Generate Image 3 to Video Refrence...")
            ...
            task_id, response_contents, video_url, additional_id = enviar_tarea(jwt_token1, content, image_id_1, image_id_2, x2, y2, image_id_3, image_id_4, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version)
            os.environ["TASK_ID"] = task_id
            os.environ["VIDEO_URL"] = video_url
            os.environ["ADDITIONAL_ID"] = additional_id
            print("Generate Image 3 to Video Refrence...")
...
def procesoo_tarea(jwt_token, task_id):
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            print(f"Error en la solicitud. Código: {response.status_code}. Detalle: {response.text}")

        # Esperar 10 segundos antes del próximo intento
        time.sleep(10)

def enviar_tarea(jwt_token, content, image_id_1, image_id_2, x2, y2, image_id_3, image_id_4, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version):
    ...
    data = {
        "input": {
            "prompts": [
                ...
                {
                    "type": "image",
                    "content": f"ssupload:?id={image_id_1}",
                    "src_imgs": [f"ssupload:?id={image_id_2}"],
                    "selected_region": {
                        "top_left": {"x": 0, "y": 0},
                ...
                {
                    "type": "image",
                    "content": f"ssupload:?id={image_id_3}",
                    "src_imgs": [f"ssupload:?id={image_id_4}"],
                    "selected_region": {
                        "top_left": {"x": 0, "y": 0},
                        "bottom_right": {"x": x4, "y": y4}
                    }
                }
            ],
            "enhance": enhance
...
def img2_to_reference(content, image_id_1, x2, y2, image_id_3, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos):
    ...
    if API_KEY:
        ...
        jwt_token=API_KEY
        # Llamar a la función y mostrar el resultado
        credits = get_credits_from_api(jwt_token, content, image_id_1, image_id_1, x2, y2, image_id_3, image_id_3, x4, y4, enhance, media_type, style, duration, resolution, movement_amplitude, aspect_ratio, sample_count, schedule_mode, model_version, creditos)
