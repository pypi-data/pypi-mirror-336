...
"""
from dartsdwell.two_phase.define_pipe_geometry import PipeGeometry
from dartsdwell.two_phase.define_fluid_model import FluidModel
from dartsdwell.utilities.units import *
...
class WellLateralHeatTransfer:
    def __init__(self, pipe_name: str, pipe_geometry: PipeGeometry, earth_thermal_props: dict, outermost_layer_OD: float,
        ...
        Availabe options are "Ramey" and "Chiu&Thakur". Default is "Chiu&Thakur"
...
class ConstantMassRateSource:
    def __init__(self, pipe_name: str, pipe_geom: PipeGeometry, fluid_model: FluidModel, segment_index: int,
                 start_time: float, stop_time, flow_direction: list, mass_rate: float, isothermal: bool,
                 injected_fluid_mole_fractions: list = None, injected_fluid_specific_enthalpy: float = None, verbose: bool = False):
        """
        ...
        :param flow_direction: First element of the list is either "inflow" or "outflow". Second element of the list is
        either "along the pipe" or "perpendicular to the pipe"
        :type flow_direction: list
        :param mass_rate:
        :type mass_rate: float
        :param isothermal: Whether or not the system is isothermal
        :type isothermal: bool
        :param injected_fluid_mole_fractions: The composition of the injected fluid [list of mole fractions]. If
        flow_direction is outflow, this input argument is not needed.
        :type injected_fluid_mole_fractions: list
        :param injected_fluid_specific_enthalpy: The specific enthalpy of the injected fluid. If flow_direction is
        outflow, this input argument is not needed.
        :type injected_fluid_specific_enthalpy: float
        :param verbose: Whether to display extra info about ConstantMassRateSource
        :type verbose: boolean
        """
        assert pipe_geom.pipe_name == pipe_name, \
            "The names of the pipes for the geometry and ConstantMassRateSource are not identical!"
        assert fluid_model.pipe_name == pipe_name, \
            "The names of the pipes for the fluid model and ConstantMassRateSource are not identical!"
        ...
        assert len(flow_direction) == 2, "Flow direction must be a 2-element list!"
        assert flow_direction[0] in ["inflow", "outflow"], \
            "First element of flow_direction must be either inflow or outflow!"
        assert flow_direction[1] in ["along the pipe", "perpendicular to the pipe"], \
            "Second element of flow_direction must be either along the pipe or perpendicular to the pipe!"
        self.flow_direction = flow_direction

        if flow_direction[0] == "inflow":
            assert injected_fluid_mole_fractions is not None, \
                "If flow_direction is inflow, injected_fluid_mole_fractions is needed!"
            assert (1 - 1e-10) <= sum(injected_fluid_mole_fractions) <= (1 + 1e-10), \
                "Summation of injected fluid mole fractions must be equal to 1!"
            assert len(injected_fluid_mole_fractions) == fluid_model.num_components, \
                "Number of specified injected fluid mole fractions must be equal to the number of components in the fluid model!"
            self.injected_fluid_mole_fractions = np.array(injected_fluid_mole_fractions)

            # Calculate mass fractions of components
            self.injected_fluid_mass_fractions = self.injected_fluid_mole_fractions * fluid_model.MW / sum(self.injected_fluid_mole_fractions * fluid_model.MW)

            assert isinstance(mass_rate, (int, float)), "A single total mass rate must be provided!"
            self.mass_rate = mass_rate
            self.injected_fluid_components_mass_rates = mass_rate * self.injected_fluid_mass_fractions

            if not isothermal:
                assert injected_fluid_specific_enthalpy is not None, \
                    "If flow_direction is inflow and model is non-isothermal, injected_fluid_specific_enthalpy is needed."
                assert isinstance(injected_fluid_specific_enthalpy, (int, float)), \
                    "A single total injected fluid specific enthalpy must be provided."
                self.injected_fluid_specific_enthalpy = injected_fluid_specific_enthalpy
                self.injected_fluid_enthalpy_rate = mass_rate * injected_fluid_specific_enthalpy

        elif flow_direction[0] == "outflow":
            assert injected_fluid_mole_fractions is None, \
                "If flow_direction is outflow, injected_fluid_mole_fractions is not needed!"
            assert injected_fluid_specific_enthalpy is None, \
                "If flow_direction is outflow, injected_fluid_specific_enthalpy is not needed!"
    ...
    def evaluate_components_mass_rate(self, simulation_timer, zc_segment):
        """
        :param simulation_timer: Simulation time up to now
        :param zc_segment: Overall mole fractions of components in the segment for which the ConstantMassRateSource
        is defined.
        """
        source_bool = ((self.stop_time == "end" and self.start_time <= simulation_timer) or
                       (self.start_time <= simulation_timer <= self.stop_time))
        if source_bool == True:
            if self.flow_direction[0] == "inflow":
                return self.injected_fluid_components_mass_rates
            elif self.flow_direction[0] == "outflow":
                segment_fluid_mass_fractions = zc_segment * self.fluid_model.MW / sum(zc_segment * self.fluid_model.MW)
                segment_fluid_components_mass_rates = self.mass_rate * segment_fluid_mass_fractions
                return segment_fluid_components_mass_rates
        else:
            return np.zeros(self.fluid_model.num_components)
    ...
    def evaluate_enthalpy_rate(self, simulation_timer):
        ...
        if source_bool == True:
            if self.flow_direction[0] == "inflow":
                return self.injected_fluid_enthalpy_rate
            elif self.flow_direction[0] == "outflow":
    ...
    def evaluate_specific_kinetic_energy0(self, simulation_timer, sG0, rhoG0, rhoL0):   # I'm not sure if this simulation_timer with previous time step (0) is going to work correctly
        """
        :param simulation_timer: Simulation time up to now
        :param sG0: The saturation of the gas at the previous time step in the segment for which the ConstantMassRateSource is defined.
        :param rhoG0: The density of the gas at the previous time step in the segment for which the ConstantMassRateSource is defined.
        :param rhoL0: The density of the liquid at the previous time step in the segment for which the ConstantMassRateSource is defined.
        :return:
        """
        source_bool = ((self.stop_time == "end" and self.start_time <= simulation_timer) or
                       (self.start_time <= simulation_timer <= self.stop_time))
...
class Perforation:
    ...
    """
    def __init__(self, pipe_name, segment_index, start_time, stop_time, pipe_geom, fluid_model, reservoir_pressure, reservoir_fluid_mole_fractions,
                 reservoir_cell_perm_list, reservoir_cell_size_list, perforation_hole_diameter=None, skin=0, verbose: bool = False):
        """
        ...
        :param reservoir_fluid_mole_fractions: The composition of the reservoir fluid [a list of mole fractions]
        :type reservoir_fluid_mole_fractions: list
        ...
        assert fluid_model.pipe_name == pipe_name, \
            "The names of the pipes for the fluid model and Perforation are not identical!"
        self.fluid_model = fluid_model
        self.pipe_name = pipe_name
        ...
        assert (1 - 1e-10) <= sum(reservoir_fluid_mole_fractions) <= (1 + 1e-10), \
            "Summation of reservoir fluid mole fractions must be equal to 1!"
        assert len(reservoir_fluid_mole_fractions) == fluid_model.num_components, \
            "Number of specified reservoir fluid mole fractions must be equal to the number of components in the fluid model!"
        self.reservoir_fluid_mole_fractions = np.array(reservoir_fluid_mole_fractions)

        # Calculate mass fractions of components
        self.reservoir_fluid_mass_fractions = self.reservoir_fluid_mole_fractions * fluid_model.MW / sum(self.reservoir_fluid_mole_fractions * fluid_model.MW)
    ...
    def evaluate_components_mass_rate(self, simulation_timer, iter_counter, total_iter_counter, flag, segment_pressure,
                                      xG_mass_up, xL_mass_up, rhoG_up_at_perf, rhoL_up_at_perf, miuG_up_at_perf,
                                      miuL_up_at_perf, KrG_up_at_perf, KrL_up_at_perf, flow_direction):
        """
        :param simulation_timer: Simulation time up to now
        :param iter_counter:
        :param total_iter_counter:
        :param flag:
        ...
        if perforation_bool == True:
            ...
            rw = self.well_radius
            # This paper "Advanced well model for superhot and saline geothermal reservoirs" does not use any -1/2
            ...
            if rhoG_up_at_perf != 0 and miuG_up_at_perf != 0:
                well_index_gas = (rhoG_up_at_perf * KrG_up_at_perf / miuG_up_at_perf) * 2 * np.pi * K * dz / (np.log(re / rw) + self.skin)
            elif rhoG_up_at_perf == 0 and miuG_up_at_perf == 0:
                well_index_gas = 0
            else:
                raise Exception("There is a problem with the calculated phase density and viscosity values!")

            if rhoL_up_at_perf != 0 and miuL_up_at_perf != 0:
                well_index_liquid = (rhoL_up_at_perf * KrL_up_at_perf / miuL_up_at_perf) * 2 * np.pi * K * dz / (np.log(re / rw) + self.skin)
            elif rhoL_up_at_perf == 0 and miuL_up_at_perf == 0:
                well_index_liquid = 0
            else:
                raise Exception("There is a problem with the calculated phase density and viscosity values!")
            ...
            if flow_direction == "inflow":
                total_mass_rate = gas_mass_rate_at_perf + liquid_mass_rate_at_perf
                self.components_mass_rates = total_mass_rate * self.reservoir_fluid_mass_fractions

            elif flow_direction == "outflow":
                self.components_mass_rates = xG_mass_up * gas_mass_rate_at_perf + xL_mass_up * liquid_mass_rate_at_perf

        elif perforation_bool == False:
            # Store mass rates in self in order to use in evaluate_enthalpy_rate method
            self.gas_mass_rate_at_perf = 0
            self.liquid_mass_rate_at_perf = 0
            self.components_mass_rates = np.zeros(self.fluid_model.num_components)

        return self.components_mass_rates
    ...
    def evaluate_enthalpy_rate(self, hG_up_at_perf, hL_up_at_perf):
        ...
        """
        # Because evaluate_components_mass_rate is called before this method, gas_mass_rate_at_perf and
        # liquid_mass_rate_at_perf are already stored in the instance of the class.
    ...
    def evaluate_specific_kinetic_energy0(self, sG0_up, sL0_up, rhoG0_up, rhoL0_up):
        ...
        if sG0_up == 0:
            vG0 = 0
            vL0 = liquid_mass_rate0 / rhoL0_up / (perforation_hole_area * 1)
        elif sG0_up == 1:
            vL0 = 0
            vG0 = gas_mass_rate0 / rhoG0_up / (perforation_hole_area * 1)
        elif 0 < sG0_up < 1:
            vG0 = gas_mass_rate0 / rhoG0_up / (perforation_hole_area * sG0_up)
            vL0 = liquid_mass_rate0 / rhoL0_up / (perforation_hole_area * (1 - sG0_up))
        else:
            raise Exception("sG0_up is out of correct range (from 0 to 1)!")
