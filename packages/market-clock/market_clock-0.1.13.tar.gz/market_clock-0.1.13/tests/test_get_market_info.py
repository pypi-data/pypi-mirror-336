...
def test_required_info():
    for market, info in ALL_MARKET_INFO.items():
        ...
        assert isinstance(info["half_days"], set), (
            f"Half days for {market} is not a set."
        )
...
def test_no_duplication_holiday_halfday():
    for market, info in ALL_MARKET_INFO.items():
...
def test_outdated_holiday_halfday():
    for market, info in ALL_MARKET_INFO.items():
...
def test_start_half_end_time_order():
    for market, info in ALL_MARKET_INFO.items():
...
def test_lunch_break_is_specified():
    for market, info in ALL_MARKET_INFO.items():
...
def test_half_day_end_is_specified():
    for market, info in ALL_MARKET_INFO.items():
        if info["half_days"]:
