...
class Group_Orgunits:
    ...
    def save_orgunits_to_csv(self, filepath: str):
        ...
        sample = self.members[0]
        orgunits = self.members
        valid_attrs = [
            attr
            for attr in dir(sample)
            if not attr.startswith("_")
            and not callable(getattr(sample, attr))
            and isinstance(getattr(sample, attr), (int, float, str))
        ]
        ...
        with open(filepath, mode="w", newline="", encoding="utf-8") as file:
            writer = csv.DictWriter(file, fieldnames=valid_attrs)
            writer.writeheader()
            ...
            for obj in orgunits:
                row = {
                    ...
                    if isinstance(getattr(obj, attr), (int, float, str))
                }
                writer.writerow(row)
...
class Orgunit:
    ...
    def get_gain_verif_for_period_verif(self, taux_validation):
        ...
        if self.id == "HxCZ2481wSw":
            quantite_period_verif_total.to_csv(
                f"{workspace.files_path}/PBF burundi extraction/data/debug/{self.id}_{self.period}.csv"
            )
            ...
            for dx in quantite_period_verif_total.service.unique():
                ...
                if len(quantite_period_verif) > 0:
                    ...
                    if taux_validation < 1:
                        quantite_period_verif[
                            ...
                            * list(
                            ...
                            > 0
                            else x.subside_sans_verification_method_dpdt
                            * self.taux_validation_par_service[
                                "taux_validation"
                            ].mean(),
                            axis=1,
                    ...
                    if self.id == "HxCZ2481wSw":
                        print(
                            dx,
                            quantite_period_verif.dec,
                            quantite_period_verif.val,
                            quantite_period_verif.tarif,
                        )
                        print(
                            list(
                                self.taux_validation_par_service[
    ...
    def get_taux_validation_median(self):
        ...
        self.taux_validation_par_service = self.quantite_window.groupby(
