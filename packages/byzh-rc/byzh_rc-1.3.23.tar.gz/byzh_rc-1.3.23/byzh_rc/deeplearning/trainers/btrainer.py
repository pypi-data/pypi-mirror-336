...
class BTrainer(Byzh):
    ...
    def _stop_and_reload(self, train_loss, train_acc, val_acc):
        ...
        if self._stop_by_loss_delta(train_loss=train_loss):
            ...
            self._print_and_toWriter(info, if_print=False)
            return "break"
        if self._stop_by_byzh(train_loss=train_loss, train_acc=train_acc, val_acc=val_acc):
            info = f'[stop] 模型在连续{self._stop_by_byzh.func.rounds}个epoch内停滞, 触发stop_by_byzh'
            self._print_and_toWriter(info)
            info = "[stop] " + str(self._stop_by_byzh.func.flags)
            self._print_and_toWriter(info, if_print=False)
            return "break"
        ##### 过拟合
    ...
    def set_stop_by_loss(self, rounds=10, delta=.0, target_loss=0.01):
        ...
        '''
        self._stop_by_loss.func = BStopByLoss(rounds=rounds, delta=delta, target_loss=target_loss)
    ...
    def set_stop_by_byzh(self, rounds=10, loss_delta=0.002, acc_delta=0.003):
        ...
        '''
        self._stop_by_byzh.func = BStopByBYZH(rounds=rounds, loss_delta=loss_delta, acc_delta=acc_delta)
    ...
    def draw(self, path: Path, if_show=False):
        ...
        if suffix == 'csv':
            df = pd.DataFrame(
                data=[self.train_loss_lst, self.train_acc_lst, self.val_acc_lst],
