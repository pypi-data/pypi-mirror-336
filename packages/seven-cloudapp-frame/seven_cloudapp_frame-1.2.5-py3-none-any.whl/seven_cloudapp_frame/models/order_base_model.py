...
@LastEditTime: 2025-03-20 15:51:15
...
class OrderBaseModel(FrameBaseModel):
    ...
    def get_horseracelamp_list(self, app_id, act_id, module_id, is_search_module=False, condition="", params=[], page_size=30):
        ...
        if condition:
            condition_where.add_condition(condition)
            params_list.extend(params)
        act_prize_list_dict = ActPrizeModel(context=self.context, is_auto=True).get_cache_dict_list("act_id=%s and is_prize_notice=1 and is_del=0", params=[act_id],cache_expire=60)
        ...
        prize_roster_list = prize_roster_model.get_cache_dict_list(condition_where.to_string(), "", "create_date desc", str(page_size), "user_id,user_nick,prize_name,tag_id,module_name", params_list, cache_expire=60)
        total = int(len(prize_roster_list))
        if total == 0:
            prize_roster_list = []
        else:
            user_ids = SevenHelper.get_row_by_dict_list(prize_roster_list, "user_id")
            user_info_list_dict = UserInfoModel(context=self.context).set_sub_table(app_id).get_dict_list("act_id=%s and " + SevenHelper.get_condition_by_int_list("user_id", user_ids), params=[act_id])
        ...
        if total < page_size:
            ...
            if is_search_module == True:
                ...
                act_module_list = ActModuleModel(context=self.context, is_auto=True).get_cache_list(module_condition, params=module_params,cache_expire=60)
            ...
            if len(false_act_prize_list) > 0 and add_num > 0:
                ...
                user_info_list_dict = UserInfoModel(context=self.context, is_auto=True).set_sub_table(false_act_prize_list[0].app_id).get_dict_list("act_id<%s and create_date>%s", params=[act_id,now_datetime], limit=str(add_num))
