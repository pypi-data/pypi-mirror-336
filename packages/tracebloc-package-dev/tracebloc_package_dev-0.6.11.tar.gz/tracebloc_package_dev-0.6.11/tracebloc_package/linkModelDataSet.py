...
class LinkModelDataSet:
    ...
    def __init__(self, **kwargs):
        ...
        self.__dataset_type = kwargs.get(
            "dataset_type", "welds_data"
        )  # Default to welds_data
        if kwargs["feature_modification"]:
            print(
                "User can create new features for this dataset."
                "\nFor this you can use 'feature_interaction' method for this. To get feature list use 'get_features' method."
            )
    ...
    def resetTrainingPlan(self):
        ...
        self.__epochs = 10
        self.__cycles = 1
        self.__image_shape = self.default_dict["image_size"]
        self.__image_type = "rgb"
        self.__optimizer = "sgd"
        ...
        self.__learningRate = {TYPE: CONSTANT, VALUE: 0.001}
        self.__seed = False
        self.__batchSize = self.__default_batchSize(self.default_dict["batchsize"])
        self.__featurewise_center = False
        self.__samplewise_center = False
        self.__featurewise_std_normalization = False
        self.__samplewise_std_normalization = False
    ...
    def imageType(self, image_type: str):
        ...
        try:
            if type(image_type) is str:
                ...
                error_msg = f"imageType:enter image type as string\n"
                self.__print_error(error_msg)
        except:
    ...
    def __default_batchSize(self, batchsize=None):  # pragma: no cover
        """
    ...
    def lossFunction(self, lossFunction: dict):
        ...
        try:
            if self.__framework == TENSORFLOW_FRAMEWORK:
                ...
                if self.__category == TEXT_CLASSIFICATION:
                    l = ["crossentropy"]
                elif self.__category == GENERIC_CLASSIFICATION:
                    l = ["mse"]
                else:
    ...
    def __get_feature_list(self):
        ...
        feature_list = DATASET_FEATURE_MAPPING.get(self.__dataset_type)
        ...
        if feature_list is None:
            try:
                ...
                # Make GET request to fetch schema for dataset_type
                response = requests.get(
                    f"{self.__url}global-metadata/{self.__dataset_type}/",
                    headers=header,
                )
                ...
                if response.status_code == 200:
                    ...
                    if not feature_list:
                        print(
                            f"Warning: No features found for dataset type '{self.__dataset_type}'"
                        )
                else:
                    print(
    ...
    def get_features(self):  # pragma: no cover
        if not self.__feature_list:
            print(
                f"Warning: No features available for dataset type '{self.__dataset_type}'"
            )
        else:
            pprint.pprint(
                f"The feature list for {self.__dataset_type} is {self.__feature_list}"
            )
    ...
    def feature_interaction(self, dict):
        ...
        """
        if not self.__feature_list:
            error_msg = f"feature_interaction: No features available for dataset type '{self.__dataset_type}'\n"
            self.__print_error(error_msg)
            return
        ...
        if method in ["include", "exclude"]:
            ...
            # Validate all features in the list
            invalid_features = [
            ...
            if dict["feature1"] not in self.__feature_list:
                error_msg = "feature_interaction: Invalid feature name provided\n"
                self.__print_error(error_msg)
                return
            elif (
                "feature2" in dict.keys()
                and dict["feature2"] not in self.__feature_list
            ):
                error_msg = "feature_interaction: Invalid feature name provided\n"
                self.__print_error(error_msg)
                return
    ...
    def set_lora_parameters(
        ...
        try:
            ...
            if self.__framework == PYTORCH_FRAMEWORK and self.__category == TEXT_CLASSIFICATION:
                ...
                task_class = task_classes_dict.get((TEXT_CLASSIFICATION, PYTORCH_FRAMEWORK))
                ...
                if task_class:
                    ...
                    model_upload_params = {
                        ...
                        "batch_size": self.__batchSize,
                        "weights": self.__weights,
                        "input_shape": (self.__input_shape,self.__input_shape) if self.__input_shape else (self.__image_shape,),
                        "model_path": self.model_path,
                        "weights_path": self.tmp_path,
                        "progress_bar_1": None,
                    ...
                    print("Could not validate LoRA configuration - task class not found.")
    ...
    def getTrainingPlan(self):
        if self.__eligibility_passed:
            print(
                ...
                f"feature_interaction: {self.__feature_interaction}\n",
