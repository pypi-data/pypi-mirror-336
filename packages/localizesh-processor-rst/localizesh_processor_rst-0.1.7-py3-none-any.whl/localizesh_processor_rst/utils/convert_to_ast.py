    ...
    def hast_to_document_recursive(node) -> LayoutRoot:
        ...
        if node and "children" in node:
            for child in node["children"]:
                ...
                if child.get("type") == "element" or child["value"] is not None and len(child["value"]) > 0:
                    content_length += 1
                ...
                if is_contain_text_node_content is False:
                    is_contain_text_node_content = child["type"] == "text" and child["value"] is not None and len(child["value"]) > 0
