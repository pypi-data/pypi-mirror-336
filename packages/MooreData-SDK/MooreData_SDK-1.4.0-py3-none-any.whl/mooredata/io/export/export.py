...
class ExportData():
        ...
        if isinstance(data, dict):
            for k, v in data.items():
                if isinstance(v, list):
                    data[f'{k}'] = [self.moore2dict(d) for d in v]
                elif repr(v) == 'molar_ai_format':
                    data[f'{k}'] = self.moore2dict(v)
        elif repr(data) == 'molar_ai_format':
            data = self.moore2dict(vars(data))
        return data
    ...
    def _query(self, key, value):
        ...
        if key == add_location:
            ...
            return MOORE({f"{add_key}": add_value(value)})
        else:
            return MOORE({})