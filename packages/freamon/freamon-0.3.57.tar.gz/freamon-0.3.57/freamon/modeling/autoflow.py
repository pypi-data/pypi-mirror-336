...
class AutoModelFlow:
    ...
    def _train_model(
        ...
        # Ensure we don't include 'log_loss' unless we have probabilities
        if 'log_loss' in metrics and self.problem_type != 'classification':
            metrics = [m for m in metrics if m != 'log_loss']
    ...
    def _train_model_with_tuning(
        ...
        for train_idx, test_idx in cv.split(X, y):
            ...
            try:
                ...
                print(f"Warning: Error during model prediction in CV: {str(e)}")
                continue
            ...
            if self.problem_type == 'classification' and hasattr(cv_model, 'predict_proba'):
                try:
                    ...
                    print(f"Warning: Error during probability prediction in CV: {str(e)}")
            
            # Calculate metrics
            try:
                fold_metrics = calculate_metrics(
                    y_test, y_pred, problem_type=self.problem_type, y_proba=y_prob)
                
                # Store results
                for metric, value in fold_metrics.items():
                    # Initialize the metric list if it's not already in cv_metrics
                    if metric not in cv_metrics:
                        cv_metrics[metric] = []
                    cv_metrics[metric].append(value)
            except Exception as e:
                print(f"Warning: Error during metrics calculation in CV: {str(e)}")
        ...
        for metric, values in cv_metrics.items():
            if len(values) > 0:
                ...
                # Handle case where metric calculation failed for all folds
                cv_results[f"{metric}_mean"] = float('nan')
                cv_results[f"{metric}_std"] = float('nan')
                cv_results[f"{metric}_values"] = []
