...
def _flag_similar_records_chunked(
    ...
    # Initialize similarity tracking data structures
    similarity_flags = pd.Series(False, index=pandas_df.index)
    similarity_scores = pd.Series(0.0, index=pandas_df.index)
    
    # Create a graph to track similar records across chunks
    G = nx.Graph()
    ...
    def process_chunk_pair(chunk1_idx, chunk2_idx):
        ...
        chunk1 = pandas_df.iloc[start1:end1]
        ...
        if chunk1_idx == chunk2_idx:
            ...
            for i in range(len(chunk1)):
                for j in range(i+1, len(chunk1)):
                    ...
                    # Calculate similarity using the same logic as in standard approach
            ...
            chunk2 = pandas_df.iloc[start2:end2]
            edges = []
            scores = {}
            
            # Compare all pairs between the two chunks
            for i in range(len(chunk1)):
    ...
    # Generate all chunk pairs to process
    chunk_pairs = []
    for i in range(n_chunks):
    ...
    if n_jobs > 1:
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            futures = {executor.submit(process_chunk_pair, i, j): (i, j) for i, j in chunk_pairs}
    ...
    for edge in edges_list:
        ...
        # Store similarity scores if requested
        if similarity_column:
            sim = scores_dict.get(edge, 0.0)
            if sim > similarity_scores.iloc[i]:
                similarity_scores.iloc[i] = sim
            if sim > similarity_scores.iloc[j]:
                similarity_scores.iloc[j] = sim
    
    # Find connected components (clusters of similar records)
    clusters = list(nx.connected_components(G))
    
    # Initialize group IDs if needed
    if group_column:
        group_ids = pd.Series(0, index=pandas_df.index)
