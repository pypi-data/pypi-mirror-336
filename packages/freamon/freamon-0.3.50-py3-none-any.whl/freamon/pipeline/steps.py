...
class HyperparameterTuningStep(PipelineStep):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None, **kwargs) -> HyperparameterTuningStep:
        ...
        # Fit the model with best parameters - filter incompatible parameters
        import lightgbm
        
        # Create a dictionary of valid parameters
        fit_params = {}
        
        # Add categorical_feature if it's provided and not empty
        if self.categorical_features:
            fit_params['categorical_feature'] = self.categorical_features
            
        # Handle early_stopping_rounds properly for LightGBM models
        if self.early_stopping_rounds is not None:
            # For LightGBM models, early_stopping_rounds is applied via callbacks
            if hasattr(self.model, 'booster_') and self.model.booster_ == 'lightgbm':
                # Create a validation sample since early stopping requires validation data
                from sklearn.model_selection import train_test_split
                ...
                self.model.fit(
                    ...
                    callbacks=[lightgbm.early_stopping(stopping_rounds=self.early_stopping_rounds)],
                    **fit_params
                )
            else:
                # For other models that support early_stopping_rounds directly
