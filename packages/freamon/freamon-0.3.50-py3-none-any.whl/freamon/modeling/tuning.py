        ...
        def objective(trial):
            ...
            for train_idx, valid_idx in cv_splitter.split(X, y):
                ...
                try:
                    ...
                    # Check LightGBM version to handle API differences
                    import lightgbm
                    if hasattr(model.model, 'booster_') and model.model.booster_ == 'lightgbm':
                        # New LightGBM API uses callbacks for early stopping
                        fit_params = {
                            'eval_set': [eval_set],
                        ...
                        try:
                            ...
                            # If verbose isn't supported, try without it
                            model.fit(
                                X_train, y_train,
                                eval_set=[eval_set],
                                early_stopping_rounds=early_stopping_rounds,
