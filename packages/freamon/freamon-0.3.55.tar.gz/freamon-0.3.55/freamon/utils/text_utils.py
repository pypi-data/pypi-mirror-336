...
def create_topic_model_optimized_from_matrix(tfidf_matrix, feature_names, n_topics=5, 
                                    random_state=42, **kwargs):
    """
    Create a topic model directly from a pre-computed TF-IDF matrix.
    
    This function is particularly useful when you already have a vectorized representation
    of your documents and want to apply topic modeling without repeating the vectorization step.
    
    Parameters:
    -----------
    tfidf_matrix : scipy.sparse.csr_matrix
        Pre-computed TF-IDF matrix where rows are documents and columns are terms
    feature_names : list
        List of feature names (terms) corresponding to the columns in tfidf_matrix
    n_topics : int, default=5
        Number of topics to extract
    random_state : int, default=42
        Random state for reproducibility
    **kwargs : dict
        Additional parameters for the NMF model
        
    Returns:
    --------
    tuple
        A tuple containing (topics_df, topic_terms, doc_topics):
        - topics_df: DataFrame with topic-term weights
        - topic_terms: List of top terms for each topic
        - doc_topics: Document-topic matrix
    """
    # Extract supported parameters from kwargs, ignoring any unsupported ones like sampling_ratio
    supported_params = {'alpha', 'l1_ratio', 'beta_loss', 'solver', 'tol', 'max_iter',
                       'regularization', 'init', 'verbose'}
    ...
    n_top_words = 10  # Number of top words to include
    topic_terms = []
    for topic_idx, topic in enumerate(topic_term_matrix):
        top_indices = topic.argsort()[:-n_top_words-1:-1]
        top_terms = [feature_names[idx] for idx in top_indices]
        topic_terms.append((topic_idx, top_terms))
    
    # Create topics DataFrame
    topics_df = pd.DataFrame()
    for topic_idx, terms in topic_terms:
        topics_df[f"Topic {topic_idx+1}"] = pd.Series(terms)
    
    return topics_df, topic_terms, doc_topics
