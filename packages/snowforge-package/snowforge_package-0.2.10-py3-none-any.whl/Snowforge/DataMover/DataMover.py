...
class Engine():
    ...
    @staticmethod
    def parallel_process(worker_func: object, args_list: list[tuple], num_workers: int = None, use_shared_queue: bool = False, multi_queue: bool = False, single_queue = None, queue1 = None, queue2 = None):
        '''
        ...
        process_list = []
        if use_shared_queue and single_queue is None:
        ...
        for i in range(num_processes):
            ...
            if use_shared_queue:
                process = mp.Process(target=worker_func, args=(*args_list[i], single_queue))
            elif multi_queue:
                process = mp.Process(target=worker_func, args=(*args_list[i], queue1, queue2))
            else:
