...
def plot_batch_effect(
    ...
    *,
    ...
    neighbors_key='neighbors', # harmony_neighbors
    ...
    if not use_rep in adata.obsm.keys():
        sc.tl.pca(adata, svd_solver="arpack", n_comps=50, mask_var=mask_var)
        sc.pl.pca_variance_ratio(adata, n_pcs=50, show=False)
        _saveimg(f"PCA_variance_ratio")
    if not neighbors_key in adata.uns.keys():
        sc.pp.neighbors(adata, n_neighbors=n_neighbors, n_pcs=n_pcs,use_rep=use_rep,method='umap',key_added=neighbors_key,random_state=0)
    sc.tl.umap(adata,method='umap',min_dist=0.5,spread=1.0,init_pos='spectral',neighbors_key=neighbors_key,random_state=0)
    ...
    ,metric='cosine'
    use_rep='X_pca'
    sc.tl.leiden(adata, key_added=cluster_key, resolution=0.5,neighbors_key='pca_neighbors')#, flavor='igraph',n_iterations=2
    ...
    _dimplot(reduction='umap',color='celltype',filename=f"celltype_UMAP")
