...
from eventix.pydantic.settings import EventixClientSettings, EventixServerSettings
...
class EventixClient:
    ...
    def config(cls, config: dict):
        ...
        if not settings.eventix_delay_tasks:
            log.info(
                "ATTENTION!!!: Delay tasks disabled. All tasks are run directly. No delay."
            )
    ...
    @classmethod
    def get_task_by_unique_key_and_namespace(cls,unique_key:str,namespace:str=None) -> TEventixTask:
        r = cls.interface.get(f"/task/by_unique_key/{namespace}/{unique_key}")
        with raise_errors(r, []):
            return r
