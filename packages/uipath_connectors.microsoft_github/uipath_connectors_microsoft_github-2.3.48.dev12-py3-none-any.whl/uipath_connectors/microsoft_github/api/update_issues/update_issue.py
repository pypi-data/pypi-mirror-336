...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "patch",
        "url": "/{repo}/update_issues/{issue_number}".format(
            repo=repo,
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[DefaultError, UpdateIssueResponse]]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    Args:
        ...
        issue_number (str): The number that identifies the issue
        body (UpdateIssueRequest):
    ...
    if not issue_number and issue_number_lookup:
        filter = issue_number_lookup
        lookup_response_raw = client.get_httpx_client().request(
            method="get",
            url=f"/search_issues?sort=updated?query=type:issue org:orgName %22{filter}%22in:title",
        )
        lookup_response = lookup_response_raw.json()
        ...
        if not found_items:
            raise ValueError(
                "No matches found for issue_number_lookup in search_issues?sort=updated"
            )
        if len(found_items) > 1:
            print(
                "Warning: Multiple matches found for issue_number_lookup in search_issues?sort=updated. Using the first match."
            )

        issue_number = found_items[0]["number"]
...
def sync(
    ...
    *,
    ...
    Returns:
        Union[DefaultError, UpdateIssueResponse]
    """
