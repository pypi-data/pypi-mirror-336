...
class BatchDeleter:
    ...
    def check_if_some_batch_was_not_found_for_deletion(self):
        ...
        batches_not_found_for_deletion = [
            batch
        ...
        if batches_not_found_for_deletion:
            logger.info(
                f"Some batches were not found for deletion in {self.practice}."
                f"Batches not found: {batches_not_found_for_deletion}"
            )
    ...
    def check_if_all_batches_were_deleted(self):
        ...
        if self.total_number_of_batches_in_the_practice - len(self.batches_not_secured_to_thoughtful) == len(
            data_items
        ):
            ...
            for index, data_item in enumerate(data_items):
                ...
                for description in batches_secured_to_thoughtful:
                    if description in next_gen_description:
                        batches_not_deleted.append(description)
                        break
            if batches_not_deleted:
                logger.warning(
                    f"Some batches were not deleted in {self.practice}. Batches not deleted: {batches_not_deleted}"
                )
                # todo - export a json file with the batches not deleted
...
def delete_batches(next_gen: NextGenPMCore, batches_to_delete: list[dict], database: str = Config.DATABASES.TEST):
    ...
    """
    next_gen.login(database=database)
