...
def show_single_dose_slice(dose, slice_num, dose_min, dose_max, colormap):
    """
    显示单个剂量分布的切片。
    Args:
    ...
    """
    fig, ax = plt.subplots(figsize=(6, 6), dpi=100)
    img = ax.imshow(dose[slice_num], cmap=colormap, interpolation='none', vmin=dose_min, vmax=dose_max)
...
def show_single_slice_interactive(dose, colormap):
    """
    显示单个剂量分布的切片, 交互式版本。
    Args:
    ...
    interact(lambda slice_num: show_single_dose_slice(dose, slice_num, dose.min(), dose.max(), colormap),
