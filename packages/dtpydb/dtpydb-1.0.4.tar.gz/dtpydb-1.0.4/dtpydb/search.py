...
def filters_mapper(db: Session, pre_conditions: list, joins: list[dict], filter_item, conditions):
    ...
    if columns_type == 'select':
        ...
        for column in columns:
            query = db.query(
                ...
                if filter_item.get('is_json', False) else
                column
            )
            ...
            distinct_names_query.append(query.filter(*pre_conditions, *conditions).distinct())

        values = list(map(lambda x: x[0], db.execute(union(*distinct_names_query)).fetchall()))
...
def get_filters_value(db: Session, pre_conditions: list, joins: list[dict], filters: list, names_conditions: dict):
    available_filters = []
...
def make_selected_filters(
    ...
    for filter_item in filters:
        ...
        if isinstance(selected_data, dict) and len([k for k, v in selected_data.items() if v is not None]) == 0:
            continue
        ...
        if filter_item['type'] == 'select':
            ...
            if minimum is not None and maximum is not None:
                result_label = f"{filter_item['label']} (between {minimum} and {maximum})"
    ...
    if columns_type == 'search':
        if is_json_column:
            ...
            return logic_function(
                *[
                    or_(*[func.lower(cast(column, String)).like(func.lower(f"%\"{x}\"%")) for x in filter_value])
                    for column in columns
                ]
            )
        else:
            ...
            return logic_function(
                *[
                    func.lower(cast(column, String)).like(func.lower(f"%{filter_value}%"))
                    for column in columns
                ]
            )
...
def get_list(
    ...
    if distinct is True:
        count_query = db.query(func.count(distinct_func(getattr(model, primary_column))))
    elif distinct:
    ...
    for filter_item in filters:
        ...
        target_condition = make_condition(
            filter_item=filter_item,
            values=values
        )
