...
class DatabaseInstance:
    def __init__(self, config: DatabaseConfig):
        ...
        if db_url:
            ...
            self.async_database_path_write = db_url.replace("postgresql://", "postgresql+asyncpg://", 1)
        else:
        ...
        if db_url_read:
            ...
            self.async_database_path_read = db_url_read.replace("postgresql://", "postgresql+asyncpg://", 1)
        else:
    ...
    def get_clean_db_write(self):
        with self.get_clean_db_cm(force="write") as db:
            yield db

    @contextmanager
    def get_clean_db_cm_read(self):
        with self.get_clean_db_cm(force="read") as db:
            yield db

    @contextmanager
    def get_clean_db_cm_write(self):
        yield from self.get_clean_db_cm(force="write")
    ...
    async def async_get_db_cm_read(self):
        async for db in self.async_get_db_cm(force="read"):
            yield db

    @asynccontextmanager
    async def async_get_db_cm_write(self):
        async for db in self.async_get_db_cm(force="write"):
            yield db
    ...
    async def async_get_clean_db_cm_read(self):
        async with self.async_get_clean_db_cm(force="read") as db:
            yield db

    @asynccontextmanager
    async def async_get_clean_db_cm_write(self):
        async with self.async_get_clean_db_cm(force="write") as db:
            yield db
