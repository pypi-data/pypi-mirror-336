...
from ._serialization import deserialize_json, deserialize_png, deserialize_tiff
...
# allows to disable varnish for testing purposes - see test_caching_udfs.py
_DISABLE_VARNISH = False
...
def _parse_realtime_response(r: requests.Response, content_type: str):
    if content_type == "application/octet-stream":  # parquet
        data = parquet_to_df(r.content)
    elif content_type == "image/png":
        data = deserialize_png(r.content)
    elif content_type == "image/tiff":
        data = deserialize_tiff(r.content)
    elif content_type == "application/json":
        data = deserialize_json(r.content)
    else:
        data = r.content if len(r.content) > 0 else None  # TODO

    return UdfEvaluationResult(
        data=data,
        time_taken_seconds=0.0,  # TODO
    )


async def _parse_realtime_response_async(content: bytes, content_type: str):
    if content_type == "application/octet-stream":  # parquet
        data = parquet_to_df(content)
    elif content_type == "image/png":
    ...
    **params,
    ...
    r = requests.get(
        ...
        headers={
            ...
            **({"X-Fused-Cache-Disable": "true"} if _DISABLE_VARNISH else {}),
        },
    )
    exc = _realtime_raise_for_status(r)
    if exc is not None:
        return exc
...
async def _run_tile_async(
    ...
    **params,
    ...
    if OPTIONS.pyodide_async_requests:
        ...
        exc = _realtime_raise_for_status_async(r)
        if exc is not None:
            return exc
