...
class BaseUdf(BaseModel):
    ...
    code: str
    headers: Union[HeaderSequence, Sequence[Header]] = Field(default_factory=list)
    metadata: UserMetadataType = None
    _globals: Optional[CompiledAttrs] = None

    @model_validator(mode="after")
    def _process_import(self) -> Self:
        if not OPTIONS.never_import:
            if not self.code:
