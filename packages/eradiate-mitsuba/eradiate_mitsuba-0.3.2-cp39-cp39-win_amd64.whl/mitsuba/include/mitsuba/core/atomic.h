...
public:
    ...
    explicit AtomicFloat(Type v = 0.f) { m_bits = dr::memcpy_cast<Storage>(v); }

    /// Convert the AtomicFloat into a normal floating point value
    operator Type() const { return dr::memcpy_cast<Type>(m_bits.load(std::memory_order_relaxed)); }

    /// Overwrite the AtomicFloat with a floating point value
    AtomicFloat &operator=(Type v) { m_bits = dr::memcpy_cast<Storage>(v); return *this; }
