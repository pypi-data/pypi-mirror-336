...
class CcApiContextQuery(ContextLookupRoleOptions):
    def __init__(
        ...
        *,
        ...
        :param exact_identifier: Identifier of the resource to look up using ``GetResource``. Specifying exactIdentifier will return exactly one result, or throw an error. Default: - Either exactIdentifier or propertyMatch should be specified.
        ...
        :param property_match: Returns any resources matching these properties, using ``ListResources``. Specifying propertyMatch will return 0 or more results. Notes on property completeness CloudControl API's ``ListResources`` may return fewer properties than ``GetResource`` would, depending on the resource implementation. The resources that ``propertyMatch`` matches against will *only ever* be the properties returned by the ``ListResources`` call. Default: - Either exactIdentifier or propertyMatch should be specified.
        '''
        if __debug__:
