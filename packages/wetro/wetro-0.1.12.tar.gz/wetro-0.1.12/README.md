...
rag_client.collection.get_or_create_collection_id(collection_id="my_unique_collection_id")

# Create a collection
rag_client.collection.create_collection_id(collection_id="my_unique_collection_id")

# Get all collections
all_collection_resp = rag_client.collection.get_all()

# Insert a web resource
insert_response = rag_client.collection.insert(collection_id="my_unique_collection_id",resource="https://example.com/article", type="web")
...
rag_client.collection.create_collection_id(collection_id="research_papers")

# Insert documents from different sources
rag_client.collection.insert(collection_id="research_papers",resource="https://medium.com/@wetrocloud/why-legal-tech-needs-wetrocloud-ai-rag-and-the-future-of-legal-practice-66fb38c4df09", type="web")
rag_client.collection.insert(resource ="This is a sample text document about AI.", type ="text")
```
...
```python
# Simple query
response = rag_client.collection.query(collection_id="research_papers",request_query="What are the main findings in the research?")
print(response)
```
...
#### `collection.create_collection_id(collection_id)`
Creates a new collection.

#### `collection.get_all()`
Gets all existing colllections

#### `collection.insert(collection_id, resource, type)`
Inserts a document into the collection.
- `collection_id`: Collection ID
- `resource`: URL, text content, or file path
- `type`: "web", "text", "json", or "file"

#### `collection.query(collection_id, request_query, model=None, json_schema=None, json_schema_rules=None, stream=False)`
