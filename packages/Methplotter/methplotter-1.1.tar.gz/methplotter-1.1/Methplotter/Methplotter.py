...
packages = ['pandas', 'numpy', 'seaborn', 'matplotlib', 'easygui','pyqt5']
...
# 根据gff文件生成相应的可以进行绘图（箱型图）的文件
def GenerateFileToDrawBoxplot(CX_file,gff_file_list,context):
    # 参数
    # CX_file：cx文件，df格式
...
# 将绘图功能包装成函数  绘制箱型图
def Draw_box(file,context):
    # 使用seaborn绘制箱型图
    seaborn.boxplot(x='gene', y='ratio', data=file,  # 箱子垂直显示，默认为'h'水平显示
                    fliersize=4,  # 设置离散值marker大小，默认为5
...
# # 执行代码
# # 读取CX_report-txt文件
CX_file_df_path  = easygui.enterbox("请输入CX_file文件地址:", title="获取文件地址")
# CX_file_df_pat = 'F:/bi_ye_she_ji/share_WGBS_results/Arabidopsis_tair10/Tair10_methylation_CX_report.txt'
CX_file = open(CX_file_df_path , 'r', encoding='utf-8')
...
gff_file_path = easygui.enterbox("请输入gff_file文件地址:", title="获取文件地址")
gff_file = pandas.read_csv(gff_file_path, sep='\t', header=None)
# # 为列添加名称
