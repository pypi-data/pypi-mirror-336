...
class EmbeddingsManager:  # noqa: D101
    ...
    def load_embeddings(self):
        ...
        Returns:
            None
        """
        for data_field in ["container_types", "ports", "terminals", "depots", "item_codes"]:
    ...
    def setup_item_code_embeddings(self, computed_item_code_names, computed_item_code_embeddings):
        """
...
def clean_item_description(lineitem: str):
    """
    This function removes dates, month names, whitespaces, currency patterns and container numbers from the given line item string.
    ...
    """
    # Patterns
    date_explicit_patterns = r'\b\d{1,2} (?:january|february|march|april|may|june|july|august|september|october|november|december|januar|februar|märz|mai|juni|juli|oktober|dezember) \d{4}\b'
    date_abbrevation_patterns = r'\b(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC|JAN|FEB|MRZ|APR|MAI|JUN|JUL|AUG|SEP|OKT|NOV|DEZ)\b'
    currency_codes_pattern = r'\b(USD|EUR|JPY|GBP|CAD|AUD|CHF|CNY|SEK|NZD|KRW|SGD|INR|BRL|ZAR|RUB|MXN|HKD|NOK|TRY|IDR|MYR|PHP|THB|VND|PLN|CZK|HUF|ILS|AED|SAR|QAR|KWD|EGP|NGN|ARS|CLP|COP|PEN|UYU|VEF|INR|PKR|BDT|LKR|NPR|MMK)\b'
...
async def format_all_entities(result, embed_manager, document_type_code, llm_client):
    ...
    if document_type_code == "partnerInvoice":
        for line_item in aggregated_data.get("lineItem", []):
            line_item["itemCode"] = associate_forto_item_code(embed_manager, line_item["lineItemDescription"]["formattedValue"])
