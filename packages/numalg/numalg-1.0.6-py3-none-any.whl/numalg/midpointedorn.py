...
'''Midpoint method for solving a system of ordinary differential equations (ODEs) with a given vector of initial conditions.'''

def midpointedorn(f, y0, t0, tn, n = 10):
    '''
    ...
    for i in range(n):
        k1 = f(t[i], y[i, :]) 
        y[i+1, :] = y[i, :] + h * f(t[i] + h/2, y[i, :] + h/2 * k1)
    yn = y[-1, :]  
    return yn, y, t, h
...
def teste():
    ...
    yn, t, y, h = midpointedorn(f, y0, t0, tn, n)
    print(yn, t, y, h)
