...
'''Heun method for solving ordinary differential equations (ODEs) with a given initial value.'''


from math import exp
import numpy as np
def heun(f, y0, t0, tn, n=10) :
    '''
    heun finds solutions of initial value problems
    y '= f(t, y), y(t0) = y0
...
def teste():
    ''' y' = 2 * exp(-y), y(0) = 2'''
    print("heun(lambda t, y: 2*exp(-y), 2, 0, 1, 5)")
    fun = lambda t, y: 2*exp(-y)
    print(heun(fun, 2, 0, 1, 5))
