...
def cluster():
    url = 'https://drive.google.com/file/d/1Piv9F0xAkg2n3vRLofkGGjbqLNZlapE5/view?usp=sharing'
    url = 'https://drive.google.com/uc?export=download&id='+url.split('/')[-2]
...
def ukdata():
    url = 'https://drive.google.com/file/d/1kzFpEJvzT2d5RIkf6vCF4hVxabRjsE6j/view?usp=sharing'
    url = 'https://drive.google.com/uc?export=download&id='+url.split('/')[-2]
    macro=pd.read_csv(url,index_col=0)
    macro=macro.drop(index=macro.index[0])
    macro.index=pd.to_datetime(macro.index)
...
def usdata1():
    url = 'https://drive.google.com/file/d/172VsCJMRvIuO9GNhuW9iEsZpt6tS7_nh/view?usp=sharing'
    url = 'https://drive.google.com/uc?export=download&id='+url.split('/')[-2]
    macro=pd.read_csv(url,index_col=0)
    macro=macro.drop(index=macro.index[0])
    macro.index=pd.to_datetime(macro.index)
