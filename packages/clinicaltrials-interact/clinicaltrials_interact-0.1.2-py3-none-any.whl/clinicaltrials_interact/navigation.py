...
class ClinicalTrialsNavigator:
    ...
    def search_and_build_graph(
        ...
        """
        # Search for studies
        df = self.api.search_to_dataframe_IDs(search_expr, max_studies=max_studies)

        # print(df)
        # print("Columns in dataframe:", df.columns.tolist())  # Add this line to see column names

        df["briefSummary"] = df["NCTId"].apply(lambda x: self.api.extract_brief_summary(x))
