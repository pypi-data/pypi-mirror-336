...
def _arrow2568(alternate_name: str | None=None, measurement_method: str | None=None, description: str | None=None, category: OntologyAnnotation | None=None, value: Value | None=None, unit: OntologyAnnotation | None=None) -> ProcessParameterValue:
    return ProcessParameterValue.create_as_pv(alternate_name, measurement_method, description, category, value, unit)


ROCrate_decoder: Decoder_1[ProcessParameterValue] = decoder(_arrow2568)
...
def ISAJson_encoder(id_map: Any | None, oa: ProcessParameterValue) -> IEncodable:
    ...
    def _arrow2570(value: ProtocolParameter, id_map: Any=id_map, oa: Any=oa) -> IEncodable:
        return encoder_1(id_map, value)

    def _arrow2571(value_1: Value, id_map: Any=id_map, oa: Any=oa) -> IEncodable:
        return encoder_2(id_map, value_1)

    def _arrow2572(oa_1: OntologyAnnotation, id_map: Any=id_map, oa: Any=oa) -> IEncodable:
        return OntologyAnnotation_ISAJson_encoder(id_map, oa_1)

    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include("category", _arrow2570, oa.Category), try_include("value", _arrow2571, oa.Value), try_include("unit", _arrow2572, oa.Unit)]))
...
def _arrow2577(get: IGetters) -> ProcessParameterValue:
    def _arrow2574(__unit: None=None) -> ProtocolParameter | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("category", decoder_1)

    def _arrow2575(__unit: None=None) -> Value | None:
    ...
    return ProcessParameterValue(_arrow2574(), _arrow2575(), _arrow2576())


ISAJson_decoder: Decoder_1[ProcessParameterValue] = object(_arrow2577)
