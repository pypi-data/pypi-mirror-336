...
def encoder(obj: LDGraph) -> IEncodable:
    ...
    def _arrow1924(__unit: None=None, obj: Any=obj) -> IEnumerable_1[tuple[str, IEncodable | None]]:
        def _arrow1919(value: str) -> IEncodable:
        ...
        def _arrow1923(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
            def _arrow1922(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                def _arrow1920(kv: Any) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    l: str = kv[0].lower()
                ...
                def _arrow1921(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    return singleton(("@graph", seq(map_1(encoder_2, obj.Nodes))))

                return append(collect(_arrow1920, obj.GetProperties(True)), delay(_arrow1921))

            return append(singleton(try_include("@context", encoder_1, obj.TryGetContext())), delay(_arrow1922))

        return append(singleton(try_include("@id", _arrow1919, obj.Id)), delay(_arrow1923))

    values_1: FSharpList[tuple[str, IEncodable]] = choose(chooser, to_list(delay(_arrow1924)))
...
class ObjectExpr1927(Decoder_1[LDGraph]):
...
decoder: Decoder_1[LDGraph] = ObjectExpr1927()
