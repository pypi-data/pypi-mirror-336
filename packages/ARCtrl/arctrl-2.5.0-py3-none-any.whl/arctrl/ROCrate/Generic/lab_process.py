...
class LDLabProcess:
    ...
    def try_get_agent(lp: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> LDNode | None:
        ...
        if match_value is not None:
            def _arrow1518(__unit: None=None) -> bool:
    ...
    def get_agent(lp: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> LDNode:
        match_value: LDNode | None = lp.TryGetPropertyAsSingleNode(LDLabProcess.agent(), graph, context)
        if match_value is not None:
            def _arrow1519(__unit: None=None) -> bool:
                ld_object: LDNode = match_value
                return LDPerson.validate(ld_object, context)
    ...
    def gen_id(name: Any, assay_name: Any | None=None, study_name: Any | None=None) -> str:
        def _arrow1521(__unit: None=None) -> str:
            ...
            return ((("#Process_" + str(study_1)) + "_") + str(name)) + ""

        def _arrow1522(__unit: None=None) -> str:
            assay_1: __B = value(assay_name)
            return ((("#Process_" + str(assay_1)) + "_") + str(name)) + ""

        def _arrow1523(__unit: None=None) -> str:
            assay: __B = value(assay_name)
            study: __C = value(study_name)
            return ((((("#Process_" + str(study)) + "_") + str(assay)) + "_") + str(name)) + ""

        return clean((_arrow1521() if (study_name is not None) else (("#Process_" + str(name)) + "")) if (assay_name is None) else (_arrow1522() if (study_name is None) else _arrow1523()))
...
LDLabProcess_reflection = _expr1524
