...
class DTOType(Union):
    ...
    @staticmethod
    def cases() -> list[str]:
        return ["ISA_Assay", "ISA_Study", "ISA_Investigation", "ISA_Datamap", "JSON", "Markdown", "CWL", "YAML", "PlainText", "Cli"]


DTOType_reflection = _expr3099

def _expr3107() -> TypeInfo:
...
CLITool_reflection = _expr3107

def _expr3109() -> TypeInfo:
...
class Contract(Record):
    ...
    def create_execute(dto: CLITool, path: str | None=None) -> Contract:
        return Contract("EXECUTE", default_arg(path, ""), DTOType(9), DTO(2, dto))
