...
def DateTime_tryFromString(s: str) -> Any | None:
    try: 
        def _arrow3490(__unit: None=None) -> Any:
...
def ColumnIndex_tryInt(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3491(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3492(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3491, _arrow3492)), out_arg)
    if match_value[0]:
...
BaseTypes_reflection = _expr3493
...
ProcessConversion_reflection = _expr3494
...
def ProcessConversion_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 0:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3495(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 3:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3496(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 2:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3497(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 1:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3498(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 4:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3499(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetProtocolREFGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 8:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3500(i: int, matrix: Any=matrix) -> str:
...
def ProcessConversion_tryGetProtocolDescriptionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 5:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3501(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetProtocolURIGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 6:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3502(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 7:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3503(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetInputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 11:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3504(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetOutputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 12:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3505(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 14:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3506(i: int, matrix: Any=matrix) -> str:
...
def ProcessConversion_tryGetPerformerGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 9:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3507(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], LDNode]:
    ...
    def chooser(tupled_arg: tuple[int, tuple[int, CompositeHeader]], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_1(tupled_arg_1: tuple[int, tuple[int, CompositeHeader]], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_2(tupled_arg_2: tuple[int, tuple[int, CompositeHeader]], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_3(tupled_arg_3: tuple[int, tuple[int, CompositeHeader]], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_4(tupled_arg_4: tuple[int, CompositeHeader], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_9(tupled_arg_9: tuple[int, CompositeHeader], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    def chooser_10(tupled_arg_10: tuple[int, CompositeHeader], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], str] | None:
    ...
    def chooser_11(tupled_arg_11: tuple[int, CompositeHeader], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    if match_value is None:
        def _arrow3509(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3508(i_1: int) -> Array[LDNode]:
        ...
        def _arrow3511(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3510(i_2: int) -> Array[LDNode]:
                return []

            return _arrow3510

        input_getter_1 = _arrow3509 if (length(char_getters) != 0) else _arrow3511

    else: 
        input_getter: Callable[[Any, int], LDNode] = match_value
        def _arrow3513(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3512(i: int) -> Array[LDNode]:
        ...
        input_getter_1 = _arrow3513

    output_getter_1: Callable[[Any, int], Array[LDNode]]
    def chooser_12(tupled_arg_12: tuple[int, CompositeHeader], process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[Any, int], LDNode] | None:
    ...
    if match_value_1 is None:
        def _arrow3515(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3514(i_4: int) -> Array[LDNode]:
        ...
        def _arrow3517(matrix_5: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3516(i_5: int) -> Array[LDNode]:
                return []

            return _arrow3516

        output_getter_1 = _arrow3515 if (length(factor_value_getters) != 0) else _arrow3517

    else: 
        output_getter: Callable[[Any, int], LDNode] = match_value_1
        def _arrow3519(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3518(i_3: int) -> Array[LDNode]:
        ...
        output_getter_1 = _arrow3519

    def _arrow3522(matrix_6: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], LDNode]:
        def _arrow3521(i_6: int) -> LDNode:
            def mapping_4(tuple_1: tuple[int, int]) -> int:
                return tuple_1[1]

            class ObjectExpr3520:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            pn: str = process_name_root if ((1 + max(map_1(mapping_4, matrix_6.keys()), ObjectExpr3520())) == 1) else ProcessConversion_composeProcessName(process_name_root, i_6)
            ...
            return LDLabProcess.create(pn, match_value, match_value_1, None, agent, protocol, paramvalues, None, comments)
...
def ProcessConversion_groupProcesses_Z27F0B586(processes: FSharpList[LDNode], graph: LDGraph | None=None, context: LDContext | None=None) -> FSharpList[tuple[str, FSharpList[LDNode]]]:
    def projection(p: LDNode, processes: Any=processes, graph: Any=graph, context: Any=context) -> str:
        ...
        if match_value is not None:
            if ProcessConversion_decomposeProcessName_Z721C83C5(match_value)[1] is not None:
                ...
                def _arrow3523(__unit: None=None, p: Any=p) -> bool:
                ...
                if _arrow3523():
                    ...
                    def _arrow3524(__unit: None=None, p: Any=p) -> bool:
                ...
                def _arrow3525(__unit: None=None, p: Any=p) -> bool:
            ...
            def _arrow3526(__unit: None=None, p: Any=p) -> bool:
    ...
    class ObjectExpr3528:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3527(x: str, y: str) -> bool:
                return x == y
    ...
    return List_groupBy(projection, processes, ObjectExpr3528())
...
def ProcessConversion_processToRows_Z6839B9E8(p: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> Array[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow3537(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3529(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3536(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3530(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                ...
                def _arrow3535(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3531(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    ...
                    def _arrow3534(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3532(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        def _arrow3533(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(_arrow3532(), delay(_arrow3533))

                    return append(_arrow3531(), delay(_arrow3534))

                return append(_arrow3530(), delay(_arrow3535))

            return append(_arrow3529(), delay(_arrow3536))

        prot_vals = to_list(delay(_arrow3537))
    ...
    def _arrow3538(Value: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value

    def _arrow3539(Value_1: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_1

    def _arrow3540(Value_2: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_2

    def _arrow3541(Value_3: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_3

    pattern_input: tuple[Array[LDNode | None], Array[LDNode | None]] = ((ResizeArray_create(len(outputs), None), ResizeArray_map(_arrow3538, outputs))) if ((len(outputs) != 0) if (len(inputs) == 0) else False) else (((ResizeArray_map(_arrow3539, inputs), ResizeArray_create(len(inputs), None))) if ((len(outputs) == 0) if (len(inputs) != 0) else False) else ((ResizeArray_map(_arrow3540, inputs), ResizeArray_map(_arrow3541, outputs))))
    ...
    if (len(outputs_1) == 0) if (len(inputs_1) == 0) else False:
        ...
        def _arrow3543(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
            def _arrow3542(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                return pvs

            return append(components, delay(_arrow3542))

        class ObjectExpr3544:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping, sort_by(projection, to_list(delay(_arrow3543)), ObjectExpr3544()))
        def _arrow3547(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3546(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3545(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    return comments

                return append(vals, delay(_arrow3545))

            return append(prot_vals, delay(_arrow3546))

        return ResizeArray_singleton(to_list(delay(_arrow3547)))
        ...
        def f_5(tupled_arg: tuple[LDNode | None, LDNode | None], p: Any=p, graph: Any=graph, context: Any=context) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3551(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                def _arrow3550(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                    def _arrow3549(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                        def _arrow3548(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                            return factors

                        return append(pvs, delay(_arrow3548))

                    return append(components, delay(_arrow3549))

                return append(chars, delay(_arrow3550))

            class ObjectExpr3552:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            vals_1: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_1, sort_by(projection_1, to_list(delay(_arrow3551)), ObjectExpr3552()))
            def _arrow3557(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3556(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3555(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3554(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                            def _arrow3553(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                                return singleton(BaseTypes_decomposeProcessOutput_Z2F770004(value_4(o), context)) if (o is not None) else empty_1()

                            return append(comments, delay(_arrow3553))

                        return append(vals_1, delay(_arrow3554))

                    return append(prot_vals, delay(_arrow3555))

                return append(singleton(BaseTypes_decomposeProcessInput_Z2F770004(value_4(i), context)) if (i is not None) else empty_1(), delay(_arrow3556))

            return to_list(delay(_arrow3557))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        def _arrow3560(__unit: None=None, this: Any=this) -> LDNode:
        ...
        return singleton_2(_arrow3560())

    else: 
        def _arrow3561(i: int, this: Any=this) -> LDNode:
        ...
        return List_distinct(initialize(this.RowCount, _arrow3561), ObjectExpr3562())



def ARCtrl_ArcTable__ArcTable_GetProcesses(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        return singleton_2(LDLabProcess.create(this.Name))

    else: 
        getter: Callable[[Any, int], LDNode] = ProcessConversion_getProcessGetter(this.Name, this.Headers)
        def _arrow3565(__unit: None=None, this: Any=this) -> IEnumerable_1[LDNode]:
            def _arrow3564(i: int) -> LDNode:
                return getter(this.Values)(i)

            return map_1(_arrow3564, range_big_int(0, 1, this.RowCount - 1))

        return to_list(delay(_arrow3565))
...
def ARCtrl_ArcTables__ArcTables_GetProcesses(this: ArcTables) -> FSharpList[LDNode]:
    def mapping(t: ArcTable, this: Any=this) -> FSharpList[LDNode]:
...
DatamapConversion_reflection = _expr3569
...
PersonConversion_reflection = _expr3570
...
def PersonConversion_composeAddress_Z721C83C5(address: str) -> Any:
    try: 
        def _arrow3571(__unit: None=None) -> LDNode:
...
ScholarlyArticleConversion_reflection = _expr3572
...
def ScholarlyArticleConversion_composeScholarlyArticle_D324A6D(publication: Publication) -> LDNode:
    ...
    def _arrow3574(__unit: None=None, publication: Any=publication) -> IEnumerable_1[LDNode]:
        def _arrow3573(__unit: None=None) -> IEnumerable_1[LDNode]:
        ...
        return append(singleton(LDPropertyValue.create_doi(value_4(publication.DOI))) if ((value_4(publication.DOI) != "") if (publication.DOI is not None) else False) else empty_1(), delay(_arrow3573))

    identifiers: Array[LDNode] = list(to_list(delay(_arrow3574)))
...
AssayConversion_reflection = _expr3575
...
def AssayConversion_composeAssay_1501C0F8(assay: ArcAssay) -> LDNode:
    ...
    process_sequence: Array[LDNode] | None = Option_fromSeq(list(ARCtrl_ArcTables__ArcTables_GetProcesses(ArcTables(assay.Tables))))
    ...
    def _arrow3576(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds_1: Array[LDNode] = fragment_descriptors
        return fds_1

    def _arrow3577(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        vm_1: LDNode = variable_measured
        return ResizeArray_singleton(vm_1)

    def _arrow3578(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds: Array[LDNode] = fragment_descriptors
        vm: LDNode = variable_measured
        return ResizeArray_appendSingleton(vm, fds)

    variable_measureds: Array[LDNode] | None = (None if (fragment_descriptors is None) else _arrow3576()) if (variable_measured is None) else (_arrow3577() if (fragment_descriptors is None) else _arrow3578())
    ...
    return LDDataset.create_assay(assay.Identifier, None, None, creators, data_files, measurement_method, measurement_technique, variable_measureds, process_sequence, comments)
...
def AssayConversion_decomposeAssay_Z6839B9E8(assay: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcAssay:
    ...
    return ArcAssay.create(LDDataset.get_identifier_as_string(assay, context), variable_measured, measurement_method, measurement_technique, tables.Tables, data_map, perfomers, comments)


def _expr3579() -> TypeInfo:
...
StudyConversion_reflection = _expr3579

def StudyConversion_composeStudy_1680536E(study: ArcStudy) -> LDNode:
    def _arrow3580(s: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3580, study.SubmissionDate)
    def _arrow3581(s_1: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any | None = bind(_arrow3581, study.PublicReleaseDate)
    ...
    process_sequence: Array[LDNode] | None = Option_fromSeq(list(ARCtrl_ArcTables__ArcTables_GetProcesses(ArcTables(study.Tables))))
...
def StudyConversion_decomposeStudy_Z6839B9E8(study: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcStudy:
    def _arrow3583(d: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3583, LDDataset.try_get_date_created_as_date_time(study, context))
    def _arrow3585(d_1: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3585, LDDataset.try_get_date_published_as_date_time(study, context))
...
InvestigationConversion_reflection = _expr3589
...
def InvestigationConversion_composeInvestigation_Z720BD3FF(investigation: ArcInvestigation) -> LDNode:
    ...
    def _arrow3590(s: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3590, investigation.SubmissionDate)
    def _arrow3591(s_1: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any = default_arg(bind(_arrow3591, investigation.PublicReleaseDate), now())
    ...
    def _arrow3592(__unit: None=None, investigation: Any=investigation) -> Array[LDNode]:
    ...
    has_parts: Array[LDNode] | None = Option_fromSeq(_arrow3592())
...
def InvestigationConversion_decomposeInvestigation_Z6839B9E8(investigation: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcInvestigation:
    ...
    def _arrow3593(d: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3593, LDDataset.try_get_date_created_as_date_time(investigation, context))
    def _arrow3594(d_1: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3594, LDDataset.try_get_date_published_as_date_time(investigation, context))
    ...
    return ArcInvestigation.create(LDDataset.get_identifier_as_string(investigation, context), title, LDDataset.try_get_description_as_string(investigation, context), date_created, date_published, None, publications, creators, assays, studies, None, comments)
