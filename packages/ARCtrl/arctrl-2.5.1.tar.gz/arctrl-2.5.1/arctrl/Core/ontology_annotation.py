...
class OntologyAnnotation:
    ...
    def to_string_object(oa: OntologyAnnotation, as_ontobee_purl_url_if_short: bool | None=None) -> dict[str, Any]:
        ...
        def _arrow628(__unit: None=None) -> str:
            url: str = oa.TermAccessionAndOntobeeUrlIfShort
            return default_arg(oa.TermAccessionNumber, "") if (url == "") else url

        return {
            "TermAccessionNumber": _arrow628() if as_ontobee_purl_url_if_short_1 else default_arg(oa.TermAccessionNumber, ""),
            "TermName": TermName,
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow636(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow629(__unit: None=None) -> str:
                arg: str = value_4(this.Name)
                return to_text(printf("Name = %A"))(arg)

            def _arrow635(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow630(__unit: None=None) -> str:
                    arg_1: str = value_4(this.TermSourceREF)
                    return to_text(printf("TSR = %A"))(arg_1)

                def _arrow634(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow631(__unit: None=None) -> str:
                        arg_2: str = value_4(this.TermAccessionNumber)
                        return to_text(printf("TAN = %A"))(arg_2)

                    def _arrow633(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow632(__unit: None=None) -> str:
                            arg_3: Array[Comment] = this.Comments
                            return to_text(printf("Comments = %A"))(arg_3)

                        return singleton(_arrow632()) if (len(this.Comments) != 0) else empty()

                    return append(singleton(_arrow631()) if (this.TermAccessionNumber is not None) else empty(), delay(_arrow633))

                return append(singleton(_arrow630()) if (this.TermSourceREF is not None) else empty(), delay(_arrow634))

            return append(singleton(_arrow629()) if (this.Name is not None) else empty(), delay(_arrow635))

        ignore(StringBuilder__Append_Z721C83C5(sb, join("; ", to_list(delay(_arrow636)))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "}"))
        return to_string(sb)
    ...
    def __hash__(self, __unit: None=None) -> Any:
        ...
        def _arrow640(__unit: None=None) -> IEnumerable_1[Any]:
            ...
            return append(singleton(box_hash_option(this.Name)), delay(_arrow639))

        return box_hash_array(to_array(delay(_arrow640)))
...
OntologyAnnotation_reflection = _expr641
