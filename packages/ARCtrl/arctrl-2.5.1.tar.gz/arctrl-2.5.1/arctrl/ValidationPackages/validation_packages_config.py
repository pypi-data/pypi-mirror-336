...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3071(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3070(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3069(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3068(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3067(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3066(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3066))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3067))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3068))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3069))

            return append(singleton("{"), delay(_arrow3070))

        return join("\n", to_list(delay(_arrow3071)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3080(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3079(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3079, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3080))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3089
