...
gitignore_contract: Contract = Contract.create_create(".gitignore", DTOType(8), DTO(1, dgi))

gitattributes_contract: Contract = Contract.create_create(".gitattributes", DTOType(8), DTO(1, dga))

def _expr3292() -> TypeInfo:
...
Init_reflection = _expr3292

def _expr3293() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow3299(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow3298(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3297(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3296(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3295(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3294(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow3294))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow3295))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow3296))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow3297))

        return append(singleton(Clone_get_clone()), delay(_arrow3298))

    return create_git_contract(to_array(delay(_arrow3299)))
