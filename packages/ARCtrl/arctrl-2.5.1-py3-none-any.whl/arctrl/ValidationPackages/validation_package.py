...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2945(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2944(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2943(__unit: None=None) -> IEnumerable_1[str]:
                    ...
                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2942))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2943))

            return append(singleton("{"), delay(_arrow2944))

        return join("\n", to_list(delay(_arrow2945)))
...
ValidationPackage_reflection = _expr2946
