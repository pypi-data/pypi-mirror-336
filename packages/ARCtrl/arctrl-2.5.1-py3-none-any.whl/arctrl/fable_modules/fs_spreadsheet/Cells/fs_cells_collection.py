...
def FsCellsCollection__RemoveCellAt_Z37302880(this: FsCellsCollection, row: int, column: int) -> None:
    ...
    if (row == this._maxRowUsed) if row_removed else False:
        ...
        this._maxRowUsed = (max(this._rowsUsed.keys(), ObjectExpr158()) if (not is_empty(this._rowsUsed.keys())) else 0) or 0
    ...
    if (column == this._maxColumnUsed) if column_removed else False:
        ...
        this._maxColumnUsed = (max(this._columnsUsed.keys(), ObjectExpr159()) if (not is_empty(this._columnsUsed.keys())) else 0) or 0
    ...
    if match_value is None:
        ...
        def _arrow160(__unit: None=None, this: Any=this, row: Any=row, column: Any=column) -> bool:
...
def FsCellsCollection__GetCells_6611854F(this: FsCellsCollection, row_start: int, column_start: int, row_end: int, column_end: int, predicate: Callable[[FsCell], bool]) -> IEnumerable_1[FsCell]:
    ...
    def _arrow164(__unit: None=None, this: Any=this, row_start: Any=row_start, column_start: Any=column_start, row_end: Any=row_end, column_end: Any=column_end, predicate: Any=predicate) -> IEnumerable_1[FsCell]:
        def _arrow163(ro: int) -> IEnumerable_1[FsCell]:
            ...
            if match_value is None:
                ...
                def _arrow162(co: int) -> IEnumerable_1[FsCell]:
                ...
                return collect(_arrow162, range_big_int(column_start, 1, final_column))


        return collect(_arrow163, range_big_int(row_start, 1, final_row))
