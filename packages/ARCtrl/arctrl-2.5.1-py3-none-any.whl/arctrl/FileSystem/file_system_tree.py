...
class FileSystemTree(Union):
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow444(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow443(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow443))

        file_paths: Array[str] = to_array(delay(_arrow444))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow445(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr447:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow446(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr447()), None)
            if parent.tag == 1:
                def _arrow449(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow448(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow448))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow449)))
        ...
        class ObjectExpr452:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow451(x: Array[str], y: Array[str]) -> bool:
                    def _arrow450(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow450, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr452()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def to_file_paths(remove_root: bool | None=None) -> Callable[[FileSystemTree], Array[str]]:
        def _arrow456(root: FileSystemTree) -> Array[str]:
    ...
    def filter_files(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow461(tree: FileSystemTree) -> FileSystemTree | None:
