...
def from_sparse_table(matrix: SparseTable) -> FSharpList[ArcAssay]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow1194(i: int, matrix: Any=matrix) -> ArcAssay:
        ...
        return initialize(matrix.ColumnCount, _arrow1194)
...
def to_sparse_table(assays: FSharpList[ArcAssay]) -> SparseTable:
    ...
    class ObjectExpr1196:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1195(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1196())), matrix.ColumnCount)
