...
class LDPostalAddress:
    ...
    def gen_id(address_country: str | None=None, postal_code: str | None=None, street_address: str | None=None) -> str:
        def _arrow1473(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow1472(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow1471(__unit: None=None) -> IEnumerable_1[str]:
                    return singleton("streetAddress") if (street_address is not None) else empty()

                return append(singleton("postalCode") if (postal_code is not None) else empty(), delay(_arrow1471))

            return append(singleton("addressCountry") if (address_country is not None) else empty(), delay(_arrow1472))

        items: FSharpList[str] = to_list(delay(_arrow1473))
        def _arrow1474(__unit: None=None) -> str:
        ...
        return clean(create_missing_identifier() if is_empty(items) else _arrow1474())
...
LDPostalAddress_reflection = _expr1475
