...
def encoder(id_map: Any | None, value: Value) -> IEncodable:
    if value.tag == 1:
        ...
        class ObjectExpr2198(IEncodable):
        ...
        return ObjectExpr2198()



def _arrow2200(Item: int) -> Value:
    return Value(1, Item)


def _arrow2201(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow2202(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow2203(Item_3: str) -> Value:
    return Value(3, Item_3)


decoder: Decoder_1[Value] = one_of(of_array([map(_arrow2200, int_1), map(_arrow2201, float_1), map(_arrow2202, OntologyAnnotation_ISAJson_decoder), map(_arrow2203, string)]))
