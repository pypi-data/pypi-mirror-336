...
def _arrow2622(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2623(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2624(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2622, ROCrate_decoder_1), map(_arrow2623, ROCrate_decoder_2), map(_arrow2624, ROCrate_decoder_3)]))
...
def _arrow2626(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2627(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2628(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2626, ISAJson_decoder_1), map(_arrow2627, ISAJson_decoder_2), map(_arrow2628, ISAJson_decoder_3)]))
