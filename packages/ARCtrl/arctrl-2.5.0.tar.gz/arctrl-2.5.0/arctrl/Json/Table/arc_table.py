...
def encoder(table: ArcTable) -> IEncodable:
    def _arrow2741(__unit: None=None, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow2730(__unit: None=None) -> IEncodable:
        ...
        def _arrow2740(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow2731(__unit: None=None) -> IEnumerable_1[IEncodable]:
                return map(encoder_1, table.Headers)

            def _arrow2739(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                def key_encoder(tupled_arg: tuple[int, int]) -> IEncodable:
                    def _arrow2733(value: int, tupled_arg: Any=tupled_arg) -> IEncodable:
                    ...
                    def _arrow2735(value_2: int, tupled_arg: Any=tupled_arg) -> IEncodable:
                    ...
                    return tuple2(_arrow2733, _arrow2735, tupled_arg[0], tupled_arg[1])

                def _arrow2737(__unit: None=None) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    def _arrow2736(match_value: Any) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    ...
                    return collect(_arrow2736, table.Values)

                class ObjectExpr2738:
                    @property
                    def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
                        return compare_arrays

                return singleton(("values", map_1(key_encoder, encoder_2, of_seq(to_list(delay(_arrow2737)), ObjectExpr2738())))) if (len(table.Values) != 0) else empty()

            return append(singleton(("header", list_1(to_list(delay(_arrow2731))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2739))

        return append(singleton(("name", _arrow2730())), delay(_arrow2740))

    values: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2741))
...
def _arrow2748(get: IGetters) -> ArcTable:
    def _arrow2743(__unit: None=None) -> FSharpList[CompositeHeader] | None:
        arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("header", arg_1)

    decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2743(), empty_1()))
    def _arrow2744(__unit: None=None) -> Any | None:
        arg_3: Decoder_1[Any] = map_0027(tuple2_1(int_1, int_1), decoder_2)
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("values", arg_3)

    class ObjectExpr2745:
        @property
        def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
            return compare_arrays
    ...
    decoded_values: Any = Dictionary(default_arg(_arrow2744(), empty_2(ObjectExpr2745())), ObjectExpr2746())
    def _arrow2747(__unit: None=None) -> str:
        object_arg_2: IRequiredGetter = get.Required
        return object_arg_2.Field("name", string)

    return ArcTable.create(_arrow2747(), decoded_header, decoded_values)


decoder: Decoder_1[ArcTable] = object(_arrow2748)
...
def encoder_compressed_column(column_index: int, row_count: int, cell_table: Any, table: ArcTable) -> IEncodable:
    if True if table.Headers[column_index].IsIOType else (row_count < 100):
        def _arrow2750(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2749(r: int) -> IEncodable:
                return encode_cell(cell_table, get_item_from_dict(table.Values, (column_index, r)))

            return map(_arrow2749, range_big_int(0, 1, row_count - 1))

        values: Array[IEncodable] = to_array(delay(_arrow2750))
        ...
        from_: int = 0
        def _arrow2772(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2759(i: int) -> IEnumerable_1[IEncodable]:
                next_1: CompositeCell = get_item_from_dict(table.Values, (column_index, i))
                def _arrow2757(__unit: None=None) -> IEncodable:
                    def _arrow2753(__unit: None=None) -> IEncodable:
                    ...
                    def _arrow2755(__unit: None=None) -> IEncodable:
                    ...
                    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2753()), ("t", _arrow2755()), ("v", encode_cell(cell_table, current))])
                ...
                def _arrow2758(__unit: None=None) -> IEnumerable_1[IEncodable]:
                    nonlocal current, from_
                    current = next_1
                    from_ = i or 0
                    return empty()

                return append(singleton(_arrow2757()), delay(_arrow2758)) if (not equals(next_1, current)) else empty()

            def _arrow2771(__unit: None=None) -> IEnumerable_1[IEncodable]:
                def _arrow2770(__unit: None=None) -> IEncodable:
                    ...
                    values_2: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2762()), ("t", _arrow2766()), ("v", encode_cell(cell_table, current))])
                ...
                return singleton(_arrow2770())

            return append(collect(_arrow2759, range_big_int(1, 1, row_count - 1)), delay(_arrow2771))

        values_3: Array[IEncodable] = to_array(delay(_arrow2772))
        class ObjectExpr2775(IEncodable):
    ...
    class ObjectExpr2786(Decoder_1[None]):
        def Decode(self, helper: IDecoderHelpers_1[Any], column: Any, cell_table: Any=cell_table, table: Any=table, column_index: Any=column_index) -> FSharpResult_2[None, tuple[str, ErrorReason_1[__A_]]]:
            ...
            if match_value.tag == 1:
                def _arrow2780(get: IGetters) -> None:
                    from_: int
                ...
                range_decoder: Decoder_1[None] = object(_arrow2780)
...
def arrayi(decoderi: Callable[[int], Decoder_1[_VALUE]]) -> Decoder_1[Array[Any]]:
    class ObjectExpr2800(Decoder_1[Array[_VALUE_]]):
                ...
                def folder(acc: FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]], value_1: __A_) -> FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]]:
                    ...
                    if acc.tag == 0:
                        ...
                        if match_value.tag == 0:
                            ...
                            def _arrow2797(__unit: None=None, acc: Any=acc, value_1: Any=value_1) -> tuple[str, ErrorReason_1[__A_]]:
                            ...
                            return FSharpResult_2(1, _arrow2797())
...
def encoder_compressed(string_table: Any, oa_table: Any, cell_table: Any, table: ArcTable) -> IEncodable:
    def _arrow2809(__unit: None=None, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow2808(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow2803(__unit: None=None) -> IEnumerable_1[IEncodable]:
                return map(encoder_1, table.Headers)

            def _arrow2807(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                if len(table.Values) != 0:
                    row_count: int = table.RowCount or 0
                    def _arrow2805(__unit: None=None) -> IEnumerable_1[IEncodable]:
                        def _arrow2804(c: int) -> IEncodable:
                            return encoder_compressed_column(c, row_count, cell_table, table)

                        return map(_arrow2804, range_big_int(0, 1, table.ColumnCount - 1))

                    columns: Array[IEncodable] = to_array(delay(_arrow2805))
                    ...
                    return singleton(("c", ObjectExpr2806()))

                else: 
                    return empty()


            return append(singleton(("h", list_1(to_list(delay(_arrow2803))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2807))

        return append(singleton(("n", encode_string(string_table, table.Name))), delay(_arrow2808))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2809))
...
def decoder_compressed(string_table: Array[str], oa_table: Array[OntologyAnnotation], cell_table: Array[CompositeCell]) -> Decoder_1[ArcTable]:
    def _arrow2821(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table) -> ArcTable:
        def _arrow2812(__unit: None=None) -> FSharpList[CompositeHeader] | None:
            arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
            object_arg: IOptionalGetter = get.Optional
            return object_arg.Field("h", arg_1)

        decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2812(), empty_1()))
        def _arrow2814(__unit: None=None) -> str:
            arg_3: Decoder_1[str] = decode_string(string_table)
            object_arg_1: IRequiredGetter = get.Required
            return object_arg_1.Field("n", arg_3)

        class ObjectExpr2816:
            @property
            def Equals(self) -> Callable[[tuple[int, int], tuple[int, int]], bool]:
                return equal_arrays
        ...
        table: ArcTable = ArcTable.create(_arrow2814(), decoded_header, Dictionary([], ObjectExpr2816()))
        def _arrow2820(__unit: None=None) -> Array[None] | None:
            def _arrow2818(column_index: int) -> Decoder_1[None]:
                return decoder_compressed_column(cell_table, table, column_index)

            arg_5: Decoder_1[Array[None]] = arrayi(_arrow2818)
