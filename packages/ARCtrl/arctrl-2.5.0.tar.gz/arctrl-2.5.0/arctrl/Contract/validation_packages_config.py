...
ValidationPackagesConfigHelper_ReadContract: Contract = Contract("READ", ValidationPackagesConfigHelper_ConfigFilePath, DTOType_1(7), None)

def ValidationPackagesConfigExtensions__007CValidationPackagesYamlPath_007C__007C(input: Array[str]) -> str | None:
    (pattern_matching_result,) = (None,)
    def _arrow3273(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 2) if (not equals_with(_arrow3273, input, None)) else False:
        if input[0] == ".arc":
...
def ARCtrl_ValidationPackages_ValidationPackagesConfig__ValidationPackagesConfig_ToCreateContract(this: ValidationPackagesConfig) -> Contract:
    return Contract.create_create(ValidationPackagesConfigHelper_ConfigFilePath, DTOType_1(7), DTO(1, ARCtrl_ValidationPackages_ValidationPackagesConfig__ValidationPackagesConfig_toYamlString_Static_71136F3F()(this)))
        ...
        if c.DTOType is not None:
            if c.DTOType.tag == 7:
                if c.DTO is not None:
                    if c.DTO.tag == 1:
                        def _arrow3279(__unit: None=None, c: Any=c) -> bool:
