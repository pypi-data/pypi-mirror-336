...
class MaterialAttributeValue(Record):
    ...
    def PrintCompact(self, __unit: None=None) -> str:
        ...
        def _arrow671(__unit: None=None) -> str:
            value_2: str = value
            return value_2

        def _arrow672(__unit: None=None) -> str:
            category_2: str = category
            return (category_2 + ":") + "No Value"

        def _arrow673(__unit: None=None) -> str:
            category_1: str = category
            value_1: str = value
            return (category_1 + ":") + value_1

        return ("" if (value is None) else _arrow671()) if (category is None) else (_arrow672() if (value is None) else _arrow673())
...
MaterialAttributeValue_reflection = _expr674
...
def MaterialAttributeValue__SetCategory_ZDED3A0F(this: MaterialAttributeValue, c: OntologyAnnotation) -> MaterialAttributeValue:
    def _arrow675(__unit: None=None, this: Any=this, c: Any=c) -> MaterialAttribute | None:
    ...
    return MaterialAttributeValue(this.ID, _arrow675(), this.Value, this.Unit)
