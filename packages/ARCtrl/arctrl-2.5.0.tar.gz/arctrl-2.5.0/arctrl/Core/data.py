...
class Data:
    ...
    def Name(self, __unit: None=None) -> str | None:
        ...
        def _arrow642(__unit: None=None) -> str | None:
            p_1: str = match_value
            return p_1

        def _arrow643(__unit: None=None) -> str | None:
            p: str = match_value
            s: str = match_value_1
            return DataAux_nameFromPathAndSelector(p, s)

        return None if (match_value is None) else (_arrow642() if (match_value_1 is None) else _arrow643())
    ...
    def GetAbsolutePathForAssay(self, assay_identifier: str, check_existence_from_root: Callable[[str], bool] | None=None) -> str:
        this: Data = self
        def _arrow644(_arg: str) -> bool:
            return False

        check_existence_from_root_1: Callable[[str], bool] = default_arg(check_existence_from_root, _arrow644)
        ...
        if match_value is None:
            ...
            p: str = match_value
            return p if (True if (True if (True if (True if check_existence_from_root_1(p) else starts_with_exact(p, "assays/")) else starts_with_exact(p, "studies/")) else starts_with_exact(p, "http:")) else starts_with_exact(p, "https:")) else ((("assays/" + assay_identifier) + "/dataset/") + trim_start(p, "/"))


    def GetAbsolutePathForStudy(self, study_identifier: str, check_existence_from_root: Callable[[str], bool] | None=None) -> str:
        this: Data = self
        def _arrow645(_arg: str) -> bool:
            return False

        check_existence_from_root_1: Callable[[str], bool] = default_arg(check_existence_from_root, _arrow645)
        match_value: str | None = this.FilePath
        if match_value is None:
            raise Exception("Data does not have a file path")
            ...
            p: str = match_value
            return p if (True if (True if (True if (True if check_existence_from_root_1(p) else starts_with_exact(p, "assays/")) else starts_with_exact(p, "studies/")) else starts_with_exact(p, "http:")) else starts_with_exact(p, "https:")) else ((("studies/" + study_identifier) + "/resources/") + trim_start(p, "/"))
