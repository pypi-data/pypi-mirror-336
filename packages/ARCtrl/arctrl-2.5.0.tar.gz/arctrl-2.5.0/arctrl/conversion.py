...
def DateTime_tryFromString(s: str) -> Any | None:
    try: 
        def _arrow3489(__unit: None=None) -> Any:
...
def ColumnIndex_tryInt(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3490(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3491(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3490, _arrow3491)), out_arg)
    if match_value[0]:
...
BaseTypes_reflection = _expr3492
...
ProcessConversion_reflection = _expr3499
...
def ProcessConversion_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 0:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3500(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 3:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3501(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 2:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3502(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 1:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3503(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetProtocolREFGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 8:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3505(i: int, matrix: Any=matrix) -> str:
...
def ProcessConversion_tryGetProtocolURIGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 6:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3507(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 7:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3508(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetInputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 11:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3509(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetOutputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 12:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3510(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 14:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3511(i: int, matrix: Any=matrix) -> str:
...
def ProcessConversion_tryGetPerformerGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 9:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3512(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], LDNode]:
    ...
    if match_value is None:
        def _arrow3514(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3513(i_1: int) -> Array[LDNode]:
        ...
        def _arrow3516(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3515(i_2: int) -> Array[LDNode]:
                return []

            return _arrow3515

        input_getter_1 = _arrow3514 if (length(char_getters) != 0) else _arrow3516

    else: 
        input_getter: Callable[[Any, int], LDNode] = match_value
        def _arrow3518(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3517(i: int) -> Array[LDNode]:
    ...
    if match_value_1 is None:
        def _arrow3520(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3519(i_4: int) -> Array[LDNode]:
        ...
        def _arrow3522(matrix_5: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3521(i_5: int) -> Array[LDNode]:
                return []

            return _arrow3521

        output_getter_1 = _arrow3520 if (length(factor_value_getters) != 0) else _arrow3522

    else: 
        output_getter: Callable[[Any, int], LDNode] = match_value_1
        def _arrow3524(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3523(i_3: int) -> Array[LDNode]:
        ...
        output_getter_1 = _arrow3524

    def _arrow3528(matrix_6: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], LDNode]:
        def _arrow3527(i_6: int) -> LDNode:
            def mapping_4(tuple_1: tuple[int, int]) -> int:
                return tuple_1[1]

            class ObjectExpr3525:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            pn: str = process_name_root if ((1 + max(map_1(mapping_4, matrix_6.keys()), ObjectExpr3525())) == 1) else ProcessConversion_composeProcessName(process_name_root, i_6)
...
def ProcessConversion_groupProcesses_Z27F0B586(processes: FSharpList[LDNode], graph: LDGraph | None=None, context: LDContext | None=None) -> FSharpList[tuple[str, FSharpList[LDNode]]]:
    def projection(p: LDNode, processes: Any=processes, graph: Any=graph, context: Any=context) -> str:
        ...
        if match_value is not None:
            if ProcessConversion_decomposeProcessName_Z721C83C5(match_value)[1] is not None:
                ...
                def _arrow3530(__unit: None=None, p: Any=p) -> bool:
                ...
                if _arrow3530():
                    ...
                    def _arrow3531(__unit: None=None, p: Any=p) -> bool:
                ...
                def _arrow3532(__unit: None=None, p: Any=p) -> bool:
            ...
            def _arrow3533(__unit: None=None, p: Any=p) -> bool:
    ...
    class ObjectExpr3535:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3534(x: str, y: str) -> bool:
                return x == y
    ...
    return List_groupBy(projection, processes, ObjectExpr3535())
...
def ProcessConversion_processToRows_Z6839B9E8(p: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> Array[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow3547(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3539(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3546(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3540(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                ...
                def _arrow3545(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3541(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    ...
                    def _arrow3544(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3542(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        def _arrow3543(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(_arrow3542(), delay(_arrow3543))

                    return append(_arrow3541(), delay(_arrow3544))

                return append(_arrow3540(), delay(_arrow3545))

            return append(_arrow3539(), delay(_arrow3546))

        prot_vals = to_list(delay(_arrow3547))
    ...
    def _arrow3548(Value: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value

    def _arrow3549(Value_1: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_1

    def _arrow3550(Value_2: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_2

    def _arrow3551(Value_3: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_3

    pattern_input: tuple[Array[LDNode | None], Array[LDNode | None]] = ((ResizeArray_create(len(outputs), None), ResizeArray_map(_arrow3548, outputs))) if ((len(outputs) != 0) if (len(inputs) == 0) else False) else (((ResizeArray_map(_arrow3549, inputs), ResizeArray_create(len(inputs), None))) if ((len(outputs) == 0) if (len(inputs) != 0) else False) else ((ResizeArray_map(_arrow3550, inputs), ResizeArray_map(_arrow3551, outputs))))
    ...
    if (len(outputs_1) == 0) if (len(inputs_1) == 0) else False:
        ...
        def _arrow3553(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
            def _arrow3552(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                return pvs

            return append(components, delay(_arrow3552))

        class ObjectExpr3554:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping, sort_by(projection, to_list(delay(_arrow3553)), ObjectExpr3554()))
        def _arrow3557(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3556(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                ...
                return append(vals, delay(_arrow3555))

            return append(prot_vals, delay(_arrow3556))

        return ResizeArray_singleton(to_list(delay(_arrow3557)))
        ...
        def f_5(tupled_arg: tuple[LDNode | None, LDNode | None], p: Any=p, graph: Any=graph, context: Any=context) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3561(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                def _arrow3560(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                    def _arrow3559(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                        def _arrow3558(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                            return factors

                        return append(pvs, delay(_arrow3558))

                    return append(components, delay(_arrow3559))

                return append(chars, delay(_arrow3560))
            ...
            vals_1: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_1, sort_by(projection_1, to_list(delay(_arrow3561)), ObjectExpr3562()))
            def _arrow3567(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3566(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3565(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3564(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                            def _arrow3563(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                                return singleton(BaseTypes_decomposeProcessOutput_Z2F770004(value_4(o), context)) if (o is not None) else empty_1()

                            return append(comments, delay(_arrow3563))

                        return append(vals_1, delay(_arrow3564))

                    return append(prot_vals, delay(_arrow3565))

                return append(singleton(BaseTypes_decomposeProcessInput_Z2F770004(value_4(i), context)) if (i is not None) else empty_1(), delay(_arrow3566))

            return to_list(delay(_arrow3567))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        def _arrow3568(__unit: None=None, this: Any=this) -> LDNode:
        ...
        return singleton_2(_arrow3568())

    else: 
        def _arrow3569(i: int, this: Any=this) -> LDNode:
        ...
        return List_distinct(initialize(this.RowCount, _arrow3569), ObjectExpr3570())
...
def ARCtrl_ArcTable__ArcTable_GetProcesses(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        ...
        def _arrow3572(__unit: None=None, this: Any=this) -> IEnumerable_1[LDNode]:
            def _arrow3571(i: int) -> LDNode:
                return getter(this.Values)(i)

            return map_1(_arrow3571, range_big_int(0, 1, this.RowCount - 1))

        return to_list(delay(_arrow3572))
...
DatamapConversion_reflection = _expr3573
...
PersonConversion_reflection = _expr3574
...
def PersonConversion_composeAddress_Z721C83C5(address: str) -> Any:
    try: 
        def _arrow3575(__unit: None=None) -> LDNode:
...
ScholarlyArticleConversion_reflection = _expr3576
...
def ScholarlyArticleConversion_composeScholarlyArticle_D324A6D(publication: Publication) -> LDNode:
    ...
    def _arrow3578(__unit: None=None, publication: Any=publication) -> IEnumerable_1[LDNode]:
        def _arrow3577(__unit: None=None) -> IEnumerable_1[LDNode]:
        ...
        return append(singleton(LDPropertyValue.create_doi(value_4(publication.DOI))) if ((value_4(publication.DOI) != "") if (publication.DOI is not None) else False) else empty_1(), delay(_arrow3577))

    identifiers: Array[LDNode] = list(to_list(delay(_arrow3578)))
...
AssayConversion_reflection = _expr3579
...
def AssayConversion_composeAssay_1501C0F8(assay: ArcAssay) -> LDNode:
    ...
    def _arrow3580(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds_1: Array[LDNode] = fragment_descriptors
        return fds_1

    def _arrow3581(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        vm_1: LDNode = variable_measured
        return ResizeArray_singleton(vm_1)

    def _arrow3582(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds: Array[LDNode] = fragment_descriptors
        vm: LDNode = variable_measured
        return ResizeArray_appendSingleton(vm, fds)

    variable_measureds: Array[LDNode] | None = (None if (fragment_descriptors is None) else _arrow3580()) if (variable_measured is None) else (_arrow3581() if (fragment_descriptors is None) else _arrow3582())
...
StudyConversion_reflection = _expr3583

def StudyConversion_composeStudy_1680536E(study: ArcStudy) -> LDNode:
    def _arrow3584(s: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3584, study.SubmissionDate)
    def _arrow3585(s_1: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any | None = bind(_arrow3585, study.PublicReleaseDate)
...
def StudyConversion_decomposeStudy_Z6839B9E8(study: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcStudy:
    def _arrow3586(d: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3586, LDDataset.try_get_date_created_as_date_time(study, context))
    def _arrow3587(d_1: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3587, LDDataset.try_get_date_published_as_date_time(study, context))
...
InvestigationConversion_reflection = _expr3588
...
def InvestigationConversion_composeInvestigation_Z720BD3FF(investigation: ArcInvestigation) -> LDNode:
    ...
    def _arrow3589(s: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3589, investigation.SubmissionDate)
    def _arrow3590(s_1: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any = default_arg(bind(_arrow3590, investigation.PublicReleaseDate), now())
    ...
    def _arrow3591(__unit: None=None, investigation: Any=investigation) -> Array[LDNode]:
    ...
    has_parts: Array[LDNode] | None = Option_fromSeq(_arrow3591())
...
def InvestigationConversion_decomposeInvestigation_Z6839B9E8(investigation: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcInvestigation:
    ...
    def _arrow3592(d: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3592, LDDataset.try_get_date_created_as_date_time(investigation, context))
    def _arrow3593(d_1: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3593, LDDataset.try_get_date_published_as_date_time(investigation, context))
