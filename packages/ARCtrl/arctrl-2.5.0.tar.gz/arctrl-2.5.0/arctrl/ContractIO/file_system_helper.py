...
def directory_exists_async(path: str) -> Async[bool]:
    def _arrow3402(__unit: None=None, path: Any=path) -> Async[bool]:
        return singleton.Return(Path(path).is_dir())

    return singleton.Delay(_arrow3402)


def create_directory_async(path: str) -> Async[None]:
    def _arrow3403(__unit: None=None, path: Any=path) -> Async[None]:
        Path(path).mkdir(parents=True, exist_ok=True)
        return singleton.Zero()

    return singleton.Delay(_arrow3403)


def ensure_directory_async(path: str) -> Async[None]:
    def _arrow3405(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3404(_arg: bool) -> Async[None]:
            return singleton.ReturnFrom(create_directory_async(path)) if (not _arg) else singleton.Zero()

        return singleton.Bind(directory_exists_async(path), _arrow3404)

    return singleton.Delay(_arrow3405)


def ensure_directory_of_file_async(file_path: str) -> Async[None]:
    def _arrow3406(__unit: None=None, file_path: Any=file_path) -> Async[None]:
        return singleton.ReturnFrom(ensure_directory_async(Path(file_path).parent))

    return singleton.Delay(_arrow3406)


def file_exists_async(path: str) -> Async[bool]:
    def _arrow3407(__unit: None=None, path: Any=path) -> Async[bool]:
        return singleton.Return(Path(path).is_file())

    return singleton.Delay(_arrow3407)


def read_file_text_async(path: str) -> Async[str]:
    def _arrow3409(__unit: None=None, path: Any=path) -> Async[str]:
        def _arrow3408(__unit: None=None) -> str:
            with open(path, 'r', encoding='utf-8') as f: return f.read()

        return singleton.Return(_arrow3408())

    return singleton.Delay(_arrow3409)


def read_file_binary_async(path: str) -> Async[bytearray]:
    def _arrow3411(__unit: None=None, path: Any=path) -> Async[bytearray]:
        def _arrow3410(__unit: None=None) -> bytearray:
            with open(path, 'rb') as f: return f.read()

        return singleton.Return(_arrow3410())

    return singleton.Delay(_arrow3411)


def read_file_xlsx_async(path: str) -> Async[FsWorkbook]:
    def _arrow3412(__unit: None=None, path: Any=path) -> Async[FsWorkbook]:
        return singleton.Return(FsSpreadsheet_FsWorkbook__FsWorkbook_fromXlsxFile_Static_Z721C83C5(path))

    return singleton.Delay(_arrow3412)


def move_file_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3413(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
        shutil.move(old_path, new_path)
        return singleton.Zero()

    return singleton.Delay(_arrow3413)
...
def delete_file_async(path: str) -> Async[None]:
    def _arrow3414(__unit: None=None, path: Any=path) -> Async[None]:
        try:
            os.remove(path)
    ...
    return singleton.Delay(_arrow3414)


def delete_directory_async(path: str) -> Async[None]:
    def _arrow3415(__unit: None=None, path: Any=path) -> Async[None]:
        shutil.rmtree(path, ignore_errors=True)
        return singleton.Zero()

    return singleton.Delay(_arrow3415)


def write_file_text_async(path: str, text: str) -> Async[None]:
    def _arrow3416(__unit: None=None, path: Any=path, text: Any=text) -> Async[None]:
        with open(path, 'w') as f: f.write(text)
        return singleton.Zero()

    return singleton.Delay(_arrow3416)


def write_file_binary_async(path: str, bytes: bytearray) -> Async[None]:
    def _arrow3417(__unit: None=None, path: Any=path, bytes: Any=bytes) -> Async[None]:
        with open(path, 'wb') as f: f.write(bytes)
        return singleton.Zero()

    return singleton.Delay(_arrow3417)


def write_file_xlsx_async(path: str, wb: FsWorkbook) -> Async[None]:
    def _arrow3418(__unit: None=None, path: Any=path, wb: Any=wb) -> Async[None]:
        FsSpreadsheet_FsWorkbook__FsWorkbook_toXlsxFile_Static(path, wb)
        return singleton.Zero()

    return singleton.Delay(_arrow3418)
...
def get_sub_directories_async(path: str) -> Async[Array[str]]:
    def _arrow3419(__unit: None=None, path: Any=path) -> Async[Array[str]]:
    ...
    return singleton.Delay(_arrow3419)


def get_sub_files_async(path: str) -> Async[Array[str]]:
    def _arrow3420(__unit: None=None, path: Any=path) -> Async[Array[str]]:
        paths: Array[str] = [str(entry) for entry in Path(path).iterdir() if entry.is_file()]
        return singleton.Return(map(standardize_slashes, paths, None))

    return singleton.Delay(_arrow3420)


def get_all_file_paths_async(directory_path: str) -> Async[Array[str]]:
    directory_path_1: str = standardize_slashes(directory_path)
    def _arrow3426(__unit: None=None, directory_path: Any=directory_path) -> Async[Array[str]]:
        def all_files(dirs: IEnumerable_1[str]) -> Async[IEnumerable_1[str]]:
            def _arrow3424(__unit: None=None, dirs: Any=dirs) -> Async[IEnumerable_1[str]]:
                def _arrow3423(_arg: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                    sub_files_1: IEnumerable_1[str] = concat(_arg)
                    def _arrow3422(_arg_1: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                        def _arrow3421(_arg_2: Array[IEnumerable_1[str]]) -> Async[IEnumerable_1[str]]:
                            sub_dir_contents_1: IEnumerable_1[str] = concat(_arg_2)
                            return singleton.Return(append(sub_dir_contents_1, sub_files_1))

                        return singleton.Bind(all(map_1(all_files, _arg_1)), _arrow3421)

                    return singleton.Bind(all(map_1(get_sub_directories_async, dirs)), _arrow3422)

                return singleton.Return(empty()) if is_empty(dirs) else singleton.Bind(all(map_1(get_sub_files_async, dirs)), _arrow3423)

            return singleton.Delay(_arrow3424)

        def _arrow3425(_arg_3: IEnumerable_1[str]) -> Async[Array[str]]:
        ...
        return singleton.Bind(all_files(to_enumerable([directory_path_1])), _arrow3425)

    return singleton.Delay(_arrow3426)


def rename_file_or_directory_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3429(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
        def _arrow3428(_arg: bool) -> Async[None]:
            def _arrow3427(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(old_path), _arrow3427)

        return singleton.Bind(file_exists_async(old_path), _arrow3428)

    return singleton.Delay(_arrow3429)


def delete_file_or_directory_async(path: str) -> Async[None]:
    def _arrow3432(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3431(_arg: bool) -> Async[None]:
            def _arrow3430(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(path), _arrow3430)

        return singleton.Bind(file_exists_async(path), _arrow3431)

    return singleton.Delay(_arrow3432)
