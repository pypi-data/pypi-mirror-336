...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow2951(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2950(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2949(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2948(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2947(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2946(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2946))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2947))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2948))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2949))

            return append(singleton("{"), delay(_arrow2950))

        return join("\n", to_list(delay(_arrow2951)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2954(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2953(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2953, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2954))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2955
