...
class SASTBox(object):
    ...
    def _scan_diff(self, code_dir, current_commit, previous_commit, log):
        ...
        environment = {
            ...
            'SASTBOX_REPORT_DIR': '/tmp',  # Directory where reports are generated
            'SASTBOX_REPORT_PATTERN': '*.sarif',  # Pattern for report files
            'SASTBOX_CODE_DIR': self.CONTAINER_CODE_DIR,
        }

        # Build the command as a list for clarity, then join into a string
        command_parts = [
        ...
        try:
            ...
            # Call recovery function (assumed to be correctly implemented)
            self.recovery_technologies_file()

            # Wait for container completion (redundant after logs, but kept for clarity)
