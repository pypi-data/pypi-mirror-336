...
class ModelErrorInputMessages(Enum):
    MULTI_INPUT_ERR = "The provided model has more than one input. Model should have exactly one input."
    DIMENSION_ERR = ("The provided model has less or more than 4 dimensions. MCT currently supports models"
                     " with 4 dimensions [e.g. batch=1, channels, height, width].")
    BATCH_SIZE_ERR = "The provided model has batch-size > 1. MCT currently supports batch-size of 1 only."
    DYNAMIC_INPUT_ERR = ("The provided model has a dynamic input tensor shape. "
...
class UnsupportedModelInputError(UnsupportedError):
    ...
    def __init__(self, reason: ModelErrorInputMessages):
