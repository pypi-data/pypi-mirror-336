    ...
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
    ...
    if response.status_code == 200:
        ...
        for response_200_item_data in _response_200:
            response_200_item = GetEventList.from_dict(response_200_item_data)
...
def sync_detailed(
    *,
    ...
    """Get Event List (Calendar Event)

    Args:
        until (datetime.datetime): Indicates the date and time until which to search for events
        filter_ (Optional[str]): The OData query parameter to retrieve a subset of the
            CuratedCalendarEvents
        calendar_id (Optional[str]): The calendar for the event. If not indicated, the user's
            default calendar will be used.
        size (Optional[str]): The maximum number of events to be returned. Default: '50'.
        from_ (datetime.datetime): Indicates the date and time as of when to search for events
        output_timezone (Optional[str]): Timezone for current event. If left blank will be
            defaulted to UTC.
