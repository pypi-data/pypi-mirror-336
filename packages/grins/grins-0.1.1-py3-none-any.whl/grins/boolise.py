...
from grins.ising_bool import run_simulations
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, help="mode of simulation. Choose between async, sync", default="async"
    )
    parser.add_argument(
        "--flipvalue", type=str, help="replacement values set. 0 = [Low 0, High 1]; -1 = [Low -1, High 1]", default="0"
    )
    ...
    replacement_dict = {"0": jnp.array([0,1]),
                        "-1": jnp.array([-1,1])}
