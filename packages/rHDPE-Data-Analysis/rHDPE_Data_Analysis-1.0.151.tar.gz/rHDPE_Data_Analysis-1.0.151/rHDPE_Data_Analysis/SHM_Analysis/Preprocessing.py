...
def write_csv( output_directory, file_data, data, name_appendage = "" ):
    ...
    for i, f in enumerate( file_data ):
        ...
        np.savetxt( output_directory + "SHM/Condensed_Data/Resin{}_{}_".format( f[0], f[1] ) + name_appendage + ".csv", array, delimiter = ",", fmt = "%.4f" )
...
def copy_data( output_directory, name_appendage_1, name_appendage_2 ):
    ...
    for path in glob.glob( output_directory + "SHM/Condensed_Data/*" ):
        ...
        if pattern.search( path ):
            ...
            shutil.copyfile( path, output_directory + "SHM/Condensed_Data/Resin" + str( resin ) + "_" + str( specimen ) + "_" + name_appendage_2 + ".csv" )

    shutil.copyfile( output_directory + "SHM/File_Data/file_data" + name_appendage_1 + ".csv", output_directory + "SHM/File_Data/file_data" + name_appendage_2 + ".csv" )
...
def delete_data( output_directory, name_appendage ):
    ...
    for path in glob.glob( output_directory + "SHM/File_Data/*" ):

        if pattern.search( path ):

            os.remove( path )

    for path in glob.glob( output_directory + "SHM/Features/*" ):
