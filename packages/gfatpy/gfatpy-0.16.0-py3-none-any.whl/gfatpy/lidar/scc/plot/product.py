...
from gfatpy.lidar.scc.plot import PLOT_INFO
from gfatpy.lidar.scc.plot.utils import id2str
    ...
    def plot(
        ...
        **kwargs,
        ...
        for channel_ in data.channels.values:
            ...
            colors = color_list(
                data.sizes["time"], cmap=PLOT_INFO["wavelength2cmaps"][wavelength]
            )
...
class SCC_elpp(SCC_product):
    ...
    def __init__(self, path: Path):
        ...
        self.string_ID = id2str(self.productID)
    ...
    def rayleight_fit(
        ...
        Args:
            ...
            attenuated_backscatter_limits (tuple[float, float] | None, optional): Attenuated backscatter limits. Defaults to None means automatic limits from `gfatpy.lidar.scc.plot.PLOT_INFO`.
        ...
        for idx, channel_ in enumerate(raw_data.channel):
            ...
            string_list = [id2str(int(ID)) for ID in IDs]
            channelID_ = os.path.commonprefix(string_list)
            ...
            for idx, time_ in enumerate(raw_data.time.values):
                ...
                (1e6 * normalized_rcs.sel(time=time_, channel=channel_)).plot(
                    ...
                    + f"[{channelID_}] | {time_.astype(str).split('.')[0]}",
                    color=colors[idx],
                    linewidth=1.0,
                )
                (
            ...
            for idx, time_ in enumerate(elpp.time.values):
                try:
                    elpp.sel(time=time_, channel=product_)[
                        ...
                        x="altitude",
                        ax=ax,
                        label=f"{channelID_} | {time_.astype(str).split('.')[0]}",
                        color=colors[idx],
                        linewidth=1.0,
                    )
                except:
...
class SCC_elda(SCC_product):
        ...
        for time_ in elda.time.values:
            try:
                (elda.sel(time=time_)["backscatter"] * 1e6).plot(
                    ...
                    color=PLOT_INFO["backscatter2color"][self.string_ID],
                    linewidth=1.0,
                )
            except:
    ...
    def plot_extinction(
        ...
        elda = xr.open_dataset(self.path)
        elda["altitude"] = elda["altitude"] / 1e3
        color = PLOT_INFO["extinction2color"][self.string_ID]
        for time_ in elda.time.values:
            try:
    ...
    def plot_depo(
        ...
        elda = xr.open_dataset(self.path)
        elda["altitude"] = elda["altitude"] / 1e3
        color = PLOT_INFO["depolarization2color"][self.string_ID]
        for time_ in elda.time.values:
            try:
    ...
    def plot_lidar_ratio(
        ...
        elda = xr.open_dataset(self.path)
        elda["altitude"] = elda["altitude"] / 1e3
        color = PLOT_INFO["lidar_ratio2color"][self.string_ID]
        for time_ in elda.time.values:
            try:
                (
