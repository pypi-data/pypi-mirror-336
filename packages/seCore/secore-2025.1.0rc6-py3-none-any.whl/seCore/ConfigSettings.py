...
from seCore.templates import ConfigSettings, Keys
...
VALIDATION_RULES = {
    "log_level": "ERROR",
    "log_appname": "ERROR",
    "log_inc_resp_body": "ERROR",
    "log_format": "WARNING",
}

# Constant for encryption key
...
class ConfigManager:
    ...
    def _setup_directories_and_configs(self):
        ...
        Supported directories include the configuration directory and secret directory.

        The method also ensures environmental and secret configuration files exist with defaults,
        ...
        self._initialize_file(os.path.join(self.config_dir, ENV_FILE_SETTINGS), Template(ConfigSettings.DEFAULT_ENV_SETTINGS).safe_substitute(fn_key=Fernet.generate_key().decode()))
    ...
    def _parse_line(line: str) -> tuple[str, str]:
        ...
        return key.strip().upper(), value.strip().strip('"\'')  # Clean up quotes
...
class Settings(BaseSettings):
    ...
    :ivar VERSION: The current version of the package.
    :type VERSION: str
    :ivar FN_KEY: The key used for specific functionality, defaulting to a
                  system environment variable if available.
    :type FN_KEY: str
    :ivar PROJECT_ROOT: The root directory of the project, automatically
...
def env_secrets_settings(_settings: BaseSettings) -> dict[str, Any]:
    ...
    This function utilizes a YAML-safe loader to transform configuration
