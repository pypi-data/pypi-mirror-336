...
def gen_check_against_c_main(classes, *, tab='  '):
    out = """
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
class XCType:
    ...
    def legal_c_struct(self):
        if self._legal_c_struct is None:
            if self.is_primitive() or self.is_array():
                ...
                if var_size_cnt > 1:
                    logging.debug(
                        f'{self._type_name} not legal struct because it contains {var_size_cnt} variable size members'
                    )
                    self._legal_c_struct = False
    ...
    def __init__(self, init=None, *, packed=False, fill_byte: int | None = None):
        ...
        if init is not None:
            self.do_init(init, fill_byte=fill_byte)

    def do_init(self, init, *, fill_byte: int | None = None):
        if init is not None:
