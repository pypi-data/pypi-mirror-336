...
import numpy as np
from abstract_utilities import eatAll,safe_read_from_json,safe_dump_to_file,eatAll
...
def get_text_constants(video_text):
    ...
    for j,frame in enumerate(video_text):
        text = frame.get('text')
        frame = os.path.splitext(frame.get('frame'))[0].split('_')
        text_spl = text.split('\n')
...
def extract_subtitles(ocr_data, total_frames, frequency_threshold=0.2, min_subtitle_count=1, max_subtitle_count=10):
    text_contents = ocr_data["text_constants"]
...
def derive_all_video_meta(video_path,output_dir=None,video_text_path=None,keywords=None,description=None,title=None):
    ...
    video_info['keywords'] = video_info
    video_info['title'] = title
    video_info['description'] = description
    video_json = {"video_file_path":video_path,'thumbnail_texts':thumbnail_texts,"video_info":video_info,"video_text":video_text,"text_constants":text_constants,"subtitles":subtitles}
