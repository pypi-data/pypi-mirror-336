...
def get_file(basename,directory):
    file=None
    for item in os.listdir(directory):
        basename = os.path.splitext(item)[0]
        if basename and str(video_id) == str(basename):
            file = item
            break
    return file
...
def derive_all_video_meta(video_file_path,output_dir=None):
    ...
    video_text = {"video_file_path":video_file_path,'thumbnail_texts':thumbnail_texts,"video_info":video_info,"video_text":video_text,"text_contants":text_contants}
    safe_dump_to_file(data=video_text,file_path=video_text_path)
