...
from fast_calc_harmonic_coefficients_modify import fast_calc_harmonic_coefficients_modify


def compute_harmonic_coefficients(ai, harmonic_index):
    """单独计算一个谐波的系数"""
    return fast_calc_harmonic_coefficients_modify(ai, harmonic_index + 1, 0)
...
def fourier_approx_norm_modify(ai, n, m, normalized, option):
    ...
    results = np.array([compute_harmonic_coefficients(ai, i) for i in range(n)])
    # 收集计算结果
    ...
    return output, a, b, c, d, a0, b0, c0, d0, A1, C1
