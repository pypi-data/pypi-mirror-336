...
from fi_instrumentation.instrumentation.logging import logger
...
class _WrappedSpan(wrapt.ObjectProxy):  # type: ignore[misc]z
...
class FITracer(wrapt.ObjectProxy):  # type: ignore[misc]
    def __init__(self, wrapped: Tracer, config: TraceConfig) -> None:
        super().__init__(wrapped)
        self._self_config = config
        self._self_id_generator = UuidIdGenerator()
