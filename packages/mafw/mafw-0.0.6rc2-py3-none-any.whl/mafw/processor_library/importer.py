...
the input set. Those elements can be everything, the results of an experiment or of a simulation, information
retrieved from webscraping or other data sources.
...
This module provides a generic processor that the user can subclass and customise to their needs to import
...
class FilenameElement:
    ...
    The element is characterised by a name, a regular expression, the expected python type for the parsed value and an optional
    ...
    To make a filename element optional, it is just enough to provide a default value different from None.
...
class FilenameParser:
    ...
    If something goes wrong with the parsing, meaning that a compulsory element is not found, then the
