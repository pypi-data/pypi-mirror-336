...
class SQLDataRetriever(DataRetriever):
    """
    A specialised data retriever to get a data frame from a database table.
...
class GenericPlotter(Processor, metaclass=PlotterMeta):
    ...
    as you would do for a normal Processor, but you will have to implement the following methods:
        ...
        * :meth:`~.plot`.
            ...
            This method is where the real plotting is taking place. Use the :attr:`~.data_frame` to plot the quantities
            you want.

        * :meth:`~.customize_plot`.

            This method can be optionally used to customise the appearance of the facet grid produced by the
    ...
    def is_output_existing(self) -> bool:
        ...
        generate one or more graphical output file.
    ...
    def customize_plot(self) -> None:
        """
        The customise plot method.

        The user can overload this method to customise the output produced by the :meth:`~.plot` method, like for
    ...
    def _update_plotter_db(self) -> None:
        ...
        A plotter subclass is producing in most of the cases just a plot as an output, so no additional information
        should be stored in the database, but this is already enough to avoid executing the processor if not needed.
