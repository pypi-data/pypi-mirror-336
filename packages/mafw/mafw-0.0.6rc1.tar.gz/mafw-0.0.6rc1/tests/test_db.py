...
def test_trigger_creation():
    with pytest.raises(MissingSQLStatement):
        new_trigger = Trigger(
            ...
            (TriggerWhen.Before, TriggerAction.Insert),
            'my_table',
            safe=True,
            for_each_row=True,
        )
        sql = new_trigger.create()
        print(sql)

    new_trigger = Trigger(
        'trigger_before_insert', (TriggerWhen.Before, TriggerAction.Insert), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
        )
    )

    expected = """CREATE TRIGGER IF NOT EXISTS trigger_before_insert
...
END;
"""
    assert new_trigger.create() == expected

    new_trigger = Trigger(
        'trigger_after_insert', (TriggerWhen.After, TriggerAction.Insert), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER IF NOT EXISTS trigger_after_insert
...
"""
    ...
    new_trigger = Trigger(
        'trigger_before_delete', (TriggerWhen.Before, TriggerAction.Delete), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER IF NOT EXISTS trigger_before_delete
...
"""
    ...
    new_trigger = Trigger(
        'trigger_after_delete', (TriggerWhen.After, TriggerAction.Delete), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER IF NOT EXISTS trigger_after_delete
...
"""
    ...
    new_trigger = Trigger(
        'trigger_before_update', (TriggerWhen.Before, TriggerAction.Update), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER IF NOT EXISTS trigger_before_update
...
"""
    ...
    new_trigger = Trigger(
        'trigger_after_update', (TriggerWhen.After, TriggerAction.Update), 'my_table', safe=True, for_each_row=True
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER IF NOT EXISTS trigger_after_update
...
"""
    ...
    new_trigger = Trigger(
        'trigger_after_update', (TriggerWhen.After, TriggerAction.Update), 'my_table', safe=True, for_each_row=False
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
...
"""
    ...
    new_trigger = Trigger(
        'trigger_after_update', (TriggerWhen.After, TriggerAction.Update), 'my_table', safe=False, for_each_row=False
    )
    (
        new_trigger.add_sql('INSERT INTO another_table (col1, col2) VALUES (1, 2)').add_sql(
            'INSERT INTO third_table (col1, col2) VALUES (2, 3)'
    ...
    expected = """CREATE TRIGGER  trigger_after_update
AFTER UPDATE  ON my_table
...
"""
    ...
    expected = """CREATE TRIGGER  trigger_after_update
AFTER UPDATE OF (col3, col4) ON my_table
...
"""
    ...
    expected = """CREATE TRIGGER  trigger_after_insert
AFTER INSERT  ON my_table
...
def test_trigger_dropping():
    trig = Trigger('mytable_after_insert', (TriggerWhen.After, TriggerAction.Insert), 'my_table', safe=True)
    assert trig.drop() == 'DROP TRIGGER IF EXISTS mytable_after_insert'
    assert trig.drop(False) == 'DROP TRIGGER  mytable_after_insert'


def test_modification_trigger():
    trig = Trigger('mytable_after_insert', (TriggerWhen.After, TriggerAction.Insert), 'my_table', safe=True)
    trig.add_sql('INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2)')
    assert (
        trig.create()
...
BEGIN
	INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2);
END;
"""
    )
    assert trig.safe
    assert not trig.for_each_row

    trig.safe = False
    trig.for_each_row = True
    assert (
        trig.create()
...
def test_trigger_with_when():
    trig = Trigger('mytable_after_insert', (TriggerWhen.After, TriggerAction.Insert), 'my_table', safe=True)
    trig.add_sql('INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2)')
    trig.add_when('a == b', 'b == c')
    trig.add_when('d == e')
    assert (
        trig.create()
        == """CREATE TRIGGER IF NOT EXISTS mytable_after_insert
AFTER INSERT  ON my_table
 WHEN ((a == b) AND (b == c)) AND ((d == e))
BEGIN
	INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2);
END;
"""
    ...
    trig = Trigger('mytable_after_insert', (TriggerWhen.After, TriggerAction.Insert), 'my_table', safe=True)
    trig.add_sql('INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2)')
    trig.add_when(or_(and_('a == b', 'b == c'), and_('d == e', 'f == g')))
    assert (
        trig.create()
        == """CREATE TRIGGER IF NOT EXISTS mytable_after_insert
AFTER INSERT  ON my_table
 WHEN ((((a == b) AND (b == c)) OR ((d == e) AND (f == g))))
BEGIN
	INSERT INTO target_table (id__id, half_double) VALUES (NEW.id_, NEW.double / 2);
END;
"""
...
def test_triggers_automatically_created_within_class():
...
def test_triggers_automatically_created_within_class_with_peewee_query():
    class TargetTable:
        pass
...
def test_failed_trigger_creation_with_class():
    class MyTable(MAFwBaseModel):
        id_ = AutoField(primary_key=True)
        integer = IntegerField()
        double = FloatField()
    ...
    database.connect()
    database_proxy.initialize(database)
    with pytest.raises(peewee.OperationalError, match='THIS'):
        database.create_tables([MyTable], safe=True)
...
def test_trigger_with_when_conditions():
    class TriggerStatus(MAFwBaseModel):
        """A Model for the trigger status"""

        trigger_type_id = AutoField(primary_key=True)
        trigger_type = peewee.TextField(
            help_text='You can use it to specify the type (DELETE/INSERT/UPDATE) or the name of a specif trigger'
        )
        status = IntegerField(default=1, help_text='0 = disable / 1 = enable')
    ...
    database.connect()
    database_proxy.initialize(database)
    database.create_tables([MyTable, TargetTable, TriggerStatus], safe=True)

    MyTable.delete().execute()
    TargetTable.delete().execute()
    TriggerStatus.delete().execute()

    data = [
        dict(trigger_type='DELETE'),
        dict(trigger_type='INSERT'),
        dict(trigger_type='UPDATE'),
    ]

    TriggerStatus.insert_many(data).execute()
    ...
    # check that the target table got the right entries because the status is active
    for row in MyTable.select(MyTable.float_num, TargetTable.half_float_num).join(TargetTable).namedtuples():
        assert row.float_num == 2 * row.half_float_num

    # now disable the trigger
    TriggerStatus.update({TriggerStatus.status: 0}).where(TriggerStatus.trigger_type == 'INSERT').execute()

    # add some data for testing to the first table, but with the trigger disabled
    data = []
    for i in range(100):
        data.append(dict(integer=random.randint(i, 10 * i), float_num=random.gauss(i, 2 * i)))
    MyTable.insert_many(data).execute()

    assert MyTable.select().count() == 200
    assert TargetTable.select().count() == 100
    assert MyTable.select(MyTable.float_num, TargetTable.half_float_num).join(TargetTable).count() == 100

    # restore the trigger
    TriggerStatus.update({TriggerStatus.status: 1}).where(TriggerStatus.trigger_type == 'INSERT').execute()

    # add some data for testing to the first table, but with the trigger re-enabled
    data = []
    for i in range(100):
        data.append(dict(integer=random.randint(i, 10 * i), float_num=random.gauss(i, 2 * i)))
    MyTable.insert_many(data).execute()

    assert MyTable.select().count() == 300
    assert TargetTable.select().count() == 200
    assert MyTable.select(MyTable.float_num, TargetTable.half_float_num).join(TargetTable).count() == 200
