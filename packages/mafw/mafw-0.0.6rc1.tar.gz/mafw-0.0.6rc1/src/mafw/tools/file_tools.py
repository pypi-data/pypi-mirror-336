...
def remove_widow_db_rows(models: list[Model | type[Model]] | Model | type[Model]) -> None:
    ...
    It is to say that a processor is getting from a database table a list of item to be processed, and it is updating a
    result table with the newly obtained outputs.
    ...
    has been deleted. A typical example it the following: the user wants to a certain processor to regenerate a given
...
def verify_checksum(models: list[Model | type[Model]] | Model | type[Model]) -> None:
    ...
    and compare it with the actual stored value. If the freshly calculated value differs from the stored one,
    then the row in the model is deleted because it is not any more actual.
