...
To achieve your complex analytical task, the solution proposed by MAFw is to chain many processors one of the other. The following processors can start where the previous one stopped so that like in a building game, scientists can put together their analytical solution with simple blocks.
...
Let's have a look at the snippet here below:
...
It might sound a bit too technical for this tutorial, but it is worth mentioning an interesting implementation detail.
If you look at the constructor of the :class:`~mafw.processor.Processor` class, you will notice that you can provide some resources, like the Timer and UserInterface even though so far we have never done this. The idea is that when you execute a single processor, it is fully responsible of creating the required resources by itself, using them during the execution and then closing them when finishing.

Just as an example, consider the case of the use of a database interface. The processor is opening the connection, doing all the needed transactions and finally closing the connection. This approach is also very practical because it is much easier to keep under control exceptions.

If you run a :class:`~mafw.processor.ProcessorList`, you may want to move the responsibility of handling the resources from the single Processor to the output ProcessorList. In this way, all processors are sharing the same resources without the overhead of opening and closing the database connection everytime the processor list move to the next item.
