...
Sometimes you have tasks that can be performed in one go on all your dataset, like producing a graph from a dataset, some other times you need to repeat the same process on a list of similar items, or you have to repeat a process until you have reached a certain condition. The |processor| can actually accomplish all those different execution workflows just by changing a variable: :class:`the loop type <mafw.enumerators.LoopType>`.
...
In python a for loop is generally performed on a list of items and MAFw is actually following the same strategy. Here below is schematic workflow.
...
perform some real analysis, the user has to subclass it and overload some methods. Having understood what is the role of each steps in the execution workflow, read the list of methods that can be overloaded here below and then you are ready to see some simple :ref:`examples`.
...
The :meth:`~mafw.processor.Processor.while_condition` method.
    ...
    This is where the decision about to continue or to stop the loop is taken. This is required only for **while_loop**
...
The :meth:`~mafw.processor.Processor.format_progress_message` (optional).
    ...
    be relevant to have every now and then an update on the progress. The |processor| will automatically will display regular messages via the
...
to configure your processor. As always the best is somewhere in the middle.

You can have parameters implemented in the processor subclass as normal attributes, but then you would need to modify
the code in order to change them and this is far not practical. You can have them as specific variables passed to the
...
.. code-block:: python
    ...
    class MyProcessor(Processor):
        ...
        Let's put this in a processor parameter.
...
Some important notes: input_folder is defined as a class attribute and not an instance attribute, it is to say it is
...
execution and you inspect the type of `self.input_folder` you will see that is a Path object. This is because
...
but if you try to access those attribute from input_folder you will get an error message, because class Path does not
...
Let's summarize all this with an example:
...
You have probably noticed that for both :class:`~mafw.processor.ActiveParameter` and :class:`~mafw.processor.PassiveParameter` you have the possibility to pass a default value, that is a very good practice, especially for very advanced parameters that most of the time will remain untouched.

If you want to set a value for a parameter, the easiest way is via the processor __init__ method. The basic Processor accepts any number of keywords arguments that can be used exactly for this purpose. Just add a keyword argument named after the parameter and the processor will take care of the rest.
...
Note that the best way is to avoid to explicitly include the parameter names in the init signature. They will collected anyhow by keyword arguments and then automatically registered.

The second approach is to use a configuration object, it is to say a dictionary containing all the parameters key and value pairs. This is particularly handy when using a configuration file. Exactly for this reason, the configuration object can have one of the two following structure.
...
Now let us make a step forward and complicate a bit the things. See the next snippet:
...
a class). For these two elements, the generated instances will use no constructor arguments. You may think that
the default values are dumped to the output file, but surprise this is not the case. When you have created the
...
So in the end, what values will be dumped depends on the type of parameter (Active / Passive) and in case of Passive
...
This approach, even though possible, is rather risky, we all know how dangerous ``eval`` can be especially when using it directly with information coming from external files. Moreover, one should ask himself if it is really so important to have a custom class as a parameter in a processor and very likely there are, simpler and safer, solutions.
