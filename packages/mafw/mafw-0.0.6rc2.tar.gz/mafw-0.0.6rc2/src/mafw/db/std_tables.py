...
tables and of initializing them. If the user opens the connection and pass it to a Processor or ProcessorList,
then the user is responsible to create the standard tables and to initialise them.

All standard tables must derive from the :class:`StandardTable` in order to have the same interface for the
initialisation.

Users can create their own standard tables and export them as pluggable object using the same approach as for custom
...
class PlotterOutput(StandardTable):
    ...
    plotter_name = TextField(primary_key=True, help_text='The plotter processor name')
