...
MAFw is meant to work without the need for a user interface, but having some communication between the process
execution and the user is a feature that is often very much appreciated by the user.

There are several different interfaces and different interface types (Command Line, Textual, Graphical...) and
everyone has its own preferences. In order to be the more generic as possible, MAFw is allowing for an abstract
