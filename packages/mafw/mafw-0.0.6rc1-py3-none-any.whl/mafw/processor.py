...
class PassiveParameter(Generic[ParameterType]):
    ...
    .. seealso::
        ...
        A explanation on how processor parameters work and should be used is given in :ref:`Understanding processor
...
class Processor(metaclass=ProcessorMeta):
    ...
    def acquire_resources(self) -> None:
        ...
        if self._database is None and self._database_conf is None:
            ...
            # we can make a db.
            # This processor will try to make a valid connection and in case it succeds it will add the database to
            # the resource stack.
            # The database has an enter method but it is to generate transcation.
            ...
            self._database = connect(conf.get('URL'), pragmas=conf.get('pragmas', {}))  # type: ignore # peewee is not returning a DB
    ...
    def _remove_orphan_files(self) -> None:
        ...
        OrphanFile = standard_tables['OrphanFile']

        if OrphanFile._meta.table_name not in self._database.get_tables():  # type: ignore # submit PR https://github.com/python/typeshed/pull/13230
    ...
    def acquire_resources(self) -> None:
        ...
        if self._database is None and self._database_conf is None:
            ...
            if 'DBConfiguration' in self._database_conf:
                ...
                conf = self._database_conf  # type2
            self._database = connect(conf.get('URL'), pragmas=conf.get('pragmas', {}))  # type: ignore  # peewee does not return a DB.
            try:
                self._database.connect()
                self._resource_stack.callback(self._database.close)
            except peewee.OperationalError as e:
