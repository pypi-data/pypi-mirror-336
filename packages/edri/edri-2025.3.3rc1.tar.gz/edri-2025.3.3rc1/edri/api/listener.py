from asyncio import get_event_loop, create_task, wait, Event as AEvent
...
from edri.config.setting import HOST, PORT, API_WORKERS, API_RESPONSE_TIMEOUT, SSL_KEY, SSL_CERTIFICATE, CORS_CREDENTIALS
...
class Listener(Process):
    ...
    def run(self) -> None:
        if SSL_KEY and SSL_CERTIFICATE:
            config = Config(self.App(self.api_broker_queue, self.middlewares, self.json_encoder), host=HOST, port=PORT, log_level="debug",
                            workers=API_WORKERS, log_config=None, ssl_keyfile=SSL_KEY, ssl_certfile=SSL_CERTIFICATE)
        else:
            config = Config(self.App(self.api_broker_queue, self.middlewares, self.json_encoder), host=HOST, port=PORT, log_level="debug",
                            workers=API_WORKERS, log_config=None)
                ...
                if event_constructors:
                    headers = NormalizedDefaultDict[str, Headers](list, {
                        ...
                        "Access-Control-Allow-Credentials": [
                            "True" if CORS_CREDENTIALS else "False"
                        ]
                    })
