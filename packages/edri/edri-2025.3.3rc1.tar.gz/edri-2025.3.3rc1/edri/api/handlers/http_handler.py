...
from edri.config.setting import MAX_BODY_SIZE, ASSETS_PATH, UPLOAD_FILES_PREFIX, ENVIRONMENT, CORS_HEADERS, CORS_ORIGINS
            ...
            if part.startswith("{") and part.endswith("}"):  # Detect dynamic part
                ...
                for child_url, child_node in node.children.items():
                    if regex_pattern.match(child_url):
                        raise ValueError("Dynamic url in conflict with static url: %s %s", child_url, event)
                ...
                for child_pattern, child_node in node.dynamic_children.items():
                    if child_pattern.match(part):
                        raise ValueError("Dynamic url in conflict with static url: %s %s", part, event)
                if part not in node.children:
                    node.children[part] = URLNode(part)
                node = node.children[part]

        if method in node.methods:
            raise KeyError(f"Method %s already exists for this URL: %s", method, url)
        if method is None:
            raise ValueError("Method for event %s is not defined", event)
    ...
    def _convert_to_regex(cls, parameter_name: str, type_: type) -> tuple[Pattern[str], bool]:
        ...
        if origin is not None:
            ...
            if not regex_function:
                regex_function = cls.DATA_TYPE_REGEX.get(type_.__base__) #TODO: How about deeper inheritance?!
...
class HTTPHandler[T: HTTPResponseDirective](BaseHandler, ABC):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if headers:
            if headers.startswith(","):
                headers = headers[1:]
            cls.cors["headers"] += f",{headers}"

        if CORS_ORIGINS:
            cls.cors["origins"] = [origin.strip() for origin in CORS_ORIGINS.split(",")]
    ...
    def handle_custom(self, data: BinaryIO) -> None:
        with NamedTemporaryFile(prefix=UPLOAD_FILES_PREFIX, delete=False) as received_temp_file:
            received_temp_file.write(data.read())
