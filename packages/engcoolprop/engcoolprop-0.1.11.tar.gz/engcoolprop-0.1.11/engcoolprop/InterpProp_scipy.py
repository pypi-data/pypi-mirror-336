...
def get_density_interpolator( ec_inc ):
    """
    Create a cubic spline interpolator for D = f(T)
    T = degR
    D = lbm/cuft
    """

    # P = 5000 psia for CoolProp density method
    Psi = PSI_fromEng( 5000 )
    def get_dens( T ): # T=degR
        Tsi = TSI_fromEng( T )
        # try:
        D = Deng_fromSI( PropsSI('D', 'T',Tsi,'P',Psi, ec_inc.fluid) )
        return D # lbm/cuft
    ...
    # print( "tL=", ' '.join(['%g'%t for t in tL]) )

    dL = [ get_dens(T) for T in tL ]
    # print( "dL=", ' '.join(['%g'%d for d in dL]) )

    Dterp = InterpProp(tL, dL, extrapOK=1)
...
class InterpProp:
    ...
    def __init__(self, xInp=[0,1], yInp=[10,110], extrapOK=1, linear=0,
        ...
        if len(self.x) > 1:
            ...
            # most smooth interpolation in my testing of rocketProps fluid properties
            # self.interpFunc = interpolate.PchipInterpolator(self.x, self.y)
            
            # self.interpFunc = interpolate.Akima1DInterpolator(self.x, self.y)

            # for fitting INCOMP fluids gives best results by far
