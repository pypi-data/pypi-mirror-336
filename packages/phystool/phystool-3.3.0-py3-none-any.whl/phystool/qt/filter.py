...
class FilterWidget(QWidget):
    ...
    def __init__(self):
        ...
        self._checked_filters = dict()
        self._query = ""
        self._discarded_tags = Tags.validate(
            settings.value(
                "FilterWidget/discarded_tags",
                defaultValue=""
            )
        )
        self._selected_tags = Tags.validate(
            settings.value(
        ...
        for file_type in VALID_TYPES:
            ...
            button.setChecked(file_type in self._file_types)
    ...
    def update_filters(self) -> None:
        ...
        for category, tags in Tags.TAGS:
            ...
            for tag_name in tags:
                ...
                elif tag_name in self._discarded_tags[category]:
    ...
    def get_filter_data(self) -> dict[str, str | set[str] | Tags]:
        return {
            ...
            'discarded_tags': self._discarded_tags
        }
    ...
    def _filter_file_type(self) -> None:
        button = self.sender()
        if button.isChecked():
            self._file_types.add(button.file_type)
        else:
            self._file_types.remove(button.file_type)
        self.sig_filter_updated.emit(self.get_filter_data())
    ...
    def __del__(self):
        settings = QSettings()
        settings.setValue("FilterWidget/filetypes", ",".join(self._file_types))
        settings.setValue("FilterWidget/selected_tags", str(self._selected_tags))
        settings.setValue("FilterWidget/discarded_tags", str(self._discarded_tags))
