...
class DmenuPhys:
    ...
    DMENU_GIT = 8
    PDB_PNG_PATH = "unused/"
    PNG_NOT_FOUND = PDB_PNG_PATH + '_file_not_found.png'
    ...
    def _xclip(self, uuid: str, fname: str) -> None:
        ...
        png_file = (self.PDB_PNG_PATH / fname).with_suffix('.png')
    ...
    def _dmenu_in(self, how: int) -> str | None:
        if how == self.DMENU_OPT:
            ...
            self._vim(config.get_new_pdb_filename(), check_path=False)
            return None
    ...
    def _dmenu_out(self, how: int, selected: str) -> None:
        ...
        if how == self.DMENU_PDF:
            self._pdf((config.PDB_DB_DIR / uuid).with_suffix('.pdf'))
        elif how == self.DMENU_VIM:
            self._vim((config.PDB_DB_DIR / uuid).with_suffix('.tex'))
        else:
