...
class PlugFlowReactor(ReactorAbstract, CPFRSoot):
    ...
    def start(self):
        ...
        self.T = self.inlet.T;
        self.P = self.inlet.P;
        self.set_norm_mdot(self.inlet.mdot, self.area);
        self.build_arrays();
        ...
        if self.check_soot_array(inlet_soot):
            ...
            y0 = np.hstack((0.0, 0.0, self.get_norm_mdot() * (1.0 - self.soot.volume_fraction), self.inlet.T, self.inlet.P, self.inlet.Y, inlet_soot));
    ...
    def area(self, area):
        self.set_area(area);
        self.set_perimeter_by_area(area);
    ...
    def gas_carbon_flux(self) -> float:
        return self.soot_gas.elemental_mass_fraction('C') * self.get_norm_mdot();
    
    @property
    def soot_carbon_flux(self) -> float:
        return self.soot_wrapper.particle_dynamics_model.carbon_mass() * self.get_norm_mdot();    
    ...
    def total_hydrogen_flux(self) -> float:
        ...
        return hydrogen_mass * self.get_norm_mdot();
        
        
    @property
    def gas_hydrogen_flux(self) -> float:
        return self.soot_gas.elemental_mass_fraction('H') * self.get_norm_mdot();
    ...
    def soot_hydrogen_flux(self) -> float:
        return self.soot_wrapper.particle_dynamics_model.hydrogen_mass() * self.get_norm_mdot();
    ...
    def gas_elemental_flux(self, element_name):
        return self.soot_gas.elemental_mass_fraction(element_name) * self.get_norm_mdot();
    ...
    def total_carbon_flux(self):
        return (
                (self.soot_gas.elemental_mass_fraction('C') +
                 self.soot_wrapper.particle_dynamics_model.carbon_mass()) * self.get_norm_mdot()
        );
