...
def enviarDocumentos(pastaDocumentos):
    """
    ...
    file_paths = [os.path.join(pastaDocumentos, f) for f in os.listdir(pastaDocumentos) if os.path.isfile(os.path.join(pastaDocumentos, f))]
...
def main():
    ...
    status_code, resposta_api, documentos_true = enviarDocumentos(pastaDocumentos)
