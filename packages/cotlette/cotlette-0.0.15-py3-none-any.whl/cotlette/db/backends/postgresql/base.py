"""
PostgreSQL database backend for Cotlette.
...
from cotlette.db import DatabaseError as WrappedDatabaseError
from cotlette.db import connections
from cotlette.db.backends.base.base import NO_DB_ALIAS, BaseDatabaseWrapper
from cotlette.db.backends.utils import CursorDebugWrapper as BaseCursorDebugWrapper
...
from cotlette.utils.version import get_version_tuple
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def pool(self):
        ...
        if self.alias not in self._connection_pools:
            ...
            # Ensure we run in autocommit, Cotlette properly sets it later on.
    ...
    def is_usable(self):
        if self.connection is None:
            return False
        try:
            # Use a psycopg cursor directly, bypassing Cotlette's utilities.
    ...
    def _nodb_cursor(self):
        ...
        try:
            ...
            if cursor is not None:
                raise
            warnings.warn(
                "Normally Cotlette will use a connection to the 'postgres' database "
                "to avoid running initialization queries against the production "
                "database when it's not needed (for example, when running tests). "
                "Cotlette was unable to create a connection to the 'postgres' database "
