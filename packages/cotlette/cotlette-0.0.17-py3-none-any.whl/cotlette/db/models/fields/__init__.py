...
from cotlette.core import checks, exceptions, validators
from cotlette.db import connection, connections, router
from cotlette.db.models.constants import LOOKUP_SEP
from cotlette.db.models.query_utils import DeferredAttribute, RegisterLookupMixin
from cotlette.db.utils import NotSupportedError
from cotlette.utils import timezone
from cotlette.utils.choices import (
    BlankChoiceIterator,
    CallableChoiceIterator,
    flatten_choices,
    normalize_choices,
)
from cotlette.utils.datastructures import DictWrapper
from cotlette.utils.dateparse import (
    parse_date,
    parse_datetime,
    parse_duration,
    parse_time,
)
from cotlette.utils.duration import duration_microseconds, duration_string
from cotlette.utils.functional import Promise, cached_property
from cotlette.utils.ipv6 import MAX_IPV6_ADDRESS_LENGTH, clean_ipv6_address
from cotlette.utils.text import capfirst
...
class Field(RegisterLookupMixin):
    ...
    # The auto_creation_counter is used for fields that Cotlette implicitly
    ...
    def deconstruct(self):
         ...
         * The import path of the field, including the class, e.g.
           cotlette.db.models.IntegerField. This should be the most portable
        ...
        for name, default in possibles.items():
            ...
            if name in equals_comparison:
                ...
                if value is not default:
                    keywords[name] = value
        # Work out path - we shorten it for known Cotlette core fields
        path = "%s.%s" % (self.__class__.__module__, self.__class__.__qualname__)
        if path.startswith("cotlette.db.models.fields.related"):
            path = path.replace("cotlette.db.models.fields.related", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields.files"):
            path = path.replace("cotlette.db.models.fields.files", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields.generated"):
            path = path.replace("cotlette.db.models.fields.generated", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields.json"):
            path = path.replace("cotlette.db.models.fields.json", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields.proxy"):
            path = path.replace("cotlette.db.models.fields.proxy", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields.composite"):
            path = path.replace("cotlette.db.models.fields.composite", "cotlette.db.models")
        elif path.startswith("cotlette.db.models.fields"):
            path = path.replace("cotlette.db.models.fields", "cotlette.db.models")
    ...
    def to_python(self, value):
        ...
        cotlette.core.exceptions.ValidationError if the data can't be converted.
    ...
    def db_type(self, connection):
        ...
        # which *preexisting* Cotlette Field class it's most similar to -- i.e.,
        # a custom field might be represented by a TEXT column type, which is
        # the same as the TextField Cotlette field type, which means the custom
        # field's get_internal_type() returns 'TextField'.
        #
        # But the limitation of the get_internal_type() / data_types approach
        # is that it cannot handle database column types that aren't already
        # mapped to one of the built-in Cotlette field types. In this case, you
    ...
    def db_returning(self):
        """Private API intended only to be used by Cotlette itself."""
        return (
    ...
    def formfield(self, form_class=None, choices_form_class=None, **kwargs):
        """Return a cotlette.forms.Field instance for this field."""
        defaults = {
...
class CharField(Field):
    ...
    def slice_expression(self, expression, start, length):
        from cotlette.db.models.functions import Substr
            ...
            return [
                checks.Warning(
                    ...
                    hint=(
                        ...
                        "as default, use `cotlette.utils.timezone.now`"
                    ),
                    obj=self,
                    id="fields.W161",
                )
...
class AutoFieldMeta(type):
    ...
    In many areas Cotlette also relies on using isinstance() to check for an
