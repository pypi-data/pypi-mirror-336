    ...
    "tasks":[
        {
            ...
            "shell":{
                "chdir": "{{py_env}}/site-packages/qd2_controller",
                "cmd": "nohup python3 controller.py > stdout.txt 2> stderr.txt &"
            },
        }
    ]
...
def get_receiver_play(host):
    ...
    play[0]["hosts"] = host
    play[0]["tasks"][0]["shell"]["cmd"] = "nohup python3 receive_qd2.py "+str(host)+" > stdout_receiver.txt 2> stderr_receiver.txt &"
    return play

def get_receptor_play(host, IP):
    play = start_receive_play
    play[0]["hosts"] = host
    play[0]["tasks"][0]["shell"]["cmd"] = "nohup python3 http_receptor.py "+str(IP)+" 8000 "+str(host)+" > stdout_receptor.txt 2> stderr_receptor.txt &"
    return play
