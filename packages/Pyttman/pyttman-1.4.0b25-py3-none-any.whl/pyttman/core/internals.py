...
class PyttmanApp(PrettyReprMixin):
    ...
    def execute_plugins_before_router(self, message: MessageMixin):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.before_router):
    ...
    def execute_plugins_before_entity_extraction(self, message: MessageMixin):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.before_entity_extraction):
    ...
    def execute_plugins_after_entity_extraction(self, message: MessageMixin):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.after_entity_extraction):
    ...
    def execute_plugins_before_intent(self, message: MessageMixin):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.before_intent):
    ...
    def execute_plugins_after_intent(self, reply: Reply):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.after_intent):
    ...
    def execute_plugins_before_app_start(self):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PPyttmanPlugin.PluginInterceptPoint.before_app_start):
                plugin.before_app_start(self)

    def execute_plugins_after_app_stop(self):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PPyttmanPlugin.PluginInterceptPoint.after_app_stops):
                plugin.after_app_stops(self)

    def execute_plugins_no_intent_match(self, message: MessageMixin):
        reply = None
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PPyttmanPlugin.PluginInterceptPoint.no_intent_match):
