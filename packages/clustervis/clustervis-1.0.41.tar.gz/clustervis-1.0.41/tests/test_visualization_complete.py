...
ensemble_classifier_plot(X, bag, colors, resolution, 'Bagging Classifier', show=False, ax=ax_ensemble)
...
for i, base_estimator in enumerate(bag.estimators_):
    base_classifier_plot(X, base_estimator, colors, resolution, f'KNN #{i+1}', show=False, ax=axes[i])
