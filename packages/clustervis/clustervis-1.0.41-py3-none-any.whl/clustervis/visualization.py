...
def ensemble_classifier_plot(X, ensembleClassifier, colors, resolution, plotTitle, show, plotPath=None, filename="", ax=None):
    """
...
def base_classifier_plot(X, baseClassifier, colors, resolution, plotTitle, show, plotPath=None, filename="", ax=None):
    """
    ...
    for i in range(xx.shape[0]):
        for j in range(yy.shape[0]):
            ...
            class_weights = [1 if k == prediction else 0 for k in range(len(colors_normalized))]
            rgb_grid[i, j] = compute_weighted_rgb(class_weights, colors_normalized)
