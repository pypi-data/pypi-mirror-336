...
TEMPLATE_NAMES, NICE_TEMPLATE_NAMES, OPEN_PORTS, AUTOMOUNT_FOLDERS = utils.get_template_info()
...
@click.option('--template', type=click.Choice(TEMPLATE_NAMES), default='base',
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                try:
                    ...
                    progress.update(task, description=f"Waiting for instance to be ready...")
                ...
                progress.update(task, description="Continuing SSH connection...")
                ...
                progress.update(task, description=f"Retrying SSH connection... ({str(e)})")
                time.sleep(interval)
            except (socket.timeout, TimeoutError) as e:
                last_error = f"Connection timeout: {str(e)}"
                progress.update(task, description=f"Connection timed out, retrying...")
                time.sleep(interval)
            except Exception as e:
                last_error = f"Unexpected error: {str(e)}"
                progress.update(task, description=f"Retrying connection... ({str(e)})")
                time.sleep(interval)
            finally:
                # Ensure socket is closed in case of any exception
                progress.stop()
                if socket_test:
                    try:
                        socket_test.close()
...
def connect(tunnel, instance_id=None, mount=None, nomount=False, debug=False):
    ...
    try:
        ...
        try:
            ssh = robust_ssh_connect(ip, keyfile, max_wait=120, interval=1, username="ubuntu")
        except click.ClickException as e:
            # Pass through Click exceptions directly
            raise
        except Exception as e:
            raise click.ClickException(f"Failed to connect to instance {instance_id}: {str(e)}")
...
def scp(source_paths, destination_path):
    ...
    try:
        ...
        if len(all_instances) > 1:
            raise click.ClickException("Cannot transfer files between different instances. Please transfer through your local machine first.")
        ...
        if source_count > 1:
            if not dst_path.endswith('/') and not dst_path.endswith('\\'):
                raise click.ClickException("When copying multiple files, destination must be a directory (end with / or \\)")
        ...
        with Progress(
            ...
            if total_files > 0:
                ...
                progress.update(task, description=f"Successfully transferred {total_files} file{'s' if total_files != 1 else ''} ({_format_size(total_transferred)})")
