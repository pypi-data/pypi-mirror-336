...
class DebugHelper:
    ...
    def installer( self, command, shell = False ):
        self.log_step( f"command : { command }", success = None )
        result = subprocess.run( command, shell = shell, capture_output=True, text=True)
        self.log_step( f"result  : { result.returncode }", success = None )
        self.log_step( f"stdout  :\n{ result.stdout     }", success = None )
        self.log_step( f"stderr  :\n{ result.stderr     }", success = None )
