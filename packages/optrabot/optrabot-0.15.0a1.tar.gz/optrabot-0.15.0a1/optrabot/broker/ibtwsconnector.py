...
class IBTWSTConnector(BrokerConnector):
	...
	async def placeOrder(self, managed_trade: ManagedTrade, order: GenericOrder, parent_order: GenericOrder = None) -> bool:
		...
		async with self._lock:
			...
			logger.debug(f'Account: {managed_trade.template.account} Entry Order {ibOrder.orderId} placed for Trade: {trade} Number of contracts: {order.quantity}')
			return True
