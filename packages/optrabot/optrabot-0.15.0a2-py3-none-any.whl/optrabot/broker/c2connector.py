...
class C2Connector(BrokerConnector):
	...
	async def placeOrder(self, managed_trade: ManagedTrade, order: GenericOrder, parent_order: GenericOrder = None) -> bool:
		...
		"""
		if len(order.legs) > 1:
			logger.error('Only single leg orders are supported for Collective2 Strategies!')
			return False
		
		c2order: C2Order = order.brokerSpecific.get('c2order')
		if c2order is None:
			logger.error('C2 order not prepared!')
			return False
		...
		if response.status_code != status.HTTP_200_OK:
			logger.error('Failed to place order: {}', response.text)
			return False
	...
	async def prepareOrder(self, order: GenericOrder, need_valid_price_data: bool = True) -> bool:
		...
		if need_valid_price_data:
			strike_price_data = self._broker_connector.get_option_strike_price_data(order.symbol, current_date, leg.strike)
	...
	async def requestTickerData(self, symbols: List[str]):
		"""
		Ticker data don't need to be requested for C2 as they're consumed from the configured broker connection.
		"""
		pass
	...
	def _log_rate_limit_info(self, response: httpx.Response):
		...
		logger.info(f'Rate Limit: {rate_limit}/{rate_limit_type} Remaining: {rate_limit_remaining} Reset: {rate_limit_reset}')
