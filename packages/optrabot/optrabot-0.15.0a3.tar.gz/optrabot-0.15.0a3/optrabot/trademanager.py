	...
	async def _monitorOpenTrades(self):
		...
		for managedTrade in self._trades:
			...
			if managedTrade.template.has_soft_take_profit() == False:
				...
				if now.minute != self._last_trade_monitoring_time.minute:
					logger.debug(f'Check if Take Profit has been reached for trade {managedTrade.trade.id}')
					logger.debug(f'Current Price: {managedTrade.current_price} Take Profit Price: {managedTrade.takeProfitOrder.price}')
					if managedTrade.current_price >= managedTrade.takeProfitOrder.price:
