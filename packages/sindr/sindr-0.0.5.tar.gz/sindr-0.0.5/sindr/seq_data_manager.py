...
# %% ../nbs/03_seq_data_manager.ipynb 7
class WgsData:

    def __init__(self, file_paths: dict[dict], sample_data: dict[dict] | None = None):
        """
    ...
    @classmethod
    def from_samplesheet(cls, sample_sheet: Path, base_folder: Path | None = None):
        """
        ...
        for index, row in file_paths_df.iterrows():
            ...
            if not sample_name in sample_name_count:
                sample_name_count[sample_name] = 1
                file_paths[sample_name] = row.to_dict()
                uniq_sample_names.append(sample_name)
            else:
                sample_name_count[sample_name] += 1
                file_paths[f"{sample_name}_{sample_name_count[sample_name]}"] = (
                    row.to_dict()
                )
                uniq_sample_names.append(
                    f"{sample_name}_{sample_name_count[sample_name]}"
                )
                print(
                    f"Duplicate sample name {sample_name} in provided sample sheet. Using sample name {sample_name}_{sample_name_count[sample_name]} for uniquification"
                )
        if base_folder:
            ...
            for sample, files in file_paths.items():
                if not Path(files["assembly_file"]).is_absolute():
                    files["assembly_file"] = base_folder.joinpath(
                        files["assembly_file"]
                    ).resolve()
                if not Path(files["r1_file"]).is_absolute():
                    files["r1_file"] = base_folder.joinpath(files["r1_file"]).resolve()
                if not Path(files["r2_file"]).is_absolute():
                    files["r2_file"] = base_folder.joinpath(files["r2_file"]).resolve()
    ...
    @classmethod
    def from_folders(
        cls, assembly_data_folder: Path = None, paired_end_read_data_folder: Path = None
    ):
        if assembly_data_folder:
            assembly_data_dict = cls.parse_folder_for_fasta_files(
                data_folder=assembly_data_folder,
    ...
    def get_missing_files(self):
        ...
        for sample_name, assembly_file, r1_file, r2_file in self.filepaths():
            ...
            if r1_file and r2_file and (not r1_file.exists() or not r2_file.exists()):
    ...
    def parse_folder_for_paired_end_reads(
        data_folder: Path, paired_end_reads_pattern: str
    ) -> tuple[dict[dict], set]:
        """
    ...
    def parse_folder_for_fasta_files(
        data_folder: Path, fasta_file_pattern: str
    ) -> tuple[dict[dict], set]:
        """
        Parses a provided folder for paired end reads matching the provided regex pattern
        Returns a dictionary with sample names as keys and a dict with keys "r1_file" and "r2_file" and filenames as values
