...
class PPOCR(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        results = self.reader.ocr(img_new)
        ocr_out = []

        if results[0] is not None and results:
            for item in results:
                for coord_res, text_res in item:
                    coord_res_np = np.array(coord_res).astype(np.float32)
                    text_content = text_res[0]  # 检测结果内容
                    text_score = text_res[1]
