...
def EJ_EC_fr_g_to_coupled_freq(EJ, EC, fr_b, g, ng=0.5, r_levels=50, q_levels=5):
    """
    For a transmon-resonator pair, from EJ, EC, fr_b(bare uncoupled resonator frequency) and g,
    calculate coupled values f01_c, alpha_c, fr_c, chi_c. All units here are in [GHz].
    """
    # Transmon energies spectrum.
    transmon = scq.Transmon(EJ=EJ, EC=EC, ng=ng, ncut=q_levels*2+5, truncated_dim=q_levels*2+5)
    energies = transmon.eigenvals(evals_count=q_levels)

    # Charge operator for both.
    n_tmon = qt.Qobj(transmon.matrixelement_table(operator='n_operator', evals_count=q_levels))
    n_rsnt = 1j * (qt.create(r_levels) - qt.destroy(r_levels))

    # Composite Qobj.
    H_q = qt.tensor( qt.Qobj(np.diag(energies)), qt.identity(r_levels) )
    H_r = qt.tensor( qt.identity(q_levels), qt.Qobj(np.diag([0.5 + i * fr_b for i in range(r_levels)])) )
    V = g * qt.tensor(n_tmon, n_rsnt)
    H = H_q + H_r + V
    ...
    eigenenergies_sorted = {}
    for eigenenergy, eigenstate in zip(eigenenergies, eigenstates):
        population = np.abs(eigenstate.full()) ** 2
        index = np.argmax(population)  # q * r_levels + r
        q = index // r_levels
        r = index % r_levels
        eigenenergies_sorted[(q, r)] = eigenenergy

    f01_c = eigenenergies_sorted[(1, 0)] - eigenenergies_sorted[(0, 0)]
...
def coupled_freq_to_EJ_EC_fr_g(f01_m, alpha_m, fr_m, chi_m, ng=0.5, r_levels=50, q_levels=5, 
                               method='BFGS', options={'gtol': 1e-07}):
    """
    For a transmon-resonator pair, from measured values f01_m, alpha_m, fr_m, chi_m,
    calculate EJ, EC, fr_b(bare uncoupled resonator frequency) and g. All units here are in [GHz].
    We will do it by numerically minimizing an objective function here.
    """
    if r_levels * q_levels > 1000: print('Hilbert dimension exceed 1000 may take very long time (>10mins) to compute result!')
    if r_levels < 4 or q_levels < 4: raise ValueError('Both transmon and resonator need to take at least 4 levels for accuracy.')

    EC_guess = -alpha_m if alpha_m < 0 else alpha_m
    EJ_guess = (f01_m + EC_guess)**2 / (8*EC_guess)
    _, _, fr_b_guess, g_guess = get_bare_frequency(f01_m, alpha_m, fr_m, chi_m)

    def obj_fun(params):
        f01_c, alpha_c, fr_c, chi_c, _ = EJ_EC_fr_g_to_coupled_freq(params[0], params[1], params[2], params[3], 
                                                                    ng, r_levels, q_levels)
        return (f01_m - f01_c)**2 + (fr_m - fr_c)**2 + 1e2 * (alpha_m - alpha_c)**2 + 1e4 * (chi_m - chi_c)**2

    result = minimize(obj_fun, x0=(EJ_guess, EC_guess, fr_b_guess, g_guess),
                      method=method, options=options)
    EJ, EC, fr_b, g = result.x
    return EJ, EC, fr_b, np.abs(g)
