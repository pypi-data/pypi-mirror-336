...
class OBOFramework:
    ...
    def handle_request(self, request):
        ...
        if handler_data is not None:
            ...
            if inspect.isclass(handler):
                handler = getattr(handler(), request.method.casefold(), None)
                if handler is None:
                    return self.method_not_allowed_response(response)
            else:
                if request.method.casefold() not in allowed_methods:
                    return self.method_not_allowed_response(response)
            try:
    ...
    def add_route(self, path, handler, allowed_methods=None):
        assert path not in self.routes, "Such route already exists. Please change the route path."
        if allowed_methods is None:
            allowed_methods = ["get", "post", "put", "delete", "patch", "options", "head", "connect", "trace"]
