...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(
            description="Extract and structure data from the German Bundestag's API"
        )

        # Basic options
        parser.add_argument(
            "--api-key",
            help="API key for the Bundestag API (optional, defaults to public key)",
        )
        parser.add_argument(
            "--period", type=int, default=20, help="Legislative period (default: 20)"
        )
        parser.add_argument(
            "--output-dir", default="output", help="Output directory for extracted data"
        )

        # Extraction control
        parser.add_argument(
            "--limit", type=int, help="Limit the number of protocols to extract"
        )
        parser.add_argument(
            "--offset",
            type=int,
        ...
        parser.add_argument(
            "--list-progress", action="store_true", help="List available progress files"
        )
        ...
        log_level.add_argument(
            "--quiet", "-q", action="store_true", help="Minimal console output"
        )
        ...
        if args.list_progress:
            ...
            if not available_progress:
                ...
                for i, p in enumerate(available_progress):
                    logger.info(
                        f"{i+1}. Job ID: {p['job_id']}, WP{p['wahlperiode']}, "
                        f"Status: {p['status']}, "
        ...
        try:
            ...
            logger.info(
                "To resume from this point, run the command again with --resume parameter"
            )
            return 1
            ...
            logger.error(
                "To resume, use the --resume parameter with the latest progress file"
            )
            return 1
