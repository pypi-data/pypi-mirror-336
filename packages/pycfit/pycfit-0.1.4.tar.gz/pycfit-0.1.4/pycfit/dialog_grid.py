...
class GridDialog(QDialog):
    ...
    def __init__(self, grid_model, parent=None, init_ap=None):
        ...
        if init_ap is None:
            ...
            self.GM.analysis_point.set_point((init_lambda, init_x, init_y)) # Midpoint inidices, rounded down when odd
        else:
            self.GM.analysis_point.set_point(init_ap)
        ...
        self.button_redoFit_grid.clicked.connect(self.redo_fit_grid)
    ...
    def set_point(self, lambda_index=None, x_index=None, y_index=None, from_text=True):
        if from_text:
            ...
            if lambda_index is None:
                lambda_index = self.GM.analysis_point.get_index('lambda_index')
            if x_index is None:
                x_index = self.GM.analysis_point.get_index('x_index')
            if y_index is None:
                y_index = self.GM.analysis_point.get_index('y_index')
            self.lambda_index.setText(f'{lambda_index}')
            self.x_index.setText(f'{x_index}')
            self.y_index.setText(f'{y_index}')
        
        self.GM.analysis_point.set_point([lambda_index, x_index, y_index])        
    ...
    def mask_point(self):
        point_to_mask = self.GM.analysis_point.get_point()
        self.GM._mask[point_to_mask] = np.invert(self.GM._mask[point_to_mask]) # Swaps np.bool_ value of mask
        if point_to_mask not in self.mask_list:
            self.mask_list.append(point_to_mask)
        else:
            self.mask_list.remove(point_to_mask)

    def set_mask_checkbox_state(self):
        ap = self.GM.analysis_point.get_point()
        if ap in self.mask_list:
            self.checkBox_maskPoint.setChecked(True)
            if ap in self.initial_mask:
    ...
    def generate_mask_list(self):
        mask_shape = self.GM._mask.shape
        self.mask_list = []
        for li in range(mask_shape[0]):
            for xi in range(mask_shape[1]):
                for yi in range(mask_shape[2]):
                    if self.GM._mask[li, xi, yi]:
                        self.mask_list.append((li, xi, yi))
    ...
    def new_slice_axis(self):
        if self.GM.analysis_point.get_index('lambda_index') is None: # Index choice to check is arbitrary. If one is None, all will be None
    ...
    def generate_origdata_line(self):
        ...
        if x_axis == 'X':
            y_data = self.GM.get_data_subset(self.GM._intensity, fixed_lambda=True, fixed_y=True)
        elif x_axis == 'Y':
            y_data = self.GM.get_data_subset(self.GM._intensity, fixed_lambda=True, fixed_x=True)
        else: # λ
            y_data = self.GM.get_data_subset(self.GM._intensity, fixed_x=True, fixed_y=True)
    ...
    def generate_selectvar_line(self):
        ...
        if x_axis == 'X':
            y_data = self.GM.get_data_subset(data, fixed_y=True)
        else: # Y
            y_data = self.GM.get_data_subset(data, fixed_x=True)
        self.graph_selectvar_line.setLabel(axis='bottom', text=x_axis)
        self.graph_selectvar_line.setLabel(axis='left', text='Intensity')
    ...
    def generate_residuals_line(self):
        ...
        if x_axis == 'X':
            y_data = self.GM.get_data_subset(self.GM.residual_vals, fixed_lambda=True, fixed_y=True)
        elif x_axis == 'Y':
            y_data = self.GM.get_data_subset(self.GM.residual_vals, fixed_lambda=True, fixed_x=True)
        else: # λ
            y_data = self.GM.get_data_subset(self.GM.residual_vals, fixed_x=True, fixed_y=True)
    ...
    def generate_origdata_slice(self):
        ...
        if 'X' not in axes_list:
            data_for_image = self.GM.get_data_subset(self.GM._intensity, fixed_x=True)
        elif 'Y' not in axes_list:
            data_for_image = self.GM.get_data_subset(self.GM._intensity, fixed_y=True)
        else: # lambda (λ)
            data_for_image = self.GM.get_data_subset(self.GM._intensity, fixed_lambda=True)
            if y_axis == 'X':
    ...
    def generate_residuals_slice(self):
        ...
        axes_list = [x_axis, y_axis]
        if 'X' not in axes_list:
            data_for_image = self.GM.get_data_subset(self.GM.residual_vals, fixed_x=True)
        elif 'Y' not in axes_list:
            data_for_image = self.GM.get_data_subset(self.GM.residual_vals, fixed_y=True)
        else: # lambda (λ)
            data_for_image = self.GM.get_data_subset(self.GM.residual_vals, fixed_lambda=True)
            if y_axis == 'X':
                data_for_image = np.swapaxes(data_for_image, 0, 1)
        if y_axis == 'λ': # By default, if present, λ is always the x-axis after using get_data_subset()
            data_for_image = np.swapaxes(data_for_image, 0, 1)
    ...
    def generate_graphs_all(self):
        ...
        self.generate_graph_line(self.graph_spectra_fit, self.GM._wavelength, self.GM.get_data_subset(self.GM._intensity, fixed_x=True, fixed_y=True),
                                 symbol='o', pen=None)
    ...
    def redo_fit_point(self):
        fitter_xvals = self.GM._wavelength.copy()
        fitter_yvals = self.GM.get_data_subset(self.GM._intensity, fixed_x=True, fixed_y=True).copy()
        nanvals_mask = self.GM.get_data_subset(self.GM._mask, fixed_x=True, fixed_y=True).copy()
        fitter = FunctionFitter(fitter_xvals[~nanvals_mask], fitter_yvals[~nanvals_mask], function=self.GM.__getitem__(None)) # TODO: Needs uncertainties
        redo_fit_dialog = FitDialog(fitter, limited_adjustment=True)
        grid_key = (self.GM.analysis_point.get_index('x_index'), self.GM.analysis_point.get_index('y_index'))
        if redo_fit_dialog.exec():
            self.GM.__setitem__(grid_key, redo_fit_dialog.get_model())
            self.generate_graphs_all()

    def redo_fit_grid(self):
        parallel = self.checkBox_fitGridParallel.isChecked()
        self.GM.fit(parallel=parallel)
        self.generate_graphs_all()
    ...
    def draw_ap_crosshair(self, graph):
        ...
        ap_x_val = self.GM.analysis_point.get_index(x_label)
        ...
        if 'slice' in graph_name:
            ...
            ap_y_val = self.GM.analysis_point.get_index(y_label) + 0.5
