import pandas as pd
from strideutils import stride_config as stride_config, stride_requests as stride_requests
...
def get_tvl(chain_config: stride_config.ChainConfig, api_token: str | None = None, cache_response: bool = False) -> float: ...
def get_token_price_history(token: str, num_days: int = 90, api_token: str | None = None, _memo: dict[tuple[str, int], pd.DataFrame] | None = None) -> pd.DataFrame: ...
def get_token_volume_history(token: str, num_days: int = 90, api_token: str | None = None, _memo: dict[tuple, pd.DataFrame] | None = None) -> pd.DataFrame: ...
