...
from c2d.constants import CURRENT_ARTEFACTS_INFRA_VERSION, SUPPORTED_IMAGE_TAGS
...
def get_base_image(packaging: Packaging) -> str:
    ...
    try:
        ...
        # Everything gpu for now, could specify gpu / non-gpu based on tiers in future
        tag += f"-{simulator_version}-gpu-{CURRENT_ARTEFACTS_INFRA_VERSION}"
    except IndexError:
        # Cannot split means simulator version not specified, attempt just simulator (e.g turtlesim, maniskill)
        tag += f"-{packaging.simulator}-{CURRENT_ARTEFACTS_INFRA_VERSION}"
        logging.warning(
