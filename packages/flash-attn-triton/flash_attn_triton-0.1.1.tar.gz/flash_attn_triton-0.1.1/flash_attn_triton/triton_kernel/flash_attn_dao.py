...
flash_attn_qkvpacked_func_triton = FlashAttnQKVPackedFunc.apply
...
flash_attn_kvpacked_func_triton = FlashAttnKVPackedFunc.apply
...
flash_attn_func_triton = FlashAttnFunc.apply
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dropout_p, dtype=torch.float16):
    ...
    tri_out = flash_attn_func_triton(q, k, v, causal, sm_scale).half()
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, mode, provider, dropout_p=0.0, device=DEVICE):
    ...
    if "triton" in provider:
        ...
        fn = lambda: flash_attn_func_triton(q, k, v, causal, sm_scale)
        if mode == "bwd":
            o = fn()
    ...
    if provider == "flash":
        ...
        fn = lambda: flash_attn_func_triton(qkv, causal=causal)
        if mode == "bwd":
            o = fn()
            do = torch.randn_like(o)
            fn = lambda: o.backward(do, retain_graph=True)
