...
for /f "Tokens=* Delims=" %%x in (%ENV_DIR%\tiling-advisor\jib-classpath-file) do set CPText=!CPText!%%x

REM replace / with \
set "CP=!CPText:/=\!"
REM replace tiling-advisor\libs to libs
set "CP=!CP:tiling-advisor\libs=libs!"
REM replace : with ;ENV_DIR
set "CP=!CP::=;%ENV_DIR%!"
REM append ENV_DIR at the start of text
set "CP=%ENV_DIR%%CP%"

set /p JIB_MC=<%ENV_DIR%\tiling-advisor\jib-main-class-file
