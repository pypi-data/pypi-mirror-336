...
class HydraEmailManager:
    ...
    def baixar_emails(self, user_email, folder_id, is_read=None, file_format="eml", subject_filter=None, from_filter=None, body_filter=None, order_by=None, limit=10, only_attachments=False):
        ...
        if response.status_code == 200:
            # Cria a pasta 'emails' se ela não existir
            ...
            for email in emails:
                subject = email.get("subject", "sem_assunto").replace("/", "_").replace("\\", "_")
                file_name = f"{subject}.{file_format}"
                file_path = os.path.join("emails", file_name)
    
                # Salva o conteúdo do email no formato especificado
                with open(file_path, "w", encoding="utf-8") as file:
                    file.write(email.get("body", ""))
    
                saved_files.append(file_path)
    
            # Retorna os arquivos salvos e os dados dos emails
            return {
                "success": True,
                "message": f"{len(saved_files)} emails salvos na pasta 'emails'.",
