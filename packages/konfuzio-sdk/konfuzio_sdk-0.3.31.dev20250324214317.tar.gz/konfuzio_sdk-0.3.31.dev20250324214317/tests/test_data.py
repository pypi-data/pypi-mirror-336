...
class TestOnlineProject(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.test_document_id = [document for document in cls.project.documents if document.text == original_document_text][0].id_
...
class TestOfflineExampleData(unittest.TestCase):
    ...
    def test_find_outlier_annotations_by_regex(self):
        ...
        outliers_with_test = label.get_probable_outliers_by_regex(project_regex.categories, use_test_docs=True, top_worst_percentage=1.0)
    ...
    def test_find_outlier_annotations_by_confidence(self):
        ...
        pipeline.df_train, pipeline.label_feature_list = pipeline.feature_function(documents=pipeline.documents, require_revised_annotations=False)
        pipeline.fit()
    ...
    def test_find_outlier_annotations_error(self):
        ...
        with pytest.raises(ValueError, match='search modes disabled'):
            label.get_probable_outliers(self.project.categories, regex_search=False, confidence_search=False, normalization_search=False)
    ...
    def test_get_category_by_name(self):
        ...
        assert self.project.get_category_by_id(63).name == self.project.get_category_by_name(category_name='Lohnabrechnung').name
        assert self.project.get_category_by_id(63).name_clean == self.project.get_category_by_name(category_name='Lohnabrechnung').name_clean
...
class TestEqualityAnnotation(unittest.TestCase):
    ...
    def test_overlapping_correct_same_label(self):
        ...
        _ = Annotation(document=document, spans=[first_span], label_set=self.label_set, label=self.label_one, is_correct=True)

        with pytest.raises(ValueError) as e:
            _ = Annotation(document=document, spans=[second_span], label_set=self.label_set, label=self.label_one, is_correct=True)
    ...
    def test_overlapping_wrong_same_label(self):
        ...
        first_span = Span(start_offset=1, end_offset=2)
        second_span = Span(start_offset=1, end_offset=2)
        _ = Annotation(document=document, spans=[first_span], label_set=self.label_set, label=self.label_one, is_correct=False)

        with pytest.raises(ValueError) as e:
            _ = Annotation(document=document, spans=[second_span], label_set=self.label_set, label=self.label_one, is_correct=False)
            assert 'is a duplicate of' in str(e)
    ...
    def test_overlapping_correct_other_label(self):
        ...
        first_span = Span(start_offset=1, end_offset=2)
        second_span = Span(start_offset=1, end_offset=2)
        _ = Annotation(document=document, spans=[first_span], label_set=self.label_set, label=self.label_two, is_correct=True)
    ...
    def test_overlapping_wrong_other_label(self):
        ...
        _ = Annotation(document=document, spans=[second_span], label_set=self.label_set, label=self.label_two, is_correct=False)
...
class TestOfflineDataSetup(unittest.TestCase):
    ...
    def test_add_annotation_with_complete_bbox_data(self):
        ...
        document = Document(project=self.project, category=self.category, text='hello', bbox_validation_type=BboxValidationTypes.STRICT)
    ...
    def test_document_check_bbox_coordinates(self):
        ...
        document_bbox = {'0': {'x0': 0, 'x1': 1, 'y0': 0, 'y1': 1, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'h'}}
    ...
    def test_document_check_bbox_zero_height_allowed(self):
        ...
        document_bbox = {'0': {'x0': 0, 'x1': 2, 'y0': 0, 'y1': 0, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'h'}}
        document = Document(project=self.project, category=self.category, text='h', bbox=document_bbox)
        _ = Page(id_=1, number=1, original_size=(595.2, 841.68), document=document, start_offset=0, end_offset=1)
        self.assertTrue(document.bboxes)
    ...
    def test_document_check_bbox_zero_width_allowed(self):
        ...
        document_bbox = {'0': {'x0': 0, 'x1': 0, 'y0': 0, 'y1': 2, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'h'}}
        document = Document(project=self.project, category=self.category, text='h', bbox=document_bbox)
        _ = Page(id_=1, number=1, original_size=(595.2, 841.68), document=document, start_offset=0, end_offset=1)
        self.assertTrue(document.bboxes)
    ...
    def test_document_text_modified(self):
        ...
        document_bbox = {'0': {'x0': 0, 'x1': 1, 'y0': 0, 'y1': 2, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'h'}}
        document = Document(
            project=self.project,
            category=self.category,
            text='hello',
    ...
    def test_document_spans(self):
        ...
        _ = Annotation(document=document, is_correct=True, label=self.label, label_set=self.label_set, spans=[span1, span2])
    ...
    def test_page_text(self):
        """Test text Page."""
        document_bbox = {'0': {'x0': 0, 'x1': 1, 'y0': 0, 'y1': 2, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'p'}}
    ...
    def test_page_text_without_specifying_offsets(self):
        ...
        document_bbox = {'0': {'x0': 0, 'x1': 1, 'y0': 0, 'y1': 2, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'p'}}
        document = Document(project=self.project, category=self.category, text='page1\fpage2\fpage3', bbox=document_bbox)
    ...
    def test_page_annotations(self):
        ...
        annotation1 = Annotation(document=document, is_correct=True, label=self.label, label_set=self.label_set, spans=[span1, span2])
        annotation2 = Annotation(document=document, is_correct=True, label=self.label, label_set=self.label_set, spans=[span3])
    ...
    def test_document_check_bbox_invalid_height_coordinates(self):
        ...
        document_bbox = {'0': {'x0': 1, 'x1': 0, 'y0': 0, 'y1': 2, 'top': 10, 'bottom': 11, 'page_number': 1, 'text': 'h'}}
        document = Document(project=self.project, category=self.category, text='h', bbox=document_bbox)
        _ = Page(id_=1, number=1, original_size=(595.2, 841.68), document=document, start_offset=0, end_offset=1)
    ...
    def test_to_add_two_annotations_to_a_document(self):
        ...
        second_annotation = Annotation(document=document, spans=[second_span, third_span], label_set=self.label_set, label=self.label)
    ...
    def test_to_add_a_correct_annotation_with_a_duplicated_span_to_a_document(self):
        ...
        second_span = Span(start_offset=1, end_offset=2)
        third_span = Span(start_offset=2, end_offset=3)
        _ = Annotation(document=document, spans=[first_span], label_set=self.label_set, label=self.label, is_correct=True)
    ...
    def test_create_subdocument_from_page_range(self):
        ...
        new_doc = test_document.create_subdocument_from_page_range(test_document.pages()[0], test_document.pages()[1], include=True)
...
class TestKonfuzioDataSetup(unittest.TestCase):
    ...
    @pytest.mark.xfail(reason='Too strict limitation and does not coincide with limitations on app; we allow uploading duplicates.')
    ...
    @unittest.skip(reason='Skip: Changes in AbstractExtractionAI Annotation needed to require a Label for every Annotation init.')
    ...
    @unittest.skip(reason='Skip: Changes in AbstractExtractionAI Annotation needed to require a Document for every ' 'Annotation init.')
    ...
    def test_add_annotation_for_no_category_document(self):
        ...
        with pytest.raises(ValueError, match='We cannot add'):
            _ = Annotation(label=label, annotation_set=annotation_set, label_set=label_set, document=test_document, spans=[span])
...
class TestKonfuzioForceOfflineData(unittest.TestCase):
    ...
    def test_force_offline_project(self):
        ...
        annotation = Annotation(doc, annotation_set=annotation_set, label_set=label_set, label=label, id_=1, spans=[Span(0, 1)])
