...
from DIRAC.WorkloadManagementSystem.Client.JobStateUpdateClient import (
...
class DownloadInputData:
    ...
    def __checkDiskSpace(self, totalSize):
        ...
        if (data + totalSize) < availableBytes:
            ...
            msg = (
                "Not enough disk space available for download %s (including %dGB buffer) > %s bytes"
                % (
                    (data + totalSize),
                    bufferGBs,
                    availableBytes,
    ...
    def _downloadFromBestSE(self, lfn, reps, guid):
        ...
        """
        self.log.verbose(
            "Attempting to download file from all SEs", f"({','.join(reps)}): {lfn}"
        )
        diskSEs = set()
        tapeSEs = set()
        ...
        for seName in list(diskSEs) + list(tapeSEs):
            if seName in diskSEs or _isCached(lfn, seName):
                ...
                if result["OK"]:
                    ...
                    self.log.error(
                        f"Download failed \n Tried downloading {lfn} from SE {seName}: {result['Message']}"
                    )
    ...
    def _downloadFromSE(self, lfn, seName, reps, guid):
        ...
        # derive voName from lfn to be able to download both lfns starting with /vo.cta.in2p3.fr and /ctao
    ...
    def __setJobParam(self, name, value):
        ...
        self.log.verbose(
            "setting job parameters", f"setJobParameter({self.jobID},{name},{value})"
        )
        jobParam = JobStateUpdateClient().setJobParameter(
            int(self.jobID), str(name), str(value)
        )
        if not jobParam["OK"]:
            self.log.warn(f"Failed to set job parameters {jobParam['Message']}")
    ...
    def __cleanFailedFile(self, lfn, downloadDir):
        ...
        self.log.info(
            f"Trying to remove file after failed download Local path: {filePath}"
        )
        if os.path.exists(filePath):
            try:
                os.remove(filePath)
                self.log.info(
                    f"Removed file remnant after failed download \n Local path: {filePath}"
                )
            except OSError as e:
                self.log.info(f"Failed to remove file after failed download {repr(e)}")
