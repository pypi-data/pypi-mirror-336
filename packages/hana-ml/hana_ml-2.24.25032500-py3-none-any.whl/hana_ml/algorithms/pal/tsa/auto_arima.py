...
class AutoARIMA(_AutoARIMABase):
    ...
    where the seasonality (m) can be estimated using seasonal_decompose() function, and 'd' and 'D' are usually determined first due to information criterion considerations.
    ...
    more efficient but may not yield the optimal result. The constant part's inclusion depends on the criterion information, mainly when d + D isn't more than 1.
