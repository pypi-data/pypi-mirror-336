...
class OutlierDetectionTS(PALBase):#pylint:disable=too-many-instance-attributes
    ...
    voting_config : dict, optional
        ...
        - 'dbscan': ``eps``, ``minpts``, ``distiance_method``, ``dbscan_normalization``, ``dbscan_outlier_from_cluster``.
    ...
    __init_param_dict = {'auto' : ('AUTO', bool),
                         ...
                         'minpts' : ('MINPTS', int),
                         'eps' : ('RADIUS', float),
                         'distiance_method' : ('DISTANCE_METHOD', int, {'manhattan':1, 'euclidean':2, 'minkowski':3, 'chebyshev':4, 'standardized_euclidean':5, 'cosine':6}),
        ...
        if self.massive is True:
            ...
            if not self.is_index_int:# timestamp
                ...
                for group in data_groups:
                    group_val = group if 'INT' in group_key_type else f"'{group}'"
            ...
            for group in self.__pal_params:
                ...
                for name, value_type in self.__pal_params[group].items():
                    if "VOTING_OUTLIER_METHOD" in name and name != 'VOTING_OUTLIER_METHOD_CRITERION':
                        tpl = [(group_val, "VOTING_OUTLIER_METHOD", value_type[0], None, None)]
                    else:
            ...
            self._disable_hana_execution):
                msg = 'The version of your SAP HANA does not support massive outlier detection!'
                logger.error(msg)
                raise ValueError(msg)
            try:
