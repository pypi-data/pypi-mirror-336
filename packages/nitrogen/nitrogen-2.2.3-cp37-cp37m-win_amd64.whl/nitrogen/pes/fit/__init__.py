...
class InternuclearExp(nitrogen.dfun.DFun):
    ...
    The :math:`y_{ij}` functions are ordered :math:`y_{12}, y_{13}, \\ldots, y_{1n}, y_{23}, \\ldots, y_{n-1,n}`.
    ...
    def __init__(self, n, a, r0 = 0.0, offset = 0.0 ):
        ...
        a : float
            The exponential length parameter.
        r0 : float, optional
            A reference length. The default is 0.
        offset : float, optional
            Offset of each coordinate function. The default is 0.
...
class ExpPIP(nitrogen.dfun.DFun):
    """
    ...
    def __init__(self, a, natoms, coeff, terms,
        ...
        Notes
        -----
        `coeff` will be cast to np.float64 
        ...
        self.coeff = np.array(coeff).astype(np.float64) 
    ...
    def _ffun(self, X, deriv = 0, out = None, var = None):
        ...
        evalexppip(Y, f, deriv, table, 
                   self.a, self.natoms, self.maxmon,
                   self.coeff, self.terms)
