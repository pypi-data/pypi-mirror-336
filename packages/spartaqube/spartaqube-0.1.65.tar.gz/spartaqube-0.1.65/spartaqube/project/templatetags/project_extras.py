...
def get_item(dictionary,key):
	A=settings.IS_VITE
	if A:
		if os.environ.get('CYPRESS_TEST_APP','0')=='1':A=False
	if A:return f"http://localhost:3000/src/{key}"
	B=dictionary.get(key,'');C=B.lstrip('/');D=urljoin(settings.STATIC_URL,C);E=str(PurePosixPath(D));return E