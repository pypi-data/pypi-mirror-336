...
class SenderKernel:
	...
	async def send_zmq_request(B,sender_dict,b_send_websocket_msg=True):
		...
		while True:
			A=json.loads(await B.zmq_socket.recv_string());C=A[_A];logger.debug('response_dict');logger.debug(A);logger.debug(f"service >>> {C}")
			if C in['exec','execute_code','execute_shell','execute',_E,'deactivate_venv','reset_kernel_workspace',_F,'set_workspace_variables','set_workspace_variable_from_datasource']:
