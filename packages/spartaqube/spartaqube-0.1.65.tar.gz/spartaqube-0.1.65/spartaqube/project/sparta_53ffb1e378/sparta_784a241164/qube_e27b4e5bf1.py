...
from project.sparta_53ffb1e378.sparta_ee33375414.qube_a3d5668237 import IPythonKernel
...
class ReceiverKernel:
	...
	async def process_request(C,identity,request_dict):
		S='value';R='name';Q='kernel_variable';P='cellId';O='cmd';J='json_data';E=identity;A=request_dict;D=A[_E];B=C.ipython_kernel;B.set_zmq_identity(E);B.set_zmq_request(A)
		if D=='execute_code':H=A[O];await B.execute_code(H,websocket=C.socket_zmq)
		elif D=='execute_shell':H=A[O];F=json.loads(A[J]);await B.execute_shell(H,websocket=C.socket_zmq,cell_id=F[P])
		elif D=='execute':H=A[O];F=json.loads(A[J]);await B.execute(H,websocket=C.socket_zmq,cell_id=F[P])
		...
		elif D=='get_kernel_variable_repr':K=A[Q];U=B._method_get_kernel_variable_repr(kernel_variable=K);G={_A:1,_B:U};await C.send_response(E,G,A)
		elif D=='get_workspace_variable':K=A[Q];V=await B._method_get_workspace_variable(kernel_variable=K);W=base64.b64encode(cloudpickle.dumps(V)).decode(_C);G={_A:1,_B:W};await C.send_response(E,G,A)
		...
		elif D=='set_workspace_variable':F=json.loads(A[J]);await B._method_set_workspace_variable(name=F[R],value=json.loads(F[S]));await C.terminate(E)
			...
			await C.terminate(E)
		elif D=='set_workspace_variable_from_datasource':F=json.loads(A[J]);I=json.loads(F[S]);Y=pd.DataFrame(I['data'],columns=I['columns'],index=I['index']);await B._method_set_workspace_variable(name=F[R],value=Y);await C.terminate(E)
