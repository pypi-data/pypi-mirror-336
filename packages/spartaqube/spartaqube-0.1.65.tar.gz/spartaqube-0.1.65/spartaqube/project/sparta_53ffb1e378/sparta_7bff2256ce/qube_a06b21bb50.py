...
def sparta_ae893e8a12(user_obj):
	...
	return B
def sparta_a2301c325f(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_4c7896ad88()[_a],_K,_b)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_7207c4b91f(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_H];A=sparta_14c28d1bf8(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_ae893e8a12(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_a2301c325f(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_fad313911c(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_3187e2bfc9.sparta_cf55247498(A,user_obj)
def sparta_9f1f7f7084(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_ae893e8a12(E)
...
def sparta_b364820721(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_ae893e8a12(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_2a3d3b5038(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_b77967a45f(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_0eb129de9f(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_Y];E=_D
	if G:E=A[_j];E=qube_59c94ebf0e.sparta_431fee2c5d(E)
	O=A[_Z];P=A[_I];Q=A[_Q];D=A[_H];D=sparta_14c28d1bf8(D);R=A[_k];S=A[_l];G=A[_Y];T=A[_m];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_I]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_4c7896ad88()[_o];M=os.path.join(D,_p,_q,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_8deca42bf5());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_0eb129de9f(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_ae893e8a12(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_4c7896ad88()[_o];S=os.path.join(b,_p,_q,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_43c63fb94c(json_data,user_obj):
	...
	return{_B:1}
def sparta_6c08e43c01(json_data,user_obj):
	...
	return{_B:1}
def sparta_2a3d3b5038(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_59c94ebf0e.sparta_bc9cf31dc0(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_fd20ea3cd5(json_data,user_obj):A=sparta_14c28d1bf8(json_data[_H]);return sparta_a9ba571667(A)
def sparta_fe4a0a2f1e(json_data,user_obj):A=sparta_14c28d1bf8(json_data[_H]);return sparta_4a491c9c75(A)
def sparta_315e851b97(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_14c28d1bf8(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec_DEPREC(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			H=qube_02c3b53459.sparta_a612941571(A);I=H['cells']
			for B in I:
...
async def notebook_permission_code_exec(json_data):
	...
	return''
def sparta_9fb5fb1621(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_6b70dbe6cd(project_path,python_executable=_u):
...
def sparta_0a52e658e7():
	...
	return B
def sparta_baa2347945(json_data,user_obj):
	D=sparta_14c28d1bf8(json_data[_H]);A=D;B=os.path.join(sparta_4c7896ad88()[_a],_K,_b);C=os.path.join(A,_O)
...
def sparta_ec70f6a8c4(json_data,user_obj):
	A=sparta_14c28d1bf8(json_data[_H]);A=os.path.join(A,_O);G,C,D=sparta_6b70dbe6cd(A);B=_A;E=1;F=''
	if D:
...
def sparta_70baf4f9d0(project_path,python_executable=_u):
...
def sparta_01197c3438(json_data,user_obj):
	A=sparta_14c28d1bf8(json_data[_H]);A=os.path.join(A,_O);B,C=sparta_70baf4f9d0(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_y:C,_E:E}
def sparta_8fffdd62f1(json_data,user_obj):return{_B:1}
def sparta_355d96cecf(json_data,user_obj):return{_B:1}