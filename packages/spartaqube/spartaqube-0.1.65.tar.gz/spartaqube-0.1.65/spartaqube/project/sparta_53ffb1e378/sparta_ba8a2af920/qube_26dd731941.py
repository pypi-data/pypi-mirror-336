...
def sparta_33b3dd8ff6():return str(uuid.uuid4())
def sparta_25973fc219(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_f656a1f8ab(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_5a0e210d50(path):
	with open(path,'a'):os.utime(path,None)
def sparta_f57a107648(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_f656a1f8ab(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_5a0e210d50(B)
	...
	return{_A:1}
def sparta_b2e90ebe40(json_data,userObj):
	...
	return{_A:1}
def sparta_a2f118a154(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_f656a1f8ab(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_722fbe711f(json_data,userObj):A=json_data[_B];B=qube_b9f5298885.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_8bcaabc343(json_data,userObj):
	A=json_data
	try:
...
def sparta_989c74bf29(json_data,userObj):
	...
	return{_A:1}
def sparta_362a2d26e3(json_data,userObj):
	...
	return{_A:1}
def sparta_d8beb84d1b(json_data,userObj):
...
def sparta_cbab33bde7(json_data,userObj):
	...
	return{_A:-1}
def sparta_7d933e4950(json_data,userObj):
