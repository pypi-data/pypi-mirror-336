...
def sparta_5d97bd97c7(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_570773adab(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_26dd731941.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_d5d6983327(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();logger.debug('realtime_output 1');logger.debug(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);logger.debug('realtime_output 2');logger.debug(B)
def sparta_ef416d1c26(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_570773adab(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_9f029faa21(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_570773adab(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_e9de139420(repo,user_obj):
...
def sparta_41323f2383(webSocket,json_data,user_obj):
...
def sparta_cf55247498(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_bc2fd243f3(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_5d97bd97c7(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_e9de139420(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_e9de139420(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_d5d6983327(K);os.chdir(L)
	return{_A:1}
def sparta_e000ee980f(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_5d97bd97c7(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_e9de139420(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_4771c13c29(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_5d97bd97c7(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_e9de139420(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_d5d6983327(E);os.chdir(F)
	return{_A:1}
@sparta_ef416d1c26
def sparta_2be4452cb9(json_data,user_obj):return{_A:1}
def sparta_7226902f59(json_data,user_obj):
	B=json_data[_C];C=sparta_5d97bd97c7(B)
	if C:A=git.Repo(B);sparta_e9de139420(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_dd41074471(json_data,user_obj):
	B=json_data[_C];C=sparta_5d97bd97c7(B)
	if C:A=git.Repo(B);sparta_e9de139420(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_ee8c862cf3(json_data,user_obj):
	A=json_data[_C];C=sparta_5d97bd97c7(A)
	if C:
		B=git.Repo(A);sparta_e9de139420(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_d1652b9340(json_data,user_obj):A=json_data[_C];B=sparta_5d97bd97c7(A);return{_A:1,_L:B}
def sparta_93dc1af7e0(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_5d97bd97c7(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_b459ee737e(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_5d97bd97c7(C)
	if I:
		B=git.Repo(C);sparta_e9de139420(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_bbfd2df267(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_5d97bd97c7(C)
	if E:A=git.Repo(C);sparta_e9de139420(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_ef416d1c26
def sparta_cb29b2beaf(json_data,user_obj):return{_A:1}
def sparta_922f3b44ff(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_5d97bd97c7(C)
	if E:
		B=git.Repo(C);sparta_e9de139420(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_154ceab619(json_data,user_obj):return{_A:1}
def sparta_0e1caa72ab(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_5d97bd97c7(E)
	if F:
		A=git.Repo(E);sparta_e9de139420(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_20e5f75a7c(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_5d97bd97c7(B)
	if E:
		C=git.Repo(B);sparta_e9de139420(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_8a94d40a97(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_5d97bd97c7(C)
	if F:
		A=git.Repo(C);sparta_e9de139420(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_ca7f8d6619(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_5d97bd97c7(A)
	if F:
		E=git.Repo(A);sparta_e9de139420(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_4344d89432(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_5d97bd97c7(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_e9de139420(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}