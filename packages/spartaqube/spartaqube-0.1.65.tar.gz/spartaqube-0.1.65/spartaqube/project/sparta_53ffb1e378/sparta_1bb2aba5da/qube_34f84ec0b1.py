...
from project.sparta_9e6f96b177.sparta_fed68a0eab.qube_3b035725a9 import sparta_c8ecd38ed6,sparta_650014e5e3,sparta_f6ca77941d
from project.sparta_53ffb1e378.sparta_0a2f6496d5.qube_8fbd96b92d import sparta_648c3cdcbc
from project.logger_config import logger
def sparta_1790654d02(json_data,user_obj):
...
def sparta_f639f150ef(message,typeCase=0,companyName=_G):
	...
	if C.count()>0:
		...
		A.send()
def sparta_9153949604(json_data,userObj):
	...
	return B
def sparta_bcd4c67c9e(json_data,userObj):
	...
	return A
def sparta_3f4570f203(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_648c3cdcbc(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_8852efdf14():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_1b9f0731eb(json_data):
	A=json_data;C=A[_F];E=A[_H];B=A[_I];F=A[_R]
	if not sparta_f6ca77941d(E):return{_A:-1,_B:_J}
...
def sparta_cb0ad2fc71(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_H];G=sparta_c8ecd38ed6(E)
	if G[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_650014e5e3(F):return{_A:-1,_B:_J}
...
def sparta_678f81d840(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_H];H=A[_U];E=A[_I];I=A[_R];J=sparta_c8ecd38ed6(F)
	if J[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_650014e5e3(G):return{_A:-1,_B:_J}
