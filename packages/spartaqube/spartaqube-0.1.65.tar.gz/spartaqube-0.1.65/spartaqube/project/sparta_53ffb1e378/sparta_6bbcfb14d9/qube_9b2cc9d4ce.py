...
def sparta_434703c19a(json_data,user_obj):
...
def sparta_7525322325(message,typeCase=0,companyName=_E):
	...
	if E.count()>0:
		...
		A.send()
def sparta_172af3d50d(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_6abff05ba1(model_to_dict(D));del A[G];A[_F]=naturalday(parser.parse(str(A[_F])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_6abff05ba1(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_e90c992dde(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_6abff05ba1(model_to_dict(L));A[_F]=naturalday(parser.parse(str(A[_F])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_7ffc427fca(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_7525322325(F,A.type_ticket,_E)
...
def sparta_bf6bc52cac(json_data,user_obj):
	...
	return{_A:1}
def sparta_b5ec1d2192(json_data):
	...
	return{_A:1}
def sparta_03a7eb0564(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}