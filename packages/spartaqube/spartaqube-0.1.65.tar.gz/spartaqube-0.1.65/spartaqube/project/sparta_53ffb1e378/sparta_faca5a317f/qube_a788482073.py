...
def sparta_c53eb7ce6e():B='db-conn';A=B.encode(_A);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_A));return A.decode(_A)
def sparta_3389c4a8dd(password_to_encrypt):A=password_to_encrypt;A=A.encode(_A);C=Fernet(sparta_c53eb7ce6e().encode(_A));B=C.encrypt(A).decode(_A);B=base64.b64encode(B.encode(_A)).decode(_A);return B
def sparta_0354577766(password_e):B=Fernet(sparta_c53eb7ce6e().encode(_A));A=base64.b64decode(password_e);A=B.decrypt(A).decode(_A);return A
def sparta_afe21dc634():
