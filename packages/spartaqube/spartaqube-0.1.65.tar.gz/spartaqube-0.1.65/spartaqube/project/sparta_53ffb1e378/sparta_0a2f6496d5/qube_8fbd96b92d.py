...
from project.sparta_53ffb1e378.sparta_f1549b2883 import qube_99425fc462 as qube_99425fc462
from project.sparta_53ffb1e378.sparta_40a51d32d8.qube_8721075617 import Email as Email
from project.logger_config import logger
def sparta_43c7b997ad(function):
	def A(request,*E,**D):
		A=request;B=_B
		...
		if not B:
			if C is not _E:F=qube_99425fc462.sparta_0d06b5607d(C);login(A,F)
		else:0
		return function(A,*E,**D)
	return A
def sparta_795c1e5190(function):
	def A(request,*C,**D):
	...
	return A
def sparta_b9a6882d05(function):
	def A(request,*B,**C):
	...
	return A
def sparta_711f7ee2a4(function):
	C=function
	def A(request,*D,**E):
		A=request;F=_C
		try:
			G=json.loads(A.body);H=json.loads(G[_J]);I=H[_I];B=qube_99425fc462.sparta_0d06b5607d(I)
...
def sparta_d366209995():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_5b07d8f3a3(json_data):
	D='ip_addr';A=json_data;del A[_K];del A[_L]
	try:A[D]=sparta_d366209995()
	except:A[D]=-1
...
def sparta_bf73a4a00d(file_path,text):
	A=file_path
	try:
...
def sparta_035c659038(json_data,hostname_url):
	P='emailExist';O='passwordConfirm';K='email';B=json_data;F={O:'The two passwords must be the same...',K:'Email address is not valid...','form':'The form you sent is not valid...',P:'This email is already registered...'};E=_C;Q=B['firstName'].capitalize();R=B['lastName'].capitalize();C=B[K].lower();L=B[_K];S=B[_L];T=B['code'];M=B['captcha'];B['app_id']=sparta_26b6cfeed9()
	if M=='cypress'and C=='cypress_tests@gmail.com':
		if int(os.environ.get('CYPRESS_TEST_APP','0'))==1:
			try:from project.sparta_fc96e589c2.sparta_1f599b79d9.qube_d6007c5161 import sparta_e03498c682 as U;U()
			except Exception as V:W='C:\\Users\\benme\\Desktop\\LOG_DEBUG_CYPRESS.txt';sparta_bf73a4a00d(W,str(V))
	else:
		X=sparta_c8ecd38ed6(M)
		if X[_A]!=1:return{_A:-1,_D:_H}
	if not sparta_648c3cdcbc(T):return{_A:-1,_D:'Invalid spartaqube code, please contact your administrator'}
	if L!=S:E=_B;G=F[O]
	if not re.match('[^@]+@[^@]+\\.[^@]+',C):E=_B;G=F[K]
	if User.objects.filter(username=C).exists():E=_B;G=F[P]
	if not E:
		Y=sparta_5b07d8f3a3(B);N=_B;Z=Y[_G]
		if not Z:N=_C
...
def sparta_f845278f09():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_6a0ca5372e(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_237bc12875(guest_code,user_obj):
	...
	return _C
def sparta_fcf454e400(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_3d8ee85896(email,captcha):
	D=sparta_c8ecd38ed6(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_0faa26020a(A.user,C);return{_A:1}
def sparta_0faa26020a(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_29eb898050(captcha,token,email,password):
	D=sparta_c8ecd38ed6(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
