...
There are also several other methods that can be used from the Apollo class:
...
embeddings = azure.get_embeddings("Hello, world!")

# Get embeddings for multiple texts
texts = ["Hello, world!", "Another text"]
embeddings = azure.get_embeddings(texts)
```
...
response = azure.chat_completion(messages=messages, is_stream=True)
print(azure.get_content(response))  # Returns the complete response as a string

# Works the same way for non-streaming responses
response = azure.chat_completion(messages=messages, is_stream=False)
print(azure.get_content(response))  # Also returns the complete response as a string
```
