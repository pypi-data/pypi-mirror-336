...
def run_runbot(
    ...
    for step in project_config.steps:
        if only_action is not None and step.action != only_action:
            ...
            continue
        if step_names and not step_names.intersection({None, "all", step.name}) and step.action != StepAction.INSTALL:
            _logger.info("Skip %s", step.name)
            continue
...
def write_exit_code(step: RunbotStepConfig, *, return_code: int, failfast: bool) -> None:
    if env.in_ci:
        datas = {}
