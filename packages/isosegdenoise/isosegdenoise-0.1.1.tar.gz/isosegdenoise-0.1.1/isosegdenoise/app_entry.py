    ...
    def img_entry_func(self, directory: str = None, resolutions = None, from_mcds = True) -> None:
        ...
        if directory is None:
            directory = "not a directory"   
                    ## if the directory remains None, odd behaviours can result because of default arguments in downstram functions ---> 
        ...
        try:
            try:
                ...
                return
            if len(os.listdir(directory + "/images")) == 0:
...
class License_window(ctk.CTkToplevel, metaclass = CtkSingletonWindow):
    ...
    def __init__(self, master):
        ...
        label = ctk.CTkLabel(master = self, text = """Copyright Medical University of South Carolina 2024   (BSD-3 license)
                    ...
                    Much of this project is also heavily based on / derived from a number of existing open-source packages:
                        \t Steinbock (https://github.com/BodenmillerGroup/steinbock, MIT license, Python): much of MCD and Image processing, project directory structure, steinbock_for_deepcell_alone.py file
                        \t singletons (https://github.com/jmaroeder/python-singletons, MIT, python): Utils/shared_classes, for windows and loggers
                        \t scikit-image (https://github.com/scikit-image/scikit-image, BSD-3, python): simple denoising uses techniques discussed in skimage documentation
                    ...
                    Also, note that the deepcell / Mesmer models are licensed for "non-commercial" use only, and the mian cellpose models were trained on "non-commercial" 
