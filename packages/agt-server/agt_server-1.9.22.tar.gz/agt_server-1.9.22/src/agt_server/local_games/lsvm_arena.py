...
# TODO: Multiprocess this shit so that it runs faster 
...
class LSVMArena(LocalArena):
                    ...
                    for i in range(len(curr_players)): 
                        ...
                        for j in range(len(curr_players)): 
                            if i != j: 
                                ...
                                curr_players[j].valuations = curr_players[j].proximity(np.random.uniform(3, 20, shape))
    ...
    def prune_valid_bids(player, my_bids):
        ...
        for good, bid in my_bids.items():
            if bid is None:
                invalid_reasons[good] = "None value"
                continue

            if good not in player._goods_to_index:
                invalid_reasons[good] = "Unknown good"
                continue
            ...
            if not revealed_preference:
                invalid_reasons[good] = "Revealed preference violated"
            else:
                valid_bids[good] = bid
