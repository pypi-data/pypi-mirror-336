...
def test_partition(pdf, kwargs, response, mocker):
    ...
    resp_object.iter_bytes.return_value = byteses.split(sep=b"\n")

    mocker.patch("httpx.stream").return_value.__enter__.return_value = resp_object
...
def test_partition_it(pdf, kwargs, response):
    ...
    assert len(response_data["elements"]) == len(new_response["elements"])
    for el1, el2 in zip(response_data["elements"], new_response["elements"]):
        assert el1 == el2
...
def test_partition_with_unsupported_file_format():
    with pytest.raises(PartitionError):
        with open(
            RESOURCE_DIR / "image" / "unsupported-format-test-document-image.heic", "rb"
        ) as f:
            partition_file(f)
...
def test_partition_it_no_api_key():
    with pytest.raises(HTTPStatusError) as einfo:
...
def test_partition_file_async_submit(mocker):
    ...
    mocked_response.iter_bytes.return_value = (data,)

    mocker.patch("httpx.stream").return_value.__enter__.return_value = mocked_response
...
def test_partiton_file_async_url_forwarding(mocker):
    ...
    def call_partition_file(base_url: str):
        ...
        partition_file_async_submit(
            dummy,
            aps_url="https://example.com/v1/document/partition",
            docparse_url=base_url,
        )
    ...
    nonstandard_async_url_example = nonstandard_url_example.replace(
        "/v1/", "/v1/async/submit/"
    )
...
def test_partition_file_async_with_unsupported_file_format():
    ...
    while True:
        ...
        if (
            actual_result["task_status"] != "pending"
            or time.time() - start >= ASYNC_TIMEOUT
        ):
            break
        time.sleep(1)
...
def test_multiple_partition_file_async():
    ...
    for i in range(num_tasks):
        ...
        task_id = partition_file_async_submit(
            RESOURCE_DIR / "pdfs" / "FR-2002-05-03-TRUNCATED-40.pdf"
        )["task_id"]
...
def test_smoke_webhook(mocker):
    ...
    mocked_response = mocker.Mock()
    mocked_response.status_code = 202
    mocked_response.iter_bytes.return_value = data.split(sep=b"\n")

    def check_webhook(*args, request, **kwargs):
        assert "X-Aryn-Webhook" in request.headers
        assert request.headers.get("X-Aryn-Webhook") == webhook_url
        return mocked_response

    fake_post = mocker.patch("httpx.Client.send", side_effect=check_webhook)

    partition_file_async_submit(
        RESOURCE_DIR / "pdfs" / "3m_table.pdf", webhook_url=webhook_url
    )
