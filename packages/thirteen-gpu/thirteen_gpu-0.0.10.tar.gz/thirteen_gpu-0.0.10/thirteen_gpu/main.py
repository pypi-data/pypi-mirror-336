...
def main():
    ...
    for _ in range(5):
        ...
        if datetime.now() - worker_pool_last_update >= timedelta(seconds=10):
            ...
            workers = [workers_dict[worker_name] for worker_name in sorted(workers_dict.keys())]
        ...
        for project_path in glob(f"{WORKSPACE_DIR}/*"):
            ...
            new_projects.append(Project(project_path, user, submit_at, executed_filename))
        ...
        if first_iteration:
            ...
            for project_name, job, status_value in zip(project_names, job_names, status_values):
                assign_status = False
        ...
        if len(deleted_projects) > 0 or len(deleted_jobs) > 0:
            ...
            threads = [threading.Thread(target=delete_jobs_in_worker, args=(worker,)) for worker in workers]
            execute_by_threads(threads)
        ...
        available_gpus = {}
        threads = [threading.Thread(target=get_available_gpus, args=(worker, available_gpus)) for worker in workers]
        execute_by_threads(threads)
        ...
        for worker in workers:
            ...
            jobs = [job for job in jobs if job not in pending_jobs_for_user[: len(gpus)]]
            
            print(f"{worker.name} 에 뿌릴 pending jobs: {[job.job_name for job in pending_jobs_for_user[: len(gpus)]]}")
            
            # jobs = jobs[len(gpus) :]

        # 현재 pending 중인 job 들을 각 서버에 dispatch 한다.
        threads = [
            threading.Thread(target=dispatch_jobs, args=job_dispatch_arg) for job_dispatch_arg in job_dispatch_args
        ]
        execute_by_threads(threads)
        ...
        for project in projects:
            ...
            if all(
                [
                    job.status
                    in (JobStatus.SUCCESS, JobStatus.FAILED, JobStatus.CRASHED, JobStatus.STOPPED, JobStatus.UNKNOWN)
                    for job in project.jobs.values()
                ]
            ):
                if os.path.exists(f"{project.project_path}/alarm.txt"):
                    from .slack import slack_alarm
                    slack_alarm(message=f"✅ 스케줄러 프로젝트 `{project.project_name}` 가 완료되었습니다. ✅", tag_user=project.user)
