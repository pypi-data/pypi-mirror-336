...
def main():
    ...
    if ssh.is_exists(f"{WORKSPACE_DIR}/{args.project}"):
        ...
        ssh.ssh_exec_command(f"echo {args.user} > {WORKSPACE_DIR}/{args.project}/user.txt")
        
        submit_at = datetime.now(timezone(timedelta(hours=9))).strftime('%Y-%m-%d %H:%M:%S')
        ssh.ssh_exec_command(f"echo {submit_at} > {WORKSPACE_DIR}/{args.project}/submit_at.txt")
        
        if args.alarm:
            ssh.ssh_exec_command(f"echo {args.project} > {WORKSPACE_DIR}/{args.project}/alarm.txt")
