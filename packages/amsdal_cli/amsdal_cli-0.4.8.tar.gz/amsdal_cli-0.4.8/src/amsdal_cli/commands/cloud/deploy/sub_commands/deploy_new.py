    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as _temp_dir:
        ...
        if AmsdalConfigManager().get_config().async_mode:
            ...
            asyncio.run(_async_check_missing_generated_migrations(cli_config, manager, app_source_path))
        else:
    ...
    try:
        ...
        if str(e) in ['Same environment name', 'same_environment_name']:
            ...
            rprint(
                rich_error(
                    f'Environment {rich_highlight(str(from_env))} is not deployed. Please check the environment name.'
                )
            )
