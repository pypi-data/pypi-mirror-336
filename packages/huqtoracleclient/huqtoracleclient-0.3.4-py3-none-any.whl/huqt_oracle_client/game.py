...
class GameInterface(ABC):
    ...
    def start_game(self):
        ...
        for account in self.accounts:
            ...
            self.admin.set_position(usd_pos, CURRENCY, account)
        ...
        for symbol in config:
            self.admin.add_symbol(self.partition, self.partition + symbol, symbol)
            self.init_account_positions(self.partition + symbol, config[symbol])
    ...
    def settle_game(self, settle_prices: Dict[str, int]):
        ...
        for symbol in settle_prices:
            prices[self.partition + symbol] = settle_prices[symbol]
        ...
        try:
            ...
            async for market_data in stream:
                ...
                if field == "orderbook":
                    ...
                    book['asks'] = list(map(lambda x: PublicOrder(x.symbol, int(x.size), int(x.price)), orderbook.book.bids))
                    self.orderbook[symbol] = book
