...
def test_sort_by_id_and_ts():
    data = pl.DataFrame({"id": [2, 2, 1, 1], "ts": [3, 2, 1, 0], "value": [10, 20, 30, 40]})

    dataset = TSDataset(data, id_column_name="id", ts_column_name="ts")

    expected_sorted_data = pl.DataFrame({"id": [1, 1, 2, 2], "ts": [0, 1, 2, 3], "value": [40, 30, 20, 10]})

    assert dataset.data.equals(expected_sorted_data)


def test_sort_already_sorted():
    data = pl.DataFrame({"id": [1, 1, 2, 2], "ts": [0, 1, 2, 3], "value": [40, 30, 20, 10]})
...
def test_sort_with_non_numeric_id():
    data = pl.DataFrame({"id": ["b", "b", "a", "a"], "ts": [3, 2, 1, 0], "value": [10, 20, 30, 40]})

    dataset = TSDataset(data, id_column_name="id", ts_column_name="ts")

    expected_sorted_data = pl.DataFrame({"id": ["a", "a", "b", "b"], "ts": [0, 1, 2, 3], "value": [40, 30, 20, 10]})

    assert dataset.data.equals(expected_sorted_data)


def test_add_feature():
    data = pl.DataFrame({"id": [1, 2, 3], "ts": [100, 200, 300], "value": [10, 20, 30]})
...
def test_add_feature_with_wrong_length():
    ...
    with pytest.raises(ValueError) as exc_info:
        dataset.add_feature(new_feature_name, new_values)
...
def test_add_feature_existing_column():
    ...
    with pytest.raises(ValueError) as exc_info:
        dataset.add_feature(existing_column_name, new_values)
