...
from typing import Any, Dict, List, Optional, Union, Type, TypeVar, cast, overload, get_type_hints
...
class Model(object):
    ...
    def from_datetime(self, value: Union[datetime, date, DateTime], date_format: Optional[str] = None) -> str:
        """
        ...
        if isinstance(value, DateTime):
            return value.format(date_format) if date_format else value.isoformat()

        if isinstance(value, date):
            value = Date.instance(value)

        return DateTime.instance(value).format(date_format) if date_format else DateTime.instance(value).isoformat()

    def as_datetime(self, value: Union[datetime, date, DateTime], date_format: Optional[str] = None) -> str:
        """
        Convert a value to a datetime string.
