...
"""
    ...
    if class_name and method_nodes:
    ...
    for node_item in nodes:
        ...
        if node_item.get('primaryLabel') in ['JavaMethodEntity', 'DotNetMethodEntity']:
            ...
            if annotations and any(anno.lower() in str(annotations).lower() for anno in 
                                ['getmapping', 'postmapping', 'putmapping', 'deletemapping', 
                                    'requestmapping', 'httpget', 'httppost', 'httpput', 'httpdelete']):
                rest_endpoints.append({
                    'name': node_item.get('name', ''),
                    'annotation': str([a for a in annotations if any(m in a.lower() for m in 
                                    ['mapping', 'http'])])
                })
