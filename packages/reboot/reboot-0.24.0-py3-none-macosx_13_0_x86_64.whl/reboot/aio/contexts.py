...
from reboot.time import datetime_now
from typing import (
    Awaitable,
    Callable,
    Generic,
    ...
    if effect_validation == EffectValidation.QUIET:
        if (
            identifier not in timestamps or (
                datetime_now() - timestamps[identifier]
                > _PRINT_EFFECT_VALIDATION_WARNINGS_DELTA
            )
        ):
            timestamps[identifier] = datetime_now()
...
class Context(ABC, IdempotencyManager):
    ...
    @property
    def _app_internal_authorization(self) -> Optional[str]:
        return self._headers.app_internal_authorization
