    ...
    try:
        ...
        for (pod_namespace, pod_name_prefixes) in pods:
            for pod_name_prefix in pod_name_prefixes:
                ...
                wait_for_running_task = asyncio.create_task(
                    k8s_client.pods.wait_for_running_with_prefix(
                        namespace=pod_namespace,
                        name_prefix=pod_name_prefix,
                    ),
                    name=f'k8s_client.pods.wait_for_running... in {__name__}',
                )
                ...
                while True:
                    ...
                    if len(done) == 1 and wait_for_running_task in done:
                        break
