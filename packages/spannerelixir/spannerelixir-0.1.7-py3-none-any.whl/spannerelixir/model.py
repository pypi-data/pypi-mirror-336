"""
Model definitions for SpannerElixir.
"""
...
from spannerelixir.exceptions import ModelDefinitionError, RecordNotFoundError
from spannerelixir.fields import DateTimeField, Field
...
class SpannerModel(metaclass=ModelMeta):
    ...
    def save(self, database: Database) -> T:
        """
    ...
    def update(self, database: Database) -> T:
        """
    ...
    def delete(self, database: Database) -> bool:
        ...
        try:
            # Execute deletion using KeySet with keys
            with database.batch() as batch:
                batch.delete(table=self._table_name, keyset=KeySet(keys=[list(key_dict.values())]))
            return True
    ...
    @classmethod
    def from_query_result(cls: Type[T], result_row, fields) -> T:
        """
        Create a model instance from a query result row.

        Args:
            result_row: Row from Spanner query result
            fields: Field descriptors from Spanner result
        ...
        for i, field in enumerate(fields):
            ...
            if field_name in cls._fields:
                instance_data[field_name] = result_row[i]
