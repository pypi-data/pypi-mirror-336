...
class FullQCConf:
    ...
    def measure(self, meas):
        ...
        if meas in {"LP-MODE", "OVERVOLTAGE-PROTECTION", "UNDERSHUNT-PROTECTION"}:
            tmp_connectivity = (
                self.BASEDIR / self.MODULESN / f"{self.MODULESN}_{self.LPTYPE}.json"
            )
            qc_type = self.LPTYPE
        else:
    ...
    def get_args(self):
        ...
        parser.add_argument(
            "-c", "--config", type=Path, help="Path to hardware config file"
        )
        parser.add_argument(
            "-o",
            "--output",
    ...
    def tun(self):
        ...
        layer = get_layer_from_serial_number(self.MODULESN)
