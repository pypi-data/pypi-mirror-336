...
def try_possible_extensions(file_path: str) -> str:
    for ext in ["", ".yaml", ".json", ".txt"]:
        possible = Path(file_path).with_suffix(f".{ext}")
        if possible.exists():
            return possible
    raise ValueError(
        "Could not find file with any valid extensions (json, yaml, txt): " + file_path
    )
