...
from quizzes.questions import QuizQuestion, convert_questions, extract_feedback, get_id
...
def load_flat_file(file_list: list[str], config: Configuration) -> QuizFile:
    ...
    if "header" in file_list[0].lower():
        ...
        actual_path = try_possible_extensions(file_list[0])
        settings = read_file(actual_path, "guess")
    else:
        settings = {}
    for file_path in file_list:
        pool_name = chomp_extension(file_path)
        actual_path = try_possible_extensions(file_path)
