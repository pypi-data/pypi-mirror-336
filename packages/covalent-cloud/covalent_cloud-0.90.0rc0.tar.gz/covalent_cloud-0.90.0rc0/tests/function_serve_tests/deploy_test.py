...
def sample_func(x, /, y, *args, foo="default", k, bar="kw_default", **kwargs):
    ...
    """
    return {
        "x": x, "y": y, "args": args, "foo": foo, "k": k, "bar": bar, "kwargs": kwargs
    }


sample_service = cc.service(
    sample_func,
    executor=Mock(memory=20_480)  # will break real deployment attempt
)
...
def mock_deploy_to_cloud():
    ...
    with \
            patch.object(DeploymentAPI, "post"), \
            patch.object(FunctionService, "get_model"), \
            patch.object(AssetsMediator, "hydrate_assets_from_model"), \
            patch.object(AssetsMediator, "upload_all"), \
            patch.object(Deployment, "from_function_record"), \
            patch.object(Deployment, "attach_route_methods"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # x, y, foo, *args
        ((1, 2, "dog", 4, 5, 6), {"k": 99.9, "bar": "cat",
         "extra_kw1": "value-1", "extra_kw2": "value-2"}),
    ]
)
