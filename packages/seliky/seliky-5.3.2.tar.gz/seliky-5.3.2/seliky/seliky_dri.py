...
co = co.set_browser_path(r'C:\Program Files\Google\Chrome\Application\chrome.exe')
bs = ChromiumPage(addr_or_opts=co)
"""
...
class WebDriver:
    ...
    def open_browser(self):
        ...
        if 'chrome' in executable_path or not executable_path:  # 默认没用时用谷歌浏览器
            self.opt = ChromiumOptions()
            self.opt.set_browser_path(executable_path)  # r'C:\Program Files\Google\Chrome\Application\chrome.exe'
    ...
    def _ele_(self, locator, index=0, timeout=8.0, **kwargs):
        ...
        if self.log_locator:
            desc = desc + ' ' + locator
        ele = self.driver.eles(locator=locator, timeout=timeout)[index]
        if ele:
    ...
    def send_keys(self, locator, value, index: int = 0, timeout: int = 6, clear: bool = True, enter=False, **kwargs):
        ...
        if not value and value != 0:  # 0可以有
            return
        time.sleep(pre_sleep)
        elem = self._ele_(locator, index, timeout, raise_=True)
        elem.input(value)
        self.__bac_force_sleep__(locator, sleep=bac_sleep)
    ...
    def is_visible(self, locator, timeout=6.0):
        ...
        多元素情况值判断索引为0、-1、中间
        """
        return self.driver.ele(locator=locator, timeout=timeout)
