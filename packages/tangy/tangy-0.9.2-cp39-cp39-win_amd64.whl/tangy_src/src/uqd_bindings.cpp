...
void
CTimeTag_saveDcCalibration(CTimeTag_ptr timetag, char* filename) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
    ptr->SaveDcCalibration(filename);
}

void
CTimeTag_loadDcCalibration(CTimeTag_ptr timetag, char* filename) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
    ptr->LoadDcCalibration(filename);
}

void
CTimeTag_setFG(CTimeTag_ptr timetag, int period, int high) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
    ptr->SetFG(period, high);
}

void
CTimeTag_setFGCount(CTimeTag_ptr timetag, int period, int high, int count) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
    ptr->SetFGCount(period, high, count);
}

int
CTimeTag_getSingleCount(CTimeTag_ptr timetag, int i) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
    return ptr->GetSingleCount(i);
}

int64_t
CTimeTag_freezeSingleCounter(CTimeTag_ptr timetag) {
    auto ptr = reinterpret_cast<TimeTag::CTimeTag*>(timetag);
