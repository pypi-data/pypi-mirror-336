...
def lib(
    ...
    if not Plausipy.hasMainPackage():
        logger.warning("No main package found, consent will be asked for %s", package_name)
...
def _ppcli(pp: Plausipy) -> None:
    ...
    logger.critical(f"CLI DISABLING { pp._app_name } - {'--no-tracking' in sys.argv} - {pp.disabled}")
