...
from yeref.l_ import l_inline_demo, l_inline_bot, l_inline_post, l_inline_media, l_inline_channel, l_inline_group, \
    ...
    l_tools_has_restricted, l_bot_need_start_add, l_chn_need_boost_for_story
...
USER_VARS_ = '{"USER_TEXT": "", "USER_REACTION": "", "USER_PUSH": "", "USER_EMAIL": "", "USER_PROMO": "", "USER_CONTACT": "", "USER_GEO": "", "USER_UTM": "", "USER_ID": 0, "USER_DT": "", "USER_TZ": "+00:00", "USER_LC": "en", "USER_ISADMIN": 0, "USER_ISBLOG": 0, "USER_ISPREMIUM": 0, "USER_BALL": 0, "USER_RAND": 0, "USER_QUIZ": 0, "USER_DICE": 0, "MSGID_PAID": 0, "DATE_TIME": 0}'
    ...
    try:
        while retry > 0:
            try:
                ...
                logger.info(log_ % f"{str(e) + str(param)}")
    ...
    try:
        while retry > 0:
            try:
                ...
                logger.info(log_ % f"Error: {str(e) + str(param)} ({retry=})")
                if 'does not exist' in str(e) or 'invalid input syntax' in str(e):
                    return result
                await asyncio.sleep(round(random.uniform(1, 2), 2))
                retry -= 1
...
async def reset_database_and_role(PG_PASS_MAIN, BASE_D, is_all=False):
    ...
    try:
        ...
        roles = await conn.fetch("SELECT rolname FROM pg_roles WHERE rolname NOT IN ('postgres') AND rolname NOT LIKE 'pg_%';")
...
async def get_pay_btn(bot, chat_id, lz, payload='donate', stars=1):
    try:
        reply_markup = InlineKeyboardBuilder().add(types.InlineKeyboardButton(text=f"{l_donate[lz]} {stars}★", pay=True))
...
async def train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, EXTRA_D, BASE_D, lz, prompt='', BOT_TOKEN_=None):
    ...
    try:
        ...
        sql = f"SELECT POST_TEXT, POST_BUTTONS FROM {schema_name}_{ENT_TID}.POST"
        data = await db_select_pg(sql, (), BASE_D)
...
async def pst_gen_ent(bot, chat_id, KEYS_JSON, state, lc, lz, POST_TYPE, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D,
                      EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None):
    result = dst = None
    try:
        if prompt in ['➡️️ Next', '/Next']: prompt = ''
        openai_keys = os.path.join(EXTRA_D, 'keys.json')
        prompt_txt, prompt_img = await train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, BASE_D, EXTRA_D, BASE_D, lz, prompt, ENT_TOKEN_)
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TID, POST_TYPE, POST_MEDIA, ENT_TID, PROJECT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None, request_app=None):
    result = dst = None
    try:
        ...
        KEYS_JSON = os.path.join(EXTRA_D, 'keys.json')
        prompt_txt, prompt_img = await train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, PROJECT_TYPE, EXTRA_D, BASE_D, lz, prompt, ENT_TOKEN_)
        ...
        if POST_TYPE == 'text':
            ...
            if POST_TYPE in ['gif', 'animation', 'web']:
                ...
                if random.shuffle([True, False]):
                    ...
                    if BOT_TOKEN:
                        ...
                        POST_FID, _ = await item_to_dynamic_sticker(extra_bot, chat_id, dst, 'sticker', 'video', False, True, False, True)
                        await extra_bot.session.close()
                    else:
                        res = await bot.send_sticker(chat_id=chat_id, sticker=types.FSInputFile(dst_webp))
                ...
                dst_new = os.path.join(MEDIA_D, str(ENT_TID), datetime.now(timezone.utc).strftime(f'%d-%m-%Y_%H-%M-%S-%f.png'))
                img = Image.open(dst)
                img = await correct_orientation(img)
                new_size = (640, 640)
                resized_img = img.resize(new_size)
            ...
            POST_MEDIA.append({"file_id": POST_FID, "file_id_note": POST_FIDNOTE, "file_type": POST_TYPE, "file_name": POST_FILENAME, "file_link": POST_LNK, "file_link2": POST_LNK2})
            ...
            # MSG_TEXT = await format_text(MSG_TEXT)
            print(f"{MSG_TEXT=}")
            lst = await outsource_generate({'type': 'tts', 'prompt': MSG_TEXT, 'lc': lc, 'dir_name': os.path.join(MEDIA_D, str(ENT_TID))}, KEYS_JSON)
            if not len(lst): return
            ...
            if BOT_TOKEN:
                extra_bot = Bot(token=BOT_TOKEN)
                res = await extra_bot.send_document(chat_id=chat_id, document=types.FSInputFile(dst),
                                          thumbnail=types.FSInputFile(os.path.join(EXTRA_D, 'parse.jpg')),
                                          disable_notification=True, disable_content_type_detection=True)
                await extra_bot.delete_message(chat_id, res.message_id)
                await extra_bot.session.close()
                                    ...
                                    if provider_name == 'elevenlabs':
                                        ...
                                        async with (aioboto3.Session().client("polly",
                                            ...
                                            names = {
                                                ...
                                                'da': ['Naja', 'Mads'],
                                                'de': ['Vicki', 'Daniel'],
                                                'en': ['Olivia', 'Amy', 'Danielle', 'Joanna', 'Stephen', 'Ruth', 'Matthew'],
...
async def get_link_for_media(bot, chat_id, file_path, KEYS_JSON, is_del=True):
    ...
    try:
        ...
        base_name = str(os.path.basename(file_path).replace(' ', ''))
            ...
            try:
                ...
                try:
                    ...
                    if MSG_TYPE == 'photo':
                        ...
                        file_id, file_name_video = await send_audio_my(extra_bot, BOT_TID, BOT_USERNAME, BOT_FIRSTNAME, OWNER_TID, MEDIA, FILE_NAME,
                                                                       BASE_D, EXTRA_D)
                        if file_name_video:
                        ...
                        file_id_note, file_name_video = await send_audio_my(extra_bot, BOT_TID, BOT_USERNAME, BOT_FIRSTNAME, OWNER_TID, MEDIA,
                        ...
                        if IS_LINK: F_NAME = await download_file_my(MEDIA, BOT_TID, FILE_NAME.split(".")[-1], MEDIA_D)
...
async def return_file_link(bot, chat_id, FILE_NAME, KEYS_JSON, MSG_VID, MSG_TYPE, IS_LINK):
    ...
    try:
        ...
        if IS_LINK:
            ...
            if MSG_TYPE in ['document'] and not FILE_NAME.endswith(('.png', '.jpg', '.jpeg', '.gif', '.mp4', '.webp', '.webm', '.mp3')): return
...
async def jpg_video_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MSG_TYPE, file_id, BASE_D, MEDIA_D, BOT_TOKEN):
    ...
    try:
        if MSG_TYPE == 'photo':
            ...
            result = document_jpg
        tmpls = [photo_jpg, gif_jpg, video_jpg, video_note_jpg, audio_jpg, voice_jpg, sticker_jpg, web_jpg, document_jpg]
...
async def jpg_photo_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MSG_TYPE, MEDIA_D):
    ...
    try:
        ...
        tmpls = [photo_jpg, gif_jpg, video_jpg, video_note_jpg, audio_jpg, voice_jpg, sticker_jpg, web_jpg, document_jpg]
        if MSG_TYPE in ['audio', 'voice', 'document'] or file_link in tmpls or MSG_TYPE in ['web'] and not file_link.endswith(('.jpg', '.jpeg', '.png', '.webp', '.webm', '.mp4', '.gif')): return
...
async def send_video_my(extra_bot, OWNER_TID, MEDIA):
    result = None
    try:
        res = await extra_bot.send_video(chat_id=OWNER_TID, video=MEDIA, supports_streaming=True, disable_notification=True)
...
async def facade_get_fid(bot, chat_id, KEYS_JSON, BOT_TID, dst, MSG_VID, msg_type, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    ...
    try:
        ...
        # print(f"------------0----------")
        file_id, file_id_note, file_type, FILE_NAME, IS_LINK, BOT_TOKEN = await return_file_id(bot, BOT_TID, dst, msg_type, IS_LINK,
                                                                                    BASE_D, EXTRA_D, MEDIA_D)
        print(f"{dst=}, {FILE_NAME=}")
        print(f"{dst=}, {os.path.exists(FILE_NAME)=}")
        if FILE_NAME.endswith(('.json', '.tgs')):
            print(f"01")
            ...
            if file_link.endswith(('.mp4', '.gif', '.webm', '.mov', '.mp3')):
                file_link2 = await jpg_video_preview(bot, MSG_VID, KEYS_JSON, file_link, BOT_TID, msg_type, file_id, BASE_D, MEDIA_D, BOT_TOKEN)
            else:
...
async def podcast_start_app(app, UB_TID, lz, ENT_TID, MEDIA_D, BASE_D, BOT_TOKEN_APP, PROJECT_USERNAME):
    ...
    try:
        ...
        call = await app.invoke(CreateGroupCall(peer=peer, random_id=dt_, rtmp_stream=False, title=l_podcast_start[lz], schedule_date=dt_))
        await asyncio.sleep(10)
        ...
        while max_cnt > 0:
            ...
            for POCAST_TID in POCAST_TIDs:
                ...
                sql = f"SELECT POST_TID, POST_TYPE, POST_TEXT, POST_MEDIA FROM {schema_name}_{ENT_TID}.POST WHERE POST_TID=$1"
                ...
                for item in POST_MEDIA:
                    ...
                    try:
                        ...
                        if POST_TYPE in ['audio', 'voice'] or is_audio:
                            ...
                            if not len(extract_links_): continue
                            else: stream = MediaStream(extract_links_[0])
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTONS, ENT_TYPE,
    ...
    try:
        ...
        if PROJECT_TYPE == 'bot':
            ...
            sql = "SELECT UB_USERNAME, UB_BOTUSERNAME, UB_CHANNELLINK FROM UB WHERE UB_TID=$1"
            data_bot = await db_select_pg(sql, (ENT_TID,), BASE_D)
        ...
        if POST_LNK:
            ...
            print(f"{TMP_LNK=}")
            if TMP_TYPE in ['photo', 'gif', 'animation']:
            ...
            if ENT_TYPE == 'PST':
                ...
                for i in range(1, 4):
                    b_html = await get_row_html(msg_text, msg_btns, i * 3 - 3, i * 3, POST_ID, ENT_TID, ENT_LC,
                                                POST_LNK, BASE_D, BASE_D)
                    btns_html = f"{btns_html}{b_html}"
                btn_html = f'<div class="buttons-wrapper">{btns_html}</div>'

        sql = f"SELECT VIEW_ID FROM {''}_{ENT_TID}.VIEW WHERE ENT_VID=$1 AND ENT_TYPE=$2"
        data_views = await db_select_pg(sql, (POST_ID, ENT_TYPE,), BASE_D)
...
async def get_user_vars(chat_id, ENT_TID, BASE_D, PROJECT_USERNAME):
    USER_GAMES = {}
    USER_VARS = {}
    USER_ISPAID = 0
    try:
        ...
        sql = f"SELECT USER_GAMES, USER_VARS, USER_ISPAID FROM {schema_name}_{ENT_TID}.USER WHERE USER_TID=$1"
        data_usr = await db_select_pg(sql, (chat_id,), BASE_D)
        if not len(data_usr) or not data_usr[0][0]:
            if not len(data_usr):
                dt_now = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S')
                sql = f"INSERT INTO {schema_name}_{ENT_TID}.USER (USER_TID, USER_DT) VALUES ($1, $2) ON CONFLICT DO NOTHING"
...
async def save_posts(chat_id, lz, username, bot, data, tz, MEDIA_D, BASE_D, PROJECT_USERNAME, PROJECT_TYPE):
    result = {'result': True, 'bot': True, 'blog': None, 'hash': None}
    try:
        posts = data['posts']
        ENT_TID = str(data['ENT_TID'])
        ...
        POST_MEDIA = json.dumps(posts['POST_MEDIA'], ensure_ascii=False) if not isinstance(posts['POST_MEDIA'], str) else posts['POST_MEDIA']
        POST_BUTTONS = json.dumps(posts['POST_BUTTONS'], ensure_ascii=False) if not isinstance(posts['POST_BUTTONS'], str) else posts['POST_BUTTONS']
        ...
        POST_APPTOKEN = posts['POST_APPTOKEN'] if 'POST_APPTOKEN' in posts else ''
        POST_TZ = tz
        POST_DT = posts.get('POST_DT', None) or None
        POST_TARGET = posts.get('POST_TARGET', None)
        ...
        if PROJECT_USERNAME == 'FereyPostBot':
            ...
            web_val = f"pst-{ENT_TID}-{POST_TID}"
            HASH_VAL = hashlib.blake2b(web_val.encode('utf-8'), digest_size=8).hexdigest()
            POST_WEB = f"https://t.me/{PROJECT_USERNAME}/web?startapp=pst-{HASH_VAL}-{ENT_TID}&mode=fullscreen"

        result = {'result': True, 'bot': True, 'blog': POST_BLOG, 'hash': HASH_VAL}
        print(f"{result=}")
        if POST_ISPAY and POST_TOKEN and len(json.loads(POST_MEDIA)):
            POST_MEDIA, res_bot = await update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA, MEDIA_D, PROJECT_USERNAME)
        ...
        if POST_APPTOKEN and len(json.loads(POST_MEDIA)):
            print(f"{POST_APPTOKEN=}")
            POST_MEDIA, res_bot = await update_media(bot, my_tid, lz, ENT_TID, POST_APPTOKEN, POST_MEDIA, MEDIA_D, PROJECT_USERNAME, False)
            result = {'result': True, 'bot': res_bot, 'blog': POST_BLOG, 'hash': HASH_VAL}
            POST_MEDIA = json.dumps(POST_MEDIA) if not isinstance(POST_MEDIA, str) else POST_MEDIA

        if len(json.loads(POST_BUTTONS)):
            POST_BUTTONS = json.loads(POST_BUTTONS)

            for button in POST_BUTTONS:
                if button['knd'] == 'web':
                    web_val = f"pst-{ENT_TID}-{button['lnk']}"
        ...
        # region pay
        if POST_ISPAY and POST_TYPE in ['photo', 'video']:  # POST_STARS
            web_val = f"pay-{ENT_TID}-{POST_TID}-{''}-{ENT_TID}"
            HASH_VAL = hashlib.blake2b(web_val.encode('utf-8'), digest_size=8).hexdigest()
            POST_PAY = f"https://t.me/{PROJECT_USERNAME}/web?startapp=pay-{HASH_VAL}-{ENT_TID}"
                ...
                for button in POST_BUTTONS:
                    try:
                        ...
                        try:
                            ...
                            amount = 1
                        prices = [LabeledPrice(label="XTR", amount=amount)]
                        pay_lnk = await extra_bot.create_invoice_link(title='Post',
                                                                      description='ent_title',
                                                                      payload=f"{ENT_TID}_{POST_TID}_{button['bid']}",
                                                                      currency='XTR',
                                                                      # photo_url=post_photo,
                                                                      prices=prices)
                        button['web'] = pay_lnk
                    except TelegramRetryAfter as e:
                        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
        ...
        if POST_ISPAY and POST_STARS and POST_TOKEN:
            try:
                ...
                try:
                    ...
                    amount = 1
                prices = [LabeledPrice(label="XTR", amount=amount)]
                POST_PAYLINK = await extra_bot.create_invoice_link(title='Post',
                                                                   description='ent_title',
                                                                   payload=f"{ENT_TID}_{POST_TID}_0",
                                                                   currency='XTR',
                                                                   # photo_url=post_photo,
                                                                   prices=prices)
                await extra_bot.session.close()
        ...
        if POST_TOKEN:
            ...
            if PROJECT_USERNAME == 'FereyChannelBot':
                ...
                await db_change_pg(sql, (POST_TOKEN, ENT_TID,), BASE_D)
            elif PROJECT_USERNAME == 'FereyGroupBot':
        ...
        sql = f"""
            INSERT INTO {schema_prefix}_{ENT_TID}.POST (
                POST_TID, POST_TARGET, POST_CHATTID, POST_USERTID, POST_USERTUN, POST_TYPE, POST_TEXT, POST_MEDIA, 
                POST_BUTTONS, POST_CHKBOX, POST_WEB, POST_PAY, POST_BLOG, POST_WATER, POST_STARS, POST_ISPRIVATE, 
                POST_PAYLINK, POST_TZ, POST_DT, POST_TR
            )
            ...
            SET 
                ...
                POST_WATER = EXCLUDED.POST_WATER,
                POST_STARS = EXCLUDED.POST_STARS,
                POST_ISPRIVATE = EXCLUDED.POST_ISPRIVATE,
                POST_PAYLINK = EXCLUDED.POST_PAYLINK,
                POST_TZ = EXCLUDED.POST_TZ,
                POST_DT = EXCLUDED.POST_DT,
                POST_TR = EXCLUDED.POST_TR
        """
        params = (
            POST_TID, POST_TARGET, int(ENT_TID), chat_id, username, POST_TYPE, POST_TEXT, POST_MEDIA, POST_BUTTONS,
            POST_CHKBOX, POST_WEB, POST_PAY or None, POST_BLOG,
            int(POST_WATER) if str(POST_WATER).isdigit() else None,
            int(POST_STARS) if str(POST_STARS).isdigit() else None,
            POST_ISPRIVATE or None, POST_PAYLINK or None, POST_TZ or None, POST_DT or None, POST_TR or None
        )
        print("SQL Query:", sql)
        print("Params:", params)
        await db_change_pg(sql, (
...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, current=1):
    result = False
    try:
        POST_ID, POST_TID, POST_USERTID, POST_USERTUN, POST_TYPE, POST_TEXT, POST_MEDIA, POST_BUTTONS, POST_CHKBOX, POST_WATER, POST_STARS, POST_TZ, POST_DT = item
        ...
        if not is_private:
            ...
            try:
                ...
                HASH_VAL = hashlib.blake2b(HASH_STR.encode('utf-8'), digest_size=8).hexdigest()
                address = f"https://t.me/{PROJECT_USERNAME}/web?startapp=pst-{HASH_VAL}"
        ...
        POST_TEXT = re.sub(
            r'<div class="tgui-79024fcb6d81ad79" style="display: inline-block; white-space: nowrap; cursor: text;">'
            r'<span class="tgui-c3e2e598bd70eee6 tgui-080a44e6ac3f4d27 tgui-5b8bdfbd2af10f59 tgui-266b6ffdbad2b90e '
            r'tgui-30064fce0d501f17 tgui-16ed20e7a6e2daa0" style="line-height: normal; font-size: inherit; margin: 0px;">'
            r'(.*?)<\/span><\/div>',
            r'<blockquote>\1</blockquote>',
            POST_TEXT
        )

        POST_TEXT = re.sub(r'<span style="background-color: gray;">(.*?)<\/span>', r'<tg-spoiler>\1</tg-spoiler>', POST_TEXT)
        ...
        if POST_CHKBOX['POST_ISTAG'] and POST_TEXT and len(POST_TEXT) < 900:
            sql = f"SELECT USER_ID FROM {schema_name}_{ENT_TID}.USER"
        ...
        if POST_CHKBOX['POST_ISPAY'] and POST_TYPE not in ['photo', 'video']:
            ...
            result = await bot.send_message(chat_id=chat_id, text=str_empty, link_preview_options=lp_options,
                                            reply_markup=reply_markup.as_markup())
            sql = f"UPDATE BOT_{ENT_TID}.POST SET POST_MSGID=$1 WHERE POST_TID=$2"
            await db_change_pg(sql, (result.message_id, POST_TID,), BASE_D)
            return

        print(f"========================")
        if len(POST_MEDIA) > 1 and POST_CHKBOX['POST_ISGALLERY'] and not POST_CHKBOX['POST_ISCOLLAPSE'] and not POST_CHKBOX['POST_ISPAY']:
            POST_TEXT = '' if POST_TEXT == str_empty and POST_TYPE != 'text' else POST_TEXT
            ...
            buttons = [
                ...
                types.InlineKeyboardButton(text=f"{current}/{len_}", callback_data=f'gal_current_{ENT_TID}_{POST_TID}_{current}_{len_}'),
        ...
        if message_id:
            ...
            sql = "SELECT CHANNEL_CPAYTOKEN FROM CHANNEL WHERE CHANNEL_TID=$1"
            data_chn = await db_select_pg(sql, (ENT_TID,), BASE_D)
        ...
        if result and (int(chat_id) == int(ENT_TID) or PROJECT_USERNAME == 'FereyBotBot'):
            print('indeid')
        ...
        # region story
        try:
            if result and POST_TYPE == 'text' and POST_CHKBOX.get('POST_ISSTORY', False) and int(chat_id) == int(ENT_TID):
        ...
        # region podcast
        try:
            if result and result.chat.username and POST_CHKBOX.get('POST_ISPODCAST', False) and int(chat_id) == int(ENT_TID) and len(POST_MEDIA):
                member_ = await bot.get_chat_member(chat_id=int(ENT_TID), user_id=bot.id)

                if member_.can_manage_video_chats and member_.can_promote_members:
                    print(f"member_ can and promote")
                    sql = "SELECT USER_ISPAID FROM USER WHERE USER_TID=$1"
                    data = await db_select_pg(sql, (chat_id,), BASE_D)
        ...
        try:
            if result and POST_CHKBOX['POST_ISPIN'] and not message_id:
                await asyncio.sleep(round(random.uniform(0, 1), 2))
                msg_id = result[0].message_id if isinstance(result, list) else result.message_id
                await bot.pin_chat_message(chat_id=chat_id, message_id=msg_id, disable_notification=not POST_CHKBOX['POST_ISSOUND'])
        except Exception as e:
            logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))
        # endregion
...
async def region_blog2(bot, ENT_TID, POST_TYPE, POST_TEXT, POST_MEDIA, BASE_D, PROJECT_TYPE='bot', is_format=False):
    ...
    try:
        ...
        POST_TEXT = POST_TEXT if POST_TEXT else str_empty
        POST_TEXT = POST_TEXT.replace('<tg-spoiler>', '').replace('</tg-spoiler>', '').replace('<pre>py\n', '').replace('<pre>', '').replace('</pre>', '').replace('<br>', '')
        POST_TEXT = POST_TEXT.replace('<span style="caret-color: var(--tg-theme-hint-color);">', '')
        POST_TEXT = POST_TEXT.replace('&nbsp;', '')
        POST_TEXT = re.sub(r'<span style="background-color: rgba\(0, 123, 247, 0\.7\);">(.*?)<\/span>', r'\1', POST_TEXT)
        POST_TEXT = re.sub(r'<span style="background-color: gray;">(.*?)<\/span>', r'\1', POST_TEXT)
        POST_TEXT = re.sub(r'<span style="background-color: rgba\(0, 123, 247, 0\.8\);">(.*?)<\/span>', r'\1', POST_TEXT)
        # POST_TEXT = re.sub(r'</?div>', '\n', POST_TEXT)
        POST_TEXT = re.sub(r'</?div[^>]*>', '\n', POST_TEXT)
            ...
            try:
                if PROJECT_TYPE == 'bot':
                    ...
                    if UB_USERNAME:
                        ...
                        ENT_LINK = f"https://t.me/{UB_USERNAME}"
                    else:
...
async def update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA, MEDIA_D, PROJECT_USERNAME, is_check=True):
...
async def get_ent_rm(chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS, BASE_D, PROJECT_USERNAME):
    try:
        ...
        sql = f"SELECT BTN_BID FROM {schema_name}_{ENT_TID}.PUSH WHERE ENT_VID=$1"
        ...
        for button in sorted_buttons:
            try:
                ...
                if button['knd'] == 'button_in':
                    ...
                    if button['lnk'].strip() == '':
                        sql = f"SELECT POST_TID FROM {schema_name}_{ENT_TID}.POST"
                    ...
                    HASH_STR = f"pst-{ENT_TID}-{NEXT_POST_TID}"
                    HASH_VAL = hashlib.blake2b(HASH_STR.encode('utf-8'), digest_size=8).hexdigest()
                    # if PROJECT_USERNAME == 'FereyPostBot':
                    #     sql = "INSERT INTO \"HASH\" (HASH_STR, HASH_VAL) VALUES ($1, $2) ON CONFLICT DO NOTHING"
                    #     await db_change_pg(sql, (HASH_STR, HASH_VAL,), BASE_D)

                    url = f'https://t.me/{PROJECT_USERNAME}/web?startapp=pst-{HASH_VAL}&mode=fullscreen'
                    btn = types.InlineKeyboardButton(text=button['lbl'], url=url)
                    rows[row_index].append(btn)
                elif button['knd'] == 'nft':
...
async def py_tg_calls_fun(bot, app, chat_id, lz, r, POST_ID, ENT_TID, stream, duration, BASE_D):
    result = peer_chan = None
    try:
        ...
        async def participant_handler(_: PyTgCalls, update: Update):
            try:
                ...
                if not str(update.participant.user_id).startswith('-100'):
                    await add_to_push_podcast(bot, app, update.participant.user_id, POST_ID, BASE_D)
        ...
        for participant in participants:
            ...
            await add_to_push_podcast(bot, app, participant.user_id, POST_ID, BASE_D)
...
async def add_to_push_podcast(bot, app, user_id, ENT_TID, POST_ID, BASE_D):
    result = None
    try:
        try:
            ...
            sql = f"INSERT INTO BOT_{ENT_TID}.PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, POST_ID, BTN_BID) VALUES ($1, $2, $3, $4, $5) ON CONFLICT DO NOTHING"
        ...
        try:
            ...
            sql = f"INSERT INTO BOT_{ENT_TID}.PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, CHAT_ISPREMIUM, CHAT_LC, POST_ID, BTN_BID) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING"
    ...
    try:
        ...
        for btn in msg_btns:
            try:
                ...
                sql = "SELECT PUSH_ID FROM PUSH WHERE POST_ID=$1 AND BTN_BID=$2"
                data_push = await db_select_pg(sql, (int(MSG_VID), int(btn["i"]),), BASE_BOT)
...
def touch(path):
    if not os.path.exists(path):
        with open(path, 'a'):
            os.utime(path,None)  # async with aiofiles.open(path, 'a'):  #     await asyncio.to_thread(os.utime, path, None)
