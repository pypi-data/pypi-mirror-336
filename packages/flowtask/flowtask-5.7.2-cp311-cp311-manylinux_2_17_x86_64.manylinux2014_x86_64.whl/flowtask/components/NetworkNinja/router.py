...
class NetworkNinja(HTTPService, FlowComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.batch_id: str = kwargs.get('batch_id', None)
    ...
    async def run(self):
        ...
        if self._action == 'get_batch':
            ...
            results = await self._saving_results()
            if results:
                self._result = results
        return self._result
    ...
    async def _saving_results(self):
        ...
        results_by_type = dict(zip(data_types, results))
        ...
        async with self.semaphore:
            ...
            for item in data:
                if data_type == 'client':
                    ...
                    item.client_names = json_encoder(
                        item.client_names
                    )
                    tasks.append(
            ...
            self._logger.debug(
                f"Saving Data Type: {data_type}: {len(data)} items"
            )
