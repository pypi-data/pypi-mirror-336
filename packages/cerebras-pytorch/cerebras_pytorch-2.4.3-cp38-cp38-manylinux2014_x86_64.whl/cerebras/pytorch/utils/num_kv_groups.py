...
class GroupsAnnotationMode(AnnotationMode):
    """num_kv_groups Annotation Mode."""

    # Global config instance helps to handle nested annotation modes.
    _num_kv_groups: Optional[GroupsConfig] = None

    @property
    def config(self):
        return GroupsAnnotationMode._num_kv_groups

    @config.setter
    def config(self, config: GroupsConfig):
        GroupsAnnotationMode._num_kv_groups = config

    def get_attribute(self, config: GroupsConfig, is_backward: bool):
...
def groups_annotater(num_kv_groups: Optional[int] = None):
    ...
    return annotate(
        annotation_mode=GroupsAnnotationMode(
            config=GroupsConfig(num_kv_groups=num_kv_groups)
        )
    )
