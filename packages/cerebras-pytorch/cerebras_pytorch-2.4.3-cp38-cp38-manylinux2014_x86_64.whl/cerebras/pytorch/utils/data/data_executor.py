...
class DataExecutor(ClassLogger):
    ...
    def __init__(
        ...
        if listeners is not UNSPECIFIED:
            raise ValueError(
                "Passing listeners to the DataExecutor is no longer allowed. "
                "Please register hooks to listen on tensors using:\n\n"
                "\tfrom cerebras.pytorch.experimental.listener import register_traced_tensor_hook\n"
                "\tregister_traced_tensor_hook(lambda tensor, name: ...)\n\n"
            )

        self.op_profiler_config = None
        self._artifact_dir: Optional[Path] = None

        if cs_config is not UNSPECIFIED:
            raise ValueError(
                "Passing cs_config to the DataExecutor is no longer allowed. "
