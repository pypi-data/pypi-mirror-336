...
class DistributedSampler(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        Args:
            ...
            num_samples: The number of samples to shuffle over. In multi-epoch training,
                it is common to set this to the total number of samples acorss all epochs
                get smoother loss curves and improved convergence. If None, One epoch is
                sampled from by querying the length of the dataset.
...
class ShuffleSampler(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        Args:
            data_source: Dataset to sample from.
            num_samples: The number of samples to shuffle over. In multi-epoch training,
                it is common to set this to the total number of samples across all epochs
                get smoother loss curves and improved convergence. If None, One epoch is
                sampled from by querying the length of the dataset.
            shuffle: Whether or not to shuffle the dataset.
            seed: The RNG seed used when shuffling is on.
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            if self.num_samples > len(self._data_source):
                epochs = math.ceil(self.num_samples / len(self._data_source))
                perm = torch.cat(
                    [
                        torch.arange(len(self._data_source))
                ...
                perm = torch.cat(
                    (
                        perm,
                        torch.randperm(len(self._data_source), generator=gen),
                    )
                )
                ...
                perm = torch.randperm(len(self._data_source), generator=gen)
                perm = perm[: self.num_samples]
