...
from cerebras.pytorch.saver.storage import lazy_tensor_data_wrapper
...
class PyTorchLtcBackendImpl(BaseBackend, ClassLogger):
    ...
    def __init__(
        ...
        drop_data=UNSPECIFIED,
        log_initialization=UNSPECIFIED,
        retrace_every_iteration=UNSPECIFIED,
    ):
        ...
        deprecated_warning = (
            "Passing `{name}` to the backend is deprecated. "
            "Please set `cstorch.backends.csx.debug.{name} = "
            "{value}` instead"
        )

        if drop_data is not UNSPECIFIED:
            raise ValueError(
                deprecated_warning.format(name="drop_data", value=drop_data)
            )

        if log_initialization is not UNSPECIFIED:
            raise ValueError(
                deprecated_warning.format(
                    name="log_initialization", value=log_initialization
                )
            )

        if retrace_every_iteration is not UNSPECIFIED:
            raise ValueError(
                deprecated_warning.format(
                    name="retrace_every_iteration",
                    value=retrace_every_iteration,
                )
            )
    ...
    def mark_output(self, struct, force=False):
        ...
        def map_fn(arg):
            if isinstance(arg, torch.Tensor) and (
                ...
                if id(arg) in self._autoregress_outputs:
                    # These outputs are special; the RT writes them directly by
                    # id; even if the torch tensor is the same, the output
                    # buffer would be different.
                    # Thus, we need to prevent adding a duplicate output if
                    # this tensor is already populated by the implicit
                    # autoregressive loop logic.
                    return arg
    ...
    def initial_mark_step(self, async_compute: bool = True):
        ...
        try:
            ...
            if (
                self.device.config.drop_data
                or self.device.config.skip_weight_init
            ):
    ...
    def on_run_start(self):
        ...
        def compile(
            ...
            with self.appliance.build_worker_image(
                ...
                mount_all=cstorch.backends.csx.cluster.automount_all,
            ):
                try:
                    ...
                    if compile_hash not in self._compile_cache:
                        ...
                        if self.appliance.compile_resp is not None:
                            ...
                            # current model and appliance artefacts.
                            self._compile_cache[compile_hash] = (
                                self.appliance.compile_resp
                            )
                            # Save compile hash which includes current model with appliance
                            # artefacts and generated CrossCompileState produced by compile.
                            self._compile_cache[
        ...
        def execute(batch_size: int) -> Set[str]:
            ...
            def restart():
                # Since we restart the appliance, we no longer have appalince weights, so
    ...
    def load_deferred_tensors(self, checkpoint_file, state_dict):
        ...
        for name, tensor in state_dict.items():
            ...
            if appliance_info := appliance_data.get_appliance_info():
                ...
                if ckpt_appliance_data.shape != appliance_info.shape:
                    raise RuntimeError(
                        f"The shape of the tensor from the appliance {ckpt_appliance_data.shape} "
                ...
                if ckpt_appliance_data.shape != appliance_data.shape:
                    raise RuntimeError(
                        f"The shape of the tensor from the appliance {ckpt_appliance_data.shape} "
    ...
    def update_implicit_loop(
        ...
        self.set_attribute(output_tensor, "autoregressive", autoregressive)
        # Required to prevent adding multiple outputs for this.
        self._autoregress_outputs.add(id(output_tensor))
