...
class Registry:
    ...
    def register_decomposition(
        ...
        Args:
            op: PyTorch object for a op or it's specific overload.
            device: Device on which decomposition is registered.
        """
        ...
        def decomp_decorator(fn: Callable) -> Callable:
            ...
            for overload in filtered_overloads:
                ...
                if decomp_key in self.decompositions:
                    raise RuntimeError(
                        f"A decomposition for {op} has already been registered on {device}, "
                        f"here is the location of the first registration:\n"
                        f"{self._decompositions[decomp_key].stacktrace}"
                    )
    ...
    def remove_decomposition(
        self, op: torch._ops.OpOverload, device: Literal["CSX", "CPU"] = "CSX"
    ):
        """
        ...
        decomp_key = _DecompKey(op, device)
        if decomp_key not in self.decompositions:
            raise RuntimeError(
                f"Decomposition for {op} on {device} has not been registered."
            )

        if self.applied_decompositions:
            raise RuntimeError(
                f"Decomposition for {op} on {device} has already been applied, "
                f"remove_decomposition can only be called before the first trace."
            )
