...
def photon(wavelength, spec,time,diam,point_nb,transmission=False):
        ...
        for i in range(len(spec)):
            if spec[i] > 400 and spec[i] < 800:
                wv.append(wavelength[i])
                flux.append(spec[i])
...
def photon_pos_on_PSF(photon, psf, wv):
    ...
    # all_photons_list = []
    # for wl, t in photon:
    #     wl_proche = closest_wavelength(wl, wv)

    #     if wl_proche not in sorted_photons:
    #         sorted_photons[wl_proche] = []
    #     sorted_photons[wl_proche].append((wl, t))
    #     index = np.random.choice(len(psf.psfenergy[wl_proche]), p=psf.psfenergy[wl_proche] / np.sum(psf.psfenergy[wl_proche]))
    #     x, y = psf.psfpos[wl_proche][index]

    #     if wl_proche not in sorted_photons:
    #         sorted_photons[wl_proche] = []
        
    #     all_photons_list.append((x, y, wl, t))

    # return(all_photons_list)

    
    # Dictionnaire avec les longueur d'onde de PSF en keys et photons correspondant dedans
    # Prendre la len, choisir coordonnées avec choices, et tout mettre en list



# random.choices mais ne fonctionne que en 1D, prendre coordonnée avec weight, faire une liste et choisr index
