...
class BaseMaestro(BasePlugin[PyodideMacrosConfig]):
    ...
    def level_up_from_current_page(self, url:str=None) -> str:
        ...
        if url not in self.__case_url_cache:
            ...
            self.__case_url_cache[url] = '/'.join(out) or '.'
        return self.__case_url_cache[url]
    ...
    def setup_ctx(self, page:Optional[Page], base_url:Optional[str]=None):
        ...
        if page:
            ...
            #   - page may also be None is other cases (typically, for dev_ops operations!),
            #     then self._page must NOT be updated, but base_url doesn't matter
    ...
    def warn_unmaintained(self, that:str=None, *, msg:str=None, partial_msg:str=None):
        ...
        if self.deprecation_level == 'error' and not self._dev_mode:
