    ...
    async def modify_messages(
        ...
        for capability in self.capabilities:
            ...
            async for output in capability.modify_messages(copy(messages)):
                ...
                # This is a full message that we want to take note of.
        ...
        try:
            ...
            if isinstance(result, AsyncGenerator):
                ...
                async for intermediate in result:
                    if isinstance(intermediate, Message):
                        yield intermediate
                        continue
                    if last_received:
                        yield ToolUseChunk(
    ...
    async def run(
        ...
        **kwargs,
        ...
        while True:
            ...
            if isinstance(response.content, str) or isinstance(
            ...
            if (
                ...
                and isinstance(response.content[0], TextBlock)
            ):
                return
            ...
            for tool_call in response.content:
                if isinstance(tool_call, ToolCall):
                    ...
                    async for intermediate in self.invoke_tool(tool_call, messages):
                        if isinstance(intermediate, MessageContent):
                            yield intermediate
                        if isinstance(intermediate, Message):
                            yield intermediate
                        result = intermediate
