...
class SAMAnnotator:
    ...
    def __init__(self, 
        ...
        # Initialize file and annotation managers
        self.file_manager = FileManager(category_path, self.image_processor)
        self.annotation_manager = AnnotationManager(
            self.validation_manager,
            self.window_manager,
        ...
        self.session_manager = SessionManager(
            ...
            self.annotation_manager,
            self.window_manager,
            self.predictor  # Pass predictor instance
        )
        
        # Initialize model and load classes
        self._initialize_predictor(checkpoint_path, sam_version, model_type)
        self._load_classes(classes_csv)
    ...
    def _initialize_predictor(self, checkpoint_path: str, sam_version: str, model_type: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"Initialized {sam_version.upper()} predictor with {model_type} model")
            
        except Exception as e:
            self.logger.error(f"Error initializing predictor: {str(e)}")
            raise
