...
def delete_lines_if_needed(pdf_path: Path, output_dir: str, header_bottom: Optional[float], 
    ...
    try:
        ...
        for page_num in tqdm(range(len(doc))):
            ...
            for block in blocks:
                bbox = block["bbox"]
                # Draw a rectangle around each text block with a light gray color
                page.draw_rect(pymupdf.Rect(bbox), color=(0.7, 0.7, 0.7), width=0.5)
            ...
            print("value: ", valid_header)
            print("footer boundary: ", footer_boundary)
            print("bottom footer valid: ", bottom_footer_valid)
            print("right footer valid: ", right_footer_valid)
            print("value: ", valid_footer)
            print("---------")
