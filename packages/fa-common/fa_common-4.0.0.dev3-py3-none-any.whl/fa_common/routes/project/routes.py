...
async def list_projects(
    ...
    limit: int = 10,
    sort: list[str] = Query(
        [], description="The list of fields to sort the projects by using the syntax `['+fieldName', '-secondField']`."
    ),
    ...
    """
    projects = await service.get_projects_for_user(user=current_user, owner_only=only_mine, offset=offset, limit=limit, sort=sort)
