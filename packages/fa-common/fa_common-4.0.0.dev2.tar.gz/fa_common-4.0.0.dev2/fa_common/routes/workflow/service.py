...
from fa_common.workflow.local_utils import CustomJSONEncoder
...
async def callback(items: List[WorkflowCallBack], result: PrefectFlowRun):
    for item in items:
        ...
        data = RequestCallback(workflow=workflow_json, metadata=item.metadata, message="Local Job completed!")
