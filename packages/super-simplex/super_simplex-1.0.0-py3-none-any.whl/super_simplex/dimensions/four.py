...
def noise_4d_array(x, y, z, w, perm):
    noise = np.empty((w.size, z.size, y.size, x.size), dtype=np.double)

    for w_i in numba.prange(w.size):
        for z_i in numba.prange(z.size):
            for y_i in numba.prange(y.size):
                for x_i in numba.prange(x.size):
                    noise[w_i, z_i, y_i, x_i] = noise_4d(x[x_i], y[y_i], z[z_i], w[w_i], perm)

    return noise

@numba.njit(cache=True)
def noise_4d(x, y, z, w, perm):
