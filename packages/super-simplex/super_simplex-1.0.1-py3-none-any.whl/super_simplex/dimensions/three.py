...
def noise_3d_array(x, y, z, perm, perm_grad_index3):
    ...
    for z_i in prange(z.size):
        for y_i in prange(y.size):
            for x_i in prange(x.size):
                noise[z_i, y_i, x_i] = noise_3d(x[x_i], y[y_i], z[z_i], perm, perm_grad_index3)
    return noise

@njit(cache=True)
def noise_3d(x, y, z, perm, perm_grad_index3):
