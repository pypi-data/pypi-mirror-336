...
This will install the full version of the SDK, which includes all the features and tools. You can also install the slim version with only the dependencies you need:

```bash
pip install airflow-ai-sdk-slim[openai,...]
```
...
def deep_research_task(**context: Context) -> str:
    """
...
Currently, the Airflow AI SDK does not manage authorization against LLM providers. Instead, we follow Pydantic AI's
[models and authentication methods](https://ai.pydantic.dev/models/) (primarily using environment variables) - the benefit
of this is that authentication methods are consistent with this AI SDK and other LLM clients.

We should consider mapping Airflow connections to Pydantic AI models and use those credentials to make LLM calls. This would let
users create connections of a new type, `llm`, that contains the credentials for an LLM provider. This should include:

- API tokens
- Base URLs
- Other parameters

We could also create one connection per provider (one for each of OpenAI, Anthropic, Cohere, etc). Need to decide what the right level of abstraction is.

## Generate embeddings

One of Airflow's primary use cases is for orchestrating ETL jobs. The process to populate a vector database from structured, unstructured, and text data is a data engineering task and we've seen Airflow commonly used to do so.

We should add a `@task.embed` decorator that will generate embeddings for a given text / file. This will act as an
abstraction layer to the embedding API of your choice.

## Human in the loop

We should add human-in-the-loop functionality that will allow a human to review the output of a task and approve it before moving on to the next task. This is useful for any task that is not deterministic or should be reviewed by a human before moving on.

For now, folks have worked around this by having a "human in the loop" task that just fails and sends an alert. The user can then click on the link in the alert to manually mark the task as successful (after reviewing the current pipeline status / output), which causes the pipeline to continue running.

## Astro Observe integration

It'd be great if we got some extra observability capabilities out of the box if someone runs this on Astro. Things like:

- LLM-specific metrics: number of tokens, number of tool calls, output tokens, etc
- Prompt versions / hashes (maybe tied to dag versions post-3.0?)
- Tool calls as sub-jobs for more granular tracing?
