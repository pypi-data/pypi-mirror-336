...
def _content_access(fnc):
    ...
    def wrapper(self, *args, **kwargs):
        if self._sentences is None and self._json_data is None:
            ...
            self.__dict__.update(Analysis.parse(self._json_data, self.id).__dict__)
...
class Analysis:
    """
    :class:`Analysis` is a class that holds the results of an NLP analysis. Instances of this class can parse the NLP result and yield objects such as :class:`Sentence <wowool.annotation.sentence.Sentence>`, :class:`Token <wowool.annotation.token.Token>` and :class:`Entity <wowool.annotation.entity.Entity>` to conveniently access the data behind those annotations. Furthermore, the class also provides a static method to parse raw JSON data as returned by, for example, the Portal API.
    """
    ...
    @staticmethod
    def parse(json_data: Union[dict, str], id=None):
        """
        ...
        """
        from wowool.analysis_parser import parse_document
...
class AnalysisInputProvider(InputProvider):
    def __init__(self, analysis: Analysis, id: str) -> None:
        super(AnalysisInputProvider, self).__init__(id)
