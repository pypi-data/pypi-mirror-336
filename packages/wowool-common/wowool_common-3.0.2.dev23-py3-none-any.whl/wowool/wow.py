...
class WowTools:
    ...
    def tool_none(self, cli, id: str, doc: Document):
        pass

    @register()
    def raw(self, cli, id: str, doc: Document):
        for app in doc.app_ids():
            if app == APP_ID_ANALYSIS:
                self.console.print(f"{app=}")
                self.console.print(doc.results(app))
            elif app == "eot-print-annotations":
                            ...
                            if "streams" in data:
                                for stream in data["streams"]:
                                    if stream["type"] not in printed_stream:
                                        ...
                                        self.console.print(f"  <stem>{stream['type']: <18}</stem> : {stream['text']}")
                            print(" -" * 20)
    ...
    @register()
    def apps(self, cli, id: str, doc: Document):
        for app in doc.app_ids():
            if app != APP_ID_ANALYSIS:
                self.console.print(f"{app=}")
                self.console.print_json(doc.results(app))
    ...
    @register()
    def json(self, cli, id: str, doc: Document):
        jdoc = doc.to_json()
        print(json.dumps(jdoc, ensure_ascii=False))

    @register()
    def text(self, cli, id: str, doc: Document):
        if doc.analysis:
    ...
    @register()
    def canonical(self, cli, id: str, doc: Document):
    ...
    def input_text(self, cli, id: str, doc: Document):
        print(doc.text)

    @register()
    def grep(self, cli, id: str, doc: Document):
        from wowool.apps.grep import APP_ID
        from wowool.analysis import APP_ID as ID_ANALYSIS
    ...
    @register()
    def concepts(self, cli, id: str, doc: Document):
        assert doc.analysis
        for c in doc.analysis.concepts():
            print({**c})

    @register()
    def stagger(self, cli, id: str, doc: Document):
        from wowool.annotation import Entity

        assert doc.analysis != None
        for sentence in doc.analysis:
    ...
    @register()
    def stats(self, cli, id: str, doc: Document):
...
class CLI:
    def __init__(self, kwargs, console_=None):
        ...
        if is_valid_kwargs(kwargs, "cleanup"):
