...
def _parse_sentence(json_sentence):
    ...
    for annotation in json_sentence[3]:
        if annotation[0] == AnnotationType.TOKEN:  # Token
            ...
            token.properties = set(annotation[4])
            token.morphology = []
            ...
            concept.set_sentence_annotations_index(sentence.annotations, len(sentence.annotations))
...
def parse_document(json_str_or_obj, doc_id=None, cpp_document=None):
    """
    Parse a json string into a Analysis
    ...
    return Analysis(cpp_document, doc_id, language, sentences, json_str_or_obj, metadata)
