...
class NocodbClient:
    ...
    def get_custom_field_data(self, field_type: Union[Type[Any], HttpUrl]) -> dict:
        ...
        if isinstance(field_type, type) and issubclass(field_type, HttpUrl):
    ...
    async def synchronize_records(self, records: list[NocodbModel], upload_attachments: bool = False, remove_obsolete_records: bool = False, **mapper_kwargs):
        """
        Synchronize records with NocoDB.
        
        Args:
            self: NocoDB self
            data: Data to sync list[NocodbModel]
            language
        ...
        """
        if not records:
            self.logger.warning("No data to synchronize")
            raise ValueError("No data to synchronize")
        ...
        skip_update_column_names = nocodb_model.__skip_update_column_names__(**mapper_kwargs)
        self.logger.info(f"Syncing {len(records)} records to table {table_id}")
        new_records_map = {record.id: record for record in records}
        ...
        for record in existing_records:
            ...
            if ext_id:
                existing_lookup_map[ext_id] = record
            else:
                self.logger.debug(f"Skip from update record nocodb record without ExternalID field: {external_id_field}")

        absent_external_ids = list(set(list(existing_lookup_map.keys())) - set(list(new_records_map.keys())))
        self.logger.info(f"Found {len(existing_lookup_map)} existing records with external IDs with {absent_external_ids} absolete records")

        if remove_obsolete_records and absent_external_ids:
            self.logger.info("Remove obsolete records")
            delete_batch_size = 50
            total = len(absent_external_ids)

            for i in range(0, total, delete_batch_size):
                batch = absent_external_ids[i:i + delete_batch_size]
                batch_ids = []

                for external_id in batch:
                    absent_record = existing_lookup_map[external_id]
                    self.logger.info(f"Removing obsolete record: {absent_record['Id']}")
                    batch_ids.append(absent_record['Id'])

                await self.delete_table_record(table_id, batch_ids)
        elif absent_external_ids:
            self.logger.info(f"Skipping removal of obsolete {len(absent_external_ids)} records")
        ...
        for record_data in data:
            ...
            if not external_id:
                self.logger.debug(f"Skip from update external record with missing id field, external id must be present in 'id' field")
                continue
            del record_data["id"]
            ...
            if external_id in existing_lookup_map:
                ...
                if needs_update(self, existing_record, record_data, schema, skip_update_column_names):
                    record_data = await self.process_temporary_attachments(nocodb_model, record_data)
                    self.logger.debug(f"Updating record {external_id} with data: {record_data}")
                    update_response = await self.update_record(table_id, record_id, record_data)
                    id_map[external_id] = update_response
                    total_updated += 1
                else:
                    self.logger.debug(f"Don't need to update record {external_id}")
                    id_map[external_id] = existing_record
            else:
                # Create new record
                record_data = await self.process_temporary_attachments(nocodb_model, record_data)
                self.logger.debug(f"Creating record {external_id}")
                created_record = await self.create_record(table_id, record_data)
                id_map[external_id] = created_record
                total_created += 1
        
        self.logger.info(f"Synchronized {len(id_map)} records, {total_updated} records updated, {total_created} records created")
        return id_map