...
def blender_asset_creation_strategy() -> str:
    ...
    0. Before anything, always check the scene from blender_get_scene_info()
        ...
        2. Hyper3D(Rodin)
            ...
            - For objects/models, do the following steps:
                1. Create the model generation task
                    - Use blender_generate_hyper3d_model_via_images() if image(s) is/are given
                    - Use blender_generate_hyper3d_model_via_text() if generating 3D asset using text prompt
    ...
    5. After giving the tool location/scale/rotation information (via blender_create_object() and blender_modify_object()),
       double check the related object's location, scale, rotation, and blender_world_bounding_box using blender_get_object_info(),
