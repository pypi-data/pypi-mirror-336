...
def find_github_url(text):
    ...
    return match.group(0).rsplit(".",1)[0] if match else ""
...
def fetch_latest_arxiv_cs_papers(
    ...
    if now_in_user_tz.hour < 6:  # Early morning in IST, before new papers are announced
        # Get yesterday and day before yesterday
        search_dates = [
            (now_in_user_tz - timedelta(days=1)).strftime("%Y%m%d"),
            (now_in_user_tz - timedelta(days=2)).strftime("%Y%m%d")
        ]
    else:
        # Get today and yesterday
        search_dates = [
            now_in_user_tz.strftime("%Y%m%d"),
            (now_in_user_tz - timedelta(days=1)).strftime("%Y%m%d")
        ]
