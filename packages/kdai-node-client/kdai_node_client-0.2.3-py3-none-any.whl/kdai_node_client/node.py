...
class KDAINode:
    ...
    def register(self) -> Dict[str, Any]:
        ...
        reg_data = {
            "node_token": self.auth_token,  # Use existing token if available
            "node_info": {
                "name": self.node_name,
        ...
        try:
            ...
            # First try with allow_redirects=False to handle HTTP/HTTPS redirects
            response = requests.post(reg_url, json=reg_data, allow_redirects=False)
            
            # Handle redirect if there is one
            if response.status_code in (301, 302, 307, 308):
                redirect_url = response.headers.get('Location')
                if redirect_url:
                    logger.info(f"Received redirect to {redirect_url}")
                    # If we're redirected to HTTPS, update server_url
                    if redirect_url.startswith('https://') and self.server_url.startswith('http://'):
                        self.server_url = 'https://' + self.server_url[7:]
                        logger.info(f"Updated server URL to {self.server_url}")
                    # Follow the redirect manually
                    response = requests.post(redirect_url, json=reg_data, allow_redirects=False)
            ...
            try:
                ...
                raise ConnectionError(f"Server response wasn't valid JSON. Content: {response.text[:100]}...")
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            # First try with allow_redirects=False to handle HTTP/HTTPS redirects
            response = requests.post(auth_url, json=payload, allow_redirects=False)
            ...
            if response.status_code in (301, 302, 307, 308):
                ...
                if redirect_url:
                    ...
                    if redirect_url.startswith('https://') and self.server_url.startswith('http://'):
                        self.server_url = 'https://' + self.server_url[7:]
                        logger.info(f"Updated server URL to {self.server_url}")
                        # Update auth_url with new server_url
                        auth_url = f"{self.server_url}/api/node_heartbeat"
                    # Follow the redirect manually
                    response = requests.post(redirect_url, json=payload, allow_redirects=False)
    ...
    def heartbeat(self) -> bool:
        ...
        try:
            ...
            # First try with allow_redirects=False to handle HTTP/HTTPS redirects
            response = requests.post(heartbeat_url, json=payload, allow_redirects=False)
            ...
            if response.status_code in (301, 302, 307, 308):
                ...
                if redirect_url:
                    ...
                    if redirect_url.startswith('https://') and self.server_url.startswith('http://'):
                        self.server_url = 'https://' + self.server_url[7:]
                        logger.info(f"Updated server URL to {self.server_url}")
                        # Update heartbeat_url with new server_url
                        heartbeat_url = f"{self.server_url}/api/node_heartbeat"
                    # Follow the redirect manually
                    response = requests.post(redirect_url, json=payload, allow_redirects=False)
    ...
    def report_task_completion(self, task_id: str, result: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code in (301, 302, 307, 308):
                ...
                if redirect_url:
                    ...
                    if redirect_url.startswith('https://') and self.server_url.startswith('http://'):
                        self.server_url = 'https://' + self.server_url[7:]
                        logger.info(f"Updated server URL to {self.server_url}")
                        # Update url with new server_url
                        url = f"{self.server_url}/api/task_complete"
                    # Follow the redirect manually
                    response = requests.post(redirect_url, json=payload, allow_redirects=False)
            ...
            logger.warning(f"Failed to report task completion: Status {response.status_code}, response: {response.text}")
            return False
        except Exception as e:
    ...
    def stop(self) -> None:
        ...
        try:
            payload = {
                ...
                "uuid": self.uuid,
                "status": "offline",
                "system_info": {} # Empty system info for offline status
            }
            
            url = f"{self.server_url}/api/node_heartbeat"
            # Use allow_redirects=False and handle potential redirects manually
            response = requests.post(url, json=payload, allow_redirects=False)
            ...
            if response.status_code in (301, 302, 307, 308):
                ...
                if redirect_url:
                    logger.info(f"Received redirect to {redirect_url}")
                    # Follow the redirect manually
                    requests.post(redirect_url, json=payload, allow_redirects=False)
        except Exception as e:
