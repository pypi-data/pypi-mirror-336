...
class NetsuiteHelper:
    ...
    def normalize_account_name(account_name):
        if account_name is None or (
            isinstance(account_name, str) and not account_name.strip()
        ):
    ...
    def get_journal_amount(amount, entry_type):
        if (entry_type == "debit" and amount >= 0) or (
            entry_type == "credit" and amount < 0
        ):
            return amount
        elif (entry_type == "credit" and amount >= 0) or (
    ...
    def create_journal_item_collection(journal_items):
        journal_item_collection = []
        for item in journal_items:
            journal_item = item.to_dict()
            if ("debit" in journal_item and journal_item["debit"] != Decimal(0)) or (
            "credit" in journal_item and journal_item["credit"] != Decimal(0)
            ):
               journal_item_collection.append(item)
        return journal_item_collection
