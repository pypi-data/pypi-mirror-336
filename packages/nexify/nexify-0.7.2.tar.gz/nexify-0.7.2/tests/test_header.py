...
import pytest
from nexify import Header, status
...
def test_header(app, input, status_code, expected):
    @app.get("/items")
    def read_items(foo: Annotated[str, Header()]):
        assert foo == expected

    headers = {"foo": input} if input is not None else {}
    res = read_items({"headers": headers}, {})
...
def test_header_with_validation_int(app, input, status_code, validation):
    @app.get("/items")
    def read_items(foo: Annotated[int, Header(**validation)]): ...
...
def test_header_with_validation_str(app, input, status_code, validation):
    @app.get("/items")
    def read_items(foo: Annotated[str, Header(**validation)]): ...
...
def test_header_with_validation_float(app, input, status_code, validation):
    @app.get("/items")
    def read_items(foo: Annotated[float, Header(**validation)]): ...
...
def test_header_with_validation_decimal(app, input, status_code, validation):
    @app.get("/items")
    def read_items(foo: Annotated[Decimal, Header(**validation)]): ...
