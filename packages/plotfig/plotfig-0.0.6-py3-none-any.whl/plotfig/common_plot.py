...
def plot_one_group_bar_figure(
    ...
    y_tick_fontsize=10,
    y_tick_rotation=0,
    y_max_tick_to_one=False,
    y_max_tick_to_value=1,
    y_lim_range=None,
    math_text=True,
    ...
    **kwargs,
    ...
    for i in range(0, len(labels_name)):
        ...
        globals()["x" + str(i) + "_dots"] = (
            np.random.normal(0, 0.1, len(eval("data" + str(i)))) + i + 0
        )
        ...
        globals()["bar" + str(i) + "_SE"] = (
            eval("bar" + str(i) + "_SD") / eval("bar" + str(i) + "_len") ** 0.5
        )
    ...
    ax.errorbar(
        x, bar_mean_list, bar_SE_list, fmt="none", linewidth=1, capsize=3, color="black"
    )
    ...
    ax.tick_params(
        axis="y", which="major", labelsize=y_tick_fontsize, rotation=y_tick_rotation
    )
        ...
        for i1 in range(len(labels_name)):
            for i2 in range(i1 + 1, len(labels_name)):
                if (
                    ...
                    globals()["t_" + str(i1) + "_" + str(i2)] = p_list[
                        p_list_index
                    ]  #############################################
                    p_list_index += 1
                elif test_method == "ttest_ind":
                    (
                        globals()["s_" + str(i1) + "_" + str(i2)],
                        globals()["t_" + str(i1) + "_" + str(i2)],
                    ) = stats.ttest_ind(data[i1], data[i2])
                elif test_method == "ttest_rel":
                    (
                        globals()["s_" + str(i1) + "_" + str(i2)],
                        globals()["t_" + str(i1) + "_" + str(i2)],
                    ) = stats.ttest_rel(data[i1], data[i2])
                elif test_method == "mannwhitneyu":
                    (
                        globals()["s_" + str(i1) + "_" + str(i2)],
                        globals()["t_" + str(i1) + "_" + str(i2)],
                    ) = stats.mannwhitneyu(data[i1], data[i2], alternative="two-sided")
            ...
            for i2 in range(i1 + 1, len(labels_name)):
                if eval("t_" + str(i1) + "_" + str(i2)) <= 0.05:
                    ...
                    if 0.01 < eval("t_" + str(i1) + "_" + str(i2)) <= 0.05:
                        ax.text(
                            (i1 + i2) / 2,
                            y_max_value + count * lines_interval + star_line_interval,
                            "*",
                            c=asterisk_color,
...
def plot_one_group_violin_figure(
    ...
    **kwargs,
    ...
    ax.scatter(
        violin_position, medians, marker="o", color="white", s=30, zorder=3
    )  # 中位数
...
def plot_correlation_figure(
    ...
    x_max_tick_to_one=False,
    x_max_tick_to_value=1,
    x_math_text=True,
    x_one_decimal_place=False,
    x_percentage=False,
    y_label_name="",
    y_label_fontsize=10,
    y_tick_fontsize=10,
    ...
    if x_major_locator is not None:
        ax.xaxis.set_major_locator(
            plt.MultipleLocator(x_major_locator)
        )  # 手动设置x轴tick之间的间隔
    ax.set_xlabel(x_label_name, fontsize=x_label_fontsize)
    ax.tick_params(
        axis="x", which="major", labelsize=x_tick_fontsize, rotation=x_tick_rotation
    )
    ...
    if x_one_decimal_place:
        if x_math_text:
            print(
                "“x_one_decimal_place”会与“x_math_text”冲突，请关闭“x_math_text”后再开启！"
            )
        else:
    ...
    if y_major_locator is not None:
        ax.yaxis.set_major_locator(
            plt.MultipleLocator(y_major_locator)
        )  # 手动设置y轴tick之间的间隔
    ax.set_ylabel(y_label_name, fontsize=y_label_fontsize)
    ax.tick_params(
        axis="y", which="major", labelsize=y_tick_fontsize, rotation=y_tick_rotation
    ...
    if y_one_decimal_place:
        if y_math_text:
            print(
                "“y_one_decimal_place”会与“y_math_text”冲突，请关闭“y_math_text”后再开启！"
            )
        else:
            ax.yaxis.set_major_formatter(plt.FormatStrFormatter("%.1f"))
    # 设置y轴为百分数的显示效果，会与“y轴设置科学计数法”冲突
    ...
    ax.text(
        x_start + x_width / 10,
        y_start + 9 * y_width / 10,
        f"{r_or_rho}={str(round(s, 3))} {asterisk_text}",
        va="center",
        fontsize=asterisk_fontsize,
    )  # 参数保留小数点后3位
    return
    ...
    **kwargs,
    ...
    if colorbar:
        ...
        cax = divider.append_axes(
            "right", size="5%", pad=colorbar_label_pad
        )  # pad 控制 colorbar 与图的距离
        # 创建colorbar
        ...
        cbar.ax.tick_params(
            ...
            rotation=colorbar_tick_rotation,
        )
    ...
    plt.setp(
        ax.get_xticklabels(), rotation=x_rotation, ha="right", rotation_mode="anchor"
    )  # 其中“right”表示label最右边字母对齐到每个cell的中线
    return
