...
class Labware():
    def __init__(self, id: str, name: str, slot: Union[int, 'Module', 'Labware'], definition: dict, context: 'Context'):
        """initialization.
    ...
    def __getitem__(self, key: Union[str, int, slice, Tuple[Union[str, int], ...]]) -> Union[Well, Column, List[Well], List[Column]]:
        if isinstance(key, str):
            ...
            return [self._columns[i] for i in range(key.start or 0, (key.stop or self.__max_col))]
    ...
    def _get_slot(self) -> Union[int, 'Module', 'Labware']:
