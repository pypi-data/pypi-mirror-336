...
def plot_weekly_predictions(weekly_prediction_future, models, shift, length):
    """
    ...
    plt.title(f"Weekly Predictions for Shift={shift}, Length={length}")
    plt.legend()
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.show()

def plot_weekly_evaluation(weekly_prediction_future, models, shift, length, evaluation_metric_suffix):
    """
    ...
    plt.title(f"Weekly Evaluation for Shift={shift}, Length={length}")
    plt.legend()
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.show()
...
def plot_prediction_length_vs_metric(theD_long, metric_name):
    ...
    plt.xlabel('Number of Weeks in Forecast, averaged for all shift, wave')
    plt.title(f'Prediction Length vs {metric_name}')
    plt.show()

def plot_model_weights(stan_optimal_weights, plot_type='stacked'):
    """
    Plots model weights across shifts and prediction lengths in different formats.
    ...
    Parameters:
        ...
        plot_type (str): Type of plot to generate. Options: 'stacked', 'heatmap', 'lineplot'.
    ...
    # Sort data by shift and prediction length
    ...
    keys = [keys[i] for i in sorted_indices]  # Ensure order is maintained

    # Extract the correct model names (assuming they are the same across keys)
    model_names = model_names[0]  # Extract from the first entry (same for all keys)

    # Format x-axis labels as (shift, prediction length)
    x_labels = [f"({d}, {p})" for d, p in zip(shifts, pred_lengths)]

    if plot_type == 'stacked':
        # Stacked Bar Chart
        fig, ax = plt.subplots(figsize=(12, 6))
        bottom = np.zeros(len(x_labels))  # Initialize stacking

        for i, model in enumerate(model_names):  # Iterate over actual model names
            ax.bar(x_labels, weights[:, i], bottom=bottom, label=model)
            bottom += weights[:, i]  # Update bottom for stacking

        ax.set_xticks(range(len(x_labels)))
        ax.set_xticklabels(x_labels, rotation=45, ha="right")
        ax.set_ylabel('Weight')
        ax.set_xlabel('(Shift, Prediction Length)')
        ax.set_title('Model Weights Across Shifts and Prediction Lengths')
        ...
        for i, key in enumerate(keys):
            shift, pred_length = map(int, key.split())
            for j, weight in enumerate(weights[i]):
                data.append([shift, pred_length, model_names[j], weight])  # (shift, pred_length, model, weight)

        df = pd.DataFrame(data, columns=['Shift', 'Prediction Length', 'Model', 'Weight'])
        ...
        for i, model in enumerate(model_names):
            pivot_df = df[df['Model'] == model].pivot(index='Shift', columns='Prediction Length', values='Weight')

            sns.heatmap(pivot_df, annot=True, cmap='coolwarm', linewidths=0.5, ax=axes[i])
            axes[i].set_title(f"Model: {model}")
            axes[i].set_xlabel("Prediction Length")
            axes[i].set_ylabel("shift")

        plt.suptitle("Model Weights Across Shifts (Responsiveness) and Prediction Lengths", fontsize=14)
        plt.tight_layout()
        ...
        for i, model in enumerate(model_names):  # Iterate over models
            model_weights = [weights[j][i] for j in range(len(weights))]
            plt.plot(x_labels, model_weights, label=model, marker='o')

        plt.xticks(rotation=45, ha="right")
        plt.xlabel('(Shift (Responsiveness), Prediction Length)')
        plt.ylabel('Weight')
        plt.title('Model Weights Across shifts & Prediction Lengths')
        plt.legend(title="Models")

    else:
        raise ValueError("Invalid plot_type. Choose from 'stacked', 'heatmap', or 'lineplot'.")
