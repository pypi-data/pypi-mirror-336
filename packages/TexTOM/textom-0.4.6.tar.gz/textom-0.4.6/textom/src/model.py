...
class model:
    ...
    def __init__(self, path, classic=True, single=False):
        ...
        self.title = os.path.basename(os.path.dirname(path))
        self.path_analysis = os.path.join(path,'analysis')

        ################# Diffractlets
        self.classic = classic
        if classic:
            if os.path.isfile(os.path.join(path,'analysis','diffractlets.h5')): # loads a crystal if it exists
                t0 = time()
                with h5py.File( os.path.join(path,'analysis','diffractlets.h5'), 'r' ) as hf:
                ...
                self.Gc, self.dV, self.V_fz = rot.define_angles(self.dchi, self.sampling, self.symmetry)
                ...
                with h5py.File( os.path.join(path,'analysis','diffractlets.h5'), 'w' ) as hf:
                    hf.create_dataset('Isc', data=self.Isc, compression="lzf") 
                    hf.create_dataset('difflets', data=self.difflets) 
                    hf.create_dataset('cif_file', data=cr.cifPath)
                    hf.create_dataset('Energy_keV', data=cr.E_keV)
                    hf.create_dataset('Ang_resolution', data=self.dchi*180/np.pi)
                    hf.create_dataset('sampling',data=cr.sampling)
                    hf.create_dataset('q', data=cr.q)
                    hf.create_dataset('crystal_size', data=cr.crystalsize)
            ...
            difflets_path = os.path.join(path,'analysis','difflets.h5')
        ...
        if single: # initializes parameters if only a single image is produced
            ...
            if os.path.isfile(os.path.join(path,'analysis','projectors.h5')): # loads a tomo model if it exists
                t0 = time()
                with h5py.File( os.path.join(path,'analysis','projectors.h5'), 'r' ) as hf:
                    ...
                    # self.cutoff_low = hf['cutoff_low'][()]
                    self.Beams = hf['Beams'][()].astype(data_type)
                    self.iBeams = hf['iBeams'][()]
                    if 'fov_single' in hf:
                        self.fov_single = hf['fov_single'][()]

                    
                print("\tLoaded projectors from analysis/projectors.h5 (%2.2f s)" % ( (time()-t0)) )
            
            else:
                if os.path.isfile(os.path.join(path,'analysis/alignment_result.h5')):
                    self.Kappa, self.Omega, self.shifty, self.shiftz, self.tomogram, self.sinogram = hdl.load_shifts_mumott(
                         os.path.join(path,'analysis/alignment_result.h5') )

                    print('\tImporting geometry')
                    geo = import_module_from_path('geometry', os.path.join(path,'analysis/geometry.py'))

                    # calculate beam intensities and save them
                    self._init_projectors(geo)

                    print('\tSaving results to sample path analysis/projectors.h5')
                    with h5py.File( os.path.join(path,'analysis','projectors.h5'), 'w' ) as hf:
                        ...
                        hf.create_dataset('Ty', data=self.Ty) 
                        hf.create_dataset('Tz', data=self.Tz) 
                        hf.create_dataset('fov', data=self.fov) 
                        hf.create_dataset('x_p', data=self.x_p, compression="lzf") 
                        hf.create_dataset('p0', data=self.p0) 
                        hf.create_dataset('nVox', data=self.nVox) 
                        hf.create_dataset('mask_voxels', data=self.mask_voxels) 
                        # hf.create_dataset('cutoff_low', data=self.cutoff_low) 
                        hf.create_dataset('Beams', data=self.Beams, compression="lzf") 
                        hf.create_dataset('iBeams', data=self.iBeams, compression="lzf") 
                        hf.create_dataset('shifty', data=self.shifty) 
                        hf.create_dataset('shiftz', data=self.shiftz) 
                        if hasattr( self, 'fov_single' ):
                            hf.create_dataset('fov_single', data=self.fov_single) 
                    # hdl.save_pickle( self.tomo_pickle, self, 'tomo' )
                else:
    ...
    def _init_diffractlets(self):
        ...
        elements = np.unique(chem) # all used elements
        ...
        # pos = unit.positions/10 # coordinates of all atoms converted to nm
        ...
        # Make a crystal of several unit cells
        pos0 = pos.copy() # position of atoms in the unit cell
        chem0 = chem.copy() # elements in the unit cell
        for n,v in zip( self.crystalsize, self.lattice_vectors ): # along each lattice vector
            for k in range(n-1): # copy as many times as indicated in size
                pos_new = pos0 + v * (k+1) # shift all current atoms
                pos = np.concatenate( [pos, pos_new], axis = 0)
                chem = np.concatenate( [chem, chem0], axis = 0 )
            pos0 = pos
            chem0=chem
        self.Natom = pos.shape[0] # total number of atoms
        print('\t made crystal lattice, number of atoms: %u' % chem.shape[0] )
        ...
        FF_el = np.empty( (Nel, Ndet), data_type ) # atomic form factors for all used elements
        bool_pos_el = np.empty( (Nel, self.Natom), bool ) # positions of elements in pos and chem
        for k in range(Nel):
            # determinate the positions of element k
            bool_pos_el[k,:] = chem == elements[k]

            A, Qf = np.meshgrid( a[k], self.q/10 ) # using angstroems^-1 here for simplicity
            B, _ = np.meshgrid( b[k], self.q )
            C = c[k]
        ...
        pos_rot = rot.stack_mrot( pos.astype(data_type), self.Gc )
        # calculate structure factors
        print('\t calculating single crystal diffraction patterns \n(long calculation time possible, do not close terminal)')
        t0 = time()
        self.Isc = _single_crystal_images( Q_ew, pos_rot, FF_el, bool_pos_el )
        print('\t\t took %.2f min' % ((time()-t0)/60) )

        # Calculate powder pattern/zero order diffractlet
        difflet0_2D = ( (1/self.V_fz * self.dV) @ self.Isc ).reshape(self.detShape) # integrate over all rotations
    ...
    def _init_projectors(self, geo):
        ...
        Q_srot = np.array([rot.q_mult(qk,qo) for qo,qk in zip(Q_ome,Q_kap)])
        ...
        if not os.path.isfile(os.path.join(self.path_analysis,'voxelmask.txt')):
            ...
            for b in to_try:
                ...
                shy, shz = self.shifty, self.shiftz
        ...
        self.iBeams = (2**32-1) * np.ones( (Nrot, ntrans, lb_max_approx), np.uint32)
        tau = np.linspace(-Dmax/2, Dmax/2, int(Dmax*2)) # parameter for the beam trajectory
        dBeam_0 = geo.Dbeam/geo.Dstep # beam size relative to voxel size
        # if dBeam_0 < 1:
        neighbors = np.transpose(np.indices((3,3,3)) - 1).reshape(-1, 3) # indices of a 3x3x3 cube
        # else: # this is not yet compatible with further processing
        #     neighbors = np.transpose(np.indices((5,5,5)) - 1).reshape(-1, 3) # indices of a 5x5x5 cube
        pminmax = np.array([
    ...
    def _init_odf(self):
        ...
        if d_len > 0:
            ...
            self.Rs = self.get_Rs_stack(self.Gs)
    ...
    def set_orders( self, n_max, info=True ):
        ...
        self.ns = np.array(self.get_orders( n_max, info=False)) 
        if info:
    ...
    def get_Rs( self, g ):
        ...
        for kn, n in enumerate(self.ns):
            Rs[ self.Sn[kn,0]:self.Sn[kn,1], self.Sn[kn,0]:self.Sn[kn,1] ] = _Rs_n(
                g, n, self.Xsn[str(n)] )        
        return Rs
    ...
    def get_Rs_stack( self, G ):
        ...
        for k, n in enumerate(self.ns[1:]):
            kn = k+1
            Rs_stack[ :, self.Sn[kn,0]:self.Sn[kn,1], self.Sn[kn,0]:self.Sn[kn,1] ] = _Rs_n_stack(
                G, n, self.Xsn[str(n)] )
        return Rs_stack

    def odfFromC( self, c, K = None, clip = False, info = False ):
        ...
        if K == 'auto':
            ...
            while neg>0.01:
                ...
                odf = _odfFromC( self.V_fz, self.sHSHs, c )
        ...
        if info:
            self.get_c_weights(c)
            print('Made ODF')
            print('\tMinimum: %.3f' % odf.min() )
            print('\tMaximum: %.3f' % odf.max() )

            # estimate mean value from distribution truncated at lowest order
            if c.size > 1:
                ic_max = self.Sn[1,1]
                i_mu = np.argmax(
                    _odfFromC(self.V_fz, self.sHSHs[:ic_max-1], c[:ic_max]))
                mu = self.Gc[i_mu]
                std = self.std_sample(
                    np.array([c]), np.array([mu])
                )[0]
    ...
    def g_ML_sample( self, C, truncate = True ):
        """ Calculates ODFs from a stack of sets of sHSH coefficients
        truncated at the lowest expansion order
    ...
    def abc_pref_sample( self, g_pref ):
        ...
        a_pref, b_pref, c_pref : 2D ndarray, float
            ...
            dim: 0: voxel index, 1: [x,y,z]
        """
        lattice_vectors_norm = self.lattice_vectors/np.linalg.norm(self.lattice_vectors,axis=1)
        a_pref, b_pref, c_pref = _abc_pref_sample( g_pref, lattice_vectors_norm )
    ...
    def projection( self, g, C ):
        ...
        t0 = time()
        difflets_rot = _rotate_difflets_stack( [self.Rs[g]], self.difflets)
        images = _projection(
            g, C, 
...
@njit(parallel=True)
def _beamtranslations(
            beam_maxlength, dBeam,
            p, pminmax, mask_voxels,
            B0, Bpath0n,
            gs,
            Ty, Tz,
            beam_precision
    ...
    for t in prange(ntrans): # translations
        # # rotated translation vector T = R(omega,kappa)*(0,ty,tz)
        # T = Tg[t]
        ty,tz = Ty[t], Tz[t]
        # rotated translation vector T = R(omega,kappa)*(0,ty,tz)
        # T = np.array([
        #     -ty* np.sin(omega) + tz* np.cos(omega)*np.sin(kappa),
        #     ty* np.cos(omega) + tz* np.sin(omega)*np.sin(kappa),
        #     tz* np.cos(kappa)
        #     ])
        T = np.array([0,ty,tz],data_type) @ rot.OTP_to_matrix(gs[0],gs[1],gs[2])
        ...
        # this is valid for dBeam <=1 :
        # beam = 1/2 * ( 1 + erf( (1/2-w)/(np.sqrt(2)*sigma) ) )
        # generalisation:
        beam = 1/2 * ( erf( (1/2+w)/(np.sqrt(2)*sigma) ) + erf( (1/2-w)/(np.sqrt(2)*sigma) ) )
...
def _odf_max_stack( C, sHSHs, Gc,  V_fz ):
    ...
    for k in prange(Nvox):
        odf = _odfFromC( V_fz, sHSHs, C[k] ) # calculate the odf
...
def _odf_std_fromZero_stack( c, Rs, V_fz, sHSHs, dV, ome):
    ...
    for k in prange(c.shape[0]):
        odf = _odfFromC( V_fz, sHSHs, Rs[k] @ c[k] ) # calculate the odf
        odf[odf < 0] = 0 # clip negative values
...
def _abc_pref_sample( g_pref, lattice_vectors ):
    ...
    for k in prange(Nvox):
        ...
        R_pref  = rot.OTP_to_matrix( g_pref[k,0], g_pref[k,1], g_pref[k,2])
        # the follwoing works because its orthorhombic
...
def _projection(
    ...
    difflets_rot : 3D ndarray, float
        ...
        dim: 0: sHSH indices, 1: detector points
    ...
    images : 3D ndarray, float
        ...
        dim: 0: rotation, 1: translation, 2: detector points
    """
    images = np.array((Beams.shape[1], difflets_rot.shape[1]), data_type)
...
def _Rs_n_stack( Gs, n, Xsn ):
    ...
    for g in prange(Gs.shape[0]):
        Rs_stack[g] = _Rs_n( Gs[g], n, Xsn )
...
def _Rs_n( g, n, Xsn ):
    ...
    '''
    R = hsh.R( n, g, np.array([0.,0.,0.]) ) # this is the HSH rotation matrix
    sR = np.real( np.conj( Xsn ) @ R @ Xsn.T ) # here it's converted to sHSH
    return sR