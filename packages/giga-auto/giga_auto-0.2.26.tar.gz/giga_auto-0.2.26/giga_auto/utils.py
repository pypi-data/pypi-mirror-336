...
def get_date(days_ago, type=1):
    current_date = datetime.date.today()
    if type == 1:
        # 计算之前的日期
        days_ago = current_date - datetime.timedelta(days=days_ago)
    if type == 2:
        # 计算之后的日期
        days_ago = current_date + datetime.timedelta(days=days_ago)
...
@contextmanager
def customer_step(msg):
    """
     Context manager for cases steps .
    :param msg: steps description
    :return:
    """
    try:
        yield
...
def get_pytz_time(timezone=510):
    """
    获取指定时区的时间
    :param timezone: 时区
    :return: 指定时区的时间
...
@contextmanager
def temp_folder_context(folder_path):
    """
    上下文管理器：进入时操作 folder_path，退出时自动删除整个文件夹。
    """
    try:
