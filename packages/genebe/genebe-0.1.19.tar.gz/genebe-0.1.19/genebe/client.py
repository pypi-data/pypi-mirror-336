...
def parse_variants_df(
    ...
    """
    As parse_variants but operates on a pandas DataFrame. The DataFrame must contain a 'variant' column with variant strings.
    """

    # if there is no column variant, display warning message
    if "variant" not in df.columns:
        logging.warning("The DataFrame does not contain a 'variant' column.")
        return

    # if there are chr pos ref or alt columns, display warning message, that they will be overwritten
    ...
    # Extract variants from DataFrame as a list to pass into the parse_variants function
    ids = df["variant"].tolist()

    # Call parse_variants function to translate the variants
    ...
    # Handle empty strings by replacing them with None to avoid ValueError during splitting
    results = [x if x else None for x in results]

    # Split each variant string into separate components (chr, pos, ref, alt) and create a new DataFrame
    parsed_df = pd.DataFrame(
        [x.split("-") if x else [None, None, None, None] for x in results],
        columns=["chr", "pos", "ref", "alt"],
    )

    # Convert 'pos' column to integers, allowing None values
    parsed_df["pos"] = pd.to_numeric(parsed_df["pos"], errors="coerce").astype(
        pd.Int32Dtype()
    )

    # Concatenate original DataFrame with parsed DataFrame, aligning rows by index
