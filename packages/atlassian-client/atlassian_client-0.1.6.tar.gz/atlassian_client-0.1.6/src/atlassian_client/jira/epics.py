...
class EpicsMixin(UsersMixin):
    ...
    def download_attachment(self, issue_key: str, path: str = None) -> str:
        """
        Downloads attachments from a specified Jira issue.

        Args:
            issue_key (str): The Jira issue key (e.g., 'PROJECT-123') from which to download attachments.
            path (str, optional): The directory path where attachments should be saved.
                                 If None, attachments may be saved to a default location.

        Returns:
            str: The path where the attachments were saved.

        Raises:
            JiraError: If the issue doesn't exist or if there's an error accessing the attachments.
            IOError: If there's an error saving the attachments to the specified path.
        """
        return self.jira.download_issue_attachments(issue_key, path)
