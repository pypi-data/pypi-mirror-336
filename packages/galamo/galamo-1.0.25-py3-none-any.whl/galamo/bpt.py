...
# ANSI Escape Codes for Terminal (VS Code)
    GREEN = "\033[92m✔ Column names are correct.\033[0m"
    RED = "\033[91m❌ Incorrect or missing columns:"
    YELLOW = "\033[93mRequired SDSS format column names:"
...
# Check if running inside Jupyter Notebook
    if "ipykernel" in sys.modules:
        ...
        GREEN_HTML = '<span style="color:green; font-weight:bold;">✔ Column names are correct.</span>'
        RED_HTML = '<span style="color:red; font-weight:bold;">❌ Incorrect or missing columns: </span>'
        YELLOW_HTML = '<span style="color:orange; font-weight:bold;">Required SDSS format column names: </span>'

        if all(col in actual_columns for col in required_columns):
            display(HTML(GREEN_HTML))
        else:
            incorrect_columns = [col for col in required_columns if col not in actual_columns]
            display(HTML(f"{RED_HTML} {incorrect_columns}"))
            display(HTML(f"{YELLOW_HTML} {required_columns}"))
    else:
    # Terminal output (VS Code)
        if all(col in actual_columns for col in required_columns):
            ...
            incorrect_columns = [col for col in required_columns if col not in actual_columns]
            print(RED, incorrect_columns)
            print(YELLOW, required_columns)
