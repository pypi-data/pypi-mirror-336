...
def get_type_formatting():
    """
    Dynamically create the request type list formatting
    """
    if AbsenceRequestType.objects.exists():
        formatting = []
        for type in AbsenceRequestType.objects.all():
            with suppress(KeyError):
                formatting.append(dp.FormattingRule(condition=("==", type.id), icon=WBIcon[type.icon].icon))
        return dp.Formatting(column="type", formatting_rules=formatting)
    ...
    def get_list_display(self) -> Optional[dp.ListDisplay]:
        return dp.ListDisplay(
            fields=[
                ...
                dp.Field(key="department", label=_("Department")),
