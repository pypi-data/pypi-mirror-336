...
def _crypt_unprotect_data(
    ...
    if is_wsl:
        import jaraco.windows.dpapi as dpapi
...
def _expand_paths_impl(paths: list, os_name: str):
    ...
    if is_wsl:
        os_name = 'wsl'
    assert os_name in ['windows', 'osx', 'linux', 'wsl']
    ...
    if os_name == 'windows':
        ...
        paths = map(_expand_wsl_path, paths)
    else:
    ...
    def __add_key_and_cookie_file(self,
        ...
        if sys.platform == 'darwin':
            ...
            if sys.platform == "win32":
                ...
                if self.key_file:
                    key_file = _wsl_path_from_windows(self.key_file)
                else:
                    key_file = _expand_paths(windows_keys, 'wsl')
            ...
            if not cookie_file:
                if self.browser.lower() == 'chrome' and _windows_group_policy_path():
                    ...
                    if not is_wsl:
                        ...
                        cookie_file = _expand_paths(windows_cookies, 'wsl')
