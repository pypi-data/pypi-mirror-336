# coding: utf-8

"""
    Platform API

    _OpenAPI specification for the **Platform API** of **Helix**, the nomination platform of ElecLink_  ---  Additional documentation available in the API's [GitHub repository](https://github.com/eleclink-helix/platform-api) 

    The version of the OpenAPI document: 1.0.0
    Contact: team.eleclink@eleclink.co.uk
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from helix_platform_api_public_client_legacy.models.transmission_rights_overview import TransmissionRightsOverview  # noqa: E501

class TestTransmissionRightsOverview(unittest.TestCase):
    """TransmissionRightsOverview unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> TransmissionRightsOverview:
        """Test TransmissionRightsOverview
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `TransmissionRightsOverview`
        """
        model = TransmissionRightsOverview()  # noqa: E501
        if include_optional:
            return TransmissionRightsOverview(
                mtu_size = 'MTU_15_MINS',
                mtus = [
                    helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus.TransmissionRightsOverview_mtus(
                        mtu = '2022-01-04T10:00Z', 
                        values = [
                            helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus_values.TransmissionRightsOverview_mtus_values(
                                direction = 'GB_FR', 
                                timescales = [
                                    helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus_values_timescales.TransmissionRightsOverview_mtus_values_timescales(
                                        timescale = 'LONG_TERM', 
                                        value = 56, )
                                    ], 
                                total = 56, )
                            ], )
                    ]
            )
        else:
            return TransmissionRightsOverview(
                mtu_size = 'MTU_15_MINS',
                mtus = [
                    helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus.TransmissionRightsOverview_mtus(
                        mtu = '2022-01-04T10:00Z', 
                        values = [
                            helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus_values.TransmissionRightsOverview_mtus_values(
                                direction = 'GB_FR', 
                                timescales = [
                                    helix_platform_api_public_client_legacy.models.transmission_rights_overview_mtus_values_timescales.TransmissionRightsOverview_mtus_values_timescales(
                                        timescale = 'LONG_TERM', 
                                        value = 56, )
                                    ], 
                                total = 56, )
                            ], )
                    ],
        )
        """

    def testTransmissionRightsOverview(self):
        """Test TransmissionRightsOverview"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
