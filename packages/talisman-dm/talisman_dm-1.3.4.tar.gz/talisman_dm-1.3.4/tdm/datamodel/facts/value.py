...
class AtomValueFact(Identifiable, AbstractValueFact[AtomValueType, AbstractValue], PrunableMixin):
    ...
    def choose_one(
        ...
        if self._tuple_value() != other._tuple_value():
            return None
        links = set(self._collect_links(map(restore_element, related(self.id))))
        other_links = set(other._collect_links(map(restore_element, related(other.id))))
        ...
        if not self.value:
            mentions = set(self._collect_mentions(map(restore_element, related(self.id))))
            other_mentions = set(other._collect_mentions(map(restore_element, related(other.id))))
...
class CompositeValueFact(Identifiable, _CompositeValueFact, PrunableMixin, Duplicatable):
    ...
    def choose_one(
        ...
        components = set(self._collect_components(map(restore_element, related(self.id))))
        other_components = set(other._collect_components(map(restore_element, related(other.id))))
