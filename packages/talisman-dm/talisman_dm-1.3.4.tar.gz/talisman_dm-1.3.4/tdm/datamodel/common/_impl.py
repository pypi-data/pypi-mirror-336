...
class ViewContainer(object):
    __slots__ = (
        '_id2view', '_dependencies', '_containers', '_hash2ids', '_id2hash', '_duplicate2id', '_id2duplicates'
    )
    ...
    @staticmethod
    def _check_duplicate(element, hash2ids, id2hash, id2view, dependencies) -> Optional[Tuple[str, str]]:
        element_hash = id2hash[element.id]
        for candidate_id in hash2ids.iterate(element_hash):
            if candidate_id == element.id:
                continue
            candidate = restore_object(id2view[candidate_id], id2view)
            duplicate = element.choose_one(
                candidate,
                lambda i: map(itemgetter(0), dependencies.iterate(i)),
                lambda i: restore_object(id2view[i], id2view)
            )
