...
from .core import Storage, _get_view_classes
from .external import PythonAnalysis
from .external.axisvm import AxisVMAnalysis
from .external.dynamo import DynamoAnalysis
from .external.etabs import ETABSAnalysis
...
from .external.matlab import MatlabAnalysis
from .external.plaxis import PlaxisAnalysis
from .external.revit import RevitAnalysis
from .external.sap2000 import SAP2000Analysis
...
from .external.tekla import TeklaAnalysis
...
def _mock_params(mocked_get_entity: Callable, mocked_file_resource: type,
                 deserialize_function: Callable[[dict, dict, int, None], Munch], param_types: dict, params: dict) -> Munch:
    with mock.patch.object(API, '__init__', return_value=None), \
            mock.patch.object(API, 'get_entity', side_effect=mocked_get_entity), \
            mock.patch('viktor.parametrization.FileResource', mocked_file_resource):
        try:
            return deserialize_function(param_types, params, 1, None)
...
def _mock_View(func: Callable, args: tuple, kwargs: dict, controller_module: ModuleType,
               testing_module: ModuleType) -> Callable:
    patches = []
    for view in _get_view_classes():
        # when using `import viktor as vkt -> @vkt.XXView`
        patches.append(mock.patch(f'viktor.{view.__name__}', lambda *x, **y: lambda f: f))

        # when using `from viktor.views import XXView -> @XXView`
        patches.append(mock.patch(f'viktor.views.{view.__name__}', lambda *x, **y: lambda f: f))
...
def _mock_DynamoAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            DynamoAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_MatlabAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            MatlabAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_PythonAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            PythonAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_ETABSAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            ETABSAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_PlaxisAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            PlaxisAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_RevitAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            RevitAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_SAP2000Analysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            SAP2000Analysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
...
def _mock_TeklaAnalysis(func: Callable, args: tuple, kwargs: dict, get_output_file: Callable) -> Callable:
    with mock.patch.multiple(
            TeklaAnalysis,
            __init__=mock.Mock(return_value=None),
            execute=mock.Mock(return_value=None),
