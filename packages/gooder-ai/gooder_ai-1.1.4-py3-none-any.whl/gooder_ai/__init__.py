...
async def valuate_model(
    ...
    amplify_env: str = "seventeen",  # For development we will provide latest as the amplify env
) -> ValuateModelOutput:
    ...
    for index, model in enumerate(models):
        ...
        combined_dataframe = concat(
            [
                combined_dataframe,
                # The `scorer` function returns a NumPy array, which does not retain the original ordering
                # and may cause misalignment with the existing dataset.
                # To prevent this issue, we ensure that the new score column aligns with the index of the original DataFrame.
                # This is done by explicitly setting the `index` parameter to `combined_dataframe.index`.
                DataFrame(
                    scorer(x_data),
                    columns=score_column_names,
    ...
    if config_path is None and upload_config_to_gooder == True:
        ...
        # Ref: https://docs.python.org/3/library/asyncio-task.html#running-in-threads
        # TODO: Do we need to_thread ?
        await to_thread(lambda: save_config(config, local_config_filename))
