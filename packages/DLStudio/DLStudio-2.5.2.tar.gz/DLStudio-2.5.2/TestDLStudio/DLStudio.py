...
__date__      = '2025-March-26'                   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.5.2.html'
...
#            plt.legend()
    ...
    class VQVAE (Autoencoder):             
        ...
        """  
        ##  The embedding_dim is the size of the Codebook embedding vectors
        def __init__(self, dl_studio, encoder_out_im_size, decoder_out_im_size, encoder_out_ch, num_embeddings, 
                                   embedding_dim, commitment_cost, decay, path_saved_encoder, path_saved_decoder, path_saved_vector_quantizer ):
            super(DLStudio.VQVAE, self).__init__( dl_studio, encoder_out_im_size, decoder_out_im_size, encoder_out_ch, path_saved_model=None )
            ...
            self.pre_vq_conv   =  nn.Conv2d(in_channels=encoder_out_ch, out_channels=embedding_dim, kernel_size=1, stride=1)
            self.vector_quantizer = DLStudio.VQVAE.VectorQuantizerEMA(num_embeddings, embedding_dim, commitment_cost, decay)
            self.encoder_out_im_size = self.encoder.encoder_out_im_size
            self.encoder_out_ch  =  self.encoder.encoder_out_ch
            self.path_saved_encoder = path_saved_encoder
            self.path_saved_decoder = path_saved_decoder
        ...
        class VQVaeDecoder(nn.Module):
            ...
            def forward(self, quantized):
                decoded = self.parent_decoder( quantized.view(-1, 128, self.encoder_out_im_size[0], self.encoder_out_im_size[1]) )
                return decoded
        ...
        def run_code_for_training_VQVAE( self, vqvae, display_train_loss=False ):
            ...
            if display_train_loss:
                ...
                plt.savefig("training_losses_and_perplexity.png")
                plt.show()
