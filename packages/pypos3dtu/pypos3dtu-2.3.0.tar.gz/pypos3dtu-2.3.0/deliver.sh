...
# Prepare Unit Tests package distrib
...
if [ $1 == "test" ]; then
  echo "Test delivery"
  python3 -m twine upload --verbose --repository testpypi dist/pypos3d-${pp3dv}.tar.gz
  python3 -m twine upload --verbose --repository testpypi dist/pypos3dv-${viewer3dv}.tar.gz
  python3 -m twine upload --verbose --repository testpypi dist/pypos3dtu-${pp3dv}.tar.gz
else
