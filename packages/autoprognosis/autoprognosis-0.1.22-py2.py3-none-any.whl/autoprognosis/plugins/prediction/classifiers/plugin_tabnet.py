...
class TabNetPlugin(base.ClassifierPlugin):
    ...
    def _fit(self, X: pd.DataFrame, *args: Any, **kwargs: Any) -> "TabNetPlugin":
        if len(*args) == 0:
            raise RuntimeError("Please provide the labels for training")

        X = np.asarray(X)
        y = np.asarray(args[0]).squeeze()
    ...
    @classmethod
    def load(cls, buff: bytes) -> "TabNetPlugin":
