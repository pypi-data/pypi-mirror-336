...
class AltAction(PClass):
    ...
    def __fromdict__(json_dict):
        return AltAction(
            ...
            return_route_cards=json_dict["return_route_cards"],
            draw_faceup_unit_card_num=json_dict["draw_faceup_unit_card_num"],
            draw_faceup_spot_num=json_dict["draw_faceup_spot_num"],
            point_uuid=json_dict["point_uuid"],
            unit_combo=json_dict["unit_combo"],
        )
...
class PublicPlayerInfo(PClass):
    ...
    def __todict__(self):
        return {
            "": self.final_score.__todict__() if self.final_score else None,
            "longest_road": self.longest_road,
            "longest_road_len": self.longest_road_len,
            "num_pieces": self.num_pieces,
...
def getinitialstate(game_config):
    ...
    if getsettingvalue(fig, 'action_route_discard'):
        initial_to_play = list(range(1, game_config.num_players + 1))
    else:
...
def getlegalactions(s):
    return getlegalactions(s, gettoplay(s))
...
# end
@dispatch(State, list)
def getlegalactions(s, player_idxs):
    legal_actions = []
    for player_idx in player_idxs:
        legal_actions.extend(getlegalactions(s, player_idx))
...
def get_qvalue_trajectories(game_config, actions):
    ...
    return QValueTrajectories(scores=scores.tolist(), q_values=q_values, formulas=formulas, actions=actions)
