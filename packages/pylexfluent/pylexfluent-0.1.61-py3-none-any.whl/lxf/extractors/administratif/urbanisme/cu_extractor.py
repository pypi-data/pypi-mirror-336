...
def check_address_with_city(text:str,nlp)->tuple[bool, str,str] :
    ...
    """
    regex_adresse_avec_ville=r"(?P<adresse>.*)à(?P<ville>.*)\n?"
    match = re.search(regex_adresse_avec_ville, text, re.DOTALL)
    if match!=None :
...
def get_rue(text:str, nlp) -> str :
    ...
    """
    rue=""
    wellknwon_street2 ="17A Rue de Lattre de Tassigny"
    wellknwon_street3 ="17A Rue Maréchal de Lattre de Tassigny"
    ...
    if rue_comparator.compare_to(text)<0.75 and rue_comparator2.compare_to(text)<0.75 and rue_comparator3.compare_to(text)<0.75:
        rue=text
...
async def extract_cu_basic_data(file_path) ->tuple[int,str,str,str,str]:
    ...
    if num_cu_found==False or adresse_found == False  or cadastre_found==False :
        pages = await do_OcrImage_from_pdf(file_path)
