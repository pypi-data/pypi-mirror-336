...
class BaseVectorStorage(StorageNameSpace):
    ...
    async def upsert_with_centroid(self, data: dict[str, dict]):
        """Insert vectors and update the centroid in AstraDB or Weaviate.
    
        - Embeds content and stores it in the database.
        - Fetches the existing centroid from S3.
        - Computes and updates the centroid based on new embeddings.
        - Saves the updated centroid back to S3.
    
        Args:
            data (dict[str, dict]): Mapping of IDs to content and metadata.
    
        Returns:
            list[dict]: List of inserted items with embeddings.
        """
        raise NotImplementedError
