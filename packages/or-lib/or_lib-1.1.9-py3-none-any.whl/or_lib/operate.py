...
async def extract_entities(
    ...
    async def _user_llm_func_with_cache(
        ...
        if enable_llm_cache_for_entity_extract and llm_response_cache:
            ...
            cached_return, _1, _2, _3, _ = await handle_cache(
                llm_response_cache,
                arg_hash,
                _prompt,
                "default",
...
async def kg_query(
    ...
    cached_response, quantized, min_val, max_val, cached_image_ids = await handle_cache(
        hashing_kv, args_hash, query, query_param.mode, cache_type="query"
    )
    if cached_response is not None and cached_image_ids is not None:
        return cached_response, cached_image_ids
    ...
    context, image_chunk = await _build_query_context(
        keywords,
    ...
    image_ids = []
    if image_chunk:  # Only proceed to call LLMs if image_chunk is not empty
        csv_data = "serial number,image id,image summary\n"
        for index, item in enumerate(image_chunk, start=1):
            image_id = item["image_metadata"].get("image_id", "N/A")
            content = item["content"].replace("\n", " ")
            csv_data += f'{index},"{image_id}","{content}"\n'
        
        img_prompt_temp = PROMPTS["img_response"]
        img_prompt = img_prompt_temp.format(
            image_csv_data=csv_data,
            user_query=query
        )

        image_ids = await use_model_func(
            query,
...
async def extract_keywords_only(
    ...
    cached_response, quantized, min_val, max_val, _ = await handle_cache(
...
async def _build_query_context(
    ...
    if query_param.mode == "local":
        entities_context, relations_context, text_units_context, image_chunk = await _get_node_data(
            ll_keywords,
            knowledge_graph_inst,
            entities_vdb,
            text_chunks_db,
        ...
        entities_context, relations_context, text_units_context, image_chunk = await _get_edge_data(
            hl_keywords,
            knowledge_graph_inst,
            relationships_vdb,
            text_chunks_db,
    ...
    for chunk in ll_image_chunk + hl_image_chunk:
        image_id = chunk["image_metadata"].get("image_id")
        
        if image_id and image_id not in unique_image_chunks:
            unique_image_chunks[image_id] = chunk
...
async def _get_node_data(
    ...
    (use_text_units,image_chunks), use_relations = await asyncio.gather(
    ...
    if query_param.graph_algorithm:
        graph_analysis = GraphAlgorithms()
        algorithm_map = {
            "degree_centrality": graph_analysis.compute_degree_centrality,
            "pagerank": graph_analysis.compute_pagerank,
            "article_rank": graph_analysis.compute_article_rank,
            "betweenness_centrality": graph_analysis.compute_betweenness_centrality,
            "celf_influence": graph_analysis.celf_influence_maximization,
        }

        selected_algo = query_param.graph_algorithm 

        if selected_algo in algorithm_map:
            top_k_selected = max(5, query_param.top_k // 2)
            node_datas = algorithm_map[selected_algo](node_datas, use_relations, top_k_selected)
            use_relations = use_relations[:top_k_selected]
    ...
    return entities_context, relations_context, text_units_context, image_chunks
...
async def _find_most_related_text_unit_from_entities(
    ...
    image_text_units = [
        {
            "content": chunk["content"],
            "image_metadata": chunk["metadata"]
        }
        for chunk in results
        if chunk is not None and "metadata" in chunk and "image_id" in chunk["metadata"]
    ]
    ...
    return all_text_units, image_text_units
...
async def _get_edge_data(
    ...
    use_entities, (use_text_units, image_chunks) = await asyncio.gather(
    ...
    if query_param.graph_algorithm:
        top_k_selected = max(5, query_param.top_k // 2)
        edge_datas = edge_datas[:top_k_selected]
...
async def _find_most_related_entities_from_relationships(
    ...
    if query_param.graph_algorithm:
        ...
        if selected_algo in algorithm_map:
            top_k_selected = max(5, query_param.top_k // 2)
            node_datas = algorithm_map[selected_algo](node_datas,edge_datas, top_k_selected)
    ...
    image_text_units = [
        {
            ...
            "image_metadata": chunk["data"]["metadata"]
        }
        for chunk in all_text_units
        if chunk["data"].get("metadata") and "image_id" in chunk["data"]["metadata"]
    ]
